...
object distDocCreateNodeMod {
  ...
  trait CreateNodeContext extends StObject {
    ...
    var sourceObjects: Map[Any, typingsJapgolly.yaml.anon.Node]
  }
  object CreateNodeContext {
    ...
    inline def apply(
      ...
      onAnchor: Any => String,
      schema: Schema,
      sourceObjects: Map[Any, typingsJapgolly.yaml.anon.Node]
    ): CreateNodeContext = {
    ...
    extension [Self <: CreateNodeContext](x: Self) {
      ...
      inline def setSourceObjects(value: Map[Any, typingsJapgolly.yaml.anon.Node]): Self = StObject.set(x, "sourceObjects", value.asInstanceOf[js.Any])
    }
  }
}
