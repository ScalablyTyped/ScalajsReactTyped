...
import typingsJapgolly.wordpressComponents.checkboxControlMod.CheckboxControl.Props
...
object checkboxControlMod {
  ...
  object CheckboxControl {
    ...
    object Props {
      ...
      inline def apply(onChange: Boolean => Callback): Props = {
        val __obj = js.Dynamic.literal(onChange = js.Any.fromFunction1((t0: Boolean) => onChange(t0).runNow()))
