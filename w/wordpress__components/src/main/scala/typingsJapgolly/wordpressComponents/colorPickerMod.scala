...
import typingsJapgolly.wordpressComponents.wordpressComponentsStrings.rgb
...
object colorPickerMod {
  ...
  object ColorPicker {
    ...
    object Props {
      ...
      inline def apply(onChangeComplete: OnChangeCompleteValue => Callback): Props = {
        val __obj = js.Dynamic.literal(onChangeComplete = js.Any.fromFunction1((t0: OnChangeCompleteValue) => onChangeComplete(t0).runNow()))
      ...
      extension [Self <: Props](x: Self) {
        ...
        inline def setOnChangeComplete(value: OnChangeCompleteValue => Callback): Self = StObject.set(x, "onChangeComplete", js.Any.fromFunction1((t0: OnChangeCompleteValue) => value(t0).runNow()))
      }
    }
  }
}
