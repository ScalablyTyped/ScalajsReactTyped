...
import typingsJapgolly.wxJsSdkBrowser.anon.LocalIds
...
object ChooseImageConfig {
  ...
  inline def apply(success: LocalIds => Callback): ChooseImageConfig = {
    val __obj = js.Dynamic.literal(success = js.Any.fromFunction1((t0: LocalIds) => success(t0).runNow()))
  ...
  extension [Self <: ChooseImageConfig](x: Self) {
    ...
    inline def setSuccess(value: LocalIds => Callback): Self = StObject.set(x, "success", js.Any.fromFunction1((t0: LocalIds) => value(t0).runNow()))
  }
}
