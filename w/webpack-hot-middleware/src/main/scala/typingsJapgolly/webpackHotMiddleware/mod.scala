...
object mod {
  ...
  object EventStream {
    ...
    inline def apply(close: Callback, publish: Any => Callback): EventStream = {
      val __obj = js.Dynamic.literal(close = close.toJsFn, publish = js.Any.fromFunction1((t0: Any) => publish(t0).runNow()))
