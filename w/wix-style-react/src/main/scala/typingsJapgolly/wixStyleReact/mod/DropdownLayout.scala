package typingsJapgolly.wixStyleReact.mod

import typingsJapgolly.wixStyleReact.distTypesDropdownLayoutMod.NoneSelectedId
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("wix-style-react", "DropdownLayout")
@js.native
open class DropdownLayout ()
  extends typingsJapgolly.wixStyleReact.distTypesDropdownLayoutMod.default
/* static members */
object DropdownLayout {
  
  @JSImport("wix-style-react", "DropdownLayout")
  @js.native
  val ^ : js.Any = js.native
  
  @JSImport("wix-style-react", "DropdownLayout.NONE_SELECTED_ID")
  @js.native
  def NONE_SELECTED_ID: NoneSelectedId = js.native
  inline def NONE_SELECTED_ID_=(x: NoneSelectedId): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NONE_SELECTED_ID")(x.asInstanceOf[js.Any])
}
