...
object FormatMonthAndYear {
  ...
  extension [Self <: FormatMonthAndYear](x: Self) {
    ...
    inline def setGetFirstDayOfWeek(value: CallbackTo[Any | Double]): Self = StObject.set(x, "getFirstDayOfWeek", value.toJsFn)
    
    inline def setGetMonths(value: CallbackTo[js.Array[String]]): Self = StObject.set(x, "getMonths", value.toJsFn)
  }
}
