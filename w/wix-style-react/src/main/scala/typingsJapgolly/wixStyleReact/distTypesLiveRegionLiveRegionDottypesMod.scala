...
object distTypesLiveRegionLiveRegionDottypesMod {
  ...
  trait LiveRegionRefType extends StObject {
    ...
    def broadcast(hasMessageRole: LiveRegionState): Unit
  }
  object LiveRegionRefType {
    
    inline def apply(broadcast: LiveRegionState => Callback): LiveRegionRefType = {
      val __obj = js.Dynamic.literal(broadcast = js.Any.fromFunction1((t0: LiveRegionState) => broadcast(t0).runNow()))
      __obj.asInstanceOf[LiveRegionRefType]
    }
    
    extension [Self <: LiveRegionRefType](x: Self) {
      
      inline def setBroadcast(value: LiveRegionState => Callback): Self = StObject.set(x, "broadcast", js.Any.fromFunction1((t0: LiveRegionState) => value(t0).runNow()))
    }
  }
