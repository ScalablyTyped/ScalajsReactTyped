package typingsJapgolly.wordpressRichText.mod

import typingsJapgolly.wordpressRichText.AnonElement
import typingsJapgolly.wordpressRichText.AnonHtml
import typingsJapgolly.wordpressRichText.AnonText
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@wordpress/rich-text", "create")
@js.native
object create extends js.Object {
  def apply(): Value = js.native
  def apply(args: AnonElement): Value = js.native
  def apply(args: AnonHtml): Value = js.native
  def apply(args: AnonText): Value = js.native
}

