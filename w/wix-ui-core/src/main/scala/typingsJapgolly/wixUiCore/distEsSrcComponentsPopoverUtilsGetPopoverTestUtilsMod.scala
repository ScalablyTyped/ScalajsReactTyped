package typingsJapgolly.wixUiCore

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object distEsSrcComponentsPopoverUtilsGetPopoverTestUtilsMod {
  
  object getPopoverTestUtils {
    
    @JSImport("wix-ui-core/dist/es/src/components/popover/utils/getPopoverTestUtils", "getPopoverTestUtils")
    @js.native
    val ^ : js.Any = js.native
    
    inline def createRange(): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("createRange")().asInstanceOf[Unit]
    
    inline def generateId(): String = ^.asInstanceOf[js.Dynamic].applyDynamic("generateId")().asInstanceOf[String]
  }
}
