package typingsJapgolly.wixUiCore

import typingsJapgolly.wixUiCore.anon.Enter
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object distEsSrcComponentsPopoverUtilsShouldAnimatePopoverMod {
  
  @JSImport("wix-ui-core/dist/es/src/components/popover/utils/shouldAnimatePopover", JSImport.Namespace)
  @js.native
  val ^ : js.Any = js.native
  
  inline def shouldAnimatePopover(): Boolean = ^.asInstanceOf[js.Dynamic].applyDynamic("shouldAnimatePopover")().asInstanceOf[Boolean]
  inline def shouldAnimatePopover(timeout: Double): Boolean = ^.asInstanceOf[js.Dynamic].applyDynamic("shouldAnimatePopover")(timeout.asInstanceOf[js.Any]).asInstanceOf[Boolean]
  inline def shouldAnimatePopover(timeout: Enter): Boolean = ^.asInstanceOf[js.Dynamic].applyDynamic("shouldAnimatePopover")(timeout.asInstanceOf[js.Any]).asInstanceOf[Boolean]
}
