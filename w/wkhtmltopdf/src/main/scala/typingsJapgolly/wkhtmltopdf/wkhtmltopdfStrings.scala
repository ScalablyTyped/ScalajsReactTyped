package typingsJapgolly.wkhtmltopdf

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object wkhtmltopdfStrings {
  
  @js.native
  sealed trait A0 extends StObject
  inline def A0: A0 = "A0".asInstanceOf[A0]
  
  @js.native
  sealed trait A1 extends StObject
  inline def A1: A1 = "A1".asInstanceOf[A1]
  
  @js.native
  sealed trait A2 extends StObject
  inline def A2: A2 = "A2".asInstanceOf[A2]
  
  @js.native
  sealed trait A3 extends StObject
  inline def A3: A3 = "A3".asInstanceOf[A3]
  
  @js.native
  sealed trait A4 extends StObject
  inline def A4: A4 = "A4".asInstanceOf[A4]
  
  @js.native
  sealed trait A5 extends StObject
  inline def A5: A5 = "A5".asInstanceOf[A5]
  
  @js.native
  sealed trait A6 extends StObject
  inline def A6: A6 = "A6".asInstanceOf[A6]
  
  @js.native
  sealed trait A7 extends StObject
  inline def A7: A7 = "A7".asInstanceOf[A7]
  
  @js.native
  sealed trait A8 extends StObject
  inline def A8: A8 = "A8".asInstanceOf[A8]
  
  @js.native
  sealed trait A9 extends StObject
  inline def A9: A9 = "A9".asInstanceOf[A9]
  
  @js.native
  sealed trait B0 extends StObject
  inline def B0: B0 = "B0".asInstanceOf[B0]
  
  @js.native
  sealed trait B1 extends StObject
  inline def B1: B1 = "B1".asInstanceOf[B1]
  
  @js.native
  sealed trait B10 extends StObject
  inline def B10: B10 = "B10".asInstanceOf[B10]
  
  @js.native
  sealed trait B2 extends StObject
  inline def B2: B2 = "B2".asInstanceOf[B2]
  
  @js.native
  sealed trait B3 extends StObject
  inline def B3: B3 = "B3".asInstanceOf[B3]
  
  @js.native
  sealed trait B4 extends StObject
  inline def B4: B4 = "B4".asInstanceOf[B4]
  
  @js.native
  sealed trait B5 extends StObject
  inline def B5: B5 = "B5".asInstanceOf[B5]
  
  @js.native
  sealed trait B6 extends StObject
  inline def B6: B6 = "B6".asInstanceOf[B6]
  
  @js.native
  sealed trait B7 extends StObject
  inline def B7: B7 = "B7".asInstanceOf[B7]
  
  @js.native
  sealed trait B8 extends StObject
  inline def B8: B8 = "B8".asInstanceOf[B8]
  
  @js.native
  sealed trait B9 extends StObject
  inline def B9: B9 = "B9".asInstanceOf[B9]
  
  @js.native
  sealed trait C5E extends StObject
  inline def C5E: C5E = "C5E".asInstanceOf[C5E]
  
  @js.native
  sealed trait Comm10E extends StObject
  inline def Comm10E: Comm10E = "Comm10E".asInstanceOf[Comm10E]
  
  @js.native
  sealed trait DLE extends StObject
  inline def DLE: DLE = "DLE".asInstanceOf[DLE]
  
  @js.native
  sealed trait Executive extends StObject
  inline def Executive: Executive = "Executive".asInstanceOf[Executive]
  
  @js.native
  sealed trait Folio extends StObject
  inline def Folio: Folio = "Folio".asInstanceOf[Folio]
  
  @js.native
  sealed trait Landscape extends StObject
  inline def Landscape: Landscape = "Landscape".asInstanceOf[Landscape]
  
  @js.native
  sealed trait Ledger extends StObject
  inline def Ledger: Ledger = "Ledger".asInstanceOf[Ledger]
  
  @js.native
  sealed trait Legal extends StObject
  inline def Legal: Legal = "Legal".asInstanceOf[Legal]
  
  @js.native
  sealed trait Letter extends StObject
  inline def Letter: Letter = "Letter".asInstanceOf[Letter]
  
  @js.native
  sealed trait Portrait extends StObject
  inline def Portrait: Portrait = "Portrait".asInstanceOf[Portrait]
  
  @js.native
  sealed trait Tabloid extends StObject
  inline def Tabloid: Tabloid = "Tabloid".asInstanceOf[Tabloid]
  
  @js.native
  sealed trait abort extends StObject
  inline def abort: abort = "abort".asInstanceOf[abort]
  
  @js.native
  sealed trait error extends StObject
  inline def error: error = "error".asInstanceOf[error]
  
  @js.native
  sealed trait ignore extends StObject
  inline def ignore: ignore = "ignore".asInstanceOf[ignore]
  
  @js.native
  sealed trait info extends StObject
  inline def info: info = "info".asInstanceOf[info]
  
  @js.native
  sealed trait none extends StObject
  inline def none: none = "none".asInstanceOf[none]
  
  @js.native
  sealed trait skip extends StObject
  inline def skip: skip = "skip".asInstanceOf[skip]
  
  @js.native
  sealed trait warn extends StObject
  inline def warn: warn = "warn".asInstanceOf[warn]
}
