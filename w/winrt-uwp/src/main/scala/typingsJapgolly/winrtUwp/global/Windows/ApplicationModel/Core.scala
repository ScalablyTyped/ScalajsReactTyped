...
object Core {
  ...
  open class AppListEntry ()
    extends StObject
       with typingsJapgolly.winrtUwp.Windows.ApplicationModel.Core.AppListEntry {
    
    /** Provides an app's display name, description, and logo. */
    /* CompleteClass */
    var displayInfo: typingsJapgolly.winrtUwp.Windows.ApplicationModel.AppDisplayInfo = js.native
