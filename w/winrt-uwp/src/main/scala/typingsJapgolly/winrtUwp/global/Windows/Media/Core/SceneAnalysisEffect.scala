package typingsJapgolly.winrtUwp.global.Windows.Media.Core

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/** Represents an effect that analyzes video frames to determine if any of the supported variable photo sequence capture techniques may produce a higher-quality captured image. */
/* note: abstract class */ @JSGlobal("Windows.Media.Core.SceneAnalysisEffect")
@js.native
open class SceneAnalysisEffect ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.Media.Core.SceneAnalysisEffect
