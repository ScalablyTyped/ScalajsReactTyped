package typingsJapgolly.winrtUwp.global.Windows.UI.Input

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/** Provides access to the type of user input that triggered the edge gesture event. */
/* note: abstract class */ @JSGlobal("Windows.UI.Input.EdgeGestureEventArgs")
@js.native
open class EdgeGestureEventArgs ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.UI.Input.EdgeGestureEventArgs {
  
  /** Gets the type of user input that triggered the edge gesture event. */
  /* CompleteClass */
  var kind: typingsJapgolly.winrtUwp.Windows.UI.Input.EdgeGestureKind = js.native
}
