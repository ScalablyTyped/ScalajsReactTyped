package typingsJapgolly.winrtUwp.global.Windows.Storage

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/** A helper object that provides indexing names for Windows audio file properties. */
/* note: abstract class */ @JSGlobal("Windows.Storage.SystemAudioProperties")
@js.native
open class SystemAudioProperties ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.Storage.SystemAudioProperties {
  
  /** Gets the name of the System.Audio.EncodingBitrate property (one of the Windows audio file properties). */
  /* CompleteClass */
  var encodingBitrate: String = js.native
}
