...
open class DeviceUnpairingResult ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.Devices.Enumeration.DeviceUnpairingResult {
  
  /** Gets the paired status of the device after the unpairing action completed. */
  /* CompleteClass */
  var status: typingsJapgolly.winrtUwp.Windows.Devices.Enumeration.DeviceUnpairingResultStatus = js.native
}
