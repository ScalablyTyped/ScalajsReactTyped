...
open class EmailMailboxChangeReader ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.ApplicationModel.Email.EmailMailboxChangeReader {
  ...
  override def acceptChangesThrough(
    lastChangeToAcknowledge: typingsJapgolly.winrtUwp.Windows.ApplicationModel.Email.EmailMailboxChange
  ): Unit = js.native
