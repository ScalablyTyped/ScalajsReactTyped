package typingsJapgolly.winrtUwp.global.Windows.UI.WebUI

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/** Provides data for an app navigation event. */
/* note: abstract class */ @JSGlobal("Windows.UI.WebUI.WebUINavigatedEventArgs")
@js.native
open class WebUINavigatedEventArgs ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.UI.WebUI.WebUINavigatedEventArgs {
  
  /** Gets the app navigation operation. */
  /* CompleteClass */
  var navigatedOperation: typingsJapgolly.winrtUwp.Windows.UI.WebUI.WebUINavigatedOperation = js.native
}
