package typingsJapgolly.winrtUwp.global.Windows.Devices.Sensors

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/** Provides a controller responsible for turning on or off the phone display based on the readings from the ProximitySensor . */
/* note: abstract class */ @JSGlobal("Windows.Devices.Sensors.ProximitySensorDisplayOnOffController")
@js.native
open class ProximitySensorDisplayOnOffController ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.Devices.Sensors.ProximitySensorDisplayOnOffController {
  
  /** Disconnects this ProximitySensorDisplayOnOffController instance from the service and frees all the allocated resources. */
  /* CompleteClass */
  override def close(): Unit = js.native
}
