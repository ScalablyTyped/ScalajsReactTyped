package typingsJapgolly.winrtUwp.global.Windows.Devices.Sensors

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/** A threshold for the ProximitySensor . When the threshold conditions are met, the sensor trigger will activate. */
@JSGlobal("Windows.Devices.Sensors.ProximitySensorDataThreshold")
@js.native
open class ProximitySensorDataThreshold protected ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.Devices.Sensors.ProximitySensorDataThreshold {
  /**
    * Creates a ProximitySensorDataThreshold object.
    * @param sensor The proximity sensor.
    */
  def this(sensor: typingsJapgolly.winrtUwp.Windows.Devices.Sensors.ProximitySensor) = this()
}
