...
open class ContactMatchReason ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.ApplicationModel.Contacts.ContactMatchReason {
  
  /** Gets the contact field type that matched the search, such as name, phone number, email address and so on. */
  /* CompleteClass */
  var field: typingsJapgolly.winrtUwp.Windows.ApplicationModel.Contacts.ContactMatchReasonKind = js.native
