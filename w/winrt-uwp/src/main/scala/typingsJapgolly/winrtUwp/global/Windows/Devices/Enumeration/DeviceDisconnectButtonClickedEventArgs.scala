...
open class DeviceDisconnectButtonClickedEventArgs ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.Devices.Enumeration.DeviceDisconnectButtonClickedEventArgs {
  
  /** The device that the user clicked the disconnect button for. */
  /* CompleteClass */
  var device: typingsJapgolly.winrtUwp.Windows.Devices.Enumeration.DeviceInformation = js.native
}
