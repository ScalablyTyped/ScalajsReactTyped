package typingsJapgolly.winrtUwp.global.Windows.Devices

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* note: abstract class */ @JSGlobal("Windows.Devices.LowLevelDevicesAggregateProvider")
@js.native
open class LowLevelDevicesAggregateProvider ()
  extends StObject
     with typingsJapgolly.winrtUwp.Windows.Devices.LowLevelDevicesAggregateProvider {
  
  /* CompleteClass */
  var adcControllerProvider: Any = js.native
  
  /* unmapped type */
  /* CompleteClass */
  var gpioControllerProvider: Any = js.native
  
  /* unmapped type */
  /* CompleteClass */
  var i2cControllerProvider: Any = js.native
  
  /* unmapped type */
  /* CompleteClass */
  var pwmControllerProvider: Any = js.native
  
  /* unmapped type */
  /* CompleteClass */
  var spiControllerProvider: Any = js.native
}
