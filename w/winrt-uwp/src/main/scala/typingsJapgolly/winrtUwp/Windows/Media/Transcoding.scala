...
object Transcoding {
  ...
  object PrepareTranscodeResult {
    ...
    inline def apply(
      ...
      transcodeAsync: CallbackTo[IPromiseWithIAsyncActionWithProgress[Double]]
    ...
    extension [Self <: PrepareTranscodeResult](x: Self) {
      ...
      inline def setTranscodeAsync(value: CallbackTo[IPromiseWithIAsyncActionWithProgress[Double]]): Self = StObject.set(x, "transcodeAsync", value.toJsFn)
    }
  }
}
