...
object ClaimedSlipPrinter {
  ...
  inline def apply(
    changePrintSide: PosPrinterPrintSide => Callback,
    charactersPerLine: Double,
    closeJaws: Callback,
    colorCartridge: PosPrinterColorCartridge,
    createJob: CallbackTo[SlipPrintJob],
  ...
  extension [Self <: ClaimedSlipPrinter](x: Self) {
    ...
    inline def setOpenJaws(value: Callback): Self = StObject.set(x, "openJaws", value.toJsFn)
