...
object EasClientSecurityPolicy {
  ...
  inline def apply(
    applyAsync: CallbackTo[IPromiseWithIAsyncOperation[EasComplianceResults]],
  ...
  extension [Self <: EasClientSecurityPolicy](x: Self) {
    ...
    inline def setApplyAsync(value: CallbackTo[IPromiseWithIAsyncOperation[EasComplianceResults]]): Self = StObject.set(x, "applyAsync", value.toJsFn)
