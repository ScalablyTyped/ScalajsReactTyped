package typingsJapgolly.winrtUwp.Windows.UI.Input.Spatial

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait SpatialGestureRecognizer extends StObject {
  
  var addEventListener: Any
  
  /* unmapped type */
  var cancelPendingGestures: Any
  
  /* unmapped type */
  var captureInteraction: Any
  
  /* unmapped type */
  var gestureSettings: Any
  
  /* unmapped type */
  var onholdcanceled: Any
  
  /* unmapped type */
  var onholdcompleted: Any
  
  /* unmapped type */
  var onholdstarted: Any
  
  /* unmapped type */
  var onmanipulationcanceled: Any
  
  /* unmapped type */
  var onmanipulationcompleted: Any
  
  /* unmapped type */
  var onmanipulationstarted: Any
  
  /* unmapped type */
  var onmanipulationupdated: Any
  
  /* unmapped type */
  var onnavigationcanceled: Any
  
  /* unmapped type */
  var onnavigationcompleted: Any
  
  /* unmapped type */
  var onnavigationstarted: Any
  
  /* unmapped type */
  var onnavigationupdated: Any
  
  /* unmapped type */
  var onrecognitionended: Any
  
  /* unmapped type */
  var onrecognitionstarted: Any
  
  /* unmapped type */
  var ontapped: Any
  
  /* unmapped type */
  var removeEventListener: Any
  
  /* unmapped type */
  var trySetGestureSettings: Any
}
object SpatialGestureRecognizer {
  
  inline def apply(
    addEventListener: Any,
    cancelPendingGestures: Any,
    captureInteraction: Any,
    gestureSettings: Any,
    onholdcanceled: Any,
    onholdcompleted: Any,
    onholdstarted: Any,
    onmanipulationcanceled: Any,
    onmanipulationcompleted: Any,
    onmanipulationstarted: Any,
    onmanipulationupdated: Any,
    onnavigationcanceled: Any,
    onnavigationcompleted: Any,
    onnavigationstarted: Any,
    onnavigationupdated: Any,
    onrecognitionended: Any,
    onrecognitionstarted: Any,
    ontapped: Any,
    removeEventListener: Any,
    trySetGestureSettings: Any
  ): SpatialGestureRecognizer = {
    val __obj = js.Dynamic.literal(addEventListener = addEventListener.asInstanceOf[js.Any], cancelPendingGestures = cancelPendingGestures.asInstanceOf[js.Any], captureInteraction = captureInteraction.asInstanceOf[js.Any], gestureSettings = gestureSettings.asInstanceOf[js.Any], onholdcanceled = onholdcanceled.asInstanceOf[js.Any], onholdcompleted = onholdcompleted.asInstanceOf[js.Any], onholdstarted = onholdstarted.asInstanceOf[js.Any], onmanipulationcanceled = onmanipulationcanceled.asInstanceOf[js.Any], onmanipulationcompleted = onmanipulationcompleted.asInstanceOf[js.Any], onmanipulationstarted = onmanipulationstarted.asInstanceOf[js.Any], onmanipulationupdated = onmanipulationupdated.asInstanceOf[js.Any], onnavigationcanceled = onnavigationcanceled.asInstanceOf[js.Any], onnavigationcompleted = onnavigationcompleted.asInstanceOf[js.Any], onnavigationstarted = onnavigationstarted.asInstanceOf[js.Any], onnavigationupdated = onnavigationupdated.asInstanceOf[js.Any], onrecognitionended = onrecognitionended.asInstanceOf[js.Any], onrecognitionstarted = onrecognitionstarted.asInstanceOf[js.Any], ontapped = ontapped.asInstanceOf[js.Any], removeEventListener = removeEventListener.asInstanceOf[js.Any], trySetGestureSettings = trySetGestureSettings.asInstanceOf[js.Any])
    __obj.asInstanceOf[SpatialGestureRecognizer]
  }
  
  extension [Self <: SpatialGestureRecognizer](x: Self) {
    
    inline def setAddEventListener(value: Any): Self = StObject.set(x, "addEventListener", value.asInstanceOf[js.Any])
    
    inline def setCancelPendingGestures(value: Any): Self = StObject.set(x, "cancelPendingGestures", value.asInstanceOf[js.Any])
    
    inline def setCaptureInteraction(value: Any): Self = StObject.set(x, "captureInteraction", value.asInstanceOf[js.Any])
    
    inline def setGestureSettings(value: Any): Self = StObject.set(x, "gestureSettings", value.asInstanceOf[js.Any])
    
    inline def setOnholdcanceled(value: Any): Self = StObject.set(x, "onholdcanceled", value.asInstanceOf[js.Any])
    
    inline def setOnholdcompleted(value: Any): Self = StObject.set(x, "onholdcompleted", value.asInstanceOf[js.Any])
    
    inline def setOnholdstarted(value: Any): Self = StObject.set(x, "onholdstarted", value.asInstanceOf[js.Any])
    
    inline def setOnmanipulationcanceled(value: Any): Self = StObject.set(x, "onmanipulationcanceled", value.asInstanceOf[js.Any])
    
    inline def setOnmanipulationcompleted(value: Any): Self = StObject.set(x, "onmanipulationcompleted", value.asInstanceOf[js.Any])
    
    inline def setOnmanipulationstarted(value: Any): Self = StObject.set(x, "onmanipulationstarted", value.asInstanceOf[js.Any])
    
    inline def setOnmanipulationupdated(value: Any): Self = StObject.set(x, "onmanipulationupdated", value.asInstanceOf[js.Any])
    
    inline def setOnnavigationcanceled(value: Any): Self = StObject.set(x, "onnavigationcanceled", value.asInstanceOf[js.Any])
    
    inline def setOnnavigationcompleted(value: Any): Self = StObject.set(x, "onnavigationcompleted", value.asInstanceOf[js.Any])
    
    inline def setOnnavigationstarted(value: Any): Self = StObject.set(x, "onnavigationstarted", value.asInstanceOf[js.Any])
    
    inline def setOnnavigationupdated(value: Any): Self = StObject.set(x, "onnavigationupdated", value.asInstanceOf[js.Any])
    
    inline def setOnrecognitionended(value: Any): Self = StObject.set(x, "onrecognitionended", value.asInstanceOf[js.Any])
    
    inline def setOnrecognitionstarted(value: Any): Self = StObject.set(x, "onrecognitionstarted", value.asInstanceOf[js.Any])
    
    inline def setOntapped(value: Any): Self = StObject.set(x, "ontapped", value.asInstanceOf[js.Any])
    
    inline def setRemoveEventListener(value: Any): Self = StObject.set(x, "removeEventListener", value.asInstanceOf[js.Any])
    
    inline def setTrySetGestureSettings(value: Any): Self = StObject.set(x, "trySetGestureSettings", value.asInstanceOf[js.Any])
  }
}
