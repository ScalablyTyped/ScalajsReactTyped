...
object PhotoImportImportItemsResult {
  ...
  inline def apply(
    deleteImportedItemsFromSourceAsync: CallbackTo[
      IPromiseWithIAsyncOperationWithProgress[PhotoImportDeleteImportedItemsFromSourceResult, Double]
    ],
  ...
  extension [Self <: PhotoImportImportItemsResult](x: Self) {
    ...
    ): Self = StObject.set(x, "deleteImportedItemsFromSourceAsync", value.toJsFn)
