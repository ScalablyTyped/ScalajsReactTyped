...
object IListDataAdapter {
  ...
  extension [Self <: IListDataAdapter[?], T](x: Self & IListDataAdapter[T]) {
    ...
    inline def setGetCount(value: CallbackTo[Promise[Double]]): Self = StObject.set(x, "getCount", value.toJsFn)
    ...
    inline def setSetNotificationHandler(value: IListDataNotificationHandler[T] => Callback): Self = StObject.set(x, "setNotificationHandler", js.Any.fromFunction1((t0: IListDataNotificationHandler[T]) => value(t0).runNow()))
  }
}
