package typingsJapgolly.webcl.WEBCL

import typingsJapgolly.webcl.webclInts.`0x1140`
import typingsJapgolly.webcl.webclInts.`0x1141`
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* cl_filter_mode */
/* Rewritten from type alias, can be one of: 
  - typingsJapgolly.webcl.webclInts.`0x1140`
  - typingsJapgolly.webcl.webclInts.`0x1141`
*/
trait FilterMode extends StObject
object FilterMode {
  
  inline def FILTER_LINEAR: `0x1141` = 0x1141.asInstanceOf[`0x1141`]
  
  inline def FILTER_NEAREST: `0x1140` = 0x1140.asInstanceOf[`0x1140`]
}
