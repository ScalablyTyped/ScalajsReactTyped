...
object GeoPolygon {
  ...
  inline def apply(lines: js.Array[GeoLineString], toJSON: CallbackTo[IGeoJSONPolygon]): GeoPolygon = {
  ...
  extension [Self <: GeoPolygon](x: Self) {
    ...
    inline def setToJSON(value: CallbackTo[IGeoJSONPolygon]): Self = StObject.set(x, "toJSON", value.toJsFn)
  }
}
