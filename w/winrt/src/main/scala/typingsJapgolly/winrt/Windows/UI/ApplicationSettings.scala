...
object ApplicationSettings {
  ...
  object ISettingsPaneStatics {
    ...
    inline def apply(edge: SettingsEdgeLocation, getForCurrentView: CallbackTo[SettingsPane], show: Callback): ISettingsPaneStatics = {
      val __obj = js.Dynamic.literal(edge = edge.asInstanceOf[js.Any], getForCurrentView = getForCurrentView.toJsFn, show = show.toJsFn)
    ...
    extension [Self <: ISettingsPaneStatics](x: Self) {
      ...
      inline def setGetForCurrentView(value: CallbackTo[SettingsPane]): Self = StObject.set(x, "getForCurrentView", value.toJsFn)
