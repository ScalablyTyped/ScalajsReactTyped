...
object Portable {
  ...
  object IStorageDeviceStatics {
    ...
    inline def apply(fromId: String => StorageFolder, getDeviceSelector: CallbackTo[String]): IStorageDeviceStatics = {
      val __obj = js.Dynamic.literal(fromId = js.Any.fromFunction1(fromId), getDeviceSelector = getDeviceSelector.toJsFn)
