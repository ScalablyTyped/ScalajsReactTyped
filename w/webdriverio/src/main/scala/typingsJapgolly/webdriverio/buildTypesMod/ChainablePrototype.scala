package typingsJapgolly.webdriverio.buildTypesMod

import typingsJapgolly.std.Parameters
import typingsJapgolly.std.ReturnType
import typingsJapgolly.wdioTypes.mod.ThenArg
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Inlined {[ K in webdriverio.webdriverio/build/types.ElementQueryCommands ]: (args : std.Parameters<webdriverio.webdriverio/build/types.$ElementCommands[K]>): webdriverio.webdriverio/build/types.ChainablePromiseElement<@wdio/types.@wdio/types.ThenArg<std.ReturnType<webdriverio.webdriverio/build/types.$ElementCommands[K]>>>} & {[ K in webdriverio.webdriverio/build/types.ElementsQueryCommands ]: (args : std.Parameters<webdriverio.webdriverio/build/types.$ElementCommands[K]>): webdriverio.webdriverio/build/types.ChainablePromiseArray<@wdio/types.@wdio/types.ThenArg<std.ReturnType<webdriverio.webdriverio/build/types.$ElementCommands[K]>>>} */
trait ChainablePrototype extends StObject {
  
  @JSName("$")
  def $(
    /* import warning: parser.TsParser#functionParam Dropping repeated marker of param args because its type Parameters<$ElementCommands[K]> is not an array type */ args: Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof $ */ js.Any
    ]
  ): ChainablePromiseElement[
    ThenArg[
      ReturnType[
        /* import warning: importer.ImportType#apply Failed type conversion: typeof $ */ js.Any
      ]
    ]
  ]
  @JSName("$")
  var $_Original: js.Function1[
    /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof $ */ js.Any
    ], 
    ChainablePromiseElement[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof $ */ js.Any
        ]
      ]
    ]
  ]
  
  @JSName("$$")
  def DollarDollar(
    /* import warning: parser.TsParser#functionParam Dropping repeated marker of param args because its type Parameters<$ElementCommands[K]> is not an array type */ args: Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof $$ */ js.Any
    ]
  ): ChainablePromiseArray[
    ThenArg[
      ReturnType[
        /* import warning: importer.ImportType#apply Failed type conversion: typeof $$ */ js.Any
      ]
    ]
  ]
  @JSName("$$")
  var DollarDollar_Original: js.Function1[
    /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof $$ */ js.Any
    ], 
    ChainablePromiseArray[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof $$ */ js.Any
        ]
      ]
    ]
  ]
  
  @JSName("custom$")
  def custom$(
    /* import warning: parser.TsParser#functionParam Dropping repeated marker of param args because its type Parameters<$ElementCommands[K]> is not an array type */ args: Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$ */ js.Any
    ]
  ): ChainablePromiseElement[
    ThenArg[
      ReturnType[
        /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$ */ js.Any
      ]
    ]
  ]
  @JSName("custom$")
  var custom$_Original: js.Function1[
    /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$ */ js.Any
    ], 
    ChainablePromiseElement[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$ */ js.Any
        ]
      ]
    ]
  ]
  
  @JSName("custom$$")
  def customDollarDollar(
    /* import warning: parser.TsParser#functionParam Dropping repeated marker of param args because its type Parameters<$ElementCommands[K]> is not an array type */ args: Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$$ */ js.Any
    ]
  ): ChainablePromiseArray[
    ThenArg[
      ReturnType[
        /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$$ */ js.Any
      ]
    ]
  ]
  @JSName("custom$$")
  var customDollarDollar_Original: js.Function1[
    /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$$ */ js.Any
    ], 
    ChainablePromiseArray[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$$ */ js.Any
        ]
      ]
    ]
  ]
  
  @JSName("react$")
  def react$(
    /* import warning: parser.TsParser#functionParam Dropping repeated marker of param args because its type Parameters<$ElementCommands[K]> is not an array type */ args: Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof react$ */ js.Any
    ]
  ): ChainablePromiseElement[
    ThenArg[
      ReturnType[
        /* import warning: importer.ImportType#apply Failed type conversion: typeof react$ */ js.Any
      ]
    ]
  ]
  @JSName("react$")
  var react$_Original: js.Function1[
    /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof react$ */ js.Any
    ], 
    ChainablePromiseElement[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof react$ */ js.Any
        ]
      ]
    ]
  ]
  
  @JSName("react$$")
  def reactDollarDollar(
    /* import warning: parser.TsParser#functionParam Dropping repeated marker of param args because its type Parameters<$ElementCommands[K]> is not an array type */ args: Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof react$$ */ js.Any
    ]
  ): ChainablePromiseArray[
    ThenArg[
      ReturnType[
        /* import warning: importer.ImportType#apply Failed type conversion: typeof react$$ */ js.Any
      ]
    ]
  ]
  @JSName("react$$")
  var reactDollarDollar_Original: js.Function1[
    /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof react$$ */ js.Any
    ], 
    ChainablePromiseArray[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof react$$ */ js.Any
        ]
      ]
    ]
  ]
  
  @JSName("shadow$")
  def shadow$(
    /* import warning: parser.TsParser#functionParam Dropping repeated marker of param args because its type Parameters<$ElementCommands[K]> is not an array type */ args: Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$ */ js.Any
    ]
  ): ChainablePromiseElement[
    ThenArg[
      ReturnType[
        /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$ */ js.Any
      ]
    ]
  ]
  @JSName("shadow$")
  var shadow$_Original: js.Function1[
    /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$ */ js.Any
    ], 
    ChainablePromiseElement[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$ */ js.Any
        ]
      ]
    ]
  ]
  
  @JSName("shadow$$")
  def shadowDollarDollar(
    /* import warning: parser.TsParser#functionParam Dropping repeated marker of param args because its type Parameters<$ElementCommands[K]> is not an array type */ args: Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$$ */ js.Any
    ]
  ): ChainablePromiseArray[
    ThenArg[
      ReturnType[
        /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$$ */ js.Any
      ]
    ]
  ]
  @JSName("shadow$$")
  var shadowDollarDollar_Original: js.Function1[
    /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$$ */ js.Any
    ], 
    ChainablePromiseArray[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$$ */ js.Any
        ]
      ]
    ]
  ]
}
object ChainablePrototype {
  
  inline def apply(
    $: /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof $ */ js.Any
    ] => ChainablePromiseElement[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof $ */ js.Any
        ]
      ]
    ],
    DollarDollar: /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof $$ */ js.Any
    ] => ChainablePromiseArray[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof $$ */ js.Any
        ]
      ]
    ],
    custom$: /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$ */ js.Any
    ] => ChainablePromiseElement[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$ */ js.Any
        ]
      ]
    ],
    customDollarDollar: /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$$ */ js.Any
    ] => ChainablePromiseArray[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$$ */ js.Any
        ]
      ]
    ],
    react$: /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof react$ */ js.Any
    ] => ChainablePromiseElement[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof react$ */ js.Any
        ]
      ]
    ],
    reactDollarDollar: /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof react$$ */ js.Any
    ] => ChainablePromiseArray[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof react$$ */ js.Any
        ]
      ]
    ],
    shadow$: /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$ */ js.Any
    ] => ChainablePromiseElement[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$ */ js.Any
        ]
      ]
    ],
    shadowDollarDollar: /* args */ Parameters[
      /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$$ */ js.Any
    ] => ChainablePromiseArray[
      ThenArg[
        ReturnType[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$$ */ js.Any
        ]
      ]
    ]
  ): ChainablePrototype = {
    val __obj = js.Dynamic.literal($ = js.Any.fromFunction1($), custom$ = js.Any.fromFunction1(custom$), react$ = js.Any.fromFunction1(react$), shadow$ = js.Any.fromFunction1(shadow$))
    __obj.updateDynamic("$$")(js.Any.fromFunction1(DollarDollar))
    __obj.updateDynamic("custom$$")(js.Any.fromFunction1(customDollarDollar))
    __obj.updateDynamic("react$$")(js.Any.fromFunction1(reactDollarDollar))
    __obj.updateDynamic("shadow$$")(js.Any.fromFunction1(shadowDollarDollar))
    __obj.asInstanceOf[ChainablePrototype]
  }
  
  extension [Self <: ChainablePrototype](x: Self) {
    
    inline def set$(
      value: /* args */ Parameters[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof $ */ js.Any
        ] => ChainablePromiseElement[
          ThenArg[
            ReturnType[
              /* import warning: importer.ImportType#apply Failed type conversion: typeof $ */ js.Any
            ]
          ]
        ]
    ): Self = StObject.set(x, "$", js.Any.fromFunction1(value))
    
    inline def setCustom$(
      value: /* args */ Parameters[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$ */ js.Any
        ] => ChainablePromiseElement[
          ThenArg[
            ReturnType[
              /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$ */ js.Any
            ]
          ]
        ]
    ): Self = StObject.set(x, "custom$", js.Any.fromFunction1(value))
    
    inline def setCustomDollarDollar(
      value: /* args */ Parameters[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$$ */ js.Any
        ] => ChainablePromiseArray[
          ThenArg[
            ReturnType[
              /* import warning: importer.ImportType#apply Failed type conversion: typeof custom$$ */ js.Any
            ]
          ]
        ]
    ): Self = StObject.set(x, "custom$$", js.Any.fromFunction1(value))
    
    inline def setDollarDollar(
      value: /* args */ Parameters[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof $$ */ js.Any
        ] => ChainablePromiseArray[
          ThenArg[
            ReturnType[
              /* import warning: importer.ImportType#apply Failed type conversion: typeof $$ */ js.Any
            ]
          ]
        ]
    ): Self = StObject.set(x, "$$", js.Any.fromFunction1(value))
    
    inline def setReact$(
      value: /* args */ Parameters[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof react$ */ js.Any
        ] => ChainablePromiseElement[
          ThenArg[
            ReturnType[
              /* import warning: importer.ImportType#apply Failed type conversion: typeof react$ */ js.Any
            ]
          ]
        ]
    ): Self = StObject.set(x, "react$", js.Any.fromFunction1(value))
    
    inline def setReactDollarDollar(
      value: /* args */ Parameters[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof react$$ */ js.Any
        ] => ChainablePromiseArray[
          ThenArg[
            ReturnType[
              /* import warning: importer.ImportType#apply Failed type conversion: typeof react$$ */ js.Any
            ]
          ]
        ]
    ): Self = StObject.set(x, "react$$", js.Any.fromFunction1(value))
    
    inline def setShadow$(
      value: /* args */ Parameters[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$ */ js.Any
        ] => ChainablePromiseElement[
          ThenArg[
            ReturnType[
              /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$ */ js.Any
            ]
          ]
        ]
    ): Self = StObject.set(x, "shadow$", js.Any.fromFunction1(value))
    
    inline def setShadowDollarDollar(
      value: /* args */ Parameters[
          /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$$ */ js.Any
        ] => ChainablePromiseArray[
          ThenArg[
            ReturnType[
              /* import warning: importer.ImportType#apply Failed type conversion: typeof shadow$$ */ js.Any
            ]
          ]
        ]
    ): Self = StObject.set(x, "shadow$$", js.Any.fromFunction1(value))
  }
}
