package typingsJapgolly.officeUiFabricReact.libIndexDotbundleMod

import typingsJapgolly.uifabricUtilities.anon.Delay
import typingsJapgolly.uifabricUtilities.libDelayedRenderMod.IDelayedRenderProps
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("office-ui-fabric-react/lib/index.bundle", "DelayedRender")
@js.native
open class DelayedRender protected ()
  extends typingsJapgolly.officeUiFabricReact.mod.DelayedRender {
  def this(props: IDelayedRenderProps) = this()
}
/* static members */
object DelayedRender {
  
  @JSImport("office-ui-fabric-react/lib/index.bundle", "DelayedRender")
  @js.native
  val ^ : js.Any = js.native
  
  @JSImport("office-ui-fabric-react/lib/index.bundle", "DelayedRender.defaultProps")
  @js.native
  def defaultProps: Delay = js.native
  inline def defaultProps_=(x: Delay): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("defaultProps")(x.asInstanceOf[js.Any])
}
