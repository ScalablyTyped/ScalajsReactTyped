package typingsJapgolly.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait RequestBodyContentApplicationjsonKeyReadonly extends StObject {
  
  var parameters: `76`
  
  var requestBody: ContentApplicationjsonKeyReadonly
  
  var responses: `201ContentApplicationjsonAddedby`
}
object RequestBodyContentApplicationjsonKeyReadonly {
  
  inline def apply(
    parameters: `76`,
    requestBody: ContentApplicationjsonKeyReadonly,
    responses: `201ContentApplicationjsonAddedby`
  ): RequestBodyContentApplicationjsonKeyReadonly = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], requestBody = requestBody.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[RequestBodyContentApplicationjsonKeyReadonly]
  }
  
  extension [Self <: RequestBodyContentApplicationjsonKeyReadonly](x: Self) {
    
    inline def setParameters(value: `76`): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setRequestBody(value: ContentApplicationjsonKeyReadonly): Self = StObject.set(x, "requestBody", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `201ContentApplicationjsonAddedby`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
