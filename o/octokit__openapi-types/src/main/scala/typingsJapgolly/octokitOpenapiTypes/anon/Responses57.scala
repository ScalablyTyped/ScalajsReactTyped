package typingsJapgolly.octokitOpenapiTypes.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait Responses57 extends StObject {
  
  var parameters: PathTeamslugQueryPage
  
  var responses: `57`
}
object Responses57 {
  
  inline def apply(parameters: PathTeamslugQueryPage, responses: `57`): Responses57 = {
    val __obj = js.Dynamic.literal(parameters = parameters.asInstanceOf[js.Any], responses = responses.asInstanceOf[js.Any])
    __obj.asInstanceOf[Responses57]
  }
  
  extension [Self <: Responses57](x: Self) {
    
    inline def setParameters(value: PathTeamslugQueryPage): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setResponses(value: `57`): Self = StObject.set(x, "responses", value.asInstanceOf[js.Any])
  }
}
