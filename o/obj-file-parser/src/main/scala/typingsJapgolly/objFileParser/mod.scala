package typingsJapgolly.objFileParser
...
object mod {
  ...
  object ObjFileParser {
    ...
    inline def apply(parse: CallbackTo[ObjFile]): ObjFileParser = {
      val __obj = js.Dynamic.literal(parse = parse.toJsFn)
      __obj.asInstanceOf[ObjFileParser]
    }
    
    extension [Self <: ObjFileParser](x: Self) {
      
      inline def setParse(value: CallbackTo[ObjFile]): Self = StObject.set(x, "parse", value.toJsFn)
    }
  }
