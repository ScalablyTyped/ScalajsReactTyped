package typingsJapgolly.ontime
...
object mod {
  ...
  object OntimeReport {
    ...
    inline def apply(cancel: Callback, done: Callback): OntimeReport = {
      val __obj = js.Dynamic.literal(cancel = cancel.toJsFn, done = done.toJsFn)
