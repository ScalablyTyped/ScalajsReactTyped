package typingsJapgolly.oclazyload

import japgolly.scalajs.react.Callback
import typingsJapgolly.angular.mod.IPromise
import typingsJapgolly.angular.mod.IRequestShortcutConfig
...
object oc {
  ...
  object ILazyLoadProvider {
    ...
    inline def apply(config: IProviderConfig => Callback): ILazyLoadProvider = {
      val __obj = js.Dynamic.literal(config = js.Any.fromFunction1((t0: IProviderConfig) => config(t0).runNow()))
      __obj.asInstanceOf[ILazyLoadProvider]
    }
    
    extension [Self <: ILazyLoadProvider](x: Self) {
      
      inline def setConfig(value: IProviderConfig => Callback): Self = StObject.set(x, "config", js.Any.fromFunction1((t0: IProviderConfig) => value(t0).runNow()))
    }
  }
