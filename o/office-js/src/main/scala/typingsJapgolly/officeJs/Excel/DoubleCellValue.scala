...
import typingsJapgolly.officeJs.Excel.RangeValueType.double
...
trait DoubleCellValue extends StObject {
  ...
  /**
    ...
    * [Api set: ExcelApi 1.16]
    */
  var `type`: typingsJapgolly.officeJs.Excel.CellValueType.double | Double
}
object DoubleCellValue {
  
  inline def apply(basicValue: scala.Double, `type`: typingsJapgolly.officeJs.Excel.CellValueType.double | Double): DoubleCellValue = {
  ...
  extension [Self <: DoubleCellValue](x: Self) {
    ...
    inline def setType(value: typingsJapgolly.officeJs.Excel.CellValueType.double | Double): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
