...
import typingsJapgolly.openui5.anon.FeaturePath
...
object sapUiTestGherkinOpa5TestHarnessMod extends Shortcut {
  ...
  object opa5TestHarness {
    ...
    inline def apply(test: FeaturePath => Callback): opa5TestHarness = {
      val __obj = js.Dynamic.literal(test = js.Any.fromFunction1((t0: FeaturePath) => test(t0).runNow()))
      __obj.asInstanceOf[opa5TestHarness]
    }
    
    extension [Self <: opa5TestHarness](x: Self) {
      
      inline def setTest(value: FeaturePath => Callback): Self = StObject.set(x, "test", js.Any.fromFunction1((t0: FeaturePath) => value(t0).runNow()))
    }
  }
}
