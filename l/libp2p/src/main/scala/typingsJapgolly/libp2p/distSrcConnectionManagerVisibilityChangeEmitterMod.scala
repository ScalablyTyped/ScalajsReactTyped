...
object distSrcConnectionManagerVisibilityChangeEmitterMod {
  ...
  trait VisibilityChangeEmitterEvents extends StObject {
    ...
    var visibilityChange: CustomEvent
  }
  object VisibilityChangeEmitterEvents {
    
    inline def apply(visibilityChange: CustomEvent): VisibilityChangeEmitterEvents = {
    ...
    extension [Self <: VisibilityChangeEmitterEvents](x: Self) {
      ...
      inline def setVisibilityChange(value: CustomEvent): Self = StObject.set(x, "visibilityChange", value.asInstanceOf[js.Any])
    }
  }
}
