package typingsJapgolly.levelSupports

import typingsJapgolly.std.Record
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object anon {
  
  /* Inlined std.Partial<level-supports.level-supports.IManifest> */
  trait PartialIManifest extends StObject {
    
    var additionalMethods: js.UndefOr[Record[String, Boolean]] = js.undefined
    
    var clear: js.UndefOr[Boolean] = js.undefined
    
    var createIfMissing: js.UndefOr[Boolean] = js.undefined
    
    var deferredOpen: js.UndefOr[Boolean] = js.undefined
    
    var encodings: js.UndefOr[Record[String, Boolean]] = js.undefined
    
    var errorIfExists: js.UndefOr[Boolean] = js.undefined
    
    var events: js.UndefOr[Record[String, Boolean]] = js.undefined
    
    var getMany: js.UndefOr[Boolean] = js.undefined
    
    var iteratorAll: js.UndefOr[Boolean] = js.undefined
    
    var iteratorNextv: js.UndefOr[Boolean] = js.undefined
    
    var keyIterator: js.UndefOr[Boolean] = js.undefined
    
    var permanence: js.UndefOr[Boolean] = js.undefined
    
    var promises: js.UndefOr[Boolean] = js.undefined
    
    var seek: js.UndefOr[Boolean] = js.undefined
    
    var snapshots: js.UndefOr[Boolean] = js.undefined
    
    var status: js.UndefOr[Boolean] = js.undefined
    
    var streams: js.UndefOr[Boolean] = js.undefined
    
    var valueIterator: js.UndefOr[Boolean] = js.undefined
  }
  object PartialIManifest {
    
    inline def apply(): PartialIManifest = {
      val __obj = js.Dynamic.literal()
      __obj.asInstanceOf[PartialIManifest]
    }
    
    extension [Self <: PartialIManifest](x: Self) {
      
      inline def setAdditionalMethods(value: Record[String, Boolean]): Self = StObject.set(x, "additionalMethods", value.asInstanceOf[js.Any])
      
      inline def setAdditionalMethodsUndefined: Self = StObject.set(x, "additionalMethods", js.undefined)
      
      inline def setClear(value: Boolean): Self = StObject.set(x, "clear", value.asInstanceOf[js.Any])
      
      inline def setClearUndefined: Self = StObject.set(x, "clear", js.undefined)
      
      inline def setCreateIfMissing(value: Boolean): Self = StObject.set(x, "createIfMissing", value.asInstanceOf[js.Any])
      
      inline def setCreateIfMissingUndefined: Self = StObject.set(x, "createIfMissing", js.undefined)
      
      inline def setDeferredOpen(value: Boolean): Self = StObject.set(x, "deferredOpen", value.asInstanceOf[js.Any])
      
      inline def setDeferredOpenUndefined: Self = StObject.set(x, "deferredOpen", js.undefined)
      
      inline def setEncodings(value: Record[String, Boolean]): Self = StObject.set(x, "encodings", value.asInstanceOf[js.Any])
      
      inline def setEncodingsUndefined: Self = StObject.set(x, "encodings", js.undefined)
      
      inline def setErrorIfExists(value: Boolean): Self = StObject.set(x, "errorIfExists", value.asInstanceOf[js.Any])
      
      inline def setErrorIfExistsUndefined: Self = StObject.set(x, "errorIfExists", js.undefined)
      
      inline def setEvents(value: Record[String, Boolean]): Self = StObject.set(x, "events", value.asInstanceOf[js.Any])
      
      inline def setEventsUndefined: Self = StObject.set(x, "events", js.undefined)
      
      inline def setGetMany(value: Boolean): Self = StObject.set(x, "getMany", value.asInstanceOf[js.Any])
      
      inline def setGetManyUndefined: Self = StObject.set(x, "getMany", js.undefined)
      
      inline def setIteratorAll(value: Boolean): Self = StObject.set(x, "iteratorAll", value.asInstanceOf[js.Any])
      
      inline def setIteratorAllUndefined: Self = StObject.set(x, "iteratorAll", js.undefined)
      
      inline def setIteratorNextv(value: Boolean): Self = StObject.set(x, "iteratorNextv", value.asInstanceOf[js.Any])
      
      inline def setIteratorNextvUndefined: Self = StObject.set(x, "iteratorNextv", js.undefined)
      
      inline def setKeyIterator(value: Boolean): Self = StObject.set(x, "keyIterator", value.asInstanceOf[js.Any])
      
      inline def setKeyIteratorUndefined: Self = StObject.set(x, "keyIterator", js.undefined)
      
      inline def setPermanence(value: Boolean): Self = StObject.set(x, "permanence", value.asInstanceOf[js.Any])
      
      inline def setPermanenceUndefined: Self = StObject.set(x, "permanence", js.undefined)
      
      inline def setPromises(value: Boolean): Self = StObject.set(x, "promises", value.asInstanceOf[js.Any])
      
      inline def setPromisesUndefined: Self = StObject.set(x, "promises", js.undefined)
      
      inline def setSeek(value: Boolean): Self = StObject.set(x, "seek", value.asInstanceOf[js.Any])
      
      inline def setSeekUndefined: Self = StObject.set(x, "seek", js.undefined)
      
      inline def setSnapshots(value: Boolean): Self = StObject.set(x, "snapshots", value.asInstanceOf[js.Any])
      
      inline def setSnapshotsUndefined: Self = StObject.set(x, "snapshots", js.undefined)
      
      inline def setStatus(value: Boolean): Self = StObject.set(x, "status", value.asInstanceOf[js.Any])
      
      inline def setStatusUndefined: Self = StObject.set(x, "status", js.undefined)
      
      inline def setStreams(value: Boolean): Self = StObject.set(x, "streams", value.asInstanceOf[js.Any])
      
      inline def setStreamsUndefined: Self = StObject.set(x, "streams", js.undefined)
      
      inline def setValueIterator(value: Boolean): Self = StObject.set(x, "valueIterator", value.asInstanceOf[js.Any])
      
      inline def setValueIteratorUndefined: Self = StObject.set(x, "valueIterator", js.undefined)
    }
  }
}
