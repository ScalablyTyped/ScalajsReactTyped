...
object mod {
  ...
  object AttemptContext {
    ...
    inline def apply(abort: Callback, aborted: Boolean, attemptNum: Double, attemptsRemaining: Double): AttemptContext = {
      val __obj = js.Dynamic.literal(abort = abort.toJsFn, aborted = aborted.asInstanceOf[js.Any], attemptNum = attemptNum.asInstanceOf[js.Any], attemptsRemaining = attemptsRemaining.asInstanceOf[js.Any])
  ...
  object AttemptOptions {
    ...
    extension [Self <: AttemptOptions[?], T](x: Self & AttemptOptions[T]) {
      ...
      inline def setBeforeAttempt(value: (/* context */ AttemptContext, /* options */ AttemptOptions[T]) => Callback): Self = StObject.set(x, "beforeAttempt", js.Any.fromFunction2((t0: /* context */ AttemptContext, t1: /* options */ AttemptOptions[T]) => (value(t0, t1)).runNow()))
