...
import typingsJapgolly.libratoNode.anon.RequestCountKey
import typingsJapgolly.libratoNode.libratoNodeBooleans.`false`
import typingsJapgolly.libratoNode.libratoNodeBooleans.`true`
import typingsJapgolly.libratoNode.libratoNodeStrings.SIGINT
import typingsJapgolly.libratoNode.libratoNodeStrings.error
...
object mod {
  ...
  object LibratoRequestOptions {
    ...
    extension [Self <: LibratoRequestOptions](x: Self) {
      ...
      inline def setDelayStrategy(value: CallbackTo[Double]): Self = StObject.set(x, "delayStrategy", value.toJsFn)
