package typingsJapgolly.luminoMessaging
...
object mod {
  ...
  object IMessageHandler {
    ...
    inline def apply(processMessage: Message => Callback): IMessageHandler = {
      val __obj = js.Dynamic.literal(processMessage = js.Any.fromFunction1((t0: Message) => processMessage(t0).runNow()))
      __obj.asInstanceOf[IMessageHandler]
    }
    
    extension [Self <: IMessageHandler](x: Self) {
      
      inline def setProcessMessage(value: Message => Callback): Self = StObject.set(x, "processMessage", js.Any.fromFunction1((t0: Message) => value(t0).runNow()))
    }
  }
