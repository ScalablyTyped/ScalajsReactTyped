...
object typesRetroMod {
  ...
  object IRetroable {
    ...
    inline def apply[T](retro: CallbackTo[IIterator[T]]): IRetroable[T] = {
      val __obj = js.Dynamic.literal(retro = retro.toJsFn)
      __obj.asInstanceOf[IRetroable[T]]
    }
    
    extension [Self <: IRetroable[?], T](x: Self & IRetroable[T]) {
      
      inline def setRetro(value: CallbackTo[IIterator[T]]): Self = StObject.set(x, "retro", value.toJsFn)
    }
  }
