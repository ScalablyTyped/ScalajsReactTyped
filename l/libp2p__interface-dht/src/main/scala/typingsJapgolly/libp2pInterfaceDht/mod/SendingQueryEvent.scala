...
import typingsJapgolly.libp2pInterfaceDht.libp2pInterfaceDhtStrings.SENDING_QUERY
...
trait SendingQueryEvent
  ...
  var `type`: typingsJapgolly.libp2pInterfaceDht.mod.EventTypes.SENDING_QUERY
}
object SendingQueryEvent {
  ...
  inline def apply(
    ...
    to: PeerId,
    `type`: typingsJapgolly.libp2pInterfaceDht.mod.EventTypes.SENDING_QUERY
  ): SendingQueryEvent = {
  ...
  extension [Self <: SendingQueryEvent](x: Self) {
    ...
    inline def setType(value: typingsJapgolly.libp2pInterfaceDht.mod.EventTypes.SENDING_QUERY): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
