package typingsJapgolly.ldapjsClient
...
object mod {
  ...
  object LdapClient {
    ...
    inline def apply(
      ...
      unbind: CallbackTo[js.Promise[Any]]
    ): LdapClient = {
      val __obj = js.Dynamic.literal(add = js.Any.fromFunction2(add), bind = js.Any.fromFunction2(bind), del = js.Any.fromFunction1(del), destroy = destroy.toJsFn, modify = js.Any.fromFunction2(modify), modifyDN = js.Any.fromFunction2(modifyDN), search = js.Any.fromFunction2(search), unbind = unbind.toJsFn)
    ...
    extension [Self <: LdapClient](x: Self) {
      ...
      inline def setUnbind(value: CallbackTo[js.Promise[Any]]): Self = StObject.set(x, "unbind", value.toJsFn)
    }
  }
