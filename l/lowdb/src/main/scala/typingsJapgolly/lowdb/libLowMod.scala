...
object libLowMod {
  ...
  object Adapter {
    ...
    inline def apply[T](read: CallbackTo[js.Promise[T | Null]], write: T => js.Promise[Unit]): Adapter[T] = {
      val __obj = js.Dynamic.literal(read = read.toJsFn, write = js.Any.fromFunction1(write))
      __obj.asInstanceOf[Adapter[T]]
    }
    
    extension [Self <: Adapter[?], T](x: Self & Adapter[T]) {
      
      inline def setRead(value: CallbackTo[js.Promise[T | Null]]): Self = StObject.set(x, "read", value.toJsFn)
