...
object mod {
  ...
  type IProduce = typingsJapgolly.immer.distTypesTypesExternalMod.IProduce
  ...
  object Options {
    ...
    extension [Self <: Options[?, ?], S, A](x: Self & (Options[S, A])) {
      ...
      inline def setInitializer(value: /* store */ Store[S, A] => Callback): Self = StObject.set(x, "initializer", js.Any.fromFunction1((t0: /* store */ Store[S, A]) => value(t0).runNow()))
