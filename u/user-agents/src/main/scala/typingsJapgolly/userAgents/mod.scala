...
import typingsJapgolly.userAgents.anon.PartialData
...
object mod {
  ...
  object UserAgent {
    ...
    inline def apply(appName: String, data: Data, random: CallbackTo[UserAgent]): UserAgent = {
      val __obj = js.Dynamic.literal(appName = appName.asInstanceOf[js.Any], data = data.asInstanceOf[js.Any], random = random.toJsFn)
    ...
    extension [Self <: UserAgent](x: Self) {
      ...
      inline def setRandom(value: CallbackTo[UserAgent]): Self = StObject.set(x, "random", value.toJsFn)
    }
  }
}
