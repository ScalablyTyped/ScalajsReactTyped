...
import typingsJapgolly.uirouterAngularjs.anon.Instantiable
...
object libInterfaceMod {
  ...
  object Ng1Controller {
    ...
    inline def apply(
      ...
      uiOnParamsChanged: (Any, Transition) => Callback
    ): Ng1Controller = {
      val __obj = js.Dynamic.literal($onInit = $onInit.toJsFn, uiCanExit = js.Any.fromFunction1(uiCanExit), uiOnParamsChanged = js.Any.fromFunction2((t0: Any, t1: Transition) => (uiOnParamsChanged(t0, t1)).runNow()))
    ...
    extension [Self <: Ng1Controller](x: Self) {
      ...
      inline def setUiOnParamsChanged(value: (Any, Transition) => Callback): Self = StObject.set(x, "uiOnParamsChanged", js.Any.fromFunction2((t0: Any, t1: Transition) => (value(t0, t1)).runNow()))
    }
  }
