...
object CheckIsSupportSoterAuthenticationOptions {
  ...
  def apply(
    ...
    success: /* result */ CheckIsSupportSoterAuthenticationRes => Callback = null
  ): CheckIsSupportSoterAuthenticationOptions = {
    val __obj = js.Dynamic.literal()
    complete.foreach(p => __obj.updateDynamic("complete")(p.toJsFn))
    fail.foreach(p => __obj.updateDynamic("fail")(p.toJsFn))
    if (success != null) __obj.updateDynamic("success")(js.Any.fromFunction1((t0: /* result */ typingsJapgolly.uniApp.CheckIsSupportSoterAuthenticationRes) => success(t0).runNow()))
