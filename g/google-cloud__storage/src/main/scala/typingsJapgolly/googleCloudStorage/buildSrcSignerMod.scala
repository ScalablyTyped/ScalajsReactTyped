...
import typingsJapgolly.googleCloudStorage.googleCloudStorageStrings.GET
import typingsJapgolly.googleCloudStorage.googleCloudStorageStrings.POST
import typingsJapgolly.googleCloudStorage.googleCloudStorageStrings.PUT
...
object buildSrcSignerMod {
  ...
  object SignerExceptionMessages extends StObject {
    ...
    /* "The accessible at date provided was invalid." */ val ACCESSIBLE_DATE_INVALID: typingsJapgolly.googleCloudStorage.buildSrcSignerMod.SignerExceptionMessages.ACCESSIBLE_DATE_INVALID & String = js.native
    
    @js.native
    sealed trait EXPIRATION_BEFORE_ACCESSIBLE_DATE
      extends StObject
         with SignerExceptionMessages
    /* "An expiration date cannot be before accessible date." */ val EXPIRATION_BEFORE_ACCESSIBLE_DATE: typingsJapgolly.googleCloudStorage.buildSrcSignerMod.SignerExceptionMessages.EXPIRATION_BEFORE_ACCESSIBLE_DATE & String = js.native
    
    @js.native
    sealed trait X_GOOG_CONTENT_SHA256
      extends StObject
         with SignerExceptionMessages
    /* "The header X-Goog-Content-SHA256 must be a hexadecimal string." */ val X_GOOG_CONTENT_SHA256: typingsJapgolly.googleCloudStorage.buildSrcSignerMod.SignerExceptionMessages.X_GOOG_CONTENT_SHA256 & String = js.native
  }
  ...
  object AuthClient {
    ...
    inline def apply(getCredentials: CallbackTo[js.Promise[GetCredentialsResponse]], sign: String => js.Promise[String]): AuthClient = {
      val __obj = js.Dynamic.literal(getCredentials = getCredentials.toJsFn, sign = js.Any.fromFunction1(sign))
      __obj.asInstanceOf[AuthClient]
    }
    
    extension [Self <: AuthClient](x: Self) {
      
      inline def setGetCredentials(value: CallbackTo[js.Promise[GetCredentialsResponse]]): Self = StObject.set(x, "getCredentials", value.toJsFn)
