...
import typingsJapgolly.grpcGrpcJs.buildSrcCallStreamMod.Http2CallStream
...
import typingsJapgolly.grpcGrpcJs.buildSrcSubchannelInterfaceMod.ConnectivityStateListener
...
object buildSrcSubchannelMod {
  ...
  object SubchannelCallStatsTracker {
    ...
    inline def apply(addMessageReceived: Callback, addMessageSent: Callback): SubchannelCallStatsTracker = {
      val __obj = js.Dynamic.literal(addMessageReceived = addMessageReceived.toJsFn, addMessageSent = addMessageSent.toJsFn)
      __obj.asInstanceOf[SubchannelCallStatsTracker]
    }
    
    extension [Self <: SubchannelCallStatsTracker](x: Self) {
      
      inline def setAddMessageReceived(value: Callback): Self = StObject.set(x, "addMessageReceived", value.toJsFn)
      
      inline def setAddMessageSent(value: Callback): Self = StObject.set(x, "addMessageSent", value.toJsFn)
    }
  }
}
