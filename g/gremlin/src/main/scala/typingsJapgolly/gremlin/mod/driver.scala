...
import typingsJapgolly.gremlin.mod.process.Bytecode
import typingsJapgolly.gremlin.mod.process.Traversal
import typingsJapgolly.gremlin.mod.process.TraversalStrategy
...
object driver {
  ...
  object RemoteConnection {
    ...
    inline def apply(
      ...
      submit: Bytecode => js.Promise[Any]
    ): RemoteConnection = {
      val __obj = js.Dynamic.literal(close = close.toJsFn, open = open.toJsFn, submit = js.Any.fromFunction1(submit))
  ...
  object ResultSet {
    ...
    inline def apply(first: CallbackTo[Any], toArray: CallbackTo[js.Array[Any]]): ResultSet = {
      val __obj = js.Dynamic.literal(first = first.toJsFn, toArray = toArray.toJsFn)
