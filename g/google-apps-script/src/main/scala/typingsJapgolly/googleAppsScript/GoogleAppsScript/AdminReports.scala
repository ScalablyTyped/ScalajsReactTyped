...
import typingsJapgolly.googleAppsScript.GoogleAppsScript.AdminReports.Schema.Activities
...
object AdminReports {
  ...
  inline def apply(newChannel: CallbackTo[Channel]): AdminReports = {
    val __obj = js.Dynamic.literal(newChannel = newChannel.toJsFn)
