...
@js.native
open class AbstractPlatformObject protected () extends AbstractSelfableACLObject {
  def this(platform: Any, `object`: Any) = this()
  
  def copy(target: Any, asynchronous: Any, config: Any): Any = js.native
  
  def exportArchive(settings: Any): Any = js.native
  
  def getCluster(): Cluster = js.native
  
  def getRepository(): Repository = js.native
  
  def importArchive(settings: Any, reportFn: Any): Any = js.native
}
