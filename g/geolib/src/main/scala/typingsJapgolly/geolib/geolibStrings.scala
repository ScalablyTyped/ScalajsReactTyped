package typingsJapgolly.geolib

import typingsJapgolly.geolib.typesMod.AltitudeKeys
import typingsJapgolly.geolib.typesMod.LatitudeKeys
import typingsJapgolly.geolib.typesMod.LongitudeKeys
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

object geolibStrings {
  @js.native
  sealed trait E extends js.Object
  
  @js.native
  sealed trait ENE extends js.Object
  
  @js.native
  sealed trait ESE extends js.Object
  
  @js.native
  sealed trait N extends js.Object
  
  @js.native
  sealed trait NE extends js.Object
  
  @js.native
  sealed trait NNE extends js.Object
  
  @js.native
  sealed trait NNW extends js.Object
  
  @js.native
  sealed trait NW extends js.Object
  
  @js.native
  sealed trait S extends js.Object
  
  @js.native
  sealed trait SE extends js.Object
  
  @js.native
  sealed trait SSE extends js.Object
  
  @js.native
  sealed trait SSW extends js.Object
  
  @js.native
  sealed trait SW extends js.Object
  
  @js.native
  sealed trait W extends js.Object
  
  @js.native
  sealed trait WNW extends js.Object
  
  @js.native
  sealed trait WSW extends js.Object
  
  @js.native
  sealed trait alt extends AltitudeKeys
  
  @js.native
  sealed trait altitude extends AltitudeKeys
  
  @js.native
  sealed trait elev extends AltitudeKeys
  
  @js.native
  sealed trait elevation extends AltitudeKeys
  
  @js.native
  sealed trait lat extends LatitudeKeys
  
  @js.native
  sealed trait latitude extends LatitudeKeys
  
  @js.native
  sealed trait lng extends LongitudeKeys
  
  @js.native
  sealed trait lon extends LongitudeKeys
  
  @js.native
  sealed trait longitude extends LongitudeKeys
  
  @scala.inline
  def E: E = "E".asInstanceOf[E]
  @scala.inline
  def ENE: ENE = "ENE".asInstanceOf[ENE]
  @scala.inline
  def ESE: ESE = "ESE".asInstanceOf[ESE]
  @scala.inline
  def N: N = "N".asInstanceOf[N]
  @scala.inline
  def NE: NE = "NE".asInstanceOf[NE]
  @scala.inline
  def NNE: NNE = "NNE".asInstanceOf[NNE]
  @scala.inline
  def NNW: NNW = "NNW".asInstanceOf[NNW]
  @scala.inline
  def NW: NW = "NW".asInstanceOf[NW]
  @scala.inline
  def S: S = "S".asInstanceOf[S]
  @scala.inline
  def SE: SE = "SE".asInstanceOf[SE]
  @scala.inline
  def SSE: SSE = "SSE".asInstanceOf[SSE]
  @scala.inline
  def SSW: SSW = "SSW".asInstanceOf[SSW]
  @scala.inline
  def SW: SW = "SW".asInstanceOf[SW]
  @scala.inline
  def W: W = "W".asInstanceOf[W]
  @scala.inline
  def WNW: WNW = "WNW".asInstanceOf[WNW]
  @scala.inline
  def WSW: WSW = "WSW".asInstanceOf[WSW]
  @scala.inline
  def alt: alt = "alt".asInstanceOf[alt]
  @scala.inline
  def altitude: altitude = "altitude".asInstanceOf[altitude]
  @scala.inline
  def elev: elev = "elev".asInstanceOf[elev]
  @scala.inline
  def elevation: elevation = "elevation".asInstanceOf[elevation]
  @scala.inline
  def lat: lat = "lat".asInstanceOf[lat]
  @scala.inline
  def latitude: latitude = "latitude".asInstanceOf[latitude]
  @scala.inline
  def lng: lng = "lng".asInstanceOf[lng]
  @scala.inline
  def lon: lon = "lon".asInstanceOf[lon]
  @scala.inline
  def longitude: longitude = "longitude".asInstanceOf[longitude]
}

