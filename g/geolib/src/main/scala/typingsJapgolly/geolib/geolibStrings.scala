package typingsJapgolly.geolib

import typingsJapgolly.geolib.esTypesMod.AltitudeKeys
import typingsJapgolly.geolib.esTypesMod.LatitudeKeys
import typingsJapgolly.geolib.esTypesMod.LongitudeKeys
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object geolibStrings {
  
  @js.native
  sealed trait E extends StObject
  inline def E: E = "E".asInstanceOf[E]
  
  @js.native
  sealed trait ENE extends StObject
  inline def ENE: ENE = "ENE".asInstanceOf[ENE]
  
  @js.native
  sealed trait ESE extends StObject
  inline def ESE: ESE = "ESE".asInstanceOf[ESE]
  
  @js.native
  sealed trait N extends StObject
  inline def N: N = "N".asInstanceOf[N]
  
  @js.native
  sealed trait NE extends StObject
  inline def NE: NE = "NE".asInstanceOf[NE]
  
  @js.native
  sealed trait NNE extends StObject
  inline def NNE: NNE = "NNE".asInstanceOf[NNE]
  
  @js.native
  sealed trait NNW extends StObject
  inline def NNW: NNW = "NNW".asInstanceOf[NNW]
  
  @js.native
  sealed trait NW extends StObject
  inline def NW: NW = "NW".asInstanceOf[NW]
  
  @js.native
  sealed trait S extends StObject
  inline def S: S = "S".asInstanceOf[S]
  
  @js.native
  sealed trait SE extends StObject
  inline def SE: SE = "SE".asInstanceOf[SE]
  
  @js.native
  sealed trait SSE extends StObject
  inline def SSE: SSE = "SSE".asInstanceOf[SSE]
  
  @js.native
  sealed trait SSW extends StObject
  inline def SSW: SSW = "SSW".asInstanceOf[SSW]
  
  @js.native
  sealed trait SW extends StObject
  inline def SW: SW = "SW".asInstanceOf[SW]
  
  @js.native
  sealed trait W extends StObject
  inline def W: W = "W".asInstanceOf[W]
  
  @js.native
  sealed trait WNW extends StObject
  inline def WNW: WNW = "WNW".asInstanceOf[WNW]
  
  @js.native
  sealed trait WSW extends StObject
  inline def WSW: WSW = "WSW".asInstanceOf[WSW]
  
  @js.native
  sealed trait alt
    extends StObject
       with AltitudeKeys
  inline def alt: alt = "alt".asInstanceOf[alt]
  
  @js.native
  sealed trait altitude
    extends StObject
       with AltitudeKeys
  inline def altitude: altitude = "altitude".asInstanceOf[altitude]
  
  @js.native
  sealed trait elev
    extends StObject
       with AltitudeKeys
  inline def elev: elev = "elev".asInstanceOf[elev]
  
  @js.native
  sealed trait elevation
    extends StObject
       with AltitudeKeys
  inline def elevation: elevation = "elevation".asInstanceOf[elevation]
  
  @js.native
  sealed trait lat
    extends StObject
       with LatitudeKeys
  inline def lat: lat = "lat".asInstanceOf[lat]
  
  @js.native
  sealed trait latitude
    extends StObject
       with LatitudeKeys
  inline def latitude: latitude = "latitude".asInstanceOf[latitude]
  
  @js.native
  sealed trait lng
    extends StObject
       with LongitudeKeys
  inline def lng: lng = "lng".asInstanceOf[lng]
  
  @js.native
  sealed trait lon
    extends StObject
       with LongitudeKeys
  inline def lon: lon = "lon".asInstanceOf[lon]
  
  @js.native
  sealed trait longitude
    extends StObject
       with LongitudeKeys
  inline def longitude: longitude = "longitude".asInstanceOf[longitude]
}
