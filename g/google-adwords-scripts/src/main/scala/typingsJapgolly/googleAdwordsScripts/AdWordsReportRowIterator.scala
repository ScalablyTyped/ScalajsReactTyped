package typingsJapgolly.googleAdwordsScripts

import japgolly.scalajs.react.CallbackTo
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait AdWordsReportRowIterator extends StObject {
  
  def hasNext(): Boolean
  
  def next(): AdWordsReportRow
}
object AdWordsReportRowIterator {
  
  inline def apply(hasNext: CallbackTo[Boolean], next: CallbackTo[AdWordsReportRow]): AdWordsReportRowIterator = {
    val __obj = js.Dynamic.literal(hasNext = hasNext.toJsFn, next = next.toJsFn)
    __obj.asInstanceOf[AdWordsReportRowIterator]
  }
  
  extension [Self <: AdWordsReportRowIterator](x: Self) {
    
    inline def setHasNext(value: CallbackTo[Boolean]): Self = StObject.set(x, "hasNext", value.toJsFn)
    
    inline def setNext(value: CallbackTo[AdWordsReportRow]): Self = StObject.set(x, "next", value.toJsFn)
  }
}
