...
import typingsJapgolly.grunt.anon.Custom
...
object util {
  ...
  object ISpawnedChild {
    ...
    inline def apply(forever: Callback, kill: Callback, once: Callback, start: Callback): ISpawnedChild = {
      val __obj = js.Dynamic.literal(forever = forever.toJsFn, kill = kill.toJsFn, once = once.toJsFn, start = start.toJsFn)
    ...
    extension [Self <: ISpawnedChild](x: Self) {
      ...
      inline def setOnce(value: Callback): Self = StObject.set(x, "once", value.toJsFn)
