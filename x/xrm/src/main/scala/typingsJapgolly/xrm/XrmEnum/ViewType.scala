package typingsJapgolly.xrm.XrmEnum

import typingsJapgolly.xrm.xrmNumbers.`1039`
import typingsJapgolly.xrm.xrmNumbers.`4230`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/**
  * An enumeration for view types.
  */
/* Rewritten from type alias, can be one of: 
  - typingsJapgolly.xrm.xrmNumbers.`1039`
  - typingsJapgolly.xrm.xrmNumbers.`4230`
*/
trait ViewType extends js.Object

object ViewType {
  @scala.inline
  def SystemView: `1039` = this.cast(1039)
  @scala.inline
  def UserView: `4230` = this.cast(4230)
  @scala.inline
  /* private */ def cast[T](in: js.Any): T = in.asInstanceOf[T]
}

