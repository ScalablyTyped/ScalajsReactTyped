package typingsJapgolly.xadesjs

import typingsJapgolly.xmlCore.mod.XmlCollection
import typingsJapgolly.xmlCore.mod.XmlObject
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object buildTypesSrcXmlXmlBaseMod {
  
  /* note: abstract class */ @JSImport("xadesjs/build/types/src/xml/xml_base", "XadesCollection")
  @js.native
  open class XadesCollection[I /* <: XadesObject */] () extends XmlCollection[I] {
    def this(properties: js.Object) = this()
  }
  
  /* note: abstract class */ @JSImport("xadesjs/build/types/src/xml/xml_base", "XadesObject")
  @js.native
  open class XadesObject () extends XmlObject {
    def this(properties: js.Object) = this()
  }
}
