...
object interceptReads extends js.Object {
  ...
  def apply[V](observableSet: typingsJapgolly.mobx.internalMod.ObservableSet[V], handler: ReadInterceptor[V]): Lambda = js.native
  def apply[T](value: IObservableValue[T], handler: ReadInterceptor[T]): Lambda = js.native
  def apply[K, V](observableMap: typingsJapgolly.mobx.internalMod.ObservableMap[K, V], handler: ReadInterceptor[V]): Lambda = js.native
}
