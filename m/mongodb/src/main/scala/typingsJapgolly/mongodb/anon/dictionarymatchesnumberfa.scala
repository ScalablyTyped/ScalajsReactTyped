...
/* Inlined {  dictionary matches :number,   fast-path pages deleted :number,   internal page key bytes discarded using suffix compression :number,   internal page multi-block writes :number,   internal-page overflow keys :number,   leaf page key bytes discarded using prefix compression :number,   leaf page multi-block writes :number,   leaf-page overflow keys :number,   maximum blocks required for a page :number,   overflow values written :number,   page checksum matches :number,   page reconciliation calls :number,   page reconciliation calls for eviction :number,   pages deleted :number} & bson.bson.Document */
trait dictionarymatchesnumberfa
  extends StObject
...
object dictionarymatchesnumberfa {
