package typingsJapgolly.morphdom.mod

import org.scalajs.dom.raw.Node
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("morphdom", JSImport.Namespace)
@js.native
object ^ extends js.Object {
  def apply(fromNode: Node, toNode: String): Unit = js.native
  def apply(fromNode: Node, toNode: String, options: MorphDomOptions): Unit = js.native
  def apply(fromNode: Node, toNode: typingsJapgolly.std.Node): Unit = js.native
  def apply(fromNode: Node, toNode: typingsJapgolly.std.Node, options: MorphDomOptions): Unit = js.native
}

