package typingsJapgolly.makePlural.anon

import typingsJapgolly.makePlural.makePluralStrings.`0Dot0`
import typingsJapgolly.makePlural.makePluralStrings.`1000000Dot0`
import typingsJapgolly.makePlural.makePluralStrings.`1000000`
import typingsJapgolly.makePlural.makePluralStrings.`100000Dot0`
import typingsJapgolly.makePlural.makePluralStrings.`100000`
import typingsJapgolly.makePlural.makePluralStrings.`10000Dot0`
import typingsJapgolly.makePlural.makePluralStrings.`10000`
import typingsJapgolly.makePlural.makePluralStrings.`1000Dot0`
import typingsJapgolly.makePlural.makePluralStrings.`1000`
import typingsJapgolly.makePlural.makePluralStrings.`1001`
import typingsJapgolly.makePlural.makePluralStrings.`100Dot0`
import typingsJapgolly.makePlural.makePluralStrings.`100`
import typingsJapgolly.makePlural.makePluralStrings.`101`
import typingsJapgolly.makePlural.makePluralStrings.`10Dot0`
import typingsJapgolly.makePlural.makePluralStrings.`1Dot0`
import typingsJapgolly.makePlural.makePluralStrings.`1Dot5`
import typingsJapgolly.makePlural.makePluralStrings.`40`
import typingsJapgolly.makePlural.makePluralStrings.`50`
import typingsJapgolly.makePlural.makePluralStrings.`60`
import typingsJapgolly.makePlural.makePluralStrings.`70`
import typingsJapgolly.makePlural.makePluralStrings.`80`
import typingsJapgolly.makePlural.makePluralStrings.`90`
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait `9` extends StObject {
  
  var many: js.Tuple13[
    typingsJapgolly.makePlural.makePluralStrings.`20`, 
    typingsJapgolly.makePlural.makePluralStrings.`30`, 
    `40`, 
    `50`, 
    `60`, 
    `70`, 
    `80`, 
    `90`, 
    `100`, 
    `1000`, 
    `10000`, 
    `100000`, 
    `1000000`
  ]
  
  var one: js.Array[typingsJapgolly.makePlural.makePluralStrings.`1`]
  
  var other: js.Tuple14[
    typingsJapgolly.makePlural.makePluralStrings.`0`, 
    typingsJapgolly.makePlural.makePluralStrings.`3`, 
    typingsJapgolly.makePlural.makePluralStrings.`17`, 
    `101`, 
    `1001`, 
    `0Dot0`, 
    `1Dot0`, 
    `1Dot5`, 
    `10Dot0`, 
    `100Dot0`, 
    `1000Dot0`, 
    `10000Dot0`, 
    `100000Dot0`, 
    `1000000Dot0`
  ]
  
  var two: js.Array[typingsJapgolly.makePlural.makePluralStrings.`2`]
}
object `9` {
  
  inline def apply(
    many: js.Tuple13[
      typingsJapgolly.makePlural.makePluralStrings.`20`, 
      typingsJapgolly.makePlural.makePluralStrings.`30`, 
      `40`, 
      `50`, 
      `60`, 
      `70`, 
      `80`, 
      `90`, 
      `100`, 
      `1000`, 
      `10000`, 
      `100000`, 
      `1000000`
    ],
    one: js.Array[typingsJapgolly.makePlural.makePluralStrings.`1`],
    other: js.Tuple14[
      typingsJapgolly.makePlural.makePluralStrings.`0`, 
      typingsJapgolly.makePlural.makePluralStrings.`3`, 
      typingsJapgolly.makePlural.makePluralStrings.`17`, 
      `101`, 
      `1001`, 
      `0Dot0`, 
      `1Dot0`, 
      `1Dot5`, 
      `10Dot0`, 
      `100Dot0`, 
      `1000Dot0`, 
      `10000Dot0`, 
      `100000Dot0`, 
      `1000000Dot0`
    ],
    two: js.Array[typingsJapgolly.makePlural.makePluralStrings.`2`]
  ): `9` = {
    val __obj = js.Dynamic.literal(many = many.asInstanceOf[js.Any], one = one.asInstanceOf[js.Any], other = other.asInstanceOf[js.Any], two = two.asInstanceOf[js.Any])
    __obj.asInstanceOf[`9`]
  }
  
  extension [Self <: `9`](x: Self) {
    
    inline def setMany(
      value: js.Tuple13[
          typingsJapgolly.makePlural.makePluralStrings.`20`, 
          typingsJapgolly.makePlural.makePluralStrings.`30`, 
          `40`, 
          `50`, 
          `60`, 
          `70`, 
          `80`, 
          `90`, 
          `100`, 
          `1000`, 
          `10000`, 
          `100000`, 
          `1000000`
        ]
    ): Self = StObject.set(x, "many", value.asInstanceOf[js.Any])
    
    inline def setOne(value: js.Array[typingsJapgolly.makePlural.makePluralStrings.`1`]): Self = StObject.set(x, "one", value.asInstanceOf[js.Any])
    
    inline def setOneVarargs(value: typingsJapgolly.makePlural.makePluralStrings.`1`*): Self = StObject.set(x, "one", js.Array(value*))
    
    inline def setOther(
      value: js.Tuple14[
          typingsJapgolly.makePlural.makePluralStrings.`0`, 
          typingsJapgolly.makePlural.makePluralStrings.`3`, 
          typingsJapgolly.makePlural.makePluralStrings.`17`, 
          `101`, 
          `1001`, 
          `0Dot0`, 
          `1Dot0`, 
          `1Dot5`, 
          `10Dot0`, 
          `100Dot0`, 
          `1000Dot0`, 
          `10000Dot0`, 
          `100000Dot0`, 
          `1000000Dot0`
        ]
    ): Self = StObject.set(x, "other", value.asInstanceOf[js.Any])
    
    inline def setTwo(value: js.Array[typingsJapgolly.makePlural.makePluralStrings.`2`]): Self = StObject.set(x, "two", value.asInstanceOf[js.Any])
    
    inline def setTwoVarargs(value: typingsJapgolly.makePlural.makePluralStrings.`2`*): Self = StObject.set(x, "two", js.Array(value*))
  }
}
