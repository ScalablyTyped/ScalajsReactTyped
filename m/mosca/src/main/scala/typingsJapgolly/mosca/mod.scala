...
import typingsJapgolly.mosca.anon.CertPath
import typingsJapgolly.mosca.anon.Port
...
import typingsJapgolly.mosca.moscaStrings.ready
...
object mod {
  ...
  object persistence {
    ...
    object Persistence {
      ...
      inline def apply(wire: Server => Callback): Persistence = {
        val __obj = js.Dynamic.literal(wire = js.Any.fromFunction1((t0: Server) => wire(t0).runNow()))
        __obj.asInstanceOf[Persistence]
      }
      
      extension [Self <: Persistence](x: Self) {
        
        inline def setWire(value: Server => Callback): Self = StObject.set(x, "wire", js.Any.fromFunction1((t0: Server) => value(t0).runNow()))
      }
    }
  }
