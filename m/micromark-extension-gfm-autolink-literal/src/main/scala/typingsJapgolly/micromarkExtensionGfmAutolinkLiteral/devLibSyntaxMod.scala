package typingsJapgolly.micromarkExtensionGfmAutolinkLiteral

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object devLibSyntaxMod {
  
  @JSImport("micromark-extension-gfm-autolink-literal/dev/lib/syntax", "gfmAutolinkLiteral")
  @js.native
  val gfmAutolinkLiteral: Extension = js.native
  
  type Code = typingsJapgolly.micromarkUtilTypes.mod.Code
  
  type ConstructRecord = typingsJapgolly.micromarkUtilTypes.mod.ConstructRecord
  
  type Event = typingsJapgolly.micromarkUtilTypes.mod.Event
  
  type Extension = typingsJapgolly.micromarkUtilTypes.mod.Extension
  
  type Previous = typingsJapgolly.micromarkUtilTypes.mod.Previous
  
  type State = typingsJapgolly.micromarkUtilTypes.mod.State
  
  type Tokenizer = typingsJapgolly.micromarkUtilTypes.mod.Tokenizer
}
