...
import typingsJapgolly.mdastUtilToMarkdown.libUtilTrackMod.Point
...
object anon {
  ...
  trait SafeOptionsencodeArraystr extends StObject {
    ...
    var now: typingsJapgolly.mdastUtilToMarkdown.libTypesMod.Point
  }
  object SafeOptionsencodeArraystr {
    ...
    inline def apply(
      ...
      before: String,
      lineShift: Double,
      now: typingsJapgolly.mdastUtilToMarkdown.libTypesMod.Point
    ...
    extension [Self <: SafeOptionsencodeArraystr](x: Self) {
      ...
      inline def setNow(value: typingsJapgolly.mdastUtilToMarkdown.libTypesMod.Point): Self = StObject.set(x, "now", value.asInstanceOf[js.Any])
    }
  }
}
