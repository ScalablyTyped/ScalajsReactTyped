...
object IObjectType {
  ...
  extension [Self <: IObjectType](x: Self) {
    ...
    inline def setCanHaveItemIcons(value: CallbackTo[Boolean]): Self = StObject.set(x, "CanHaveItemIcons", value.toJsFn)
    
    inline def setClone(value: CallbackTo[IObjectType]): Self = StObject.set(x, "Clone", value.toJsFn)
