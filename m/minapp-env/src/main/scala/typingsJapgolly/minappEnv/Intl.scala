...
object Intl {
  ...
  object Collator {
    ...
    inline def apply(
      ...
      resolvedOptions: CallbackTo[ResolvedCollatorOptions]
    ): Collator = {
  ...
  object NumberFormat {
    ...
    inline def apply(format: Double => java.lang.String, resolvedOptions: CallbackTo[ResolvedNumberFormatOptions]): NumberFormat = {
      val __obj = js.Dynamic.literal(format = js.Any.fromFunction1(format), resolvedOptions = resolvedOptions.toJsFn)
    ...
    extension [Self <: NumberFormat](x: Self) {
      ...
      inline def setResolvedOptions(value: CallbackTo[ResolvedNumberFormatOptions]): Self = StObject.set(x, "resolvedOptions", value.toJsFn)
    }
  }
