...
import typingsJapgolly.marked.markedBooleans.`true`
...
import typingsJapgolly.marked.mod.marked.Renderer
import typingsJapgolly.marked.mod.marked.Token
import typingsJapgolly.marked.mod.marked.Tokenizer
...
object anon {
  ...
  object MarkedOptionsasynctrue {
    ...
    extension [Self <: MarkedOptionsasynctrue](x: Self) {
      ...
      inline def setWalkTokens(value: /* token */ Token => Callback): Self = StObject.set(x, "walkTokens", js.Any.fromFunction1((t0: /* token */ Token) => value(t0).runNow()))
