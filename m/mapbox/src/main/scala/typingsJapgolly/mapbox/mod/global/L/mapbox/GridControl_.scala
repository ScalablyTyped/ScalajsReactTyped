...
object GridControl_ {
  ...
  inline def apply(hide: CallbackTo[Any], onAdd: Map_ => Any, onRemove: Map_ => Any, setTemplate: String => Any): GridControl_ = {
    val __obj = js.Dynamic.literal(hide = hide.toJsFn, onAdd = js.Any.fromFunction1(onAdd), onRemove = js.Any.fromFunction1(onRemove), setTemplate = js.Any.fromFunction1(setTemplate))
    __obj.asInstanceOf[GridControl_]
  }
  
  extension [Self <: GridControl_](x: Self) {
    
    inline def setHide(value: CallbackTo[Any]): Self = StObject.set(x, "hide", value.toJsFn)
