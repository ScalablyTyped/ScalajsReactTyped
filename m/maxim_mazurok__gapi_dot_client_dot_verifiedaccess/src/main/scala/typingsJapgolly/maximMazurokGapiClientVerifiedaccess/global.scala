package typingsJapgolly.maximMazurokGapiClientVerifiedaccess

import typingsJapgolly.maximMazurokGapiClientVerifiedaccess.gapi.client.verifiedaccess.ChallengeResource
import typingsJapgolly.maximMazurokGapiClientVerifiedaccess.maximMazurokGapiClientVerifiedaccessStrings.v2
import typingsJapgolly.maximMazurokGapiClientVerifiedaccess.maximMazurokGapiClientVerifiedaccessStrings.verifiedaccess
import typingsJapgolly.std.PromiseLike
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object global {
  
  object gapi {
    
    object client {
      
      @JSGlobal("gapi.client")
      @js.native
      val ^ : js.Any = js.native
      
      /** Load Chrome Verified Access API v2 */
      inline def load(name: verifiedaccess, version: v2): PromiseLike[Unit] = (^.asInstanceOf[js.Dynamic].applyDynamic("load")(name.asInstanceOf[js.Any], version.asInstanceOf[js.Any])).asInstanceOf[PromiseLike[Unit]]
      inline def load(name: verifiedaccess, version: v2, callback: js.Function0[Any]): Unit = (^.asInstanceOf[js.Dynamic].applyDynamic("load")(name.asInstanceOf[js.Any], version.asInstanceOf[js.Any], callback.asInstanceOf[js.Any])).asInstanceOf[Unit]
      
      object verifiedaccess {
        
        @JSGlobal("gapi.client.verifiedaccess.challenge")
        @js.native
        val challenge: ChallengeResource = js.native
      }
    }
  }
}
