package typingsJapgolly.sketchHqSketchFileFormatTs.distCjsTypesMod

import typingsJapgolly.sketchHqSketchFileFormatTs.sketchHqSketchFileFormatTsStrings.gradientStop
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait GradientStop
  extends StObject
     with AnyObject {
  
  var _class: gradientStop
  
  var color: Color
  
  var position: UnitInterval
}
object GradientStop {
  
  inline def apply(color: Color, position: UnitInterval): GradientStop = {
    val __obj = js.Dynamic.literal(_class = "gradientStop", color = color.asInstanceOf[js.Any], position = position.asInstanceOf[js.Any])
    __obj.asInstanceOf[GradientStop]
  }
  
  extension [Self <: GradientStop](x: Self) {
    
    inline def setColor(value: Color): Self = StObject.set(x, "color", value.asInstanceOf[js.Any])
    
    inline def setPosition(value: UnitInterval): Self = StObject.set(x, "position", value.asInstanceOf[js.Any])
    
    inline def set_class(value: gradientStop): Self = StObject.set(x, "_class", value.asInstanceOf[js.Any])
  }
}
