package typingsJapgolly.snmpNative

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object snmpNativeInts {
  
  @js.native
  sealed trait `-1` extends StObject
  inline def `-1`: `-1` = -1.asInstanceOf[`-1`]
  
  @js.native
  sealed trait `0` extends StObject
  inline def `0`: `0` = 0.asInstanceOf[`0`]
  
  @js.native
  sealed trait `0x00` extends StObject
  inline def `0x00`: `0x00` = 0x00.asInstanceOf[`0x00`]
  
  @js.native
  sealed trait `0x01` extends StObject
  inline def `0x01`: `0x01` = 0x01.asInstanceOf[`0x01`]
  
  @js.native
  sealed trait `0x02` extends StObject
  inline def `0x02`: `0x02` = 0x02.asInstanceOf[`0x02`]
  
  @js.native
  sealed trait `0x03` extends StObject
  inline def `0x03`: `0x03` = 0x03.asInstanceOf[`0x03`]
  
  @js.native
  sealed trait `0x04` extends StObject
  inline def `0x04`: `0x04` = 0x04.asInstanceOf[`0x04`]
  
  @js.native
  sealed trait `0x05` extends StObject
  inline def `0x05`: `0x05` = 0x05.asInstanceOf[`0x05`]
  
  @js.native
  sealed trait `0x06` extends StObject
  inline def `0x06`: `0x06` = 0x06.asInstanceOf[`0x06`]
  
  @js.native
  sealed trait `0x30` extends StObject
  inline def `0x30`: `0x30` = 0x30.asInstanceOf[`0x30`]
  
  @js.native
  sealed trait `0x40` extends StObject
  inline def `0x40`: `0x40` = 0x40.asInstanceOf[`0x40`]
  
  @js.native
  sealed trait `0x41` extends StObject
  inline def `0x41`: `0x41` = 0x41.asInstanceOf[`0x41`]
  
  @js.native
  sealed trait `0x42` extends StObject
  inline def `0x42`: `0x42` = 0x42.asInstanceOf[`0x42`]
  
  @js.native
  sealed trait `0x43` extends StObject
  inline def `0x43`: `0x43` = 0x43.asInstanceOf[`0x43`]
  
  @js.native
  sealed trait `0x44` extends StObject
  inline def `0x44`: `0x44` = 0x44.asInstanceOf[`0x44`]
  
  @js.native
  sealed trait `0x45` extends StObject
  inline def `0x45`: `0x45` = 0x45.asInstanceOf[`0x45`]
  
  @js.native
  sealed trait `0x46` extends StObject
  inline def `0x46`: `0x46` = 0x46.asInstanceOf[`0x46`]
  
  @js.native
  sealed trait `0x80` extends StObject
  inline def `0x80`: `0x80` = 0x80.asInstanceOf[`0x80`]
  
  @js.native
  sealed trait `0x81` extends StObject
  inline def `0x81`: `0x81` = 0x81.asInstanceOf[`0x81`]
  
  @js.native
  sealed trait `0x82` extends StObject
  inline def `0x82`: `0x82` = 0x82.asInstanceOf[`0x82`]
  
  @js.native
  sealed trait `0xA0` extends StObject
  inline def `0xA0`: `0xA0` = 0xA0.asInstanceOf[`0xA0`]
  
  @js.native
  sealed trait `1` extends StObject
  inline def `1`: `1` = 1.asInstanceOf[`1`]
  
  @js.native
  sealed trait `10` extends StObject
  inline def `10`: `10` = 10.asInstanceOf[`10`]
  
  @js.native
  sealed trait `11` extends StObject
  inline def `11`: `11` = 11.asInstanceOf[`11`]
  
  @js.native
  sealed trait `12` extends StObject
  inline def `12`: `12` = 12.asInstanceOf[`12`]
  
  @js.native
  sealed trait `13` extends StObject
  inline def `13`: `13` = 13.asInstanceOf[`13`]
  
  @js.native
  sealed trait `14` extends StObject
  inline def `14`: `14` = 14.asInstanceOf[`14`]
  
  @js.native
  sealed trait `15` extends StObject
  inline def `15`: `15` = 15.asInstanceOf[`15`]
  
  @js.native
  sealed trait `16` extends StObject
  inline def `16`: `16` = 16.asInstanceOf[`16`]
  
  @js.native
  sealed trait `17` extends StObject
  inline def `17`: `17` = 17.asInstanceOf[`17`]
  
  @js.native
  sealed trait `18` extends StObject
  inline def `18`: `18` = 18.asInstanceOf[`18`]
  
  @js.native
  sealed trait `2` extends StObject
  inline def `2`: `2` = 2.asInstanceOf[`2`]
  
  @js.native
  sealed trait `3` extends StObject
  inline def `3`: `3` = 3.asInstanceOf[`3`]
  
  @js.native
  sealed trait `4` extends StObject
  inline def `4`: `4` = 4.asInstanceOf[`4`]
  
  @js.native
  sealed trait `5` extends StObject
  inline def `5`: `5` = 5.asInstanceOf[`5`]
  
  @js.native
  sealed trait `6` extends StObject
  inline def `6`: `6` = 6.asInstanceOf[`6`]
  
  @js.native
  sealed trait `7` extends StObject
  inline def `7`: `7` = 7.asInstanceOf[`7`]
  
  @js.native
  sealed trait `8` extends StObject
  inline def `8`: `8` = 8.asInstanceOf[`8`]
  
  @js.native
  sealed trait `9` extends StObject
  inline def `9`: `9` = 9.asInstanceOf[`9`]
}
