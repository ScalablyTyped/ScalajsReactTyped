package typingsJapgolly.snyk

import typingsJapgolly.snyk.distLibErrorsCustomErrorMod.CustomError
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object distLibErrorsNotFoundErrorMod {
  
  @JSImport("snyk/dist/lib/errors/not-found-error", "NotFoundError")
  @js.native
  open class NotFoundError protected () extends CustomError {
    def this(userMessage: Any) = this()
  }
  /* static members */
  object NotFoundError {
    
    @JSImport("snyk/dist/lib/errors/not-found-error", "NotFoundError")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("snyk/dist/lib/errors/not-found-error", "NotFoundError.ERROR_CODE")
    @js.native
    def ERROR_CODE: Any = js.native
    inline def ERROR_CODE_=(x: Any): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ERROR_CODE")(x.asInstanceOf[js.Any])
    
    @JSImport("snyk/dist/lib/errors/not-found-error", "NotFoundError.ERROR_MESSAGE")
    @js.native
    def ERROR_MESSAGE: Any = js.native
    inline def ERROR_MESSAGE_=(x: Any): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ERROR_MESSAGE")(x.asInstanceOf[js.Any])
  }
}
