...
- typingsJapgolly.semanticUiReact.formFieldMod.StrictFormFieldProps because var conflicts: as, children, className, disabled, error, id, label, required, `type`, width. Inlined content, control, `inline` */ trait StrictFormInputProps extends StrictInputProps {
