...
import typingsJapgolly.storefrontUiVue.anon.Headerheight
import typingsJapgolly.storefrontUiVue.storefrontUiVueStrings._empty
import typingsJapgolly.storefrontUiVue.storefrontUiVueStrings.account
import typingsJapgolly.storefrontUiVue.storefrontUiVueStrings.cart
import typingsJapgolly.storefrontUiVue.storefrontUiVueStrings.wishlist
...
object SfHeader extends Shortcut {
  ...
  object Methods {
    ...
    inline def apply(animationhandler: Double => Callback, scrollHandler: Callback): Methods = {
      val __obj = js.Dynamic.literal(animationhandler = js.Any.fromFunction1((t0: Double) => animationhandler(t0).runNow()), scrollHandler = scrollHandler.toJsFn)
      __obj.asInstanceOf[Methods]
    }
    
    extension [Self <: Methods](x: Self) {
      
      inline def setAnimationhandler(value: Double => Callback): Self = StObject.set(x, "animationhandler", js.Any.fromFunction1((t0: Double) => value(t0).runNow()))
