...
import typingsJapgolly.storefrontUiVue.storefrontUiVueStrings.`sf-product-card-horizontal__wishlist-icon`
...
object SfProductCardHorizontal extends Shortcut {
  ...
  object Methods {
    ...
    inline def apply(toggleIsOnWishlist: Callback): Methods = {
      val __obj = js.Dynamic.literal(toggleIsOnWishlist = toggleIsOnWishlist.toJsFn)
