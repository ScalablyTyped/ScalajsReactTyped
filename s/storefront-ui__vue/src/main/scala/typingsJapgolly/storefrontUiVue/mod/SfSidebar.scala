...
object SfSidebar extends Shortcut {
  ...
  object Methods {
    ...
    inline def apply(classHandler: Callback, close: Callback, keydownHandler: KeyboardEvent => Callback): Methods = {
      val __obj = js.Dynamic.literal(classHandler = classHandler.toJsFn, close = close.toJsFn, keydownHandler = js.Any.fromFunction1((t0: KeyboardEvent) => keydownHandler(t0).runNow()))
