...
object typesModulesFreeModeMod {
  ...
  object FreeModeMethods {
    ...
    inline def apply(onTouchEnd: Callback, onTouchMove: Callback): FreeModeMethods = {
      val __obj = js.Dynamic.literal(onTouchEnd = onTouchEnd.toJsFn, onTouchMove = onTouchMove.toJsFn)
    ...
    extension [Self <: FreeModeMethods](x: Self) {
      ...
      inline def setOnTouchMove(value: Callback): Self = StObject.set(x, "onTouchMove", value.toJsFn)
    }
  }
