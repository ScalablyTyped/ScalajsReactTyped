package typingsJapgolly.sharepoint.global.Microsoft.SharePoint.Client.Search

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/**Located in sp.search.apps.js*/
object Analytics {
  
  @JSGlobal("Microsoft.SharePoint.Client.Search.Analytics.AnalyticsItemData")
  @js.native
  open class AnalyticsItemData ()
    extends StObject
       with typingsJapgolly.sharepoint.Microsoft.SharePoint.Client.Search.Analytics.AnalyticsItemData
  
  @JSGlobal("Microsoft.SharePoint.Client.Search.Analytics.UsageAnalytics")
  @js.native
  open class UsageAnalytics ()
    extends StObject
       with typingsJapgolly.sharepoint.Microsoft.SharePoint.Client.Search.Analytics.UsageAnalytics
}
