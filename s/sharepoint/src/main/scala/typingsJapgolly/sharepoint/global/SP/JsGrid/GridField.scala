package typingsJapgolly.sharepoint.global.SP.JsGrid

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("SP.JsGrid.GridField")
@js.native
open class GridField protected ()
  extends StObject
     with typingsJapgolly.sharepoint.SP.JsGrid.GridField {
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Any
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Any,
    editMode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Unit,
    editMode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Any,
    editMode: Unit,
    dateOnly: Boolean
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Any,
    editMode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode,
    dateOnly: Boolean
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Unit,
    editMode: Unit,
    dateOnly: Boolean
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Unit,
    editMode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode,
    dateOnly: Boolean
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Any,
    editMode: Unit,
    dateOnly: Boolean,
    csrInfo: Any
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Any,
    editMode: Unit,
    dateOnly: Unit,
    csrInfo: Any
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Any,
    editMode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode,
    dateOnly: Boolean,
    csrInfo: Any
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Any,
    editMode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode,
    dateOnly: Unit,
    csrInfo: Any
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Unit,
    editMode: Unit,
    dateOnly: Boolean,
    csrInfo: Any
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Unit,
    editMode: Unit,
    dateOnly: Unit,
    csrInfo: Any
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Unit,
    editMode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode,
    dateOnly: Boolean,
    csrInfo: Any
  ) = this()
  def this(
    key: String,
    hasDataValue: Boolean,
    hasLocalizedValue: Boolean,
    textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection,
    defaultCellStyleId: Unit,
    editMode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode,
    dateOnly: Unit,
    csrInfo: Any
  ) = this()
  
  /* CompleteClass */
  override def CompareSingleDataEqual(dataValue1: Any, dataValue2: Any): Boolean = js.native
  
  /* CompleteClass */
  override def GetDefaultCellStyleId(): Any = js.native
  
  /* CompleteClass */
  override def GetEditMode(): typingsJapgolly.sharepoint.SP.JsGrid.EditMode = js.native
  
  /* CompleteClass */
  override def GetIsMultiValue(): Boolean = js.native
  
  /* CompleteClass */
  override def GetMultiValuePropType(): Any = js.native
  
  /* CompleteClass */
  override def GetPropType(): Any = js.native
  
  /* CompleteClass */
  override def GetSingleValuePropType(): Any = js.native
  
  /* CompleteClass */
  override def SetEditMode(mode: typingsJapgolly.sharepoint.SP.JsGrid.EditMode): Unit = js.native
  
  /* CompleteClass */
  override def SetIsMultiValue(listSeparator: Any): Unit = js.native
  
  /* CompleteClass */
  override def SetSingleValuePropType(svPropType: Any): Unit = js.native
  
  /* CompleteClass */
  var csrInfo: Any = js.native
  
  /* CompleteClass */
  var dateOnly: Boolean = js.native
  
  /* CompleteClass */
  var hasDataValue: Boolean = js.native
  
  /* CompleteClass */
  var hasLocalizedValue: Boolean = js.native
  
  /* CompleteClass */
  var key: String = js.native
  
  /* CompleteClass */
  var textDirection: typingsJapgolly.sharepoint.SP.JsGrid.TextDirection = js.native
}
