package typingsJapgolly.styledComponents

import org.scalablytyped.runtime.Shortcut
import typingsJapgolly.styledComponents.anon.A
import typingsJapgolly.styledComponents.anon.Attrs
import typingsJapgolly.styledComponents.nativeDistTypesMod.WebTarget
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object nativeDistConstructorsStyledMod extends Shortcut {
  
  @JSImport("styled-components/native/dist/constructors/styled", JSImport.Default)
  @js.native
  val default: (js.Function1[/* tag */ WebTarget, Attrs[WebTarget]]) & A = js.native
  
  type _To = (js.Function1[/* tag */ WebTarget, Attrs[WebTarget]]) & A
  
  /* This means you don't have to write `default`, but can instead just say `nativeDistConstructorsStyledMod.foo` */
  override def _to: (js.Function1[/* tag */ WebTarget, Attrs[WebTarget]]) & A = default
}
