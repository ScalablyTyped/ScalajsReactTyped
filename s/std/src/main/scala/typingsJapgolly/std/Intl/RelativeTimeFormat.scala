...
object RelativeTimeFormat {
  ...
  inline def apply(
    ...
    resolvedOptions: CallbackTo[ResolvedRelativeTimeFormatOptions]
  ): RelativeTimeFormat = {
    val __obj = js.Dynamic.literal(format = js.Any.fromFunction2(format), formatToParts = js.Any.fromFunction2(formatToParts), resolvedOptions = resolvedOptions.toJsFn)
  ...
  extension [Self <: RelativeTimeFormat](x: Self) {
    ...
    inline def setResolvedOptions(value: CallbackTo[ResolvedRelativeTimeFormatOptions]): Self = StObject.set(x, "resolvedOptions", value.toJsFn)
  }
}
