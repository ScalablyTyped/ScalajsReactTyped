package typingsJapgolly.std

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typingsJapgolly.std.stdStrings.checking
  - typingsJapgolly.std.stdStrings.closed
  - typingsJapgolly.std.stdStrings.completed
  - typingsJapgolly.std.stdStrings.connected
  - typingsJapgolly.std.stdStrings.disconnected
  - typingsJapgolly.std.stdStrings.failed
  - typingsJapgolly.std.stdStrings.`new`
*/
trait RTCIceConnectionState extends StObject
object RTCIceConnectionState {
  
  inline def checking: typingsJapgolly.std.stdStrings.checking = "checking".asInstanceOf[typingsJapgolly.std.stdStrings.checking]
  
  inline def closed: typingsJapgolly.std.stdStrings.closed = "closed".asInstanceOf[typingsJapgolly.std.stdStrings.closed]
  
  inline def completed: typingsJapgolly.std.stdStrings.completed = "completed".asInstanceOf[typingsJapgolly.std.stdStrings.completed]
  
  inline def connected: typingsJapgolly.std.stdStrings.connected = "connected".asInstanceOf[typingsJapgolly.std.stdStrings.connected]
  
  inline def disconnected: typingsJapgolly.std.stdStrings.disconnected = "disconnected".asInstanceOf[typingsJapgolly.std.stdStrings.disconnected]
  
  inline def failed: typingsJapgolly.std.stdStrings.failed = "failed".asInstanceOf[typingsJapgolly.std.stdStrings.failed]
  
  inline def `new`: typingsJapgolly.std.stdStrings.`new` = "new".asInstanceOf[typingsJapgolly.std.stdStrings.`new`]
}
