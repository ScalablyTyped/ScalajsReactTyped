package typingsJapgolly.sinonChrome.global.SinonChrome

import typingsJapgolly.sinonChrome.mod.SinonChromeStub
import typingsJapgolly.sinonChrome.mod.events.Event
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object alarms {
  
  @JSGlobal("SinonChrome.alarms")
  @js.native
  val ^ : js.Any = js.native
  
  @JSGlobal("SinonChrome.alarms.clear")
  @js.native
  def clear: SinonChromeStub = js.native
  
  @JSGlobal("SinonChrome.alarms.clearAll")
  @js.native
  def clearAll: SinonChromeStub = js.native
  inline def clearAll_=(x: SinonChromeStub): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("clearAll")(x.asInstanceOf[js.Any])
  
  inline def clear_=(x: SinonChromeStub): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("clear")(x.asInstanceOf[js.Any])
  
  @JSGlobal("SinonChrome.alarms.create")
  @js.native
  def create: SinonChromeStub = js.native
  inline def create_=(x: SinonChromeStub): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("create")(x.asInstanceOf[js.Any])
  
  @JSGlobal("SinonChrome.alarms.get")
  @js.native
  def get: SinonChromeStub = js.native
  
  @JSGlobal("SinonChrome.alarms.getAll")
  @js.native
  def getAll: SinonChromeStub = js.native
  inline def getAll_=(x: SinonChromeStub): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("getAll")(x.asInstanceOf[js.Any])
  
  inline def get_=(x: SinonChromeStub): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("get")(x.asInstanceOf[js.Any])
  
  @JSGlobal("SinonChrome.alarms.onAlarm")
  @js.native
  def onAlarm: Event = js.native
  inline def onAlarm_=(x: Event): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("onAlarm")(x.asInstanceOf[js.Any])
}
