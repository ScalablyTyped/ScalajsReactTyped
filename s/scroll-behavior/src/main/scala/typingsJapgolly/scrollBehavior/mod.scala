...
import typingsJapgolly.scrollBehavior.anon.Read
import typingsJapgolly.scrollBehavior.scrollBehaviorStrings.PUSH
...
object mod {
  ...
  object ScrollBehaviorOptions {
    ...
    inline def apply[TLocation /* <: LocationBase */, TContext](
      ...
      getCurrentLocation: CallbackTo[TLocation],
      stateStorage: Read[TLocation]
    ): ScrollBehaviorOptions[TLocation, TContext] = {
      val __obj = js.Dynamic.literal(addNavigationListener = js.Any.fromFunction1(addNavigationListener), getCurrentLocation = getCurrentLocation.toJsFn, stateStorage = stateStorage.asInstanceOf[js.Any])
    ...
    extension [Self <: ScrollBehaviorOptions[?, ?], TLocation /* <: LocationBase */, TContext](x: Self & (ScrollBehaviorOptions[TLocation, TContext])) {
      ...
      inline def setGetCurrentLocation(value: CallbackTo[TLocation]): Self = StObject.set(x, "getCurrentLocation", value.toJsFn)
