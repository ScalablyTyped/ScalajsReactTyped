...
import typingsJapgolly.smoothScrollbar.anon.PartialScrollToOptions
import typingsJapgolly.smoothScrollbar.anon.PartialSetPositionOptions
import typingsJapgolly.smoothScrollbar.interfacesData2dMod.Data2d
import typingsJapgolly.smoothScrollbar.interfacesTrackMod.TrackController
...
object interfacesScrollbarMod {
  ...
  object ScrollToOptions {
    ...
    inline def apply(callback: Callback, easing: Double => Double): ScrollToOptions = {
      val __obj = js.Dynamic.literal(callback = callback.toJsFn, easing = js.Any.fromFunction1(easing))
