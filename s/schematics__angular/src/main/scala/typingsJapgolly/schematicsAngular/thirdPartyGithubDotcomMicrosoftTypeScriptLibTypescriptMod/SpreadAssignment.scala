package typingsJapgolly.schematicsAngular.thirdPartyGithubDotcomMicrosoftTypeScriptLibTypescriptMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait SpreadAssignment
  extends StObject
     with ObjectLiteralElement
     with JSDocContainer
     with HasJSDoc
     with ObjectBindingOrAssignmentElement
     with ObjectLiteralElementLike
     with _BindingOrAssignmentElementRestIndicator {
  
  val expression: Expression = js.native
  
  @JSName("kind")
  val kind_SpreadAssignment: typingsJapgolly.schematicsAngular.thirdPartyGithubDotcomMicrosoftTypeScriptLibTypescriptMod.SyntaxKind.SpreadAssignment = js.native
  
  @JSName("parent")
  val parent_SpreadAssignment: ObjectLiteralExpression = js.native
}
