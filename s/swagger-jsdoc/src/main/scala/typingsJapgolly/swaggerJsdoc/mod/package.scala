package typingsJapgolly.swaggerJsdoc.mod

import org.scalablytyped.runtime.StringDictionary
import typingsJapgolly.swaggerJsdoc.mod.^
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}


inline def apply(): js.Object = ^.asInstanceOf[js.Dynamic].apply().asInstanceOf[js.Object]
inline def apply(options: Options): js.Object = ^.asInstanceOf[js.Dynamic].apply(options.asInstanceOf[js.Any]).asInstanceOf[js.Object]

type Callback = StringDictionary[PathItem]

type Paths = StringDictionary[PathItem]

type SecurityRequirement = StringDictionary[js.Array[String]]
