...
object anon {
  ...
  object Express {
    ...
    inline def apply(express: CallbackTo[Any]): Express = {
      val __obj = js.Dynamic.literal(express = express.toJsFn)
      __obj.asInstanceOf[Express]
    }
    
    extension [Self <: Express](x: Self) {
      
      inline def setExpress(value: CallbackTo[Any]): Self = StObject.set(x, "express", value.toJsFn)
    }
  }
