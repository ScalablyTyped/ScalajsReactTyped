package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.LA
import typingsJapgolly.baseui.baseuiStrings.Plussign856
import typingsJapgolly.baseui.baseuiStrings.`Laos LeftparenthesisລາວRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon109 extends js.Object {
  var dialCode: Plussign856
  var id: LA
  var label: `Laos LeftparenthesisລາວRightparenthesis`
}

object Anon109 {
  @scala.inline
  def apply(dialCode: Plussign856, id: LA, label: `Laos LeftparenthesisລາວRightparenthesis`): Anon109 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon109]
  }
}

