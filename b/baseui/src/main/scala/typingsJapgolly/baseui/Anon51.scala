package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.CZ
import typingsJapgolly.baseui.baseuiStrings.Plussign420
import typingsJapgolly.baseui.baseuiStrings.`Czech Republic LeftparenthesisČeská republikaRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon51 extends js.Object {
  var dialCode: Plussign420
  var id: CZ
  var label: `Czech Republic LeftparenthesisČeská republikaRightparenthesis`
}

object Anon51 {
  @scala.inline
  def apply(
    dialCode: Plussign420,
    id: CZ,
    label: `Czech Republic LeftparenthesisČeská republikaRightparenthesis`
  ): Anon51 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon51]
  }
}

