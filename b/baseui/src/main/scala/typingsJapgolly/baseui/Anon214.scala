package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.Plussign216
import typingsJapgolly.baseui.baseuiStrings.TN
import typingsJapgolly.baseui.baseuiStrings.`Tunisia Leftparenthesis‫تونس‬‎Rightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon214 extends js.Object {
  var dialCode: Plussign216
  var id: TN
  var label: `Tunisia Leftparenthesis‫تونس‬‎Rightparenthesis`
}

object Anon214 {
  @scala.inline
  def apply(dialCode: Plussign216, id: TN, label: `Tunisia Leftparenthesis‫تونس‬‎Rightparenthesis`): Anon214 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon214]
  }
}

