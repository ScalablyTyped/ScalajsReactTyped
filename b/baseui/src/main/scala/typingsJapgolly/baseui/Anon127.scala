package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.MQ
import typingsJapgolly.baseui.baseuiStrings.Martinique
import typingsJapgolly.baseui.baseuiStrings.Plussign596
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon127 extends js.Object {
  var dialCode: Plussign596
  var id: MQ
  var label: Martinique
}

object Anon127 {
  @scala.inline
  def apply(dialCode: Plussign596, id: MQ, label: Martinique): Anon127 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon127]
  }
}

