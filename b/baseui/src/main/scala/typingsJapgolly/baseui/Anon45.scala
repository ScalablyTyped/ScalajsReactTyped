package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.CR
import typingsJapgolly.baseui.baseuiStrings.Plussign506
import typingsJapgolly.baseui.baseuiStrings.`Costa Rica`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon45 extends js.Object {
  var dialCode: Plussign506
  var id: CR
  var label: `Costa Rica`
}

object Anon45 {
  @scala.inline
  def apply(dialCode: Plussign506, id: CR, label: `Costa Rica`): Anon45 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon45]
  }
}

