package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.MU
import typingsJapgolly.baseui.baseuiStrings.Plussign230
import typingsJapgolly.baseui.baseuiStrings.`Mauritius LeftparenthesisMorisRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon129 extends js.Object {
  var dialCode: Plussign230
  var id: MU
  var label: `Mauritius LeftparenthesisMorisRightparenthesis`
}

object Anon129 {
  @scala.inline
  def apply(dialCode: Plussign230, id: MU, label: `Mauritius LeftparenthesisMorisRightparenthesis`): Anon129 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon129]
  }
}

