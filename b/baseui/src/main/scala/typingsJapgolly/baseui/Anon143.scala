package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.NP
import typingsJapgolly.baseui.baseuiStrings.Plussign977
import typingsJapgolly.baseui.baseuiStrings.`Nepal LeftparenthesisनेपालRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon143 extends js.Object {
  var dialCode: Plussign977
  var id: NP
  var label: `Nepal LeftparenthesisनेपालRightparenthesis`
}

object Anon143 {
  @scala.inline
  def apply(dialCode: Plussign977, id: NP, label: `Nepal LeftparenthesisनेपालRightparenthesis`): Anon143 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon143]
  }
}

