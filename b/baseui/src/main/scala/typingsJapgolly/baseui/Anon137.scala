package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.MS
import typingsJapgolly.baseui.baseuiStrings.Montserrat
import typingsJapgolly.baseui.baseuiStrings.Plussign1664
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon137 extends js.Object {
  var dialCode: Plussign1664
  var id: MS
  var label: Montserrat
}

object Anon137 {
  @scala.inline
  def apply(dialCode: Plussign1664, id: MS, label: Montserrat): Anon137 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon137]
  }
}

