package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.Plussign967
import typingsJapgolly.baseui.baseuiStrings.YE
import typingsJapgolly.baseui.baseuiStrings.`Yemen Leftparenthesis‫اليمن‬‎Rightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon232 extends js.Object {
  var dialCode: Plussign967
  var id: YE
  var label: `Yemen Leftparenthesis‫اليمن‬‎Rightparenthesis`
}

object Anon232 {
  @scala.inline
  def apply(dialCode: Plussign967, id: YE, label: `Yemen Leftparenthesis‫اليمن‬‎Rightparenthesis`): Anon232 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon232]
  }
}

