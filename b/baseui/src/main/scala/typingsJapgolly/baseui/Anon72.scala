package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.GE
import typingsJapgolly.baseui.baseuiStrings.Plussign995
import typingsJapgolly.baseui.baseuiStrings.`Georgia LeftparenthesisსაქართველოRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon72 extends js.Object {
  var dialCode: Plussign995
  var id: GE
  var label: `Georgia LeftparenthesisსაქართველოRightparenthesis`
}

object Anon72 {
  @scala.inline
  def apply(dialCode: Plussign995, id: GE, label: `Georgia LeftparenthesisსაქართველოRightparenthesis`): Anon72 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon72]
  }
}

