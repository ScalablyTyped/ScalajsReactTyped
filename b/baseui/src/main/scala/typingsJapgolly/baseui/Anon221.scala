package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.Plussign380
import typingsJapgolly.baseui.baseuiStrings.UA
import typingsJapgolly.baseui.baseuiStrings.`Ukraine LeftparenthesisУкраїнаRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon221 extends js.Object {
  var dialCode: Plussign380
  var id: UA
  var label: `Ukraine LeftparenthesisУкраїнаRightparenthesis`
}

object Anon221 {
  @scala.inline
  def apply(dialCode: Plussign380, id: UA, label: `Ukraine LeftparenthesisУкраїнаRightparenthesis`): Anon221 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon221]
  }
}

