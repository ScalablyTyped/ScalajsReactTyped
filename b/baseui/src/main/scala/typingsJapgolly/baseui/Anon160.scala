package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.PG
import typingsJapgolly.baseui.baseuiStrings.Plussign675
import typingsJapgolly.baseui.baseuiStrings.`Papua New Guinea`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon160 extends js.Object {
  var dialCode: Plussign675
  var id: PG
  var label: `Papua New Guinea`
}

object Anon160 {
  @scala.inline
  def apply(dialCode: Plussign675, id: PG, label: `Papua New Guinea`): Anon160 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon160]
  }
}

