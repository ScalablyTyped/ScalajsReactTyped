package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.AZ
import typingsJapgolly.baseui.baseuiStrings.Plussign994
import typingsJapgolly.baseui.baseuiStrings.`Azerbaijan LeftparenthesisAzərbaycanRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon8 extends js.Object {
  var dialCode: Plussign994
  var id: AZ
  var label: `Azerbaijan LeftparenthesisAzərbaycanRightparenthesis`
}

object Anon8 {
  @scala.inline
  def apply(dialCode: Plussign994, id: AZ, label: `Azerbaijan LeftparenthesisAzərbaycanRightparenthesis`): Anon8 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon8]
  }
}

