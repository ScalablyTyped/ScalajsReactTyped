package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.IQ
import typingsJapgolly.baseui.baseuiStrings.Plussign964
import typingsJapgolly.baseui.baseuiStrings.`Iraq Leftparenthesis‫العراق‬‎Rightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon94 extends js.Object {
  var dialCode: Plussign964
  var id: IQ
  var label: `Iraq Leftparenthesis‫العراق‬‎Rightparenthesis`
}

object Anon94 {
  @scala.inline
  def apply(dialCode: Plussign964, id: IQ, label: `Iraq Leftparenthesis‫العراق‬‎Rightparenthesis`): Anon94 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon94]
  }
}

