package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.Plussign84
import typingsJapgolly.baseui.baseuiStrings.VN
import typingsJapgolly.baseui.baseuiStrings.`Vietnam LeftparenthesisViệt NamRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon230 extends js.Object {
  var dialCode: Plussign84
  var id: VN
  var label: `Vietnam LeftparenthesisViệt NamRightparenthesis`
}

object Anon230 {
  @scala.inline
  def apply(dialCode: Plussign84, id: VN, label: `Vietnam LeftparenthesisViệt NamRightparenthesis`): Anon230 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon230]
  }
}

