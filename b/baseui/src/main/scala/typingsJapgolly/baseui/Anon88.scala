package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.HK
import typingsJapgolly.baseui.baseuiStrings.Plussign852
import typingsJapgolly.baseui.baseuiStrings.`Hong Kong Leftparenthesis香港Rightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon88 extends js.Object {
  var dialCode: Plussign852
  var id: HK
  var label: `Hong Kong Leftparenthesis香港Rightparenthesis`
}

object Anon88 {
  @scala.inline
  def apply(dialCode: Plussign852, id: HK, label: `Hong Kong Leftparenthesis香港Rightparenthesis`): Anon88 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon88]
  }
}

