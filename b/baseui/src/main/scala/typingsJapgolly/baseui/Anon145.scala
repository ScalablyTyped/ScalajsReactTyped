package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.NC
import typingsJapgolly.baseui.baseuiStrings.Plussign687
import typingsJapgolly.baseui.baseuiStrings.`New Caledonia LeftparenthesisNouvelle-CalédonieRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon145 extends js.Object {
  var dialCode: Plussign687
  var id: NC
  var label: `New Caledonia LeftparenthesisNouvelle-CalédonieRightparenthesis`
}

object Anon145 {
  @scala.inline
  def apply(
    dialCode: Plussign687,
    id: NC,
    label: `New Caledonia LeftparenthesisNouvelle-CalédonieRightparenthesis`
  ): Anon145 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon145]
  }
}

