package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.BT
import typingsJapgolly.baseui.baseuiStrings.Plussign975
import typingsJapgolly.baseui.baseuiStrings.`Bhutan LeftparenthesisའབྲུགRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon18 extends js.Object {
  var dialCode: Plussign975
  var id: BT
  var label: `Bhutan LeftparenthesisའབྲུགRightparenthesis`
}

object Anon18 {
  @scala.inline
  def apply(dialCode: Plussign975, id: BT, label: `Bhutan LeftparenthesisའབྲུགRightparenthesis`): Anon18 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon18]
  }
}

