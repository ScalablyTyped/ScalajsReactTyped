package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.KG
import typingsJapgolly.baseui.baseuiStrings.Plussign996
import typingsJapgolly.baseui.baseuiStrings.`Kyrgyzstan LeftparenthesisКыргызстанRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon108 extends js.Object {
  var dialCode: Plussign996
  var id: KG
  var label: `Kyrgyzstan LeftparenthesisКыргызстанRightparenthesis`
}

object Anon108 {
  @scala.inline
  def apply(dialCode: Plussign996, id: KG, label: `Kyrgyzstan LeftparenthesisКыргызстанRightparenthesis`): Anon108 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon108]
  }
}

