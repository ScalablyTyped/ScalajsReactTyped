package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.NE
import typingsJapgolly.baseui.baseuiStrings.Plussign227
import typingsJapgolly.baseui.baseuiStrings.`Niger LeftparenthesisNijarRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon148 extends js.Object {
  var dialCode: Plussign227
  var id: NE
  var label: `Niger LeftparenthesisNijarRightparenthesis`
}

object Anon148 {
  @scala.inline
  def apply(dialCode: Plussign227, id: NE, label: `Niger LeftparenthesisNijarRightparenthesis`): Anon148 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon148]
  }
}

