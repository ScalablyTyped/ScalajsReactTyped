package typingsJapgolly.baseui

import typingsJapgolly.baseui.baseuiStrings.MF
import typingsJapgolly.baseui.baseuiStrings.Plussign590
import typingsJapgolly.baseui.baseuiStrings.`Saint Martin LeftparenthesisSaint-Martin Leftparenthesispartie françaiseRightparenthesisRightparenthesis`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait Anon176 extends js.Object {
  var dialCode: Plussign590
  var id: MF
  var label: `Saint Martin LeftparenthesisSaint-Martin Leftparenthesispartie françaiseRightparenthesisRightparenthesis`
}

object Anon176 {
  @scala.inline
  def apply(
    dialCode: Plussign590,
    id: MF,
    label: `Saint Martin LeftparenthesisSaint-Martin Leftparenthesispartie françaiseRightparenthesisRightparenthesis`
  ): Anon176 = {
    val __obj = js.Dynamic.literal(dialCode = dialCode.asInstanceOf[js.Any], id = id.asInstanceOf[js.Any], label = label.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[Anon176]
  }
}

