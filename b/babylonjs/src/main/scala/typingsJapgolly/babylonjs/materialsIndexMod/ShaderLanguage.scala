package typingsJapgolly.babylonjs.materialsIndexMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("babylonjs/Materials/index", "ShaderLanguage")
@js.native
object ShaderLanguage extends StObject {
  
  @JSBracketAccess
  def apply(value: Double): js.UndefOr[typingsJapgolly.babylonjs.materialsShaderLanguageMod.ShaderLanguage & Double] = js.native
  
  /* 0 */ val GLSL: typingsJapgolly.babylonjs.materialsShaderLanguageMod.ShaderLanguage.GLSL & Double = js.native
  
  /* 1 */ val WGSL: typingsJapgolly.babylonjs.materialsShaderLanguageMod.ShaderLanguage.WGSL & Double = js.native
}
