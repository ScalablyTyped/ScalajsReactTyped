...
object Observer {
  ...
  inline def apply[T](
    ...
    callback: (T, EventState) => Callback,
    mask: Double,
    scope: Any,
    unregisterOnNextCall: Boolean
  ): Observer[T] = {
    val __obj = js.Dynamic.literal(_willBeUnregistered = _willBeUnregistered.asInstanceOf[js.Any], callback = js.Any.fromFunction2((t0: T, t1: EventState) => (callback(t0, t1)).runNow()), mask = mask.asInstanceOf[js.Any], scope = scope.asInstanceOf[js.Any], unregisterOnNextCall = unregisterOnNextCall.asInstanceOf[js.Any])
    __obj.asInstanceOf[Observer[T]]
  }
  
  extension [Self <: Observer[?], T](x: Self & Observer[T]) {
    
    inline def setCallback(value: (T, EventState) => Callback): Self = StObject.set(x, "callback", js.Any.fromFunction2((t0: T, t1: EventState) => (value(t0, t1)).runNow()))
