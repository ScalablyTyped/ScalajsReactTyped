package typingsJapgolly.backlogJs

import typingsJapgolly.backlogJs.distTypesErrorMod.BacklogErrorNameType
import typingsJapgolly.backlogJs.distTypesOptionMod.Issue.SortKey
import typingsJapgolly.backlogJs.distTypesOptionMod.Order
import typingsJapgolly.backlogJs.distTypesOptionMod.Project.IssueTypeColor
import typingsJapgolly.backlogJs.distTypesOptionMod.Project.ProjectStatusColor
import typingsJapgolly.backlogJs.distTypesOptionMod.Project.TextFormattingRule
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object backlogJsStrings {
  
  @js.native
  sealed trait BacklogApiError
    extends StObject
       with BacklogErrorNameType
  inline def BacklogApiError: BacklogApiError = "BacklogApiError".asInstanceOf[BacklogApiError]
  
  @js.native
  sealed trait BacklogAuthError
    extends StObject
       with BacklogErrorNameType
  inline def BacklogAuthError: BacklogAuthError = "BacklogAuthError".asInstanceOf[BacklogAuthError]
  
  @js.native
  sealed trait Numbersign007e9a
    extends StObject
       with IssueTypeColor
  inline def Numbersign007e9a: Numbersign007e9a = "#007e9a".asInstanceOf[Numbersign007e9a]
  
  @js.native
  sealed trait Numbersign2779ca
    extends StObject
       with IssueTypeColor
  inline def Numbersign2779ca: Numbersign2779ca = "#2779ca".asInstanceOf[Numbersign2779ca]
  
  @js.native
  sealed trait Numbersign393939
    extends StObject
       with ProjectStatusColor
  inline def Numbersign393939: Numbersign393939 = "#393939".asInstanceOf[Numbersign393939]
  
  @js.native
  sealed trait Numbersign3b9dbd
    extends StObject
       with ProjectStatusColor
  inline def Numbersign3b9dbd: Numbersign3b9dbd = "#3b9dbd".asInstanceOf[Numbersign3b9dbd]
  
  @js.native
  sealed trait Numbersign4caf93
    extends StObject
       with ProjectStatusColor
  inline def Numbersign4caf93: Numbersign4caf93 = "#4caf93".asInstanceOf[Numbersign4caf93]
  
  @js.native
  sealed trait Numbersign666665
    extends StObject
       with IssueTypeColor
  inline def Numbersign666665: Numbersign666665 = "#666665".asInstanceOf[Numbersign666665]
  
  @js.native
  sealed trait Numbersign7ea800
    extends StObject
       with IssueTypeColor
  inline def Numbersign7ea800: Numbersign7ea800 = "#7ea800".asInstanceOf[Numbersign7ea800]
  
  @js.native
  sealed trait Numbersign814fbc
    extends StObject
       with IssueTypeColor
  inline def Numbersign814fbc: Numbersign814fbc = "#814fbc".asInstanceOf[Numbersign814fbc]
  
  @js.native
  sealed trait Numbersign868cb7
    extends StObject
       with ProjectStatusColor
  inline def Numbersign868cb7: Numbersign868cb7 = "#868cb7".asInstanceOf[Numbersign868cb7]
  
  @js.native
  sealed trait Numbersign934981
    extends StObject
       with IssueTypeColor
  inline def Numbersign934981: Numbersign934981 = "#934981".asInstanceOf[Numbersign934981]
  
  @js.native
  sealed trait Numbersign990000
    extends StObject
       with IssueTypeColor
  inline def Numbersign990000: Numbersign990000 = "#990000".asInstanceOf[Numbersign990000]
  
  @js.native
  sealed trait Numbersignb0be3c
    extends StObject
       with ProjectStatusColor
  inline def Numbersignb0be3c: Numbersignb0be3c = "#b0be3c".asInstanceOf[Numbersignb0be3c]
  
  @js.native
  sealed trait Numbersigne07b9a
    extends StObject
       with ProjectStatusColor
  inline def Numbersigne07b9a: Numbersigne07b9a = "#e07b9a".asInstanceOf[Numbersigne07b9a]
  
  @js.native
  sealed trait Numbersigne30000
    extends StObject
       with IssueTypeColor
  inline def Numbersigne30000: Numbersigne30000 = "#e30000".asInstanceOf[Numbersigne30000]
  
  @js.native
  sealed trait Numbersigne87758
    extends StObject
       with ProjectStatusColor
  inline def Numbersigne87758: Numbersigne87758 = "#e87758".asInstanceOf[Numbersigne87758]
  
  @js.native
  sealed trait Numbersignea2c00
    extends StObject
       with ProjectStatusColor
  inline def Numbersignea2c00: Numbersignea2c00 = "#ea2c00".asInstanceOf[Numbersignea2c00]
  
  @js.native
  sealed trait Numbersigneda62a
    extends StObject
       with ProjectStatusColor
  inline def Numbersigneda62a: Numbersigneda62a = "#eda62a".asInstanceOf[Numbersigneda62a]
  
  @js.native
  sealed trait Numbersignf42858
    extends StObject
       with ProjectStatusColor
  inline def Numbersignf42858: Numbersignf42858 = "#f42858".asInstanceOf[Numbersignf42858]
  
  @js.native
  sealed trait Numbersignff3265
    extends StObject
       with IssueTypeColor
  inline def Numbersignff3265: Numbersignff3265 = "#ff3265".asInstanceOf[Numbersignff3265]
  
  @js.native
  sealed trait Numbersignff9200
    extends StObject
       with IssueTypeColor
  inline def Numbersignff9200: Numbersignff9200 = "#ff9200".asInstanceOf[Numbersignff9200]
  
  @js.native
  sealed trait UnexpectedError
    extends StObject
       with BacklogErrorNameType
  inline def UnexpectedError: UnexpectedError = "UnexpectedError".asInstanceOf[UnexpectedError]
  
  @js.native
  sealed trait actualHours
    extends StObject
       with SortKey
  inline def actualHours: actualHours = "actualHours".asInstanceOf[actualHours]
  
  @js.native
  sealed trait asc
    extends StObject
       with Order
  inline def asc: asc = "asc".asInstanceOf[asc]
  
  @js.native
  sealed trait assignee
    extends StObject
       with SortKey
  inline def assignee: assignee = "assignee".asInstanceOf[assignee]
  
  @js.native
  sealed trait attachment
    extends StObject
       with SortKey
  inline def attachment: attachment = "attachment".asInstanceOf[attachment]
  
  @js.native
  sealed trait backlog
    extends StObject
       with TextFormattingRule
  inline def backlog: backlog = "backlog".asInstanceOf[backlog]
  
  @js.native
  sealed trait category
    extends StObject
       with SortKey
  inline def category: category = "category".asInstanceOf[category]
  
  @js.native
  sealed trait childIssue
    extends StObject
       with SortKey
  inline def childIssue: childIssue = "childIssue".asInstanceOf[childIssue]
  
  @js.native
  sealed trait created
    extends StObject
       with SortKey
  inline def created: created = "created".asInstanceOf[created]
  
  @js.native
  sealed trait createdUser
    extends StObject
       with SortKey
  inline def createdUser: createdUser = "createdUser".asInstanceOf[createdUser]
  
  @js.native
  sealed trait desc
    extends StObject
       with Order
  inline def desc: desc = "desc".asInstanceOf[desc]
  
  @js.native
  sealed trait dueDate
    extends StObject
       with SortKey
  inline def dueDate: dueDate = "dueDate".asInstanceOf[dueDate]
  
  @js.native
  sealed trait estimatedHours
    extends StObject
       with SortKey
  inline def estimatedHours: estimatedHours = "estimatedHours".asInstanceOf[estimatedHours]
  
  @js.native
  sealed trait issueType
    extends StObject
       with SortKey
  inline def issueType: issueType = "issueType".asInstanceOf[issueType]
  
  @js.native
  sealed trait markdown
    extends StObject
       with TextFormattingRule
  inline def markdown: markdown = "markdown".asInstanceOf[markdown]
  
  @js.native
  sealed trait milestone
    extends StObject
       with SortKey
  inline def milestone: milestone = "milestone".asInstanceOf[milestone]
  
  @js.native
  sealed trait priority
    extends StObject
       with SortKey
  inline def priority: priority = "priority".asInstanceOf[priority]
  
  @js.native
  sealed trait sharedFile
    extends StObject
       with SortKey
  inline def sharedFile: sharedFile = "sharedFile".asInstanceOf[sharedFile]
  
  @js.native
  sealed trait startDate
    extends StObject
       with SortKey
  inline def startDate: startDate = "startDate".asInstanceOf[startDate]
  
  @js.native
  sealed trait status
    extends StObject
       with SortKey
  inline def status: status = "status".asInstanceOf[status]
  
  @js.native
  sealed trait summary
    extends StObject
       with SortKey
  inline def summary: summary = "summary".asInstanceOf[summary]
  
  @js.native
  sealed trait updated
    extends StObject
       with SortKey
  inline def updated: updated = "updated".asInstanceOf[updated]
  
  @js.native
  sealed trait updatedUser
    extends StObject
       with SortKey
  inline def updatedUser: updatedUser = "updatedUser".asInstanceOf[updatedUser]
  
  @js.native
  sealed trait version
    extends StObject
       with SortKey
  inline def version: version = "version".asInstanceOf[version]
}
