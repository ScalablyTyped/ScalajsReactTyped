...
object mod {
  ...
  object Consumer {
    ...
    inline def apply(consume: (String, ConsumerCallback[Any]) => Callback): Consumer = {
      val __obj = js.Dynamic.literal(consume = js.Any.fromFunction2((t0: String, t1: ConsumerCallback[Any]) => (consume(t0, t1)).runNow()))
      __obj.asInstanceOf[Consumer]
    }
    
    extension [Self <: Consumer](x: Self) {
      
      inline def setConsume(value: (String, ConsumerCallback[Any]) => Callback): Self = StObject.set(x, "consume", js.Any.fromFunction2((t0: String, t1: ConsumerCallback[Any]) => (value(t0, t1)).runNow()))
    }
  }
