...
trait Access extends StObject {
  ...
  /**
    ...
    */
  def access(request: typingsJapgolly.hapi.mod.Request): Boolean
}
object Access {
  
  inline def apply(access: typingsJapgolly.hapi.mod.Request => Boolean): Access = {
  ...
  extension [Self <: Access](x: Self) {
    ...
    inline def setAccess(value: typingsJapgolly.hapi.mod.Request => Boolean): Self = StObject.set(x, "access", js.Any.fromFunction1(value))
  }
}
