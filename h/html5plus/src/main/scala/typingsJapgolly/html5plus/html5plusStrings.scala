package typingsJapgolly.html5plus

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object html5plusStrings {
  
  @js.native
  sealed trait default extends StObject
  inline def default: default = "default".asInstanceOf[default]
  
  @js.native
  sealed trait `1` extends StObject
  inline def `1`: `1` = "1".asInstanceOf[`1`]
  
  @js.native
  sealed trait `180` extends StObject
  inline def `180`: `180` = "180".asInstanceOf[`180`]
  
  @js.native
  sealed trait `2` extends StObject
  inline def `2`: `2` = "2".asInstanceOf[`2`]
  
  @js.native
  sealed trait `270` extends StObject
  inline def `270`: `270` = "270".asInstanceOf[`270`]
  
  @js.native
  sealed trait `90` extends StObject
  inline def `90`: `90` = "90".asInstanceOf[`90`]
  
  @js.native
  sealed trait ARGB extends StObject
  inline def ARGB: ARGB = "ARGB".asInstanceOf[ARGB]
  
  @js.native
  sealed trait Android extends StObject
  inline def Android: Android = "Android".asInstanceOf[Android]
  
  @js.native
  sealed trait Apple extends StObject
  inline def Apple: Apple = "Apple".asInstanceOf[Apple]
  
  @js.native
  sealed trait GET extends StObject
  inline def GET: GET = "GET".asInstanceOf[GET]
  
  @js.native
  sealed trait Google extends StObject
  inline def Google: Google = "Google".asInstanceOf[Google]
  
  @js.native
  sealed trait ImageSlider extends StObject
  inline def ImageSlider: ImageSlider = "ImageSlider".asInstanceOf[ImageSlider]
  
  @js.native
  sealed trait NView extends StObject
  inline def NView: NView = "NView".asInstanceOf[NView]
  
  @js.native
  sealed trait POST extends StObject
  inline def POST: POST = "POST".asInstanceOf[POST]
  
  @js.native
  sealed trait RGB565 extends StObject
  inline def RGB565: RGB565 = "RGB565".asInstanceOf[RGB565]
  
  @js.native
  sealed trait UIWebview extends StObject
  inline def UIWebview: UIWebview = "UIWebview".asInstanceOf[UIWebview]
  
  @js.native
  sealed trait WKWebview extends StObject
  inline def WKWebview: WKWebview = "WKWebview".asInstanceOf[WKWebview]
  
  @js.native
  sealed trait _docSlash extends StObject
  inline def _docSlash: _docSlash = "_doc/".asInstanceOf[_docSlash]
  
  @js.native
  sealed trait _documentsSlash extends StObject
  inline def _documentsSlash: _documentsSlash = "_documents/".asInstanceOf[_documentsSlash]
  
  @js.native
  sealed trait _downloadsSlash extends StObject
  inline def _downloadsSlash: _downloadsSlash = "_downloads/".asInstanceOf[_downloadsSlash]
  
  @js.native
  sealed trait _empty extends StObject
  inline def _empty: _empty = "".asInstanceOf[_empty]
  
  @js.native
  sealed trait _wwwSlash extends StObject
  inline def _wwwSlash: _wwwSlash = "_www/".asInstanceOf[_wwwSlash]
  
  @js.native
  sealed trait absolute extends StObject
  inline def absolute: absolute = "absolute".asInstanceOf[absolute]
  
  @js.native
  sealed trait adjustPan extends StObject
  inline def adjustPan: adjustPan = "adjustPan".asInstanceOf[adjustPan]
  
  @js.native
  sealed trait adjustResize extends StObject
  inline def adjustResize: adjustResize = "adjustResize".asInstanceOf[adjustResize]
  
  @js.native
  sealed trait all extends StObject
  inline def all: all = "all".asInstanceOf[all]
  
  @js.native
  sealed trait alldevice extends StObject
  inline def alldevice: alldevice = "alldevice".asInstanceOf[alldevice]
  
  @js.native
  sealed trait allow extends StObject
  inline def allow: allow = "allow".asInstanceOf[allow]
  
  @js.native
  sealed trait always extends StObject
  inline def always: always = "always".asInstanceOf[always]
  
  @js.native
  sealed trait amap extends StObject
  inline def amap: amap = "amap".asInstanceOf[amap]
  
  @js.native
  sealed trait auto extends StObject
  inline def auto: auto = "auto".asInstanceOf[auto]
  
  @js.native
  sealed trait back extends StObject
  inline def back: back = "back".asInstanceOf[back]
  
  @js.native
  sealed trait baidu extends StObject
  inline def baidu: baidu = "baidu".asInstanceOf[baidu]
  
  @js.native
  sealed trait barcode extends StObject
  inline def barcode: barcode = "barcode".asInstanceOf[barcode]
  
  @js.native
  sealed trait bd09 extends StObject
  inline def bd09: bd09 = "bd09".asInstanceOf[bd09]
  
  @js.native
  sealed trait bd09ll extends StObject
  inline def bd09ll: bd09ll = "bd09ll".asInstanceOf[bd09ll]
  
  @js.native
  sealed trait black extends StObject
  inline def black: black = "black".asInstanceOf[black]
  
  @js.native
  sealed trait block extends StObject
  inline def block: block = "block".asInstanceOf[block]
  
  @js.native
  sealed trait bold extends StObject
  inline def bold: bold = "bold".asInstanceOf[bold]
  
  @js.native
  sealed trait bottom extends StObject
  inline def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @js.native
  sealed trait bounce extends StObject
  inline def bounce: bounce = "bounce".asInstanceOf[bounce]
  
  @js.native
  sealed trait cacheElseNetwork extends StObject
  inline def cacheElseNetwork: cacheElseNetwork = "cacheElseNetwork".asInstanceOf[cacheElseNetwork]
  
  @js.native
  sealed trait cacheOnly extends StObject
  inline def cacheOnly: cacheOnly = "cacheOnly".asInstanceOf[cacheOnly]
  
  @js.native
  sealed trait center extends StObject
  inline def center: center = "center".asInstanceOf[center]
  
  @js.native
  sealed trait clip extends StObject
  inline def clip: clip = "clip".asInstanceOf[clip]
  
  @js.native
  sealed trait close extends StObject
  inline def close: close = "close".asInstanceOf[close]
  
  @js.native
  sealed trait destructive extends StObject
  inline def destructive: destructive = "destructive".asInstanceOf[destructive]
  
  @js.native
  sealed trait dock extends StObject
  inline def dock: dock = "dock".asInstanceOf[dock]
  
  @js.native
  sealed trait editable extends StObject
  inline def editable: editable = "editable".asInstanceOf[editable]
  
  @js.native
  sealed trait email extends StObject
  inline def email: email = "email".asInstanceOf[email]
  
  @js.native
  sealed trait favorite extends StObject
  inline def favorite: favorite = "favorite".asInstanceOf[favorite]
  
  @js.native
  sealed trait follow extends StObject
  inline def follow: follow = "follow".asInstanceOf[follow]
  
  @js.native
  sealed trait followFinger extends StObject
  inline def followFinger: followFinger = "followFinger".asInstanceOf[followFinger]
  
  @js.native
  sealed trait font extends StObject
  inline def font: font = "font".asInstanceOf[font]
  
  @js.native
  sealed trait forward extends StObject
  inline def forward: forward = "forward".asInstanceOf[forward]
  
  @js.native
  sealed trait gcj02 extends StObject
  inline def gcj02: gcj02 = "gcj02".asInstanceOf[gcj02]
  
  @js.native
  sealed trait hide extends StObject
  inline def hide: hide = "hide".asInstanceOf[hide]
  
  @js.native
  sealed trait home extends StObject
  inline def home: home = "home".asInstanceOf[home]
  
  @js.native
  sealed trait horizontal extends StObject
  inline def horizontal: horizontal = "horizontal".asInstanceOf[horizontal]
  
  @js.native
  sealed trait iOS extends StObject
  inline def iOS: iOS = "iOS".asInstanceOf[iOS]
  
  @js.native
  sealed trait image extends StObject
  inline def image: image = "image".asInstanceOf[image]
  
  @js.native
  sealed trait img extends StObject
  inline def img: img = "img".asInstanceOf[img]
  
  @js.native
  sealed trait `inline` extends StObject
  inline def `inline`: `inline` = "inline".asInstanceOf[`inline`]
  
  @js.native
  sealed trait input extends StObject
  inline def input: input = "input".asInstanceOf[input]
  
  @js.native
  sealed trait instant extends StObject
  inline def instant: instant = "instant".asInstanceOf[instant]
  
  @js.native
  sealed trait italic extends StObject
  inline def italic: italic = "italic".asInstanceOf[italic]
  
  @js.native
  sealed trait jpg extends StObject
  inline def jpg: jpg = "jpg".asInstanceOf[jpg]
  
  @js.native
  sealed trait left extends StObject
  inline def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait `line-through` extends StObject
  inline def `line-through`: `line-through` = "line-through".asInstanceOf[`line-through`]
  
  @js.native
  sealed trait long extends StObject
  inline def long: long = "long".asInstanceOf[long]
  
  @js.native
  sealed trait ltr extends StObject
  inline def ltr: ltr = "ltr".asInstanceOf[ltr]
  
  @js.native
  sealed trait menu extends StObject
  inline def menu: menu = "menu".asInstanceOf[menu]
  
  @js.native
  sealed trait middle extends StObject
  inline def middle: middle = "middle".asInstanceOf[middle]
  
  @js.native
  sealed trait mono extends StObject
  inline def mono: mono = "mono".asInstanceOf[mono]
  
  @js.native
  sealed trait noCache extends StObject
  inline def noCache: noCache = "noCache".asInstanceOf[noCache]
  
  @js.native
  sealed trait none extends StObject
  inline def none: none = "none".asInstanceOf[none]
  
  @js.native
  sealed trait normal extends StObject
  inline def normal: normal = "normal".asInstanceOf[normal]
  
  @js.native
  sealed trait nowrap extends StObject
  inline def nowrap: nowrap = "nowrap".asInstanceOf[nowrap]
  
  @js.native
  sealed trait number extends StObject
  inline def number: number = "number".asInstanceOf[number]
  
  @js.native
  sealed trait onscreen extends StObject
  inline def onscreen: onscreen = "onscreen".asInstanceOf[onscreen]
  
  @js.native
  sealed trait png extends StObject
  inline def png: png = "png".asInstanceOf[png]
  
  @js.native
  sealed trait `pop-in` extends StObject
  inline def `pop-in`: `pop-in` = "pop-in".asInstanceOf[`pop-in`]
  
  @js.native
  sealed trait `pop-out` extends StObject
  inline def `pop-out`: `pop-out` = "pop-out".asInstanceOf[`pop-out`]
  
  @js.native
  sealed trait push extends StObject
  inline def push: push = "push".asInstanceOf[push]
  
  @js.native
  sealed trait rect extends StObject
  inline def rect: rect = "rect".asInstanceOf[rect]
  
  @js.native
  sealed trait redirect extends StObject
  inline def redirect: redirect = "redirect".asInstanceOf[redirect]
  
  @js.native
  sealed trait reject extends StObject
  inline def reject: reject = "reject".asInstanceOf[reject]
  
  @js.native
  sealed trait richtext extends StObject
  inline def richtext: richtext = "richtext".asInstanceOf[richtext]
  
  @js.native
  sealed trait right extends StObject
  inline def right: right = "right".asInstanceOf[right]
  
  @js.native
  sealed trait rtl extends StObject
  inline def rtl: rtl = "rtl".asInstanceOf[rtl]
  
  @js.native
  sealed trait scheme extends StObject
  inline def scheme: scheme = "scheme".asInstanceOf[scheme]
  
  @js.native
  sealed trait search extends StObject
  inline def search: search = "search".asInstanceOf[search]
  
  @js.native
  sealed trait share extends StObject
  inline def share: share = "share".asInstanceOf[share]
  
  @js.native
  sealed trait short extends StObject
  inline def short: short = "short".asInstanceOf[short]
  
  @js.native
  sealed trait shortcut extends StObject
  inline def shortcut: shortcut = "shortcut".asInstanceOf[shortcut]
  
  @js.native
  sealed trait shrink extends StObject
  inline def shrink: shrink = "shrink".asInstanceOf[shrink]
  
  @js.native
  sealed trait silent extends StObject
  inline def silent: silent = "silent".asInstanceOf[silent]
  
  @js.native
  sealed trait `slide-in-right` extends StObject
  inline def `slide-in-right`: `slide-in-right` = "slide-in-right".asInstanceOf[`slide-in-right`]
  
  @js.native
  sealed trait `slide-out-right` extends StObject
  inline def `slide-out-right`: `slide-out-right` = "slide-out-right".asInstanceOf[`slide-out-right`]
  
  @js.native
  sealed trait slient extends StObject
  inline def slient: slient = "slient".asInstanceOf[slient]
  
  @js.native
  sealed trait static extends StObject
  inline def static: static = "static".asInstanceOf[static]
  
  @js.native
  sealed trait stereo extends StObject
  inline def stereo: stereo = "stereo".asInstanceOf[stereo]
  
  @js.native
  sealed trait stream extends StObject
  inline def stream: stream = "stream".asInstanceOf[stream]
  
  @js.native
  sealed trait system extends StObject
  inline def system: system = "system".asInstanceOf[system]
  
  @js.native
  sealed trait tel extends StObject
  inline def tel: tel = "tel".asInstanceOf[tel]
  
  @js.native
  sealed trait text extends StObject
  inline def text: text = "text".asInstanceOf[text]
  
  @js.native
  sealed trait textSlashhtml extends StObject
  inline def textSlashhtml: textSlashhtml = "text/html".asInstanceOf[textSlashhtml]
  
  @js.native
  sealed trait textSlashplain extends StObject
  inline def textSlashplain: textSlashplain = "text/plain".asInstanceOf[textSlashplain]
  
  @js.native
  sealed trait top extends StObject
  inline def top: top = "top".asInstanceOf[top]
  
  @js.native
  sealed trait transmit extends StObject
  inline def transmit: transmit = "transmit".asInstanceOf[transmit]
  
  @js.native
  sealed trait transparent extends StObject
  inline def transparent: transparent = "transparent".asInstanceOf[transparent]
  
  @js.native
  sealed trait underline extends StObject
  inline def underline: underline = "underline".asInstanceOf[underline]
  
  @js.native
  sealed trait url extends StObject
  inline def url: url = "url".asInstanceOf[url]
  
  @js.native
  sealed trait vertical extends StObject
  inline def vertical: vertical = "vertical".asInstanceOf[vertical]
  
  @js.native
  sealed trait video extends StObject
  inline def video: video = "video".asInstanceOf[video]
  
  @js.native
  sealed trait wgs84 extends StObject
  inline def wgs84: wgs84 = "wgs84".asInstanceOf[wgs84]
  
  @js.native
  sealed trait white extends StObject
  inline def white: white = "white".asInstanceOf[white]
}
