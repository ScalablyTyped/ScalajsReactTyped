package typingsJapgolly.httpCodes

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object mod {
  
  @JSImport("http-codes", "ACCEPTED")
  @js.native
  val ACCEPTED: Double = js.native
  
  @JSImport("http-codes", "ALREADY_REPORTED")
  @js.native
  val ALREADY_REPORTED: Double = js.native
  
  @JSImport("http-codes", "BAD_GATEWAY")
  @js.native
  val BAD_GATEWAY: Double = js.native
  
  @JSImport("http-codes", "BAD_REQUEST")
  @js.native
  val BAD_REQUEST: Double = js.native
  
  @JSImport("http-codes", "BANDWIDTH_LIMIT_EXCEEDED")
  @js.native
  val BANDWIDTH_LIMIT_EXCEEDED: Double = js.native
  
  @JSImport("http-codes", "CONFLICT")
  @js.native
  val CONFLICT: Double = js.native
  
  @JSImport("http-codes", "CONTINUE")
  @js.native
  val CONTINUE: Double = js.native
  
  @JSImport("http-codes", "CREATED")
  @js.native
  val CREATED: Double = js.native
  
  @JSImport("http-codes", "EXPECTATION_FAILED")
  @js.native
  val EXPECTATION_FAILED: Double = js.native
  
  @JSImport("http-codes", "FAILED_DEPENDENCY")
  @js.native
  val FAILED_DEPENDENCY: Double = js.native
  
  @JSImport("http-codes", "FORBIDDEN")
  @js.native
  val FORBIDDEN: Double = js.native
  
  @JSImport("http-codes", "FOUND")
  @js.native
  val FOUND: Double = js.native
  
  @JSImport("http-codes", "GATEWAY_TIMEOUT")
  @js.native
  val GATEWAY_TIMEOUT: Double = js.native
  
  @JSImport("http-codes", "GONE")
  @js.native
  val GONE: Double = js.native
  
  @JSImport("http-codes", "HTTP_VERSION_NOT_SUPPORTED")
  @js.native
  val HTTP_VERSION_NOT_SUPPORTED: Double = js.native
  
  @JSImport("http-codes", "IM_A_TEAPOT")
  @js.native
  val IM_A_TEAPOT: Double = js.native
  
  @JSImport("http-codes", "IM_USED")
  @js.native
  val IM_USED: Double = js.native
  
  @JSImport("http-codes", "INSUFFICIENT_STORAGE")
  @js.native
  val INSUFFICIENT_STORAGE: Double = js.native
  
  @JSImport("http-codes", "INTERNAL_SERVER_ERROR")
  @js.native
  val INTERNAL_SERVER_ERROR: Double = js.native
  
  @JSImport("http-codes", "LENGTH_REQUIRED")
  @js.native
  val LENGTH_REQUIRED: Double = js.native
  
  @JSImport("http-codes", "LOCKED")
  @js.native
  val LOCKED: Double = js.native
  
  @JSImport("http-codes", "LOOP_DETECTED")
  @js.native
  val LOOP_DETECTED: Double = js.native
  
  @JSImport("http-codes", "METHOD_NOT_ALLOWED")
  @js.native
  val METHOD_NOT_ALLOWED: Double = js.native
  
  @JSImport("http-codes", "MISDIRECTED_REQUEST")
  @js.native
  val MISDIRECTED_REQUEST: Double = js.native
  
  @JSImport("http-codes", "MOVED_PERMANENTLY")
  @js.native
  val MOVED_PERMANENTLY: Double = js.native
  
  @JSImport("http-codes", "MULTIPLE_CHOICES")
  @js.native
  val MULTIPLE_CHOICES: Double = js.native
  
  @JSImport("http-codes", "MULTI_STATUS")
  @js.native
  val MULTI_STATUS: Double = js.native
  
  @JSImport("http-codes", "NETWORK_AUTHENTICATION_REQUIRED")
  @js.native
  val NETWORK_AUTHENTICATION_REQUIRED: Double = js.native
  
  @JSImport("http-codes", "NON_AUTHORITATIVE_INFORMATION")
  @js.native
  val NON_AUTHORITATIVE_INFORMATION: Double = js.native
  
  @JSImport("http-codes", "NOT_ACCEPTABLE")
  @js.native
  val NOT_ACCEPTABLE: Double = js.native
  
  @JSImport("http-codes", "NOT_EXTENDED")
  @js.native
  val NOT_EXTENDED: Double = js.native
  
  @JSImport("http-codes", "NOT_FOUND")
  @js.native
  val NOT_FOUND: Double = js.native
  
  @JSImport("http-codes", "NOT_IMPLEMENTED")
  @js.native
  val NOT_IMPLEMENTED: Double = js.native
  
  @JSImport("http-codes", "NOT_MODIFIED")
  @js.native
  val NOT_MODIFIED: Double = js.native
  
  @JSImport("http-codes", "NO_CONTENT")
  @js.native
  val NO_CONTENT: Double = js.native
  
  @JSImport("http-codes", "OK")
  @js.native
  val OK: Double = js.native
  
  @JSImport("http-codes", "PARTIAL_CONTENT")
  @js.native
  val PARTIAL_CONTENT: Double = js.native
  
  @JSImport("http-codes", "PAYLOAD_TOO_LARGE")
  @js.native
  val PAYLOAD_TOO_LARGE: Double = js.native
  
  @JSImport("http-codes", "PAYMENT_REQUIRED")
  @js.native
  val PAYMENT_REQUIRED: Double = js.native
  
  @JSImport("http-codes", "PERMANENT_REDIRECT")
  @js.native
  val PERMANENT_REDIRECT: Double = js.native
  
  @JSImport("http-codes", "PRECONDITION_FAILED")
  @js.native
  val PRECONDITION_FAILED: Double = js.native
  
  @JSImport("http-codes", "PRECONDITION_REQUIRED")
  @js.native
  val PRECONDITION_REQUIRED: Double = js.native
  
  @JSImport("http-codes", "PROCESSING")
  @js.native
  val PROCESSING: Double = js.native
  
  @JSImport("http-codes", "PROXY_AUTHENTICATION_REQUIRED")
  @js.native
  val PROXY_AUTHENTICATION_REQUIRED: Double = js.native
  
  @JSImport("http-codes", "RANGE_NOT_SATISFIABLE")
  @js.native
  val RANGE_NOT_SATISFIABLE: Double = js.native
  
  @JSImport("http-codes", "REQUEST_HEADER_FIELDS_TOO_LARGE")
  @js.native
  val REQUEST_HEADER_FIELDS_TOO_LARGE: Double = js.native
  
  @JSImport("http-codes", "REQUEST_TIMEOUT")
  @js.native
  val REQUEST_TIMEOUT: Double = js.native
  
  @JSImport("http-codes", "RESET_CONTENT")
  @js.native
  val RESET_CONTENT: Double = js.native
  
  @JSImport("http-codes", "SEE_OTHER")
  @js.native
  val SEE_OTHER: Double = js.native
  
  @JSImport("http-codes", "SERVICE_UNAVAILABLE")
  @js.native
  val SERVICE_UNAVAILABLE: Double = js.native
  
  @JSImport("http-codes", "SWITCHING_PROTOCOLS")
  @js.native
  val SWITCHING_PROTOCOLS: Double = js.native
  
  @JSImport("http-codes", "TEMPORARY_REDIRECT")
  @js.native
  val TEMPORARY_REDIRECT: Double = js.native
  
  @JSImport("http-codes", "TOO_EARLY")
  @js.native
  val TOO_EARLY: Double = js.native
  
  @JSImport("http-codes", "TOO_MANY_REQUESTS")
  @js.native
  val TOO_MANY_REQUESTS: Double = js.native
  
  @JSImport("http-codes", "UNAUTHORIZED")
  @js.native
  val UNAUTHORIZED: Double = js.native
  
  @JSImport("http-codes", "UNAVAILABLE_FOR_LEGAL_REASONS")
  @js.native
  val UNAVAILABLE_FOR_LEGAL_REASONS: Double = js.native
  
  @JSImport("http-codes", "UNPROCESSABLE_ENTITY")
  @js.native
  val UNPROCESSABLE_ENTITY: Double = js.native
  
  @JSImport("http-codes", "UNSUPPORTED_MEDIA_TYPE")
  @js.native
  val UNSUPPORTED_MEDIA_TYPE: Double = js.native
  
  @JSImport("http-codes", "UPGRADE_REQUIRED")
  @js.native
  val UPGRADE_REQUIRED: Double = js.native
  
  @JSImport("http-codes", "URI_TOO_LONG")
  @js.native
  val URI_TOO_LONG: Double = js.native
  
  @JSImport("http-codes", "USE_PROXY")
  @js.native
  val USE_PROXY: Double = js.native
  
  @JSImport("http-codes", "VARIANT_ALSO_NEGOTIATES")
  @js.native
  val VARIANT_ALSO_NEGOTIATES: Double = js.native
}
