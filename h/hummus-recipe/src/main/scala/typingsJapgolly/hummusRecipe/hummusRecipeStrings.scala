package typingsJapgolly.hummusRecipe

import typingsJapgolly.hummusRecipe.mod.AnnotOptionsFlag
import typingsJapgolly.hummusRecipe.mod.AnnotOptionsIcon
import typingsJapgolly.hummusRecipe.mod.AnnotSubtype
import typingsJapgolly.hummusRecipe.mod.CommentOptionsFlag
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object hummusRecipeStrings {
  
  @js.native
  sealed trait Caret
    extends StObject
       with AnnotSubtype
  inline def Caret: Caret = "Caret".asInstanceOf[Caret]
  
  @js.native
  sealed trait Circle
    extends StObject
       with AnnotSubtype
  inline def Circle: Circle = "Circle".asInstanceOf[Circle]
  
  @js.native
  sealed trait Comment
    extends StObject
       with AnnotOptionsIcon
  inline def Comment: Comment = "Comment".asInstanceOf[Comment]
  
  @js.native
  sealed trait FileAttachment
    extends StObject
       with AnnotSubtype
  inline def FileAttachment: FileAttachment = "FileAttachment".asInstanceOf[FileAttachment]
  
  @js.native
  sealed trait FreeText
    extends StObject
       with AnnotSubtype
  inline def FreeText: FreeText = "FreeText".asInstanceOf[FreeText]
  
  @js.native
  sealed trait Help
    extends StObject
       with AnnotOptionsIcon
  inline def Help: Help = "Help".asInstanceOf[Help]
  
  @js.native
  sealed trait Highlight
    extends StObject
       with AnnotSubtype
  inline def Highlight: Highlight = "Highlight".asInstanceOf[Highlight]
  
  @js.native
  sealed trait Ink
    extends StObject
       with AnnotSubtype
  inline def Ink: Ink = "Ink".asInstanceOf[Ink]
  
  @js.native
  sealed trait Insert
    extends StObject
       with AnnotOptionsIcon
  inline def Insert: Insert = "Insert".asInstanceOf[Insert]
  
  @js.native
  sealed trait Key
    extends StObject
       with AnnotOptionsIcon
  inline def Key: Key = "Key".asInstanceOf[Key]
  
  @js.native
  sealed trait Line
    extends StObject
       with AnnotSubtype
  inline def Line: Line = "Line".asInstanceOf[Line]
  
  @js.native
  sealed trait NewParagraph
    extends StObject
       with AnnotOptionsIcon
  inline def NewParagraph: NewParagraph = "NewParagraph".asInstanceOf[NewParagraph]
  
  @js.native
  sealed trait Note
    extends StObject
       with AnnotOptionsIcon
  inline def Note: Note = "Note".asInstanceOf[Note]
  
  @js.native
  sealed trait Paragraph
    extends StObject
       with AnnotOptionsIcon
  inline def Paragraph: Paragraph = "Paragraph".asInstanceOf[Paragraph]
  
  @js.native
  sealed trait PolyLine
    extends StObject
       with AnnotSubtype
  inline def PolyLine: PolyLine = "PolyLine".asInstanceOf[PolyLine]
  
  @js.native
  sealed trait Polygon
    extends StObject
       with AnnotSubtype
  inline def Polygon: Polygon = "Polygon".asInstanceOf[Polygon]
  
  @js.native
  sealed trait Sound
    extends StObject
       with AnnotSubtype
  inline def Sound: Sound = "Sound".asInstanceOf[Sound]
  
  @js.native
  sealed trait Square
    extends StObject
       with AnnotSubtype
  inline def Square: Square = "Square".asInstanceOf[Square]
  
  @js.native
  sealed trait Squiggly
    extends StObject
       with AnnotSubtype
  inline def Squiggly: Squiggly = "Squiggly".asInstanceOf[Squiggly]
  
  @js.native
  sealed trait Stamp
    extends StObject
       with AnnotSubtype
  inline def Stamp: Stamp = "Stamp".asInstanceOf[Stamp]
  
  @js.native
  sealed trait StrikeOut
    extends StObject
       with AnnotSubtype
  inline def StrikeOut: StrikeOut = "StrikeOut".asInstanceOf[StrikeOut]
  
  @js.native
  sealed trait Text
    extends StObject
       with AnnotSubtype
  inline def Text: Text = "Text".asInstanceOf[Text]
  
  @js.native
  sealed trait Underline
    extends StObject
       with AnnotSubtype
  inline def Underline: Underline = "Underline".asInstanceOf[Underline]
  
  @js.native
  sealed trait cmyk extends StObject
  inline def cmyk: cmyk = "cmyk".asInstanceOf[cmyk]
  
  @js.native
  sealed trait grey extends StObject
  inline def grey: grey = "grey".asInstanceOf[grey]
  
  @js.native
  sealed trait hidden
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def hidden: hidden = "hidden".asInstanceOf[hidden]
  
  @js.native
  sealed trait invisible
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def invisible: invisible = "invisible".asInstanceOf[invisible]
  
  @js.native
  sealed trait locked
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def locked: locked = "locked".asInstanceOf[locked]
  
  @js.native
  sealed trait norotate
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def norotate: norotate = "norotate".asInstanceOf[norotate]
  
  @js.native
  sealed trait noview
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def noview: noview = "noview".asInstanceOf[noview]
  
  @js.native
  sealed trait nozoom
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def nozoom: nozoom = "nozoom".asInstanceOf[nozoom]
  
  @js.native
  sealed trait print
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def print: print = "print".asInstanceOf[print]
  
  @js.native
  sealed trait readonly
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def readonly: readonly = "readonly".asInstanceOf[readonly]
  
  @js.native
  sealed trait rgb extends StObject
  inline def rgb: rgb = "rgb".asInstanceOf[rgb]
  
  @js.native
  sealed trait togglenoview
    extends StObject
       with AnnotOptionsFlag
       with CommentOptionsFlag
  inline def togglenoview: togglenoview = "togglenoview".asInstanceOf[togglenoview]
}
