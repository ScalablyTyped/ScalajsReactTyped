...
- typingsJapgolly.highcharts.mod.SeriesOptions because var conflicts: zIndex. Inlined id, index, legendIndex, mapData, name, stack, `type`, xAxis, yAxis */ trait SeriesBubbleOptions
  extends StObject
     with PlotBubbleOptions {
...
object SeriesBubbleOptions {
  ...
  inline def apply(dataParser: Unit, dataURL: Unit, `type`: String | bubble): SeriesBubbleOptions = {
  ...
  extension [Self <: SeriesBubbleOptions](x: Self) {
    ...
    inline def setType(value: String | bubble): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
