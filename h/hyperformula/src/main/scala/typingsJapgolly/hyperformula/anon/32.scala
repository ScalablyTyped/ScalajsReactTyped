package typingsJapgolly.hyperformula.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait `32` extends StObject {
  
  var method: String
  
  var parameters: js.Array[
    DefaultValueGreaterThanMaxValue | ArgumentTypeDefaultValueGreaterThanMaxValue | DefaultValueNumberGreaterThanUndefined
  ]
}
object `32` {
  
  inline def apply(
    method: String,
    parameters: js.Array[
      DefaultValueGreaterThanMaxValue | ArgumentTypeDefaultValueGreaterThanMaxValue | DefaultValueNumberGreaterThanUndefined
    ]
  ): `32` = {
    val __obj = js.Dynamic.literal(method = method.asInstanceOf[js.Any], parameters = parameters.asInstanceOf[js.Any])
    __obj.asInstanceOf[`32`]
  }
  
  extension [Self <: `32`](x: Self) {
    
    inline def setMethod(value: String): Self = StObject.set(x, "method", value.asInstanceOf[js.Any])
    
    inline def setParameters(
      value: js.Array[
          DefaultValueGreaterThanMaxValue | ArgumentTypeDefaultValueGreaterThanMaxValue | DefaultValueNumberGreaterThanUndefined
        ]
    ): Self = StObject.set(x, "parameters", value.asInstanceOf[js.Any])
    
    inline def setParametersVarargs(
      value: (DefaultValueGreaterThanMaxValue | ArgumentTypeDefaultValueGreaterThanMaxValue | DefaultValueNumberGreaterThanUndefined)*
    ): Self = StObject.set(x, "parameters", js.Array(value*))
  }
}
