...
import typingsJapgolly.hyperformula.anon.Address
...
import typingsJapgolly.hyperformula.typingsOperationsMod.AddColumnsCommand
import typingsJapgolly.hyperformula.typingsOperationsMod.AddRowsCommand
import typingsJapgolly.hyperformula.typingsOperationsMod.ColumnsRemoval
import typingsJapgolly.hyperformula.typingsOperationsMod.Operations
import typingsJapgolly.hyperformula.typingsOperationsMod.RemoveColumnsCommand
import typingsJapgolly.hyperformula.typingsOperationsMod.RemoveRowsCommand
import typingsJapgolly.hyperformula.typingsOperationsMod.RowsRemoval
...
object typingsUndoRedoMod {
  ...
  object UndoEntry {
    ...
    inline def apply(doRedo: UndoRedo => Callback, doUndo: UndoRedo => Callback): UndoEntry = {
      val __obj = js.Dynamic.literal(doRedo = js.Any.fromFunction1((t0: UndoRedo) => doRedo(t0).runNow()), doUndo = js.Any.fromFunction1((t0: UndoRedo) => doUndo(t0).runNow()))
      __obj.asInstanceOf[UndoEntry]
    }
    
    extension [Self <: UndoEntry](x: Self) {
      
      inline def setDoRedo(value: UndoRedo => Callback): Self = StObject.set(x, "doRedo", js.Any.fromFunction1((t0: UndoRedo) => value(t0).runNow()))
      
      inline def setDoUndo(value: UndoRedo => Callback): Self = StObject.set(x, "doUndo", js.Any.fromFunction1((t0: UndoRedo) => value(t0).runNow()))
    }
  }
}
