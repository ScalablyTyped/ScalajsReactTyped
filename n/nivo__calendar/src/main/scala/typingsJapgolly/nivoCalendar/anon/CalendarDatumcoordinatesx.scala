...
/* Inlined @nivo/calendar.@nivo/calendar/dist/types/types.CalendarDatum & {  coordinates :{  x :number,   y :number},   date :std.Date,   firstWeek :number,   month :number,   year :number,   color :string,   width :number,   height :number} */
trait CalendarDatumcoordinatesx
  extends StObject
...
object CalendarDatumcoordinatesx {
