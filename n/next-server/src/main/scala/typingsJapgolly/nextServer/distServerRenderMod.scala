...
import typingsJapgolly.nextServer.anon.ErrorError
import typingsJapgolly.nextServer.distLibUtilsMod.AppType
import typingsJapgolly.nextServer.distLibUtilsMod.DocumentType
import typingsJapgolly.nextServer.distLibUtilsMod.NextPageContext
import typingsJapgolly.nextServer.distServerGetPageFilesMod.BuildManifest
...
object distServerRenderMod {
  ...
  object RenderOpts {
    ...
    extension [Self <: RenderOpts](x: Self) {
      ...
      inline def setDocumentMiddleware(value: NextPageContext => Callback): Self = StObject.set(x, "DocumentMiddleware", js.Any.fromFunction1((t0: NextPageContext) => value(t0).runNow()))
