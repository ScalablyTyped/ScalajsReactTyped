package typingsJapgolly.nodeImap

import typingsJapgolly.nodeImap.mod.SortCriteria
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object nodeImapStrings {
  
  @js.native
  sealed trait `-ARRIVAL`
    extends StObject
       with SortCriteria
  inline def `-ARRIVAL`: `-ARRIVAL` = "-ARRIVAL".asInstanceOf[`-ARRIVAL`]
  
  @js.native
  sealed trait `-CC`
    extends StObject
       with SortCriteria
  inline def `-CC`: `-CC` = "-CC".asInstanceOf[`-CC`]
  
  @js.native
  sealed trait `-DATE`
    extends StObject
       with SortCriteria
  inline def `-DATE`: `-DATE` = "-DATE".asInstanceOf[`-DATE`]
  
  @js.native
  sealed trait `-FROM`
    extends StObject
       with SortCriteria
  inline def `-FROM`: `-FROM` = "-FROM".asInstanceOf[`-FROM`]
  
  @js.native
  sealed trait `-SIZE`
    extends StObject
       with SortCriteria
  inline def `-SIZE`: `-SIZE` = "-SIZE".asInstanceOf[`-SIZE`]
  
  @js.native
  sealed trait `-SUBJECT`
    extends StObject
       with SortCriteria
  inline def `-SUBJECT`: `-SUBJECT` = "-SUBJECT".asInstanceOf[`-SUBJECT`]
  
  @js.native
  sealed trait `-TO`
    extends StObject
       with SortCriteria
  inline def `-TO`: `-TO` = "-TO".asInstanceOf[`-TO`]
  
  @js.native
  sealed trait ARRIVAL
    extends StObject
       with SortCriteria
  inline def ARRIVAL: ARRIVAL = "ARRIVAL".asInstanceOf[ARRIVAL]
  
  @js.native
  sealed trait CC
    extends StObject
       with SortCriteria
  inline def CC: CC = "CC".asInstanceOf[CC]
  
  @js.native
  sealed trait DATE
    extends StObject
       with SortCriteria
  inline def DATE: DATE = "DATE".asInstanceOf[DATE]
  
  @js.native
  sealed trait FROM
    extends StObject
       with SortCriteria
  inline def FROM: FROM = "FROM".asInstanceOf[FROM]
  
  @js.native
  sealed trait SIZE
    extends StObject
       with SortCriteria
  inline def SIZE: SIZE = "SIZE".asInstanceOf[SIZE]
  
  @js.native
  sealed trait SUBJECT
    extends StObject
       with SortCriteria
  inline def SUBJECT: SUBJECT = "SUBJECT".asInstanceOf[SUBJECT]
  
  @js.native
  sealed trait TO
    extends StObject
       with SortCriteria
  inline def TO: TO = "TO".asInstanceOf[TO]
  
  @js.native
  sealed trait attributes extends StObject
  inline def attributes: attributes = "attributes".asInstanceOf[attributes]
  
  @js.native
  sealed trait body extends StObject
  inline def body: body = "body".asInstanceOf[body]
  
  @js.native
  sealed trait end extends StObject
  inline def end: end = "end".asInstanceOf[end]
  
  @js.native
  sealed trait error extends StObject
  inline def error: error = "error".asInstanceOf[error]
  
  @js.native
  sealed trait message extends StObject
  inline def message: message = "message".asInstanceOf[message]
}
