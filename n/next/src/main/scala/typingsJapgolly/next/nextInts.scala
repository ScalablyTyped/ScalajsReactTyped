package typingsJapgolly.next

import typingsJapgolly.next.distServerDevOnDemandEntryHandlerMod.EntryTypes
import typingsJapgolly.next.distServerLibSquooshAvifAvifEncMod.AVIFTune
import typingsJapgolly.next.distServerLibSquooshMozjpegMozjpegEncMod.MozJpegColorSpace
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object nextInts {
  
  @js.native
  sealed trait `-1` extends StObject
  inline def `-1`: `-1` = -1.asInstanceOf[`-1`]
  
  @js.native
  sealed trait `0`
    extends StObject
       with AVIFTune
       with EntryTypes
       with MozJpegColorSpace
  inline def `0`: `0` = 0.asInstanceOf[`0`]
  
  @js.native
  sealed trait `1`
    extends StObject
       with AVIFTune
       with EntryTypes
       with MozJpegColorSpace
  inline def `1`: `1` = 1.asInstanceOf[`1`]
  
  @js.native
  sealed trait `100` extends StObject
  inline def `100`: `100` = 100.asInstanceOf[`100`]
  
  @js.native
  sealed trait `2`
    extends StObject
       with AVIFTune
  inline def `2`: `2` = 2.asInstanceOf[`2`]
  
  @js.native
  sealed trait `3` extends StObject
  inline def `3`: `3` = 3.asInstanceOf[`3`]
  
  @js.native
  sealed trait `301` extends StObject
  inline def `301`: `301` = 301.asInstanceOf[`301`]
  
  @js.native
  sealed trait `302` extends StObject
  inline def `302`: `302` = 302.asInstanceOf[`302`]
  
  @js.native
  sealed trait `303` extends StObject
  inline def `303`: `303` = 303.asInstanceOf[`303`]
  
  @js.native
  sealed trait `307` extends StObject
  inline def `307`: `307` = 307.asInstanceOf[`307`]
  
  @js.native
  sealed trait `308` extends StObject
  inline def `308`: `308` = 308.asInstanceOf[`308`]
  
  @js.native
  sealed trait `33` extends StObject
  inline def `33`: `33` = 33.asInstanceOf[`33`]
  
  @js.native
  sealed trait `4` extends StObject
  inline def `4`: `4` = 4.asInstanceOf[`4`]
  
  @js.native
  sealed trait `50` extends StObject
  inline def `50`: `50` = 50.asInstanceOf[`50`]
  
  @js.native
  sealed trait `6` extends StObject
  inline def `6`: `6` = 6.asInstanceOf[`6`]
  
  @js.native
  sealed trait `60` extends StObject
  inline def `60`: `60` = 60.asInstanceOf[`60`]
  
  @js.native
  sealed trait `62` extends StObject
  inline def `62`: `62` = 62.asInstanceOf[`62`]
  
  @js.native
  sealed trait `75` extends StObject
  inline def `75`: `75` = 75.asInstanceOf[`75`]
}
