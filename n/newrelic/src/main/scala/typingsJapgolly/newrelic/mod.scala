...
import typingsJapgolly.newrelic.anon.CollectPendingData
import typingsJapgolly.newrelic.anon.HasToRemoveScriptWrapper
import typingsJapgolly.newrelic.anon.ModuleName
...
object mod {
  ...
  object DistributedTracePayload {
    ...
    inline def apply(httpSafe: CallbackTo[String], text: CallbackTo[String]): DistributedTracePayload = {
      val __obj = js.Dynamic.literal(httpSafe = httpSafe.toJsFn, text = text.toJsFn)
      __obj.asInstanceOf[DistributedTracePayload]
    }
    
    extension [Self <: DistributedTracePayload](x: Self) {
      
      inline def setHttpSafe(value: CallbackTo[String]): Self = StObject.set(x, "httpSafe", value.toJsFn)
