...
trait WebAuthenticationChallengeDetails
  ...
  var challenger: typingsJapgolly.chrome.chrome.webRequest.WebAuthChallenger
...
object WebAuthenticationChallengeDetails {
  ...
  inline def apply(
    challenger: typingsJapgolly.chrome.chrome.webRequest.WebAuthChallenger,
    frameId: Double,
    isProxy: Boolean,
    method: String,
  ...
  extension [Self <: WebAuthenticationChallengeDetails](x: Self) {
    ...
    inline def setChallenger(value: typingsJapgolly.chrome.chrome.webRequest.WebAuthChallenger): Self = StObject.set(x, "challenger", value.asInstanceOf[js.Any])
