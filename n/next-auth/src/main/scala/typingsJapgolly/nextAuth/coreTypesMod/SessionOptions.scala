...
object SessionOptions {
  ...
  inline def apply(
    generateSessionToken: CallbackTo[String],
    maxAge: Double,
    strategy: SessionStrategy,
    updateAge: Double
  ): SessionOptions = {
    val __obj = js.Dynamic.literal(generateSessionToken = generateSessionToken.toJsFn, maxAge = maxAge.asInstanceOf[js.Any], strategy = strategy.asInstanceOf[js.Any], updateAge = updateAge.asInstanceOf[js.Any])
    __obj.asInstanceOf[SessionOptions]
  }
  
  extension [Self <: SessionOptions](x: Self) {
    
    inline def setGenerateSessionToken(value: CallbackTo[String]): Self = StObject.set(x, "generateSessionToken", value.toJsFn)
