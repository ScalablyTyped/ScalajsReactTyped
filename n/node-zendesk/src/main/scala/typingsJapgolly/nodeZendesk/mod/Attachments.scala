...
import typingsJapgolly.nodeZendesk.anon.Filename
...
object Attachments {
  ...
  trait UploadResponseModel extends StObject {
    ...
    var upload: typingsJapgolly.nodeZendesk.anon.Attachment
  }
  object UploadResponseModel {
    
    inline def apply(upload: typingsJapgolly.nodeZendesk.anon.Attachment): UploadResponseModel = {
    ...
    extension [Self <: UploadResponseModel](x: Self) {
      ...
      inline def setUpload(value: typingsJapgolly.nodeZendesk.anon.Attachment): Self = StObject.set(x, "upload", value.asInstanceOf[js.Any])
    }
  }
}
