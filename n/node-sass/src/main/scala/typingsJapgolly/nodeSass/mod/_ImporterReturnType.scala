package typingsJapgolly.nodeSass.mod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait _ImporterReturnType extends StObject
object _ImporterReturnType {
  
  inline def Contents(contents: String): typingsJapgolly.nodeSass.anon.Contents = {
    val __obj = js.Dynamic.literal(contents = contents.asInstanceOf[js.Any])
    __obj.asInstanceOf[typingsJapgolly.nodeSass.anon.Contents]
  }
  
  inline def Error(___SASS_ERROR___ : js.Symbol): typingsJapgolly.nodeSass.mod.types.Error = {
    val __obj = js.Dynamic.literal(___SASS_ERROR___ = ___SASS_ERROR___.asInstanceOf[js.Any])
    __obj.asInstanceOf[typingsJapgolly.nodeSass.mod.types.Error]
  }
  
  inline def File(file: String): typingsJapgolly.nodeSass.anon.File = {
    val __obj = js.Dynamic.literal(file = file.asInstanceOf[js.Any])
    __obj.asInstanceOf[typingsJapgolly.nodeSass.anon.File]
  }
  
  inline def Null(___NULL___ : js.Symbol): typingsJapgolly.nodeSass.mod.types.Null = {
    val __obj = js.Dynamic.literal(___NULL___ = ___NULL___.asInstanceOf[js.Any])
    __obj.asInstanceOf[typingsJapgolly.nodeSass.mod.types.Null]
  }
}
