...
import typingsJapgolly.nodeJose.anon.Algorithms
import typingsJapgolly.nodeJose.anon.AllowEmbeddedKey
...
object JWS {
  ...
  trait VerifyOptions extends StObject {
    ...
    var handlers: typingsJapgolly.nodeJose.anon.Exp
  }
  object VerifyOptions {
    
    inline def apply(handlers: typingsJapgolly.nodeJose.anon.Exp): VerifyOptions = {
    ...
    extension [Self <: VerifyOptions](x: Self) {
      ...
      inline def setHandlers(value: typingsJapgolly.nodeJose.anon.Exp): Self = StObject.set(x, "handlers", value.asInstanceOf[js.Any])
    }
  }
}
