package typingsJapgolly.nginstackEngine

import typingsJapgolly.nginstackEngine.libCryptoDigestTypeMod.DigestType
import typingsJapgolly.nginstackEngine.libDatasetDataSetDataTypeMod.DataSetFieldType
import typingsJapgolly.nginstackEngine.libDateDateFormatMod.DateFormatType
import typingsJapgolly.nginstackEngine.libDomNodeMod.NodeType
import typingsJapgolly.nginstackEngine.libMailPop3Mod.AuthMode
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object nginstackEngineStrings {
  
  @js.native
  sealed trait `1`
    extends StObject
       with NodeType
  inline def `1`: `1` = "1".asInstanceOf[`1`]
  
  @js.native
  sealed trait `10`
    extends StObject
       with NodeType
  inline def `10`: `10` = "10".asInstanceOf[`10`]
  
  @js.native
  sealed trait `11`
    extends StObject
       with NodeType
  inline def `11`: `11` = "11".asInstanceOf[`11`]
  
  @js.native
  sealed trait `12`
    extends StObject
       with NodeType
  inline def `12`: `12` = "12".asInstanceOf[`12`]
  
  @js.native
  sealed trait `2`
    extends StObject
       with NodeType
  inline def `2`: `2` = "2".asInstanceOf[`2`]
  
  @js.native
  sealed trait `3`
    extends StObject
       with NodeType
  inline def `3`: `3` = "3".asInstanceOf[`3`]
  
  @js.native
  sealed trait `4`
    extends StObject
       with NodeType
  inline def `4`: `4` = "4".asInstanceOf[`4`]
  
  @js.native
  sealed trait `5`
    extends StObject
       with NodeType
  inline def `5`: `5` = "5".asInstanceOf[`5`]
  
  @js.native
  sealed trait `6`
    extends StObject
       with NodeType
  inline def `6`: `6` = "6".asInstanceOf[`6`]
  
  @js.native
  sealed trait `7`
    extends StObject
       with NodeType
  inline def `7`: `7` = "7".asInstanceOf[`7`]
  
  @js.native
  sealed trait `8`
    extends StObject
       with NodeType
  inline def `8`: `8` = "8".asInstanceOf[`8`]
  
  @js.native
  sealed trait `9`
    extends StObject
       with NodeType
  inline def `9`: `9` = "9".asInstanceOf[`9`]
  
  @js.native
  sealed trait APOP
    extends StObject
       with AuthMode
  inline def APOP: APOP = "APOP".asInstanceOf[APOP]
  
  @js.native
  sealed trait DDMM
    extends StObject
       with DateFormatType
  inline def DDMM: DDMM = "DDMM".asInstanceOf[DDMM]
  
  @js.native
  sealed trait DDMMYYYY
    extends StObject
       with DateFormatType
  inline def DDMMYYYY: DDMMYYYY = "DDMMYYYY".asInstanceOf[DDMMYYYY]
  
  @js.native
  sealed trait MMMYYYY
    extends StObject
       with DateFormatType
  inline def MMMYYYY: MMMYYYY = "MMMYYYY".asInstanceOf[MMMYYYY]
  
  @js.native
  sealed trait MMYYYY
    extends StObject
       with DateFormatType
  inline def MMYYYY: MMYYYY = "MMYYYY".asInstanceOf[MMYYYY]
  
  @js.native
  sealed trait PASS
    extends StObject
       with AuthMode
  inline def PASS: PASS = "PASS".asInstanceOf[PASS]
  
  @js.native
  sealed trait WWYYYY
    extends StObject
       with DateFormatType
  inline def WWYYYY: WWYYYY = "WWYYYY".asInstanceOf[WWYYYY]
  
  @js.native
  sealed trait YYYYWW
    extends StObject
       with DateFormatType
  inline def YYYYWW: YYYYWW = "YYYYWW".asInstanceOf[YYYYWW]
  
  @js.native
  sealed trait _empty extends StObject
  inline def _empty: _empty = "".asInstanceOf[_empty]
  
  @js.native
  sealed trait arraybuffer
    extends StObject
       with DigestType
  inline def arraybuffer: arraybuffer = "arraybuffer".asInstanceOf[arraybuffer]
  
  @js.native
  sealed trait bigint extends StObject
  inline def bigint: bigint = "bigint".asInstanceOf[bigint]
  
  @js.native
  sealed trait binarystring
    extends StObject
       with DigestType
  inline def binarystring: binarystring = "binarystring".asInstanceOf[binarystring]
  
  @js.native
  sealed trait boolean
    extends StObject
       with DataSetFieldType
  inline def boolean: boolean = "boolean".asInstanceOf[boolean]
  
  @js.native
  sealed trait date
    extends StObject
       with DataSetFieldType
  inline def date: date = "date".asInstanceOf[date]
  
  @js.native
  sealed trait datetime
    extends StObject
       with DataSetFieldType
  inline def datetime: datetime = "datetime".asInstanceOf[datetime]
  
  @js.native
  sealed trait function extends StObject
  inline def function: function = "function".asInstanceOf[function]
  
  @js.native
  sealed trait int32
    extends StObject
       with DataSetFieldType
  inline def int32: int32 = "int32".asInstanceOf[int32]
  
  @js.native
  sealed trait int64
    extends StObject
       with DataSetFieldType
  inline def int64: int64 = "int64".asInstanceOf[int64]
  
  @js.native
  sealed trait memo
    extends StObject
       with DataSetFieldType
  inline def memo: memo = "memo".asInstanceOf[memo]
  
  @js.native
  sealed trait number
    extends StObject
       with DataSetFieldType
  inline def number: number = "number".asInstanceOf[number]
  
  @js.native
  sealed trait `object` extends StObject
  inline def `object`: `object` = "object".asInstanceOf[`object`]
  
  @js.native
  sealed trait string
    extends StObject
       with DataSetFieldType
  inline def string: string = "string".asInstanceOf[string]
  
  @js.native
  sealed trait symbol extends StObject
  inline def symbol: symbol = "symbol".asInstanceOf[symbol]
  
  @js.native
  sealed trait uint8array
    extends StObject
       with DigestType
  inline def uint8array: uint8array = "uint8array".asInstanceOf[uint8array]
  
  @js.native
  sealed trait undefined extends StObject
  inline def undefined: undefined = "undefined".asInstanceOf[undefined]
}
