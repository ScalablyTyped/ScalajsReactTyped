...
object libDatabaseDbtrackerMod {
  ...
  inline def apply(opt_options: typingsJapgolly.nginstackEngine.anon.Database): Unit = ^.asInstanceOf[js.Dynamic].apply(opt_options.asInstanceOf[js.Any]).asInstanceOf[Unit]
  ...
  open class ^ ()
    extends StObject
       with DBTracker {
    def this(opt_options: typingsJapgolly.nginstackEngine.anon.Database) = this()
  ...
  object DBTracker {
    ...
    extension [Self <: DBTracker](x: Self) {
      ...
      inline def setUndoChanges(value: CallbackTo[Double]): Self = StObject.set(x, "undoChanges", value.toJsFn)
    }
  }
