...
import typingsJapgolly.nobleSecp256k1.nobleSecp256k1Booleans.`false`
import typingsJapgolly.nobleSecp256k1.nobleSecp256k1Booleans.`true`
...
object mod {
  ...
  object SchnorrSignature {
    ...
    inline def apply(
      assertValidity: Callback,
      r: js.BigInt,
      s: js.BigInt,
      toHex: CallbackTo[String],
      toRawBytes: CallbackTo[js.typedarray.Uint8Array]
    ): SchnorrSignature = {
      val __obj = js.Dynamic.literal(assertValidity = assertValidity.toJsFn, r = r.asInstanceOf[js.Any], s = s.asInstanceOf[js.Any], toHex = toHex.toJsFn, toRawBytes = toRawBytes.toJsFn)
      __obj.asInstanceOf[SchnorrSignature]
    }
    
    extension [Self <: SchnorrSignature](x: Self) {
      
      inline def setAssertValidity(value: Callback): Self = StObject.set(x, "assertValidity", value.toJsFn)
      ...
      inline def setToRawBytes(value: CallbackTo[js.typedarray.Uint8Array]): Self = StObject.set(x, "toRawBytes", value.toJsFn)
    }
  }
