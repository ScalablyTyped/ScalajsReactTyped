...
import typingsJapgolly.nise.anon.Abort
import typingsJapgolly.nise.anon.PartialFakeServerOptions
...
object mod {
  ...
  object FakeXHR_ {
    ...
    inline def apply(
      ...
      val __obj = js.Dynamic.literal(FakeXMLHttpRequest = FakeXMLHttpRequest.asInstanceOf[js.Any], useFakeXMLHttpRequest = useFakeXMLHttpRequest.toJsFn)
    ...
    extension [Self <: FakeXHR_](x: Self) {
      ...
      inline def setUseFakeXMLHttpRequest(value: CallbackTo[FakeXMLHttpRequestStatic]): Self = StObject.set(x, "useFakeXMLHttpRequest", value.toJsFn)
    }
  }
