...
import typingsJapgolly.node.childProcessMod.MessageOptions
import typingsJapgolly.node.childProcessMod.SendHandle
import typingsJapgolly.node.childProcessMod.Serializable
...
import typingsJapgolly.node.nodeColonnetMod.Server
...
import typingsJapgolly.node.nodeStrings.fork
...
object clusterMod extends Shortcut {
  ...
  object ClusterSettings {
    ...
    extension [Self <: ClusterSettings](x: Self) {
      ...
      inline def setInspectPortCallbackTo(value: CallbackTo[Double]): Self = StObject.set(x, "inspectPort", value.toJsFn)
