...
import typingsJapgolly.nodemailer.libSmtpPoolMod.Options
import typingsJapgolly.nodemailer.libSmtpPoolMod.SentMessageInfo
...
object libSmtpPoolPoolResourceMod {
  ...
  open class ^ protected () extends PoolResource {
    def this(pool: typingsJapgolly.nodemailer.libSmtpPoolMod.^) = this()
  }
  ...
  trait PoolResource extends EventEmitter {
    ...
    var pool: typingsJapgolly.nodemailer.libSmtpPoolMod.^ = js.native
    ...
    def send(
      mail: typingsJapgolly.nodemailer.libMailerMailMessageMod.^[Any],
