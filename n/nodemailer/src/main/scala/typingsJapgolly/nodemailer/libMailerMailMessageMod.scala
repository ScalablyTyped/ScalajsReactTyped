...
object libMailerMailMessageMod {
  ...
  open class ^[T] protected ()
    extends StObject
       with MailMessage[T] {
    def this(mailer: typingsJapgolly.nodemailer.libMailerMod.^[T], data: Options) = this()
  }
  ...
  trait MailMessage[T] extends StObject {
    ...
    var mailer: typingsJapgolly.nodemailer.libMailerMod.^[T] = js.native
    
    var message: typingsJapgolly.nodemailer.libMimeNodeMod.^ = js.native
