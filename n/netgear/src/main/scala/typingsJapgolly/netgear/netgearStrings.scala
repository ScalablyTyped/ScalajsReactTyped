package typingsJapgolly.netgear

import typingsJapgolly.netgear.mod.Band
import typingsJapgolly.netgear.mod.Channel
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object netgearStrings {
  
  @js.native
  sealed trait `1`
    extends StObject
       with Channel
  inline def `1`: `1` = "1".asInstanceOf[`1`]
  
  @js.native
  sealed trait `10`
    extends StObject
       with Channel
  inline def `10`: `10` = "10".asInstanceOf[`10`]
  
  @js.native
  sealed trait `11`
    extends StObject
       with Channel
  inline def `11`: `11` = "11".asInstanceOf[`11`]
  
  @js.native
  sealed trait `12`
    extends StObject
       with Channel
  inline def `12`: `12` = "12".asInstanceOf[`12`]
  
  @js.native
  sealed trait `13`
    extends StObject
       with Channel
  inline def `13`: `13` = "13".asInstanceOf[`13`]
  
  @js.native
  sealed trait `2`
    extends StObject
       with Channel
  inline def `2`: `2` = "2".asInstanceOf[`2`]
  
  @js.native
  sealed trait `2Dot4G`
    extends StObject
       with Band
  inline def `2Dot4G`: `2Dot4G` = "2.4G".asInstanceOf[`2Dot4G`]
  
  @js.native
  sealed trait `3`
    extends StObject
       with Channel
  inline def `3`: `3` = "3".asInstanceOf[`3`]
  
  @js.native
  sealed trait `4`
    extends StObject
       with Channel
  inline def `4`: `4` = "4".asInstanceOf[`4`]
  
  @js.native
  sealed trait `5`
    extends StObject
       with Channel
  inline def `5`: `5` = "5".asInstanceOf[`5`]
  
  @js.native
  sealed trait `5G`
    extends StObject
       with Band
  inline def `5G`: `5G` = "5G".asInstanceOf[`5G`]
  
  @js.native
  sealed trait `5G1`
    extends StObject
       with Band
  inline def `5G1`: `5G1` = "5G1".asInstanceOf[`5G1`]
  
  @js.native
  sealed trait `6`
    extends StObject
       with Channel
  inline def `6`: `6` = "6".asInstanceOf[`6`]
  
  @js.native
  sealed trait `7`
    extends StObject
       with Channel
  inline def `7`: `7` = "7".asInstanceOf[`7`]
  
  @js.native
  sealed trait `8`
    extends StObject
       with Channel
  inline def `8`: `8` = "8".asInstanceOf[`8`]
  
  @js.native
  sealed trait `9`
    extends StObject
       with Channel
  inline def `9`: `9` = "9".asInstanceOf[`9`]
  
  @js.native
  sealed trait Allow extends StObject
  inline def Allow: Allow = "Allow".asInstanceOf[Allow]
  
  @js.native
  sealed trait Auto
    extends StObject
       with Channel
  inline def Auto: Auto = "Auto".asInstanceOf[Auto]
  
  @js.native
  sealed trait Block extends StObject
  inline def Block: Block = "Block".asInstanceOf[Block]
}
