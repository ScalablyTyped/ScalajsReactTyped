...
import typingsJapgolly.nodeObjectHash.distObjectSorterMod.SorterOptions
...
object mod {
  ...
  object Hashable {
    ...
    inline def apply(toHashableString: CallbackTo[String]): Hashable = {
      val __obj = js.Dynamic.literal(toHashableString = toHashableString.toJsFn)
      __obj.asInstanceOf[Hashable]
    }
    
    extension [Self <: Hashable](x: Self) {
      
      inline def setToHashableString(value: CallbackTo[String]): Self = StObject.set(x, "toHashableString", value.toJsFn)
    }
  }
