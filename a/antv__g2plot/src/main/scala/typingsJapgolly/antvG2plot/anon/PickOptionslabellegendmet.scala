...
/* Inlined std.Pick<@antv/g2plot.@antv/g2plot/lib/types.Options, 'label' | 'legend' | 'meta' | 'padding' | 'animation' | 'color' | 'height' | 'width' | 'appendPadding' | 'renderer' | 'pixelRatio' | 'autoFit' | 'syncViewPadding' | 'supportCSSTransform' | 'limitInPlot' | 'locale' | 'defaultInteractions' | 'localRefresh' | 'useDeferredLabel' | 'theme' | 'pattern' | 'xAxis' | 'yAxis' | 'tooltip' | 'slider' | 'scrollbar' | 'interactions' | 'annotations' | 'state'> */
trait PickOptionslabellegendmet extends StObject {
...
object PickOptionslabellegendmet {
  
  inline def apply(): PickOptionslabellegendmet = {
