package typingsJapgolly.activexWord.global.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.DiagramNodes")
@js.native
/* private */ open class DiagramNodes ()
  extends StObject
     with typingsJapgolly.activexWord.Word.DiagramNodes {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  override val Count: Double = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Item(Index: Any): typingsJapgolly.activexWord.Word.DiagramNode = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override def SelectAll(): Unit = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.DiagramNodes_typekey")
  var WordDotDiagramNodes_typekey: typingsJapgolly.activexWord.Word.DiagramNodes = js.native
}
