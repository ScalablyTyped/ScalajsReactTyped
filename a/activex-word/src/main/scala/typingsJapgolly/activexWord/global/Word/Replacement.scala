package typingsJapgolly.activexWord.global.Word

import typingsJapgolly.activexWord.Word.WdLanguageID
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.Replacement")
@js.native
/* private */ open class Replacement ()
  extends StObject
     with typingsJapgolly.activexWord.Word.Replacement {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  override def ClearFormatting(): Unit = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  var Font: typingsJapgolly.activexWord.Word.Font = js.native
  
  /* CompleteClass */
  override val Frame: typingsJapgolly.activexWord.Word.Frame = js.native
  
  /* CompleteClass */
  var Highlight: Double = js.native
  
  /* CompleteClass */
  var LanguageID: WdLanguageID = js.native
  
  /* CompleteClass */
  var LanguageIDFarEast: WdLanguageID = js.native
  
  /* CompleteClass */
  var NoProofing: Double = js.native
  
  /* CompleteClass */
  var ParagraphFormat: typingsJapgolly.activexWord.Word.ParagraphFormat = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var Style: Any = js.native
  
  /* CompleteClass */
  var Text: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.Replacement_typekey")
  var WordDotReplacement_typekey: typingsJapgolly.activexWord.Word.Replacement = js.native
}
