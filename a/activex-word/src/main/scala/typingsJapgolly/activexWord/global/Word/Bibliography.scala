package typingsJapgolly.activexWord.global.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.Bibliography")
@js.native
/* private */ open class Bibliography ()
  extends StObject
     with typingsJapgolly.activexWord.Word.Bibliography {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  var BibliographyStyle: String = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def GenerateUniqueTag(): String = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override val Sources: typingsJapgolly.activexWord.Word.Sources = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.Bibliography_typekey")
  var WordDotBibliography_typekey: typingsJapgolly.activexWord.Word.Bibliography = js.native
}
