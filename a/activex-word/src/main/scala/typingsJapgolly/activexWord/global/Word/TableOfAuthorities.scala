package typingsJapgolly.activexWord.global.Word

import typingsJapgolly.activexWord.Word.WdTabLeader
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.TableOfAuthorities")
@js.native
/* private */ open class TableOfAuthorities ()
  extends StObject
     with typingsJapgolly.activexWord.Word.TableOfAuthorities {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  var Bookmark: String = js.native
  
  /* CompleteClass */
  var Category: Double = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  var EntrySeparator: String = js.native
  
  /* CompleteClass */
  var IncludeCategoryHeader: Boolean = js.native
  
  /* CompleteClass */
  var IncludeSequenceName: String = js.native
  
  /* CompleteClass */
  var KeepEntryFormatting: Boolean = js.native
  
  /* CompleteClass */
  var PageNumberSeparator: String = js.native
  
  /* CompleteClass */
  var PageRangeSeparator: String = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var Passim: Boolean = js.native
  
  /* CompleteClass */
  override val Range: typingsJapgolly.activexWord.Word.Range = js.native
  
  /* CompleteClass */
  var Separator: String = js.native
  
  /* CompleteClass */
  var TabLeader: WdTabLeader = js.native
  
  /* CompleteClass */
  override def Update(): Unit = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.TableOfAuthorities_typekey")
  var WordDotTableOfAuthorities_typekey: typingsJapgolly.activexWord.Word.TableOfAuthorities = js.native
}
