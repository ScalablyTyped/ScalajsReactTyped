package typingsJapgolly.activexWord.global.Word

import typingsJapgolly.activexWord.Word.WdTabLeader
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.TableOfFigures")
@js.native
/* private */ open class TableOfFigures ()
  extends StObject
     with typingsJapgolly.activexWord.Word.TableOfFigures {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  var Caption: String = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  override val HeadingStyles: typingsJapgolly.activexWord.Word.HeadingStyles = js.native
  
  /* CompleteClass */
  var HidePageNumbersInWeb: Boolean = js.native
  
  /* CompleteClass */
  var IncludeLabel: Boolean = js.native
  
  /* CompleteClass */
  var IncludePageNumbers: Boolean = js.native
  
  /* CompleteClass */
  var LowerHeadingLevel: Double = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override val Range: typingsJapgolly.activexWord.Word.Range = js.native
  
  /* CompleteClass */
  var RightAlignPageNumbers: Boolean = js.native
  
  /* CompleteClass */
  var TabLeader: WdTabLeader = js.native
  
  /* CompleteClass */
  var TableID: String = js.native
  
  /* CompleteClass */
  override def Update(): Unit = js.native
  
  /* CompleteClass */
  override def UpdatePageNumbers(): Unit = js.native
  
  /* CompleteClass */
  var UpperHeadingLevel: Double = js.native
  
  /* CompleteClass */
  var UseFields: Boolean = js.native
  
  /* CompleteClass */
  var UseHeadingStyles: Boolean = js.native
  
  /* CompleteClass */
  var UseHyperlinks: Boolean = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.TableOfFigures_typekey")
  var WordDotTableOfFigures_typekey: typingsJapgolly.activexWord.Word.TableOfFigures = js.native
}
