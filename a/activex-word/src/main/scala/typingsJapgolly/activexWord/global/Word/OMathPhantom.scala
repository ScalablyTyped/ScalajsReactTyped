package typingsJapgolly.activexWord.global.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.OMathPhantom")
@js.native
/* private */ open class OMathPhantom ()
  extends StObject
     with typingsJapgolly.activexWord.Word.OMathPhantom {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override val E: typingsJapgolly.activexWord.Word.OMath = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var Show: Boolean = js.native
  
  /* CompleteClass */
  var Smash: Boolean = js.native
  
  /* CompleteClass */
  var Transp: Boolean = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.OMathPhantom_typekey")
  var WordDotOMathPhantom_typekey: typingsJapgolly.activexWord.Word.OMathPhantom = js.native
  
  /* CompleteClass */
  var ZeroAsc: Boolean = js.native
  
  /* CompleteClass */
  var ZeroDesc: Boolean = js.native
  
  /* CompleteClass */
  var ZeroWid: Boolean = js.native
}
