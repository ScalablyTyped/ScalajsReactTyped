package typingsJapgolly.activexWord.global.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.Source")
@js.native
/* private */ open class Source ()
  extends StObject
     with typingsJapgolly.activexWord.Word.Source {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  override val Cited: Boolean = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  override def Field(Name: String): String = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override val Tag: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.Source_typekey")
  var WordDotSource_typekey: typingsJapgolly.activexWord.Word.Source = js.native
  
  /* CompleteClass */
  override val XML: String = js.native
}
