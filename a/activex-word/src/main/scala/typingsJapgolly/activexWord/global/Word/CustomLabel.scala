package typingsJapgolly.activexWord.global.Word

import typingsJapgolly.activexWord.Word.WdCustomLabelPageSize
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.CustomLabel")
@js.native
/* private */ open class CustomLabel ()
  extends StObject
     with typingsJapgolly.activexWord.Word.CustomLabel {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  override val DotMatrix: Boolean = js.native
  
  /* CompleteClass */
  var Height: Double = js.native
  
  /* CompleteClass */
  var HorizontalPitch: Double = js.native
  
  /* CompleteClass */
  override val Index: Double = js.native
  
  /* CompleteClass */
  var Name: String = js.native
  
  /* CompleteClass */
  var NumberAcross: Double = js.native
  
  /* CompleteClass */
  var NumberDown: Double = js.native
  
  /* CompleteClass */
  var PageSize: WdCustomLabelPageSize = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var SideMargin: Double = js.native
  
  /* CompleteClass */
  var TopMargin: Double = js.native
  
  /* CompleteClass */
  override val Valid: Boolean = js.native
  
  /* CompleteClass */
  var VerticalPitch: Double = js.native
  
  /* CompleteClass */
  var Width: Double = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.CustomLabel_typekey")
  var WordDotCustomLabel_typekey: typingsJapgolly.activexWord.Word.CustomLabel = js.native
}
