package typingsJapgolly.activexWord.global.Word

import typingsJapgolly.activexWord.Word.WdOMathShapeType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.OMathDelim")
@js.native
/* private */ open class OMathDelim ()
  extends StObject
     with typingsJapgolly.activexWord.Word.OMathDelim {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  var BegChar: Double = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override val E: typingsJapgolly.activexWord.Word.OMathArgs = js.native
  
  /* CompleteClass */
  var EndChar: Double = js.native
  
  /* CompleteClass */
  var Grow: Boolean = js.native
  
  /* CompleteClass */
  var NoLeftChar: Boolean = js.native
  
  /* CompleteClass */
  var NoRightChar: Boolean = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var SepChar: Double = js.native
  
  /* CompleteClass */
  var Shape: WdOMathShapeType = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.OMathDelim_typekey")
  var WordDotOMathDelim_typekey: typingsJapgolly.activexWord.Word.OMathDelim = js.native
}
