package typingsJapgolly.activexWord.global.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.TextColumn")
@js.native
/* private */ open class TextColumn ()
  extends StObject
     with typingsJapgolly.activexWord.Word.TextColumn {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var SpaceAfter: Double = js.native
  
  /* CompleteClass */
  var Width: Double = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.TextColumn_typekey")
  var WordDotTextColumn_typekey: typingsJapgolly.activexWord.Word.TextColumn = js.native
}
