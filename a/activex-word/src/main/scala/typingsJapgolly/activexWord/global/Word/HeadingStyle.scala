package typingsJapgolly.activexWord.global.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.HeadingStyle")
@js.native
/* private */ open class HeadingStyle ()
  extends StObject
     with typingsJapgolly.activexWord.Word.HeadingStyle {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  var Level: Double = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var Style: Any = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.HeadingStyle_typekey")
  var WordDotHeadingStyle_typekey: typingsJapgolly.activexWord.Word.HeadingStyle = js.native
}
