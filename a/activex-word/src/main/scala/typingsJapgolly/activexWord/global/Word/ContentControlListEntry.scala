package typingsJapgolly.activexWord.global.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Word.ContentControlListEntry")
@js.native
/* private */ open class ContentControlListEntry ()
  extends StObject
     with typingsJapgolly.activexWord.Word.ContentControlListEntry {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  var Index: Double = js.native
  
  /* CompleteClass */
  override def MoveDown(): Unit = js.native
  
  /* CompleteClass */
  override def MoveUp(): Unit = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override def Select(): Unit = js.native
  
  /* CompleteClass */
  var Text: String = js.native
  
  /* CompleteClass */
  var Value: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Word.ContentControlListEntry_typekey")
  var WordDotContentControlListEntry_typekey: typingsJapgolly.activexWord.Word.ContentControlListEntry = js.native
}
