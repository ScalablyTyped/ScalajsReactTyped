package typingsJapgolly.activexWord.Word

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait ListFormat extends StObject {
  
  val Application: typingsJapgolly.activexWord.Word.Application = js.native
  
  def ApplyBulletDefault(): Unit = js.native
  def ApplyBulletDefault(DefaultListBehavior: Any): Unit = js.native
  
  def ApplyBulletDefaultOld(): Unit = js.native
  
  def ApplyListTemplate(ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate): Unit = js.native
  def ApplyListTemplate(ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate, ContinuePreviousList: Any): Unit = js.native
  def ApplyListTemplate(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Any
  ): Unit = js.native
  def ApplyListTemplate(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Any,
    DefaultListBehavior: Any
  ): Unit = js.native
  def ApplyListTemplate(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Unit,
    DefaultListBehavior: Any
  ): Unit = js.native
  def ApplyListTemplate(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Any
  ): Unit = js.native
  def ApplyListTemplate(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Any,
    DefaultListBehavior: Any
  ): Unit = js.native
  def ApplyListTemplate(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Unit,
    DefaultListBehavior: Any
  ): Unit = js.native
  
  def ApplyListTemplateOld(ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate): Unit = js.native
  def ApplyListTemplateOld(ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate, ContinuePreviousList: Any): Unit = js.native
  def ApplyListTemplateOld(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Any
  ): Unit = js.native
  def ApplyListTemplateOld(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Any
  ): Unit = js.native
  
  def ApplyListTemplateWithLevel(ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate): Unit = js.native
  def ApplyListTemplateWithLevel(ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate, ContinuePreviousList: Any): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Any,
    DefaultListBehavior: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Any,
    DefaultListBehavior: Any,
    ApplyLevel: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Any,
    DefaultListBehavior: Unit,
    ApplyLevel: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Unit,
    DefaultListBehavior: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Unit,
    DefaultListBehavior: Any,
    ApplyLevel: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Any,
    ApplyTo: Unit,
    DefaultListBehavior: Unit,
    ApplyLevel: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Any,
    DefaultListBehavior: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Any,
    DefaultListBehavior: Any,
    ApplyLevel: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Any,
    DefaultListBehavior: Unit,
    ApplyLevel: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Unit,
    DefaultListBehavior: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Unit,
    DefaultListBehavior: Any,
    ApplyLevel: Any
  ): Unit = js.native
  def ApplyListTemplateWithLevel(
    ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate,
    ContinuePreviousList: Unit,
    ApplyTo: Unit,
    DefaultListBehavior: Unit,
    ApplyLevel: Any
  ): Unit = js.native
  
  def ApplyNumberDefault(): Unit = js.native
  def ApplyNumberDefault(DefaultListBehavior: Any): Unit = js.native
  
  def ApplyNumberDefaultOld(): Unit = js.native
  
  def ApplyOutlineNumberDefault(): Unit = js.native
  def ApplyOutlineNumberDefault(DefaultListBehavior: Any): Unit = js.native
  
  def ApplyOutlineNumberDefaultOld(): Unit = js.native
  
  def CanContinuePreviousList(ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate): WdContinue = js.native
  
  def ConvertNumbersToText(): Unit = js.native
  def ConvertNumbersToText(NumberType: Any): Unit = js.native
  
  def CountNumberedItems(): Double = js.native
  def CountNumberedItems(NumberType: Any): Double = js.native
  def CountNumberedItems(NumberType: Any, Level: Any): Double = js.native
  def CountNumberedItems(NumberType: Unit, Level: Any): Double = js.native
  
  val Creator: Double = js.native
  
  val List: typingsJapgolly.activexWord.Word.List = js.native
  
  def ListIndent(): Unit = js.native
  
  var ListLevelNumber: Double = js.native
  
  def ListOutdent(): Unit = js.native
  
  val ListPictureBullet: InlineShape = js.native
  
  val ListString: String = js.native
  
  val ListTemplate: typingsJapgolly.activexWord.Word.ListTemplate = js.native
  
  val ListType: WdListType = js.native
  
  val ListValue: Double = js.native
  
  val Parent: Any = js.native
  
  def RemoveNumbers(): Unit = js.native
  def RemoveNumbers(NumberType: Any): Unit = js.native
  
  val SingleList: Boolean = js.native
  
  val SingleListTemplate: Boolean = js.native
  
  /* private */ @JSName("Word.ListFormat_typekey")
  var WordDotListFormat_typekey: ListFormat = js.native
}
