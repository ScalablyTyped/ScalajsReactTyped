...
object CoordinateSpan {
  ...
  inline def apply(
    copy: CallbackTo[CoordinateSpan],
    equals_ : CoordinateSpan => Boolean,
    latitudeDelta: Double,
    longitudeDelta: Double
  ): CoordinateSpan = {
    val __obj = js.Dynamic.literal(copy = copy.toJsFn, latitudeDelta = latitudeDelta.asInstanceOf[js.Any], longitudeDelta = longitudeDelta.asInstanceOf[js.Any])
  ...
  extension [Self <: CoordinateSpan](x: Self) {
    ...
    inline def setCopy(value: CallbackTo[CoordinateSpan]): Self = StObject.set(x, "copy", value.toJsFn)
