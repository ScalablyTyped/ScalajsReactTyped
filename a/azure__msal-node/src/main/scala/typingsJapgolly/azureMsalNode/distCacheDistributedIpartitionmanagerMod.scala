...
object distCacheDistributedIpartitionmanagerMod {
  ...
  object IPartitionManager {
    ...
    inline def apply(extractKey: AccountEntity => js.Promise[String], getKey: CallbackTo[js.Promise[String]]): IPartitionManager = {
      val __obj = js.Dynamic.literal(extractKey = js.Any.fromFunction1(extractKey), getKey = getKey.toJsFn)
    ...
    extension [Self <: IPartitionManager](x: Self) {
      ...
      inline def setGetKey(value: CallbackTo[js.Promise[String]]): Self = StObject.set(x, "getKey", value.toJsFn)
    }
  }
}
