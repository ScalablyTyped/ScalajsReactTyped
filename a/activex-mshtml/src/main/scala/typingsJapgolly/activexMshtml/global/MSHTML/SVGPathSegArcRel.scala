package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.SVGPathSegArcRel")
@js.native
/* private */ open class SVGPathSegArcRel ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.SVGPathSegArcRel {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.SVGPathSegArcRel_typekey")
  var MSHTMLDotSVGPathSegArcRel_typekey: typingsJapgolly.activexMshtml.MSHTML.SVGPathSegArcRel = js.native
  
  /* CompleteClass */
  var angle: Double = js.native
  
  /* CompleteClass */
  var largeArcFlag: Boolean = js.native
  
  /* CompleteClass */
  var pathSegType: Double = js.native
  
  /* CompleteClass */
  override val pathSegTypeAsLetter: String = js.native
  
  /* CompleteClass */
  var r1: Double = js.native
  
  /* CompleteClass */
  var r2: Double = js.native
  
  /* CompleteClass */
  var sweepFlag: Boolean = js.native
  
  /* CompleteClass */
  var x: Double = js.native
  
  /* CompleteClass */
  var y: Double = js.native
}
