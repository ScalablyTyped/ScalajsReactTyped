package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.HTMLAppBehavior")
@js.native
/* private */ open class HTMLAppBehavior ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.HTMLAppBehavior {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.HTMLAppBehavior_typekey")
  var MSHTMLDotHTMLAppBehavior_typekey: typingsJapgolly.activexMshtml.MSHTML.HTMLAppBehavior = js.native
  
  /* CompleteClass */
  var applicationName: String = js.native
  
  /* CompleteClass */
  var border: String = js.native
  
  /* CompleteClass */
  var borderStyle: String = js.native
  
  /* CompleteClass */
  var caption: String = js.native
  
  /* CompleteClass */
  override val commandLine: String = js.native
  
  /* CompleteClass */
  var contextMenu: String = js.native
  
  /* CompleteClass */
  var icon: String = js.native
  
  /* CompleteClass */
  var innerBorder: String = js.native
  
  /* CompleteClass */
  var maximizeButton: String = js.native
  
  /* CompleteClass */
  var minimizeButton: String = js.native
  
  /* CompleteClass */
  var scroll: String = js.native
  
  /* CompleteClass */
  var scrollFlat: String = js.native
  
  /* CompleteClass */
  var selection: String = js.native
  
  /* CompleteClass */
  var showInTaskBar: String = js.native
  
  /* CompleteClass */
  var singleInstance: String = js.native
  
  /* CompleteClass */
  var sysMenu: String = js.native
  
  /* CompleteClass */
  var version: String = js.native
  
  /* CompleteClass */
  var windowState: String = js.native
}
