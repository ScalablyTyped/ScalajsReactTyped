package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.HTMLPopup")
@js.native
/* private */ open class HTMLPopup ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.HTMLPopup {
  
  /* CompleteClass */
  override def Hide(): Unit = js.native
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.HTMLPopup_typekey")
  var MSHTMLDotHTMLPopup_typekey: typingsJapgolly.activexMshtml.MSHTML.HTMLPopup = js.native
  
  /* CompleteClass */
  override def Show(x: Double, y: Double, w: Double, h: Double, pElement: Any): Unit = js.native
  
  /* CompleteClass */
  override val constructor: Any = js.native
  
  /* CompleteClass */
  override val document: typingsJapgolly.activexMshtml.MSHTML.IHTMLDocument = js.native
  
  /* CompleteClass */
  override val isOpen: Boolean = js.native
}
