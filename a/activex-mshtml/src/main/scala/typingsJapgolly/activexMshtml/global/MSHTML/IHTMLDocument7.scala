package typingsJapgolly.activexMshtml.global.MSHTML

import typingsJapgolly.activexMshtml.MSHTML.IHTMLElementCollection
import typingsJapgolly.activexMshtml.MSHTML.IHTMLWindow2
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.IHTMLDocument7")
@js.native
/* private */ open class IHTMLDocument7 ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.IHTMLDocument7 {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.IHTMLDocument7_typekey")
  var MSHTMLDotIHTMLDocument7_typekey: typingsJapgolly.activexMshtml.MSHTML.IHTMLDocument7 = js.native
  
  /* CompleteClass */
  override def adoptNode(pNodeSource: typingsJapgolly.activexMshtml.MSHTML.IHTMLDOMNode): typingsJapgolly.activexMshtml.MSHTML.IHTMLDOMNode3 = js.native
  
  /* CompleteClass */
  override def all(): Any = js.native
  /* CompleteClass */
  override def all(name: Any): Any = js.native
  /* CompleteClass */
  override def all(name: Any, index: Any): Any = js.native
  /* CompleteClass */
  override def all(name: Unit, index: Any): Any = js.native
  /* CompleteClass */
  @JSName("all")
  override val all_Original: IHTMLElementCollection = js.native
  
  /* CompleteClass */
  var body: typingsJapgolly.activexMshtml.MSHTML.IHTMLElement = js.native
  
  /* CompleteClass */
  override val characterSet: String = js.native
  
  /* CompleteClass */
  override def createAttribute(bstrAttrName: String): typingsJapgolly.activexMshtml.MSHTML.IHTMLDOMAttribute = js.native
  
  /* CompleteClass */
  override def createAttributeNS(pvarNS: Any, bstrAttrName: String): typingsJapgolly.activexMshtml.MSHTML.IHTMLDOMAttribute = js.native
  
  /* CompleteClass */
  override def createCDATASection(text: String): typingsJapgolly.activexMshtml.MSHTML.IHTMLDOMNode = js.native
  
  /* CompleteClass */
  override def createElement(bstrTag: String): typingsJapgolly.activexMshtml.MSHTML.IHTMLElement = js.native
  
  /* CompleteClass */
  override def createElementNS(pvarNS: Any, bstrTag: String): typingsJapgolly.activexMshtml.MSHTML.IHTMLElement = js.native
  
  /* CompleteClass */
  override def createProcessingInstruction(bstrTarget: String, bstrData: String): typingsJapgolly.activexMshtml.MSHTML.IDOMProcessingInstruction = js.native
  
  /* CompleteClass */
  override def defaultView(pvarIndex: Any): Any = js.native
  /* CompleteClass */
  @JSName("defaultView")
  override val defaultView_Original: IHTMLWindow2 = js.native
  
  /* CompleteClass */
  override def getElementsByClassName(v: String): IHTMLElementCollection = js.native
  
  /* CompleteClass */
  override def getElementsByTagNameNS(pvarNS: Any, bstrLocalName: String): IHTMLElementCollection = js.native
  
  /* CompleteClass */
  override def getSelection(): typingsJapgolly.activexMshtml.MSHTML.IHTMLSelection = js.native
  
  /* CompleteClass */
  override def hasAttributes(): Boolean = js.native
  
  /* CompleteClass */
  override val head: typingsJapgolly.activexMshtml.MSHTML.IHTMLElement = js.native
  
  /* CompleteClass */
  override def importNode(pNodeSource: typingsJapgolly.activexMshtml.MSHTML.IHTMLDOMNode, fDeep: Boolean): typingsJapgolly.activexMshtml.MSHTML.IHTMLDOMNode3 = js.native
  
  /* CompleteClass */
  override val inputEncoding: String = js.native
  
  /* CompleteClass */
  override def normalize(): Unit = js.native
  
  /* CompleteClass */
  var onabort: Any = js.native
  
  /* CompleteClass */
  var onblur: Any = js.native
  
  /* CompleteClass */
  var oncanplay: Any = js.native
  
  /* CompleteClass */
  var oncanplaythrough: Any = js.native
  
  /* CompleteClass */
  var onchange: Any = js.native
  
  /* CompleteClass */
  var ondrag: Any = js.native
  
  /* CompleteClass */
  var ondragend: Any = js.native
  
  /* CompleteClass */
  var ondragenter: Any = js.native
  
  /* CompleteClass */
  var ondragleave: Any = js.native
  
  /* CompleteClass */
  var ondragover: Any = js.native
  
  /* CompleteClass */
  var ondrop: Any = js.native
  
  /* CompleteClass */
  var ondurationchange: Any = js.native
  
  /* CompleteClass */
  var onemptied: Any = js.native
  
  /* CompleteClass */
  var onended: Any = js.native
  
  /* CompleteClass */
  var onerror: Any = js.native
  
  /* CompleteClass */
  var onfocus: Any = js.native
  
  /* CompleteClass */
  var oninput: Any = js.native
  
  /* CompleteClass */
  var onload: Any = js.native
  
  /* CompleteClass */
  var onloadeddata: Any = js.native
  
  /* CompleteClass */
  var onloadedmetadata: Any = js.native
  
  /* CompleteClass */
  var onloadstart: Any = js.native
  
  /* CompleteClass */
  var onmssitemodejumplistitemremoved: Any = js.native
  
  /* CompleteClass */
  var onmsthumbnailclick: Any = js.native
  
  /* CompleteClass */
  var onpause: Any = js.native
  
  /* CompleteClass */
  var onplay: Any = js.native
  
  /* CompleteClass */
  var onplaying: Any = js.native
  
  /* CompleteClass */
  var onprogress: Any = js.native
  
  /* CompleteClass */
  var onratechange: Any = js.native
  
  /* CompleteClass */
  var onreset: Any = js.native
  
  /* CompleteClass */
  var onscroll: Any = js.native
  
  /* CompleteClass */
  var onseeked: Any = js.native
  
  /* CompleteClass */
  var onseeking: Any = js.native
  
  /* CompleteClass */
  var onselect: Any = js.native
  
  /* CompleteClass */
  var onstalled: Any = js.native
  
  /* CompleteClass */
  var onsubmit: Any = js.native
  
  /* CompleteClass */
  var onsuspend: Any = js.native
  
  /* CompleteClass */
  var ontimeupdate: Any = js.native
  
  /* CompleteClass */
  var onvolumechange: Any = js.native
  
  /* CompleteClass */
  var onwaiting: Any = js.native
  
  /* CompleteClass */
  override def parentWindow(pvarIndex: Any): Any = js.native
  /* CompleteClass */
  @JSName("parentWindow")
  override val parentWindow_Original: IHTMLWindow2 = js.native
  
  /* CompleteClass */
  override val xmlEncoding: String = js.native
  
  /* CompleteClass */
  var xmlStandalone: Boolean = js.native
  
  /* CompleteClass */
  var xmlVersion: String = js.native
}
