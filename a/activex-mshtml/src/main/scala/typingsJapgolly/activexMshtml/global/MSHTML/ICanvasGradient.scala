package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.ICanvasGradient")
@js.native
/* private */ open class ICanvasGradient ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.ICanvasGradient {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.ICanvasGradient_typekey")
  var MSHTMLDotICanvasGradient_typekey: typingsJapgolly.activexMshtml.MSHTML.ICanvasGradient = js.native
  
  /* CompleteClass */
  override def addColorStop(offset: Double, color: String): Unit = js.native
}
