package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.IDOMNodeIterator")
@js.native
/* private */ open class IDOMNodeIterator ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.IDOMNodeIterator {
  
  /* CompleteClass */
  override def Detach(): Unit = js.native
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.IDOMNodeIterator_typekey")
  var MSHTMLDotIDOMNodeIterator_typekey: typingsJapgolly.activexMshtml.MSHTML.IDOMNodeIterator = js.native
  
  /* CompleteClass */
  override val expandEntityReferences: Boolean = js.native
  
  /* CompleteClass */
  override val filter: Any = js.native
  
  /* CompleteClass */
  override def nextNode(): Any = js.native
  
  /* CompleteClass */
  override def previousNode(): Any = js.native
  
  /* CompleteClass */
  override val root: Any = js.native
  
  /* CompleteClass */
  override val whatToShow: Double = js.native
}
