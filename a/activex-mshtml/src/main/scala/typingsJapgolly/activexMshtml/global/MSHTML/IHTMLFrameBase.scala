package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.IHTMLFrameBase")
@js.native
/* private */ open class IHTMLFrameBase ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.IHTMLFrameBase {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.IHTMLFrameBase_typekey")
  var MSHTMLDotIHTMLFrameBase_typekey: typingsJapgolly.activexMshtml.MSHTML.IHTMLFrameBase = js.native
  
  /* CompleteClass */
  var border: Any = js.native
  
  /* CompleteClass */
  var frameBorder: String = js.native
  
  /* CompleteClass */
  var frameSpacing: Any = js.native
  
  /* CompleteClass */
  var marginHeight: Any = js.native
  
  /* CompleteClass */
  var marginWidth: Any = js.native
  
  /* CompleteClass */
  var name: String = js.native
  
  /* CompleteClass */
  var noResize: Boolean = js.native
  
  /* CompleteClass */
  var scrolling: String = js.native
  
  /* CompleteClass */
  var src: String = js.native
}
