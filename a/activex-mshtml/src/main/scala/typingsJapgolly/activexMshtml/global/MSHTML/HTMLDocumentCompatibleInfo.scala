package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.HTMLDocumentCompatibleInfo")
@js.native
/* private */ open class HTMLDocumentCompatibleInfo ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.HTMLDocumentCompatibleInfo {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.HTMLDocumentCompatibleInfo_typekey")
  var MSHTMLDotHTMLDocumentCompatibleInfo_typekey: typingsJapgolly.activexMshtml.MSHTML.HTMLDocumentCompatibleInfo = js.native
  
  /* CompleteClass */
  override val constructor: Any = js.native
}
