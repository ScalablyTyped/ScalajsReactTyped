package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.ICanvasImageData")
@js.native
/* private */ open class ICanvasImageData ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.ICanvasImageData {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.ICanvasImageData_typekey")
  var MSHTMLDotICanvasImageData_typekey: typingsJapgolly.activexMshtml.MSHTML.ICanvasImageData = js.native
  
  /* CompleteClass */
  override val data: Any = js.native
  
  /* CompleteClass */
  override val height: Double = js.native
  
  /* CompleteClass */
  override val width: Double = js.native
}
