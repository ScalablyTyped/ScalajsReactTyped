package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.IWebGeocoordinates")
@js.native
/* private */ open class IWebGeocoordinates ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.IWebGeocoordinates {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.IWebGeocoordinates_typekey")
  var MSHTMLDotIWebGeocoordinates_typekey: typingsJapgolly.activexMshtml.MSHTML.IWebGeocoordinates = js.native
  
  /* CompleteClass */
  override val accuracy: Double = js.native
  
  /* CompleteClass */
  override val altitude: Any = js.native
  
  /* CompleteClass */
  override val altitudeAccuracy: Any = js.native
  
  /* CompleteClass */
  override val heading: Any = js.native
  
  /* CompleteClass */
  override val latitude: Double = js.native
  
  /* CompleteClass */
  override val longitude: Double = js.native
  
  /* CompleteClass */
  override val speed: Any = js.native
}
