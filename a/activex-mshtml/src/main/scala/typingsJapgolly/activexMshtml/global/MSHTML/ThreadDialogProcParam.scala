package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.ThreadDialogProcParam")
@js.native
/* private */ open class ThreadDialogProcParam ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.ThreadDialogProcParam {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.ThreadDialogProcParam_typekey")
  var MSHTMLDotThreadDialogProcParam_typekey: typingsJapgolly.activexMshtml.MSHTML.ThreadDialogProcParam = js.native
  
  /* CompleteClass */
  override val document: Any = js.native
  
  /* CompleteClass */
  override val moniker: Any = js.native
  
  /* CompleteClass */
  override val optionString: Any = js.native
  
  /* CompleteClass */
  override val parameters: Any = js.native
}
