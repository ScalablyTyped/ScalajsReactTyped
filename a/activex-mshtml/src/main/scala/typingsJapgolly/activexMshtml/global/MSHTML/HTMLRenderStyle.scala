package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.HTMLRenderStyle")
@js.native
/* private */ open class HTMLRenderStyle ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.HTMLRenderStyle {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.HTMLRenderStyle_typekey")
  var MSHTMLDotHTMLRenderStyle_typekey: typingsJapgolly.activexMshtml.MSHTML.HTMLRenderStyle = js.native
  
  /* CompleteClass */
  var defaultTextSelection: String = js.native
  
  /* CompleteClass */
  var renderingPriority: Double = js.native
  
  /* CompleteClass */
  var textBackgroundColor: Any = js.native
  
  /* CompleteClass */
  var textColor: Any = js.native
  
  /* CompleteClass */
  var textDecoration: String = js.native
  
  /* CompleteClass */
  var textDecorationColor: Any = js.native
  
  /* CompleteClass */
  var textEffect: String = js.native
  
  /* CompleteClass */
  var textLineThroughStyle: String = js.native
  
  /* CompleteClass */
  var textUnderlineStyle: String = js.native
}
