package typingsJapgolly.activexMshtml.global.MSHTML

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("MSHTML.applicationCache")
@js.native
/* private */ open class applicationCache ()
  extends StObject
     with typingsJapgolly.activexMshtml.MSHTML.applicationCache {
  
  /* private */ /* CompleteClass */
  @JSName("MSHTML.applicationCache_typekey")
  var MSHTMLDotapplicationCache_typekey: typingsJapgolly.activexMshtml.MSHTML.applicationCache = js.native
  
  /* CompleteClass */
  override def abort(): Unit = js.native
  
  /* CompleteClass */
  var oncached: Any = js.native
  
  /* CompleteClass */
  var onchecking: Any = js.native
  
  /* CompleteClass */
  var ondownloading: Any = js.native
  
  /* CompleteClass */
  var onerror: Any = js.native
  
  /* CompleteClass */
  var onnoupdate: Any = js.native
  
  /* CompleteClass */
  var onobsolete: Any = js.native
  
  /* CompleteClass */
  var onprogress: Any = js.native
  
  /* CompleteClass */
  var onupdateready: Any = js.native
  
  /* CompleteClass */
  override val status: Double = js.native
  
  /* CompleteClass */
  override def swapCache(): Unit = js.native
  
  /* CompleteClass */
  override def update(): Unit = js.native
}
