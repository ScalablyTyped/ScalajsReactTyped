package typingsJapgolly.authorizenet.mod.APIContracts

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("authorizenet", "APIContracts.UpdateCustomerPaymentProfileResponse")
@js.native
open class UpdateCustomerPaymentProfileResponse protected () extends ANetApiResponse {
  def this(obj: Any, args: Any*) = this()
  def this(obj: Unit, args: Any*) = this()
  
  def getValidationDirectResponse(): Any = js.native
  
  def setValidationDirectResponse(p_validationDirectResponse: Any): Unit = js.native
  
  var validationDirectResponse: Any = js.native
}
