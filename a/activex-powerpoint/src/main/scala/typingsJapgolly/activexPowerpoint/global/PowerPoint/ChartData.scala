package typingsJapgolly.activexPowerpoint.global.PowerPoint

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("PowerPoint.ChartData")
@js.native
/* private */ open class ChartData ()
  extends StObject
     with typingsJapgolly.activexPowerpoint.PowerPoint.ChartData {
  
  /* CompleteClass */
  override def Activate(): Unit = js.native
  
  /* CompleteClass */
  override def BreakLink(): Unit = js.native
  
  /* CompleteClass */
  override val IsLinked: Boolean = js.native
  
  /* private */ /* CompleteClass */
  @JSName("PowerPoint.ChartData_typekey")
  var PowerPointDotChartData_typekey: typingsJapgolly.activexPowerpoint.PowerPoint.ChartData = js.native
  
  /* CompleteClass */
  override val Workbook: Any = js.native
}
