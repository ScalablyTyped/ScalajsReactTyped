package typingsJapgolly.activexPowerpoint.global.PowerPoint

import typingsJapgolly.activexPowerpoint.PowerPoint.MsoAnimCommandType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("PowerPoint.CommandEffect")
@js.native
/* private */ open class CommandEffect ()
  extends StObject
     with typingsJapgolly.activexPowerpoint.PowerPoint.CommandEffect {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexPowerpoint.PowerPoint.Application = js.native
  
  /* CompleteClass */
  var Command: String = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* private */ /* CompleteClass */
  @JSName("PowerPoint.CommandEffect_typekey")
  var PowerPointDotCommandEffect_typekey: typingsJapgolly.activexPowerpoint.PowerPoint.CommandEffect = js.native
  
  /* CompleteClass */
  var Type: MsoAnimCommandType = js.native
  
  /* CompleteClass */
  var bookmark: String = js.native
}
