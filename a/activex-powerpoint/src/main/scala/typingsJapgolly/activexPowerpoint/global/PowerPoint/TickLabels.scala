package typingsJapgolly.activexPowerpoint.global.PowerPoint

import typingsJapgolly.activexPowerpoint.PowerPoint.XlTickLabelOrientation
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("PowerPoint.TickLabels")
@js.native
/* private */ open class TickLabels ()
  extends StObject
     with typingsJapgolly.activexPowerpoint.PowerPoint.TickLabels {
  
  /* CompleteClass */
  var Alignment: Double = js.native
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexPowerpoint.PowerPoint.Application = js.native
  
  /* CompleteClass */
  var AutoScaleFont: Any = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Delete(): Any = js.native
  
  /* CompleteClass */
  override val Depth: Double = js.native
  
  /* CompleteClass */
  override val Font: typingsJapgolly.activexPowerpoint.PowerPoint.ChartFont = js.native
  
  /* CompleteClass */
  override val Format: typingsJapgolly.activexPowerpoint.PowerPoint.ChartFormat = js.native
  
  /* CompleteClass */
  var MultiLevel: Boolean = js.native
  
  /* CompleteClass */
  override val Name: String = js.native
  
  /* CompleteClass */
  var NumberFormat: String = js.native
  
  /* CompleteClass */
  var NumberFormatLinked: Boolean = js.native
  
  /* CompleteClass */
  var NumberFormatLocal: Any = js.native
  
  /* CompleteClass */
  var Offset: Double = js.native
  
  /* CompleteClass */
  var Orientation: XlTickLabelOrientation = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* private */ /* CompleteClass */
  @JSName("PowerPoint.TickLabels_typekey")
  var PowerPointDotTickLabels_typekey: typingsJapgolly.activexPowerpoint.PowerPoint.TickLabels = js.native
  
  /* CompleteClass */
  var ReadingOrder: Double = js.native
  
  /* CompleteClass */
  override def Select(): Any = js.native
}
