package typingsJapgolly.activexPowerpoint.global.PowerPoint

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("PowerPoint.Column")
@js.native
/* private */ open class Column ()
  extends StObject
     with typingsJapgolly.activexPowerpoint.PowerPoint.Column {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexPowerpoint.PowerPoint.Application = js.native
  
  /* CompleteClass */
  override val Cells: typingsJapgolly.activexPowerpoint.PowerPoint.CellRange = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* private */ /* CompleteClass */
  @JSName("PowerPoint.Column_typekey")
  var PowerPointDotColumn_typekey: typingsJapgolly.activexPowerpoint.PowerPoint.Column = js.native
  
  /* CompleteClass */
  override def Select(): Unit = js.native
  
  /* CompleteClass */
  var Width: Double = js.native
}
