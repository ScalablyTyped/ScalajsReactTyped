package typingsJapgolly.activexPowerpoint.global.PowerPoint

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("PowerPoint.ScaleEffect")
@js.native
/* private */ open class ScaleEffect ()
  extends StObject
     with typingsJapgolly.activexPowerpoint.PowerPoint.ScaleEffect {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexPowerpoint.PowerPoint.Application = js.native
  
  /* CompleteClass */
  var ByX: Double = js.native
  
  /* CompleteClass */
  var ByY: Double = js.native
  
  /* CompleteClass */
  var FromX: Double = js.native
  
  /* CompleteClass */
  var FromY: Double = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* private */ /* CompleteClass */
  @JSName("PowerPoint.ScaleEffect_typekey")
  var PowerPointDotScaleEffect_typekey: typingsJapgolly.activexPowerpoint.PowerPoint.ScaleEffect = js.native
  
  /* CompleteClass */
  var ToX: Double = js.native
  
  /* CompleteClass */
  var ToY: Double = js.native
}
