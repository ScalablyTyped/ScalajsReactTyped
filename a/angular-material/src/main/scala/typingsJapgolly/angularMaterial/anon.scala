...
object anon {
  ...
  object Methods {
    ...
    inline def apply(options: CallbackTo[IDialogOptions]): Methods = {
      val __obj = js.Dynamic.literal(options = options.toJsFn)
    ...
    extension [Self <: Methods](x: Self) {
      ...
      inline def setOptions(value: CallbackTo[IDialogOptions]): Self = StObject.set(x, "options", value.toJsFn)
    }
  }
