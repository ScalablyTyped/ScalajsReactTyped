package typingsJapgolly.activexExcel.global.Excel

import typingsJapgolly.activexExcel.Excel.XlCreator
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Excel.SlicerItem")
@js.native
/* private */ open class SlicerItem ()
  extends StObject
     with typingsJapgolly.activexExcel.Excel.SlicerItem {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexExcel.Excel.Application = js.native
  
  /* CompleteClass */
  override val Caption: String = js.native
  
  /* CompleteClass */
  override val Creator: XlCreator = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Excel.SlicerItem_typekey")
  var ExcelDotSlicerItem_typekey: typingsJapgolly.activexExcel.Excel.SlicerItem = js.native
  
  /* CompleteClass */
  override val HasData: Boolean = js.native
  
  /* CompleteClass */
  override val Name: String = js.native
  
  /* CompleteClass */
  override val Parent: typingsJapgolly.activexExcel.Excel.SlicerCache = js.native
  
  /* CompleteClass */
  var Selected: Boolean = js.native
  
  /* CompleteClass */
  override val SourceName: Any = js.native
  
  /* CompleteClass */
  override val SourceNameStandard: String = js.native
  
  /* CompleteClass */
  override val Value: String = js.native
}
