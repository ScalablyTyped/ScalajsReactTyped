package typingsJapgolly.activexExcel.global.Excel

import typingsJapgolly.activexExcel.Excel.XlCreator
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Excel.ToolbarButton")
@js.native
/* private */ open class ToolbarButton ()
  extends StObject
     with typingsJapgolly.activexExcel.Excel.ToolbarButton {
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexExcel.Excel.Application = js.native
  
  /* CompleteClass */
  override val BuiltIn: Boolean = js.native
  
  /* CompleteClass */
  var BuiltInFace: Boolean = js.native
  
  /* CompleteClass */
  override def Copy(Toolbar: typingsJapgolly.activexExcel.Excel.Toolbar, Before: Double): Unit = js.native
  
  /* CompleteClass */
  override def CopyFace(): Unit = js.native
  
  /* CompleteClass */
  override val Creator: XlCreator = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  override def Edit(): Unit = js.native
  
  /* CompleteClass */
  var Enabled: Boolean = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Excel.ToolbarButton_typekey")
  var ExcelDotToolbarButton_typekey: typingsJapgolly.activexExcel.Excel.ToolbarButton = js.native
  
  /* CompleteClass */
  var HelpContextID: Double = js.native
  
  /* CompleteClass */
  var HelpFile: String = js.native
  
  /* CompleteClass */
  override val ID: Double = js.native
  
  /* CompleteClass */
  override val IsGap: Boolean = js.native
  
  /* CompleteClass */
  override def Move(Toolbar: typingsJapgolly.activexExcel.Excel.Toolbar, Before: Double): Unit = js.native
  
  /* CompleteClass */
  var Name: String = js.native
  
  /* CompleteClass */
  var OnAction: String = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override def PasteFace(): Unit = js.native
  
  /* CompleteClass */
  var Pushed: Boolean = js.native
  
  /* CompleteClass */
  override def Reset(): Unit = js.native
  
  /* CompleteClass */
  var StatusBar: String = js.native
  
  /* CompleteClass */
  var Width: Double = js.native
}
