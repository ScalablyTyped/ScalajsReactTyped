package typingsJapgolly.activexExcel.Excel

import typingsJapgolly.activexExcel.activexExcelInts.`-4123`
import typingsJapgolly.activexExcel.activexExcelInts.`-4144`
import typingsJapgolly.activexExcel.activexExcelInts.`-4172`
import typingsJapgolly.activexExcel.activexExcelInts.`-4173`
import typingsJapgolly.activexExcel.activexExcelInts.`-4174`
import typingsJapgolly.activexExcel.activexExcelInts.`-4175`
import typingsJapgolly.activexExcel.activexExcelInts.`11`
import typingsJapgolly.activexExcel.activexExcelInts.`12`
import typingsJapgolly.activexExcel.activexExcelInts.`2`
import typingsJapgolly.activexExcel.activexExcelInts.`4`
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Rewritten from type alias, can be one of: 
  - typingsJapgolly.activexExcel.activexExcelInts.`-4172`
  - typingsJapgolly.activexExcel.activexExcelInts.`-4174`
  - typingsJapgolly.activexExcel.activexExcelInts.`4`
  - typingsJapgolly.activexExcel.activexExcelInts.`-4144`
  - typingsJapgolly.activexExcel.activexExcelInts.`2`
  - typingsJapgolly.activexExcel.activexExcelInts.`-4123`
  - typingsJapgolly.activexExcel.activexExcelInts.`11`
  - typingsJapgolly.activexExcel.activexExcelInts.`-4173`
  - typingsJapgolly.activexExcel.activexExcelInts.`-4175`
  - typingsJapgolly.activexExcel.activexExcelInts.`12`
*/
trait XlCellType extends StObject
object XlCellType {
  
  inline def xlCellTypeAllFormatConditions: `-4172` = -4172.asInstanceOf[`-4172`]
  
  inline def xlCellTypeAllValidation: `-4174` = -4174.asInstanceOf[`-4174`]
  
  inline def xlCellTypeBlanks: `4` = 4.asInstanceOf[`4`]
  
  inline def xlCellTypeComments: `-4144` = -4144.asInstanceOf[`-4144`]
  
  inline def xlCellTypeConstants: `2` = 2.asInstanceOf[`2`]
  
  inline def xlCellTypeFormulas: `-4123` = -4123.asInstanceOf[`-4123`]
  
  inline def xlCellTypeLastCell: `11` = 11.asInstanceOf[`11`]
  
  inline def xlCellTypeSameFormatConditions: `-4173` = -4173.asInstanceOf[`-4173`]
  
  inline def xlCellTypeSameValidation: `-4175` = -4175.asInstanceOf[`-4175`]
  
  inline def xlCellTypeVisible: `12` = 12.asInstanceOf[`12`]
}
