package typingsJapgolly.activexOffice.global.Office

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.IMsoChartData")
@js.native
/* private */ open class IMsoChartData ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.IMsoChartData {
  
  /* CompleteClass */
  override def Activate(): Unit = js.native
  
  /* CompleteClass */
  override def ActivateChartDataWindow(): Unit = js.native
  
  /* CompleteClass */
  override def BreakLink(): Unit = js.native
  
  /* CompleteClass */
  override val IsLinked: Boolean = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.IMsoChartData_typekey")
  var OfficeDotIMsoChartData_typekey: typingsJapgolly.activexOffice.Office.IMsoChartData = js.native
  
  /* CompleteClass */
  override val Workbook: Any = js.native
}
