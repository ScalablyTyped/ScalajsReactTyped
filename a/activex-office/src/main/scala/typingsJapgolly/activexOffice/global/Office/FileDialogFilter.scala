package typingsJapgolly.activexOffice.global.Office

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.FileDialogFilter")
@js.native
/* private */ open class FileDialogFilter ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.FileDialogFilter {
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override val Description: String = js.native
  
  /* CompleteClass */
  override val Extensions: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.FileDialogFilter_typekey")
  var OfficeDotFileDialogFilter_typekey: typingsJapgolly.activexOffice.Office.FileDialogFilter = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
}
