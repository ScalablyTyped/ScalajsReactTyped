package typingsJapgolly.activexOffice.global.Office

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.BalloonCheckbox")
@js.native
/* private */ open class BalloonCheckbox ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.BalloonCheckbox {
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  var Checked: Boolean = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override val Item: String = js.native
  
  /* CompleteClass */
  override val Name: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.BalloonCheckbox_typekey")
  var OfficeDotBalloonCheckbox_typekey: typingsJapgolly.activexOffice.Office.BalloonCheckbox = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var Text: String = js.native
}
