package typingsJapgolly.activexOffice.global.Office

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.SignatureSetup")
@js.native
/* private */ open class SignatureSetup ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.SignatureSetup {
  
  /* CompleteClass */
  var AdditionalXml: String = js.native
  
  /* CompleteClass */
  var AllowComments: Boolean = js.native
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override val Id: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.SignatureSetup_typekey")
  var OfficeDotSignatureSetup_typekey: typingsJapgolly.activexOffice.Office.SignatureSetup = js.native
  
  /* CompleteClass */
  override val ReadOnly: Boolean = js.native
  
  /* CompleteClass */
  var ShowSignDate: Boolean = js.native
  
  /* CompleteClass */
  override val SignatureProvider: String = js.native
  
  /* CompleteClass */
  var SigningInstructions: String = js.native
  
  /* CompleteClass */
  var SuggestedSigner: String = js.native
  
  /* CompleteClass */
  var SuggestedSignerEmail: String = js.native
  
  /* CompleteClass */
  var SuggestedSignerLine2: String = js.native
}
