package typingsJapgolly.activexOffice.global.Office

import typingsJapgolly.activexOffice.Office.MsoCTPDockPosition
import typingsJapgolly.activexOffice.Office.MsoCTPDockPositionRestrict
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.CustomTaskPane")
@js.native
/* private */ open class CustomTaskPane ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.CustomTaskPane {
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  override val ContentControl: Any = js.native
  
  /* CompleteClass */
  override def Delete(): Unit = js.native
  
  /* CompleteClass */
  var DockPosition: MsoCTPDockPosition = js.native
  
  /* CompleteClass */
  var DockPositionRestrict: MsoCTPDockPositionRestrict = js.native
  
  /* CompleteClass */
  var Height: Double = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.CustomTaskPane_typekey")
  var OfficeDotCustomTaskPane_typekey: typingsJapgolly.activexOffice.Office.CustomTaskPane = js.native
  
  /* CompleteClass */
  override val Title: String = js.native
  
  /* CompleteClass */
  var Visible: Boolean = js.native
  
  /* CompleteClass */
  var Width: Double = js.native
  
  /* CompleteClass */
  override val Window: Any = js.native
}
