package typingsJapgolly.activexOffice.global.Office

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.WebPageFont")
@js.native
/* private */ open class WebPageFont ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.WebPageFont {
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  var FixedWidthFont: String = js.native
  
  /* CompleteClass */
  var FixedWidthFontSize: Double = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.WebPageFont_typekey")
  var OfficeDotWebPageFont_typekey: typingsJapgolly.activexOffice.Office.WebPageFont = js.native
  
  /* CompleteClass */
  var ProportionalFont: String = js.native
  
  /* CompleteClass */
  var ProportionalFontSize: Double = js.native
}
