package typingsJapgolly.activexOffice.global.Office

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.IMsoInterior")
@js.native
/* private */ open class IMsoInterior ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.IMsoInterior {
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  var Color: Any = js.native
  
  /* CompleteClass */
  var ColorIndex: Any = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  var InvertIfNegative: Any = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.IMsoInterior_typekey")
  var OfficeDotIMsoInterior_typekey: typingsJapgolly.activexOffice.Office.IMsoInterior = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  var Pattern: Any = js.native
  
  /* CompleteClass */
  var PatternColor: Any = js.native
  
  /* CompleteClass */
  var PatternColorIndex: Any = js.native
}
