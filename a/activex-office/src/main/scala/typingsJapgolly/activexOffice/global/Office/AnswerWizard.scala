package typingsJapgolly.activexOffice.global.Office

import typingsJapgolly.activexOffice.Office.AnswerWizardFiles
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.AnswerWizard")
@js.native
/* private */ open class AnswerWizard ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.AnswerWizard {
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  override def ClearFileList(): Unit = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  override def Files(Index: Double): String = js.native
  /* CompleteClass */
  @JSName("Files")
  override val Files_Original: AnswerWizardFiles = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.AnswerWizard_typekey")
  var OfficeDotAnswerWizard_typekey: typingsJapgolly.activexOffice.Office.AnswerWizard = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override def ResetFileList(): Unit = js.native
}
