package typingsJapgolly.activexOffice.global.Office

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.MsoDebugOptions")
@js.native
/* private */ open class MsoDebugOptions ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.MsoDebugOptions {
  
  /* CompleteClass */
  override def AddIgnoredAssertTag(bstrTagToIgnore: String): Unit = js.native
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* CompleteClass */
  var FeatureReports: Double = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.MsoDebugOptions_typekey")
  var OfficeDotMsoDebugOptions_typekey: typingsJapgolly.activexOffice.Office.MsoDebugOptions = js.native
  
  /* CompleteClass */
  var OutputToDebugger: Boolean = js.native
  
  /* CompleteClass */
  var OutputToFile: Boolean = js.native
  
  /* CompleteClass */
  var OutputToMessageBox: Boolean = js.native
  
  /* CompleteClass */
  override def RemoveIgnoredAssertTag(bstrTagToIgnore: String): Unit = js.native
  
  /* CompleteClass */
  override val UnitTestManager: Any = js.native
}
