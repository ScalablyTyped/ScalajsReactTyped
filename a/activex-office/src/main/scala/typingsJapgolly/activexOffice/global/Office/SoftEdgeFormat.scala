package typingsJapgolly.activexOffice.global.Office

import typingsJapgolly.activexOffice.Office.MsoSoftEdgeType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Office.SoftEdgeFormat")
@js.native
/* private */ open class SoftEdgeFormat ()
  extends StObject
     with typingsJapgolly.activexOffice.Office.SoftEdgeFormat {
  
  /* CompleteClass */
  override val Application: Any = js.native
  
  /* CompleteClass */
  override val Creator: Double = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Office.SoftEdgeFormat_typekey")
  var OfficeDotSoftEdgeFormat_typekey: typingsJapgolly.activexOffice.Office.SoftEdgeFormat = js.native
  
  /* CompleteClass */
  var Radius: Double = js.native
  
  /* CompleteClass */
  var Type: MsoSoftEdgeType = js.native
}
