...
import typingsJapgolly.algoliaClientSearch.mod.AddApiKeyResponse
...
object ReadonlyWaitablePromiseAd {
  ...
  inline def apply(
    `catch`: CallbackTo[js.Promise[AddApiKeyResponse | Any]],
    `finally`: CallbackTo[js.Promise[AddApiKeyResponse]],
  ...
  extension [Self <: ReadonlyWaitablePromiseAd](x: Self) {
    ...
    inline def setCatch(value: CallbackTo[js.Promise[AddApiKeyResponse | Any]]): Self = StObject.set(x, "catch", value.toJsFn)
    
    inline def setFinally(value: CallbackTo[js.Promise[AddApiKeyResponse]]): Self = StObject.set(x, "finally", value.toJsFn)
