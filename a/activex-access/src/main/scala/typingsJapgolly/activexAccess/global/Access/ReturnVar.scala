package typingsJapgolly.activexAccess.global.Access

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Access.ReturnVar")
@js.native
/* private */ open class ReturnVar ()
  extends StObject
     with typingsJapgolly.activexAccess.Access.ReturnVar {
  
  /* private */ /* CompleteClass */
  @JSName("Access.ReturnVar_typekey")
  var AccessDotReturnVar_typekey: typingsJapgolly.activexAccess.Access.ReturnVar = js.native
  
  /* CompleteClass */
  override def IsMemberSafe(dispid: Double): Boolean = js.native
  
  /* CompleteClass */
  override val Name: String = js.native
  
  /* CompleteClass */
  override val Value: String | Double = js.native
  
  /* CompleteClass */
  override val _Value: String | Double = js.native
}
