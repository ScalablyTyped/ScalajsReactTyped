package typingsJapgolly.antd.antdMod

import typingsJapgolly.antd.Anon_PrefixCls
import typingsJapgolly.antd.Anon_Separator
import typingsJapgolly.antd.TypeofClassBreadcrumbItem
import typingsJapgolly.antd.TypeofClassBreadcrumbSeparator
import typingsJapgolly.antd.libBreadcrumbMod.default
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("antd", "Breadcrumb")
@js.native
class Breadcrumb () extends default

/* static members */
@JSImport("antd", "Breadcrumb")
@js.native
object Breadcrumb extends js.Object {
  var Item: TypeofClassBreadcrumbItem = js.native
  var Separator: TypeofClassBreadcrumbSeparator = js.native
  var defaultProps: Anon_Separator = js.native
  var propTypes: Anon_PrefixCls = js.native
}

