...
object XAsyncOutputMonitor {
  ...
  inline def apply(waitForCompletion: Callback): XAsyncOutputMonitor = {
    val __obj = js.Dynamic.literal(waitForCompletion = waitForCompletion.toJsFn)
    __obj.asInstanceOf[XAsyncOutputMonitor]
  }
  
  extension [Self <: XAsyncOutputMonitor](x: Self) {
    
    inline def setWaitForCompletion(value: Callback): Self = StObject.set(x, "waitForCompletion", value.toJsFn)
  }
}
