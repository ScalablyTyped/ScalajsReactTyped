...
object XColorSpace {
  ...
  extension [Self <: XColorSpace](x: Self) {
    ...
    inline def setGetComponentTags(value: CallbackTo[SafeArray[Double]]): Self = StObject.set(x, "getComponentTags", value.toJsFn)
    
    inline def setGetProperties(value: CallbackTo[SafeArray[PropertyValue]]): Self = StObject.set(x, "getProperties", value.toJsFn)
    
    inline def setGetRenderingIntent(value: CallbackTo[Double]): Self = StObject.set(x, "getRenderingIntent", value.toJsFn)
