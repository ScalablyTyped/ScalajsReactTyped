...
object XPublished {
  ...
  inline def apply(isPublished: CallbackTo[Boolean]): XPublished = {
    val __obj = js.Dynamic.literal(isPublished = isPublished.toJsFn)
    __obj.asInstanceOf[XPublished]
  }
  
  extension [Self <: XPublished](x: Self) {
    
    inline def setIsPublished(value: CallbackTo[Boolean]): Self = StObject.set(x, "isPublished", value.toJsFn)
  }
}
