package typingsJapgolly.activexFaxcomexlib.global.FAXCOMEXLib

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/** FaxInboundRoutingMethod Class */
@JSGlobal("FAXCOMEXLib.FaxInboundRoutingMethod")
@js.native
/* private */ open class FaxInboundRoutingMethod ()
  extends StObject
     with typingsJapgolly.activexFaxcomexlib.FAXCOMEXLib.FaxInboundRoutingMethod {
  
  /** Friendly name of the extension exposing this method */
  /* CompleteClass */
  override val ExtensionFriendlyName: String = js.native
  
  /** Image name of the extension exposing this method */
  /* CompleteClass */
  override val ExtensionImageName: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("FAXCOMEXLib.FaxInboundRoutingMethod_typekey")
  var FAXCOMEXLibDotFaxInboundRoutingMethod_typekey: typingsJapgolly.activexFaxcomexlib.FAXCOMEXLib.FaxInboundRoutingMethod = js.native
  
  /** Internal function name */
  /* CompleteClass */
  override val FunctionName: String = js.native
  
  /** GUID of the method */
  /* CompleteClass */
  override val GUID: String = js.native
  
  /** The name of the method */
  /* CompleteClass */
  override val Name: String = js.native
  
  /** Priority of the method */
  /* CompleteClass */
  var Priority: Double = js.native
  
  /** Refresh the object */
  /* CompleteClass */
  override def Refresh(): Unit = js.native
  
  /** Save the object */
  /* CompleteClass */
  override def Save(): Unit = js.native
}
