...
object srcNgtscShimsApiMod {
  ...
  object FactoryTracker {
    ...
    inline def apply(sourceInfo: Map[String, FactoryInfo], track: (SourceFile, ModuleInfo) => Callback): FactoryTracker = {
      val __obj = js.Dynamic.literal(sourceInfo = sourceInfo.asInstanceOf[js.Any], track = js.Any.fromFunction2((t0: SourceFile, t1: ModuleInfo) => (track(t0, t1)).runNow()))
    ...
    extension [Self <: FactoryTracker](x: Self) {
      ...
      inline def setTrack(value: (SourceFile, ModuleInfo) => Callback): Self = StObject.set(x, "track", js.Any.fromFunction2((t0: SourceFile, t1: ModuleInfo) => (value(t0, t1)).runNow()))
    }
  }
  ...
  object TopLevelShimGenerator {
    ...
    inline def apply(makeTopLevelShim: CallbackTo[SourceFile], shouldEmit: Boolean): TopLevelShimGenerator = {
      val __obj = js.Dynamic.literal(makeTopLevelShim = makeTopLevelShim.toJsFn, shouldEmit = shouldEmit.asInstanceOf[js.Any])
      __obj.asInstanceOf[TopLevelShimGenerator]
    }
    
    extension [Self <: TopLevelShimGenerator](x: Self) {
      
      inline def setMakeTopLevelShim(value: CallbackTo[SourceFile]): Self = StObject.set(x, "makeTopLevelShim", value.toJsFn)
