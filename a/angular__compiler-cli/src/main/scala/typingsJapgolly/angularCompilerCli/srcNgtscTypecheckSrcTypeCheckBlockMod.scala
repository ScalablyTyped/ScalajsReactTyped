...
import typingsJapgolly.angularCompilerCli.anon.ClassDeclarationClassDeclDeclarationBrand
...
object srcNgtscTypecheckSrcTypeCheckBlockMod {
  ...
  object TcbGenericContextBehavior extends StObject {
    ...
    /* 1 */ val CopyClassNodes: typingsJapgolly.angularCompilerCli.srcNgtscTypecheckSrcTypeCheckBlockMod.TcbGenericContextBehavior.CopyClassNodes & Double = js.native
    ...
    sealed trait FallbackToAny
      extends StObject
         with TcbGenericContextBehavior
    /* 2 */ val FallbackToAny: typingsJapgolly.angularCompilerCli.srcNgtscTypecheckSrcTypeCheckBlockMod.TcbGenericContextBehavior.FallbackToAny & Double = js.native
    ...
    sealed trait UseEmitter
      extends StObject
         with TcbGenericContextBehavior
    /* 0 */ val UseEmitter: typingsJapgolly.angularCompilerCli.srcNgtscTypecheckSrcTypeCheckBlockMod.TcbGenericContextBehavior.UseEmitter & Double = js.native
  }
  ...
  object TcbOp {
    ...
    inline def apply(
      circularFallback: CallbackTo[TcbOp | Expression],
      execute: CallbackTo[Expression | Null],
      optional: Boolean
    ): TcbOp = {
      val __obj = js.Dynamic.literal(circularFallback = circularFallback.toJsFn, execute = execute.toJsFn, optional = optional.asInstanceOf[js.Any])
      __obj.asInstanceOf[TcbOp]
    }
    
    extension [Self <: TcbOp](x: Self) {
      
      inline def setCircularFallback(value: CallbackTo[TcbOp | Expression]): Self = StObject.set(x, "circularFallback", value.toJsFn)
      
      inline def setExecute(value: CallbackTo[Expression | Null]): Self = StObject.set(x, "execute", value.toJsFn)
