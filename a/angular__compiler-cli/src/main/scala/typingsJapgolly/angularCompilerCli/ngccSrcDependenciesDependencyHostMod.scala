...
object ngccSrcDependenciesDependencyHostMod {
  ...
  object DependencyHost {
    ...
    inline def apply(collectDependencies: (AbsoluteFsPath, DependencyInfo) => Callback): DependencyHost = {
      val __obj = js.Dynamic.literal(collectDependencies = js.Any.fromFunction2((t0: AbsoluteFsPath, t1: DependencyInfo) => (collectDependencies(t0, t1)).runNow()))
      __obj.asInstanceOf[DependencyHost]
    }
    
    extension [Self <: DependencyHost](x: Self) {
      
      inline def setCollectDependencies(value: (AbsoluteFsPath, DependencyInfo) => Callback): Self = StObject.set(x, "collectDependencies", js.Any.fromFunction2((t0: AbsoluteFsPath, t1: DependencyInfo) => (value(t0, t1)).runNow()))
    }
  }
