package typingsJapgolly.arcgisJsApi.global.esri

import typingsJapgolly.arcgisJsApi.esri.PortalItemResourceProperties
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* This class was inferred from a value with a constructor, it was renamed because a distinct type already exists with the same name. */
@JSGlobal("__esri.PortalItemResource")
@js.native
/**
  * A reference to a portal item resource.
  *
  * [Read more...](https://developers.arcgis.com/javascript/latest/api-reference/esri-portal-PortalItemResource.html)
  */
open class PortalItemResourceCls ()
  extends StObject
     with typingsJapgolly.arcgisJsApi.esri.PortalItemResource {
  def this(properties: PortalItemResourceProperties) = this()
}
