package typingsJapgolly.atAngularCompiler.srcRender3ViewCompilerMod

import typingsJapgolly.atAngularCompiler.Anon_ExpressionType
import typingsJapgolly.atAngularCompiler.srcConstantUnderscorePoolMod.ConstantPool
import typingsJapgolly.atAngularCompiler.srcTemplateUnderscoreParserBindingUnderscoreParserMod.BindingParser
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@angular/compiler/src/render3/view/compiler", "compileBaseDefFromMetadata")
@js.native
object compileBaseDefFromMetadata extends js.Object {
  def apply(meta: R3BaseRefMetaData, constantPool: ConstantPool, bindingParser: BindingParser): Anon_ExpressionType = js.native
}

