package typingsJapgolly.atAngularCompiler.atAngularCompilerMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("@angular/compiler", "_ParseAST")
@js.native
class _ParseAST protected ()
  extends typingsJapgolly.atAngularCompiler.publicUnderscoreApiMod._ParseAST {
  def this(
    input: String,
    location: js.Any,
    absoluteOffset: Double,
    tokens: js.Array[typingsJapgolly.atAngularCompiler.srcExpressionUnderscoreParserLexerMod.Token],
    inputLength: Double,
    parseAction: Boolean,
    errors: js.Array[
        typingsJapgolly.atAngularCompiler.srcExpressionUnderscoreParserAstMod.ParserError
      ],
    offset: Double
  ) = this()
}

