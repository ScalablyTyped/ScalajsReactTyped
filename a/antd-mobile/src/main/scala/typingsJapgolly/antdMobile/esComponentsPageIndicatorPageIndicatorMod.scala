...
import typingsJapgolly.antdMobile.anon.CSSPropertiesPartialRecorActivedotborderradius
...
object esComponentsPageIndicatorPageIndicatorMod {
  
  @JSImport("antd-mobile/es/components/page-indicator/page-indicator", "PageIndicator")
  @js.native
  val PageIndicator: NamedExoticComponent[PageIndicatorProps] = js.native
  
  /* Inlined {  total :number,   current :number,   direction :'horizontal' | 'vertical' | undefined,   color :'primary' | 'white' | undefined} & antd-mobile.antd-mobile/es/utils/native-props.NativeProps<'--dot-color' | '--active-dot-color' | '--dot-size' | '--active-dot-size' | '--dot-border-radius' | '--active-dot-border-radius' | '--dot-spacing'> */
  trait PageIndicatorProps extends StObject {
  ...
  object PageIndicatorProps {
    
    inline def apply(current: Double, total: Double): PageIndicatorProps = {
      val __obj = js.Dynamic.literal(current = current.asInstanceOf[js.Any], total = total.asInstanceOf[js.Any])
      __obj.asInstanceOf[PageIndicatorProps]
    }
    
    extension [Self <: PageIndicatorProps](x: Self) {
