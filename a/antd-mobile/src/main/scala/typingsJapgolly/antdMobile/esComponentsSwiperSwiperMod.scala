...
import typingsJapgolly.antdMobile.anon.CSSPropertiesPartialRecorBlockOverflow
import typingsJapgolly.antdMobile.anon.PickPageIndicatorPropscol
import typingsJapgolly.antdMobile.anon.defaultIndexnumberundefinAllowTouchMove
...
object esComponentsSwiperSwiperMod {
  ...
  object SwiperRef {
    ...
    inline def apply(swipeNext: Callback, swipePrev: Callback, swipeTo: Double => Callback): SwiperRef = {
      val __obj = js.Dynamic.literal(swipeNext = swipeNext.toJsFn, swipePrev = swipePrev.toJsFn, swipeTo = js.Any.fromFunction1((t0: Double) => swipeTo(t0).runNow()))
      __obj.asInstanceOf[SwiperRef]
    }
    
    extension [Self <: SwiperRef](x: Self) {
      
      inline def setSwipeNext(value: Callback): Self = StObject.set(x, "swipeNext", value.toJsFn)
      
      inline def setSwipePrev(value: Callback): Self = StObject.set(x, "swipePrev", value.toJsFn)
      
      inline def setSwipeTo(value: Double => Callback): Self = StObject.set(x, "swipeTo", js.Any.fromFunction1((t0: Double) => value(t0).runNow()))
    }
  }
}
