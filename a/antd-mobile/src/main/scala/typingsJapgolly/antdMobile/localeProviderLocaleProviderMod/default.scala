package typingsJapgolly.antdMobile.localeProviderLocaleProviderMod

import typingsJapgolly.antdMobile.AnonAntLocale
import typingsJapgolly.antdMobile.AnonLocale
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("antd-mobile/lib/locale-provider/locale-provider", JSImport.Default)
@js.native
class default () extends LocaleProvider

/* static members */
@JSImport("antd-mobile/lib/locale-provider/locale-provider", JSImport.Default)
@js.native
object default extends js.Object {
  var childContextTypes: AnonAntLocale = js.native
  var propTypes: AnonLocale = js.native
}

