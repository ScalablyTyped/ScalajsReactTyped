package typingsJapgolly.activexInfopath.global.InfoPath

import typingsJapgolly.activexMsxml2.MSXML2.IXMLDOMDocument
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("InfoPath.MergeEventObject")
@js.native
/* private */ open class MergeEventObject ()
  extends StObject
     with typingsJapgolly.activexInfopath.InfoPath.MergeEventObject {
  
  /* CompleteClass */
  override val Count: Double = js.native
  
  /* CompleteClass */
  override val DOM: IXMLDOMDocument = js.native
  
  /* private */ /* CompleteClass */
  @JSName("InfoPath.MergeEventObject_typekey")
  var InfoPathDotMergeEventObject_typekey: typingsJapgolly.activexInfopath.InfoPath.MergeEventObject = js.native
  
  /* CompleteClass */
  var ReturnStatus: Boolean = js.native
  
  /* CompleteClass */
  var RollBack: Boolean = js.native
  
  /* CompleteClass */
  override val XDocument: typingsJapgolly.activexInfopath.InfoPath._XDocument = js.native
  
  /* CompleteClass */
  override val index: Double = js.native
}
