package typingsJapgolly.activexInfopath.global.InfoPath

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("InfoPath.XMLFileAdapterObject")
@js.native
/* private */ open class XMLFileAdapterObject ()
  extends StObject
     with typingsJapgolly.activexInfopath.InfoPath.XMLFileAdapterObject {
  
  /* CompleteClass */
  var FileURL: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("InfoPath.XMLFileAdapterObject_typekey")
  var InfoPathDotXMLFileAdapterObject_typekey: typingsJapgolly.activexInfopath.InfoPath.XMLFileAdapterObject = js.native
  
  /* CompleteClass */
  override val Name: String = js.native
  
  /* CompleteClass */
  override def Query(): Unit = js.native
  
  /* CompleteClass */
  override val QueryAllowed: Boolean = js.native
  
  /* CompleteClass */
  override def Submit(): Unit = js.native
  
  /* CompleteClass */
  override val SubmitAllowed: Boolean = js.native
}
