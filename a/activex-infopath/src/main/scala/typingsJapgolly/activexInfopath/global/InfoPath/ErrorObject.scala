package typingsJapgolly.activexInfopath.global.InfoPath

import typingsJapgolly.activexMsxml2.MSXML2.IXMLDOMNode
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("InfoPath.ErrorObject")
@js.native
/* private */ open class ErrorObject ()
  extends StObject
     with typingsJapgolly.activexInfopath.InfoPath.ErrorObject {
  
  /* CompleteClass */
  override val ConditionName: Any = js.native
  
  /* CompleteClass */
  var DetailedErrorMessage: String = js.native
  
  /* CompleteClass */
  var ErrorCode: Double = js.native
  
  /* private */ /* CompleteClass */
  @JSName("InfoPath.ErrorObject_typekey")
  var InfoPathDotErrorObject_typekey: typingsJapgolly.activexInfopath.InfoPath.ErrorObject = js.native
  
  /* CompleteClass */
  override val Node: IXMLDOMNode = js.native
  
  /* CompleteClass */
  var ShortErrorMessage: String = js.native
  
  /* CompleteClass */
  override val Type: String = js.native
}
