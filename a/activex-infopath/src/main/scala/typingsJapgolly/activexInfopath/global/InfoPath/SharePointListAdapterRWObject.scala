package typingsJapgolly.activexInfopath.global.InfoPath

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("InfoPath.SharePointListAdapterRWObject")
@js.native
/* private */ open class SharePointListAdapterRWObject ()
  extends StObject
     with typingsJapgolly.activexInfopath.InfoPath.SharePointListAdapterRWObject {
  
  /* CompleteClass */
  var ContentTypeId: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("InfoPath.SharePointListAdapterRWObject_typekey")
  var InfoPathDotSharePointListAdapterRWObject_typekey: typingsJapgolly.activexInfopath.InfoPath.SharePointListAdapterRWObject = js.native
  
  /* CompleteClass */
  var ListId: String = js.native
  
  /* CompleteClass */
  override val Name: String = js.native
  
  /* CompleteClass */
  override def Query(): Unit = js.native
  
  /* CompleteClass */
  override val QueryAllowed: Boolean = js.native
  
  /* CompleteClass */
  var SiteUrl: String = js.native
  
  /* CompleteClass */
  override def Submit(): Unit = js.native
  
  /* CompleteClass */
  override val SubmitAllowed: Boolean = js.native
}
