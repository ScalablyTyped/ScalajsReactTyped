package typingsJapgolly.activexInfopath.global.InfoPath

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("InfoPath.ISubmitToHostEventHandler")
@js.native
/* private */ open class ISubmitToHostEventHandler ()
  extends StObject
     with typingsJapgolly.activexInfopath.InfoPath.ISubmitToHostEventHandler {
  
  /* private */ /* CompleteClass */
  @JSName("InfoPath.ISubmitToHostEventHandler_typekey")
  var InfoPathDotISubmitToHostEventHandler_typekey: typingsJapgolly.activexInfopath.InfoPath.ISubmitToHostEventHandler = js.native
  
  /* CompleteClass */
  override def SubmitToHostEventHandler(punkSender: Any, bstrAdapterName: String, pbstrErrorMessage: String): Double = js.native
}
