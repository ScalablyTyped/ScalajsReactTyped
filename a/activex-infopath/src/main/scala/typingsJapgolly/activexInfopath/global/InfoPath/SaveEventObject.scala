package typingsJapgolly.activexInfopath.global.InfoPath

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("InfoPath.SaveEventObject")
@js.native
/* private */ open class SaveEventObject ()
  extends StObject
     with typingsJapgolly.activexInfopath.InfoPath.SaveEventObject {
  
  /* CompleteClass */
  override val FileName: String = js.native
  
  /* private */ /* CompleteClass */
  @JSName("InfoPath.SaveEventObject_typekey")
  var InfoPathDotSaveEventObject_typekey: typingsJapgolly.activexInfopath.InfoPath.SaveEventObject = js.native
  
  /* CompleteClass */
  var IsCancelled: Boolean = js.native
  
  /* CompleteClass */
  override val IsSaveAs: Boolean = js.native
  
  /* CompleteClass */
  override def PerformSaveOperation(): Boolean = js.native
  
  /* CompleteClass */
  var ReturnStatus: Boolean = js.native
  
  /* CompleteClass */
  override val XDocument: typingsJapgolly.activexInfopath.InfoPath._XDocument = js.native
}
