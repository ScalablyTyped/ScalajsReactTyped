package typingsJapgolly.angularCommon.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait DKKHRK extends StObject {
  
  var BYN: js.Array[js.UndefOr[String]]
  
  var DKK: js.Array[js.UndefOr[String]]
  
  var HRK: js.Array[js.UndefOr[String]]
  
  var ISK: js.Array[js.UndefOr[String]]
  
  var JPY: js.Array[String]
  
  var NOK: js.Array[js.UndefOr[String]]
  
  var PHP: js.Array[js.UndefOr[String]]
  
  var PLN: js.Array[js.UndefOr[String]]
  
  var SEK: js.Array[js.UndefOr[String]]
  
  var THB: js.Array[String]
  
  var TWD: js.Array[String]
  
  var ZAR: js.Array[String]
}
object DKKHRK {
  
  inline def apply(
    BYN: js.Array[js.UndefOr[String]],
    DKK: js.Array[js.UndefOr[String]],
    HRK: js.Array[js.UndefOr[String]],
    ISK: js.Array[js.UndefOr[String]],
    JPY: js.Array[String],
    NOK: js.Array[js.UndefOr[String]],
    PHP: js.Array[js.UndefOr[String]],
    PLN: js.Array[js.UndefOr[String]],
    SEK: js.Array[js.UndefOr[String]],
    THB: js.Array[String],
    TWD: js.Array[String],
    ZAR: js.Array[String]
  ): DKKHRK = {
    val __obj = js.Dynamic.literal(BYN = BYN.asInstanceOf[js.Any], DKK = DKK.asInstanceOf[js.Any], HRK = HRK.asInstanceOf[js.Any], ISK = ISK.asInstanceOf[js.Any], JPY = JPY.asInstanceOf[js.Any], NOK = NOK.asInstanceOf[js.Any], PHP = PHP.asInstanceOf[js.Any], PLN = PLN.asInstanceOf[js.Any], SEK = SEK.asInstanceOf[js.Any], THB = THB.asInstanceOf[js.Any], TWD = TWD.asInstanceOf[js.Any], ZAR = ZAR.asInstanceOf[js.Any])
    __obj.asInstanceOf[DKKHRK]
  }
  
  extension [Self <: DKKHRK](x: Self) {
    
    inline def setBYN(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "BYN", value.asInstanceOf[js.Any])
    
    inline def setBYNVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "BYN", js.Array(value*))
    
    inline def setDKK(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "DKK", value.asInstanceOf[js.Any])
    
    inline def setDKKVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "DKK", js.Array(value*))
    
    inline def setHRK(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "HRK", value.asInstanceOf[js.Any])
    
    inline def setHRKVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "HRK", js.Array(value*))
    
    inline def setISK(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "ISK", value.asInstanceOf[js.Any])
    
    inline def setISKVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "ISK", js.Array(value*))
    
    inline def setJPY(value: js.Array[String]): Self = StObject.set(x, "JPY", value.asInstanceOf[js.Any])
    
    inline def setJPYVarargs(value: String*): Self = StObject.set(x, "JPY", js.Array(value*))
    
    inline def setNOK(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "NOK", value.asInstanceOf[js.Any])
    
    inline def setNOKVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "NOK", js.Array(value*))
    
    inline def setPHP(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "PHP", value.asInstanceOf[js.Any])
    
    inline def setPHPVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "PHP", js.Array(value*))
    
    inline def setPLN(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "PLN", value.asInstanceOf[js.Any])
    
    inline def setPLNVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "PLN", js.Array(value*))
    
    inline def setSEK(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "SEK", value.asInstanceOf[js.Any])
    
    inline def setSEKVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "SEK", js.Array(value*))
    
    inline def setTHB(value: js.Array[String]): Self = StObject.set(x, "THB", value.asInstanceOf[js.Any])
    
    inline def setTHBVarargs(value: String*): Self = StObject.set(x, "THB", js.Array(value*))
    
    inline def setTWD(value: js.Array[String]): Self = StObject.set(x, "TWD", value.asInstanceOf[js.Any])
    
    inline def setTWDVarargs(value: String*): Self = StObject.set(x, "TWD", js.Array(value*))
    
    inline def setZAR(value: js.Array[String]): Self = StObject.set(x, "ZAR", value.asInstanceOf[js.Any])
    
    inline def setZARVarargs(value: String*): Self = StObject.set(x, "ZAR", js.Array(value*))
  }
}
