package typingsJapgolly.angularCommon.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait BGM extends StObject {
  
  var ATS: js.Array[String]
  
  var AUD: js.Array[String]
  
  var BGM: js.Array[String]
  
  var BGO: js.Array[String]
  
  var BYN: js.Array[js.UndefOr[String]]
  
  var CUC: js.Array[js.UndefOr[String]]
  
  var DEM: js.Array[String]
  
  var EUR: js.Array[scala.Nothing]
  
  var FKP: js.Array[js.UndefOr[String]]
  
  var GHS: js.Array[js.UndefOr[String]]
  
  var GNF: js.Array[js.UndefOr[String]]
  
  var KMF: js.Array[js.UndefOr[String]]
  
  var PHP: js.Array[js.UndefOr[String]]
  
  var RON: js.Array[js.UndefOr[String]]
  
  var RUR: js.Array[js.UndefOr[String]]
  
  var RWF: js.Array[js.UndefOr[String]]
  
  var SYP: js.Array[scala.Nothing]
  
  var THB: js.Array[String]
  
  var TWD: js.Array[String]
  
  var XXX: js.Array[scala.Nothing]
  
  var ZMW: js.Array[js.UndefOr[String]]
}
object BGM {
  
  inline def apply(
    ATS: js.Array[String],
    AUD: js.Array[String],
    BGM: js.Array[String],
    BGO: js.Array[String],
    BYN: js.Array[js.UndefOr[String]],
    CUC: js.Array[js.UndefOr[String]],
    DEM: js.Array[String],
    EUR: js.Array[scala.Nothing],
    FKP: js.Array[js.UndefOr[String]],
    GHS: js.Array[js.UndefOr[String]],
    GNF: js.Array[js.UndefOr[String]],
    KMF: js.Array[js.UndefOr[String]],
    PHP: js.Array[js.UndefOr[String]],
    RON: js.Array[js.UndefOr[String]],
    RUR: js.Array[js.UndefOr[String]],
    RWF: js.Array[js.UndefOr[String]],
    SYP: js.Array[scala.Nothing],
    THB: js.Array[String],
    TWD: js.Array[String],
    XXX: js.Array[scala.Nothing],
    ZMW: js.Array[js.UndefOr[String]]
  ): BGM = {
    val __obj = js.Dynamic.literal(ATS = ATS.asInstanceOf[js.Any], AUD = AUD.asInstanceOf[js.Any], BGM = BGM.asInstanceOf[js.Any], BGO = BGO.asInstanceOf[js.Any], BYN = BYN.asInstanceOf[js.Any], CUC = CUC.asInstanceOf[js.Any], DEM = DEM.asInstanceOf[js.Any], EUR = EUR.asInstanceOf[js.Any], FKP = FKP.asInstanceOf[js.Any], GHS = GHS.asInstanceOf[js.Any], GNF = GNF.asInstanceOf[js.Any], KMF = KMF.asInstanceOf[js.Any], PHP = PHP.asInstanceOf[js.Any], RON = RON.asInstanceOf[js.Any], RUR = RUR.asInstanceOf[js.Any], RWF = RWF.asInstanceOf[js.Any], SYP = SYP.asInstanceOf[js.Any], THB = THB.asInstanceOf[js.Any], TWD = TWD.asInstanceOf[js.Any], XXX = XXX.asInstanceOf[js.Any], ZMW = ZMW.asInstanceOf[js.Any])
    __obj.asInstanceOf[BGM]
  }
  
  extension [Self <: BGM](x: Self) {
    
    inline def setATS(value: js.Array[String]): Self = StObject.set(x, "ATS", value.asInstanceOf[js.Any])
    
    inline def setATSVarargs(value: String*): Self = StObject.set(x, "ATS", js.Array(value*))
    
    inline def setAUD(value: js.Array[String]): Self = StObject.set(x, "AUD", value.asInstanceOf[js.Any])
    
    inline def setAUDVarargs(value: String*): Self = StObject.set(x, "AUD", js.Array(value*))
    
    inline def setBGM(value: js.Array[String]): Self = StObject.set(x, "BGM", value.asInstanceOf[js.Any])
    
    inline def setBGMVarargs(value: String*): Self = StObject.set(x, "BGM", js.Array(value*))
    
    inline def setBGO(value: js.Array[String]): Self = StObject.set(x, "BGO", value.asInstanceOf[js.Any])
    
    inline def setBGOVarargs(value: String*): Self = StObject.set(x, "BGO", js.Array(value*))
    
    inline def setBYN(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "BYN", value.asInstanceOf[js.Any])
    
    inline def setBYNVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "BYN", js.Array(value*))
    
    inline def setCUC(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "CUC", value.asInstanceOf[js.Any])
    
    inline def setCUCVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "CUC", js.Array(value*))
    
    inline def setDEM(value: js.Array[String]): Self = StObject.set(x, "DEM", value.asInstanceOf[js.Any])
    
    inline def setDEMVarargs(value: String*): Self = StObject.set(x, "DEM", js.Array(value*))
    
    inline def setEUR(value: js.Array[scala.Nothing]): Self = StObject.set(x, "EUR", value.asInstanceOf[js.Any])
    
    inline def setEURVarargs(value: scala.Nothing*): Self = StObject.set(x, "EUR", js.Array(value*))
    
    inline def setFKP(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "FKP", value.asInstanceOf[js.Any])
    
    inline def setFKPVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "FKP", js.Array(value*))
    
    inline def setGHS(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "GHS", value.asInstanceOf[js.Any])
    
    inline def setGHSVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "GHS", js.Array(value*))
    
    inline def setGNF(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "GNF", value.asInstanceOf[js.Any])
    
    inline def setGNFVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "GNF", js.Array(value*))
    
    inline def setKMF(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "KMF", value.asInstanceOf[js.Any])
    
    inline def setKMFVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "KMF", js.Array(value*))
    
    inline def setPHP(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "PHP", value.asInstanceOf[js.Any])
    
    inline def setPHPVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "PHP", js.Array(value*))
    
    inline def setRON(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "RON", value.asInstanceOf[js.Any])
    
    inline def setRONVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "RON", js.Array(value*))
    
    inline def setRUR(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "RUR", value.asInstanceOf[js.Any])
    
    inline def setRURVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "RUR", js.Array(value*))
    
    inline def setRWF(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "RWF", value.asInstanceOf[js.Any])
    
    inline def setRWFVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "RWF", js.Array(value*))
    
    inline def setSYP(value: js.Array[scala.Nothing]): Self = StObject.set(x, "SYP", value.asInstanceOf[js.Any])
    
    inline def setSYPVarargs(value: scala.Nothing*): Self = StObject.set(x, "SYP", js.Array(value*))
    
    inline def setTHB(value: js.Array[String]): Self = StObject.set(x, "THB", value.asInstanceOf[js.Any])
    
    inline def setTHBVarargs(value: String*): Self = StObject.set(x, "THB", js.Array(value*))
    
    inline def setTWD(value: js.Array[String]): Self = StObject.set(x, "TWD", value.asInstanceOf[js.Any])
    
    inline def setTWDVarargs(value: String*): Self = StObject.set(x, "TWD", js.Array(value*))
    
    inline def setXXX(value: js.Array[scala.Nothing]): Self = StObject.set(x, "XXX", value.asInstanceOf[js.Any])
    
    inline def setXXXVarargs(value: scala.Nothing*): Self = StObject.set(x, "XXX", js.Array(value*))
    
    inline def setZMW(value: js.Array[js.UndefOr[String]]): Self = StObject.set(x, "ZMW", value.asInstanceOf[js.Any])
    
    inline def setZMWVarargs(value: js.UndefOr[String]*): Self = StObject.set(x, "ZMW", js.Array(value*))
  }
}
