package typingsJapgolly.antdMobileRn.cardIndexNativeMod

import typingsJapgolly.antdMobileRn.AnonFull
import typingsJapgolly.antdMobileRn.TypeofCardBody
import typingsJapgolly.antdMobileRn.TypeofCardFooter
import typingsJapgolly.antdMobileRn.TypeofCardHeader
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("antd-mobile-rn/lib/card/index.native", JSImport.Default)
@js.native
class default () extends Card

/* static members */
@JSImport("antd-mobile-rn/lib/card/index.native", JSImport.Default)
@js.native
object default extends js.Object {
  var Body: TypeofCardBody = js.native
  var Footer: TypeofCardFooter = js.native
  var Header: TypeofCardHeader = js.native
  var defaultProps: AnonFull = js.native
}

