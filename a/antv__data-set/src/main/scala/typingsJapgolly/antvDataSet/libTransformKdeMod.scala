package typingsJapgolly.antvDataSet

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object libTransformKdeMod {
  
  object default {
    
    @JSImport("@antv/data-set/lib/transform/kde", JSImport.Default)
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("@antv/data-set/lib/transform/kde", "default.BANDWIDTH_METHODS")
    @js.native
    def BANDWIDTH_METHODS: js.Array[Any] = js.native
    inline def BANDWIDTH_METHODS_=(x: js.Array[Any]): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("BANDWIDTH_METHODS")(x.asInstanceOf[js.Any])
    
    @JSImport("@antv/data-set/lib/transform/kde", "default.KERNEL_METHODS")
    @js.native
    def KERNEL_METHODS: js.Array[Any] = js.native
    inline def KERNEL_METHODS_=(x: js.Array[Any]): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("KERNEL_METHODS")(x.asInstanceOf[js.Any])
  }
}
