...
object logMod {
  ...
  object Logger_ {
    ...
    inline def apply(log: (String, Any) => Callback): Logger_ = {
      val __obj = js.Dynamic.literal(log = js.Any.fromFunction2((t0: String, t1: Any) => (log(t0, t1)).runNow()))
      __obj.asInstanceOf[Logger_]
    }
    
    extension [Self <: Logger_](x: Self) {
      
      inline def setLog(value: (String, Any) => Callback): Self = StObject.set(x, "log", js.Any.fromFunction2((t0: String, t1: Any) => (value(t0, t1)).runNow()))
    }
  }
}
