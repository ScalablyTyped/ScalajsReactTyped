...
import typingsJapgolly.ariClient.anon.DeviceName
import typingsJapgolly.ariClient.anon.DeviceNameDeviceState
...
trait DeviceStates extends StObject {
  ...
  /**
    ...
    */
  def update(params: DeviceNameDeviceState): js.Promise[Unit] = js.native
  /**
    * Change the state of a device controlled by ARI. (Note - implicitly creates the device state).
    *
    * @param params.deviceName - Name of the device.
    * @param params.deviceState - Device state value.
    */
  def update(params: DeviceNameDeviceState, callback: js.Function1[/* err */ js.Error, Unit]): Unit = js.native
}
