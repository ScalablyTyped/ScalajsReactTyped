package typingsJapgolly.activexOutlook.global.Outlook

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Outlook._RecipientControl")
@js.native
/* private */ open class RecipientControl ()
  extends StObject
     with typingsJapgolly.activexOutlook.Outlook.RecipientControl {
  
  /* CompleteClass */
  var BackColor: Double = js.native
  
  /* CompleteClass */
  var Enabled: Double = js.native
  
  /* CompleteClass */
  var Font: Any = js.native
  
  /* CompleteClass */
  var ForeColor: Double = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Outlook._RecipientControl_typekey")
  var OutlookDot_RecipientControl_typekey: typingsJapgolly.activexOutlook.Outlook.RecipientControl = js.native
  
  /* CompleteClass */
  var ReadOnly: Double = js.native
  
  /* CompleteClass */
  var SpecialEffect: Double = js.native
}
