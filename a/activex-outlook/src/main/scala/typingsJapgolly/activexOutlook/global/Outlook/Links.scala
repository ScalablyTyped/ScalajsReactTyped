package typingsJapgolly.activexOutlook.global.Outlook

import typingsJapgolly.activexOutlook.Outlook.OlObjectClass
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Outlook.Links")
@js.native
/* private */ open class Links ()
  extends StObject
     with typingsJapgolly.activexOutlook.Outlook.Links {
  
  /* CompleteClass */
  override def Add(Item: Any): typingsJapgolly.activexOutlook.Outlook.Link = js.native
  
  /* CompleteClass */
  override val Application: typingsJapgolly.activexOutlook.Outlook.Application = js.native
  
  /* CompleteClass */
  override val Class: OlObjectClass = js.native
  
  /* CompleteClass */
  override val Count: Double = js.native
  
  /* CompleteClass */
  override def Item(Index: Any): typingsJapgolly.activexOutlook.Outlook.Link = js.native
  
  /* private */ /* CompleteClass */
  @JSName("Outlook.Links_typekey")
  var OutlookDotLinks_typekey: typingsJapgolly.activexOutlook.Outlook.Links = js.native
  
  /* CompleteClass */
  override val Parent: Any = js.native
  
  /* CompleteClass */
  override def Remove(Index: Any): Unit = js.native
  
  /* CompleteClass */
  override val Session: typingsJapgolly.activexOutlook.Outlook.NameSpace = js.native
}
