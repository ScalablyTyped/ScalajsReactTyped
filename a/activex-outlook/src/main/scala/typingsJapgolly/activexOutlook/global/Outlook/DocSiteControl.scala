package typingsJapgolly.activexOutlook.global.Outlook

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Outlook._DocSiteControl")
@js.native
/* private */ open class DocSiteControl ()
  extends StObject
     with typingsJapgolly.activexOutlook.Outlook.DocSiteControl {
  
  /* private */ /* CompleteClass */
  @JSName("Outlook._DocSiteControl_typekey")
  var OutlookDot_DocSiteControl_typekey: typingsJapgolly.activexOutlook.Outlook.DocSiteControl = js.native
  
  /* CompleteClass */
  var ReadOnly: Double = js.native
  
  /* CompleteClass */
  var SuppressAttachments: Double = js.native
}
