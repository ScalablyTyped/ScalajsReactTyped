...
import typingsJapgolly.antDesignProUtils.esUseStyleTokenMod.AliasToken
...
object esUseStyleMod {
  ...
  trait UseStyleResult extends StObject {
    ...
    def wrapSSR(node: Element): Element
  }
  object UseStyleResult {
    
    inline def apply(hashId: String, wrapSSR: Element => Element): UseStyleResult = {
    ...
    extension [Self <: UseStyleResult](x: Self) {
      ...
      inline def setWrapSSR(value: Element => Element): Self = StObject.set(x, "wrapSSR", js.Any.fromFunction1(value))
    }
  }
}
