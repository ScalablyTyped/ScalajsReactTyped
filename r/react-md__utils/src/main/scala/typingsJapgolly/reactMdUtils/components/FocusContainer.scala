...
import typingsJapgolly.reactMdUtils.reactMdUtilsStrings.first
import typingsJapgolly.reactMdUtils.reactMdUtilsStrings.last
...
object FocusContainer {
  ...
  def withProps(p: FocusContainerProps & RefAttributes[HTMLDivElement]): Builder = new Builder(js.Array(this.component, p.asInstanceOf[js.Any]))
}
