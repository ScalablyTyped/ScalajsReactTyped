package typingsJapgolly.rest

import org.scalablytyped.runtime.Shortcut
import typingsJapgolly.rest.mod.Interceptor
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object interceptorIeXdomainMod extends Shortcut {
  
  @JSImport("rest/interceptor/ie/xdomain", JSImport.Namespace)
  @js.native
  val ^ : Interceptor[js.Object] = js.native
  
  type _To = Interceptor[js.Object]
  
  /* This means you don't have to write `^`, but can instead just say `interceptorIeXdomainMod.foo` */
  override def _to: Interceptor[js.Object] = ^
}
