...
import typingsJapgolly.requireHacker.anon.Path
...
object mod {
  ...
  object Unmount {
    ...
    inline def apply(unmount: Callback): Unmount = {
      val __obj = js.Dynamic.literal(unmount = unmount.toJsFn)
