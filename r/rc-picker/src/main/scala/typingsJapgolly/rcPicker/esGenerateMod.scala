...
import typingsJapgolly.rcPicker.anon.Format
...
object esGenerateMod {
  ...
  object GenerateConfig {
    ...
    extension [Self <: GenerateConfig[?], DateType](x: Self & GenerateConfig[DateType]) {
      ...
      inline def setGetNow(value: CallbackTo[DateType]): Self = StObject.set(x, "getNow", value.toJsFn)
