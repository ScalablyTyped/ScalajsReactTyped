...
import typingsJapgolly.rollingRateLimiter.rollingRateLimiterStrings.microseconds
import typingsJapgolly.rollingRateLimiter.rollingRateLimiterStrings.milliseconds
import typingsJapgolly.rollingRateLimiter.rollingRateLimiterStrings.seconds
...
object mod {
  ...
  object RedisClient {
    ...
    inline def apply(del: /* repeated */ String => Any, multi: CallbackTo[RedisBatch]): RedisClient = {
      val __obj = js.Dynamic.literal(del = js.Any.fromFunction1(del), multi = multi.toJsFn)
    ...
    extension [Self <: RedisClient](x: Self) {
      ...
      inline def setMulti(value: CallbackTo[RedisBatch]): Self = StObject.set(x, "multi", value.toJsFn)
    }
  }
