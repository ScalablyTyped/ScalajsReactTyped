...
import typingsJapgolly.reactSpringRafz.anon.Cancel
import typingsJapgolly.reactSpringRafz.reactSpringRafzStrings.always
import typingsJapgolly.reactSpringRafz.reactSpringRafzStrings.demand
...
object distDeclarationsSrcTypesMod {
  ...
  object Timeout {
    ...
    inline def apply(cancel: Callback, handler: Callback, time: Double): Timeout = {
      val __obj = js.Dynamic.literal(cancel = cancel.toJsFn, handler = handler.toJsFn, time = time.asInstanceOf[js.Any])
