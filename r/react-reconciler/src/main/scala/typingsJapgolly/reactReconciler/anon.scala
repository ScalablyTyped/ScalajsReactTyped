...
import typingsJapgolly.reactReconciler.mod.BoundingRect
...
object anon {
  ...
  object Children {
    ...
    inline def apply[T](children: T => Node): Children[T] = {
  ...
  object Disconnect {
    ...
    inline def apply(disconnect: Callback): Disconnect = {
      val __obj = js.Dynamic.literal(disconnect = disconnect.toJsFn)
