package typingsJapgolly.reactSelect.anon

import typingsJapgolly.emotionReact.mod.jsx.JSX.Element
import typingsJapgolly.reactSelect.distDeclarationsSrcComponentsMenuMod.NoticeProps
import typingsJapgolly.reactSelect.distDeclarationsSrcTypesMod.GroupBase
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@js.native
trait DefaultPropsChildren extends StObject {
  
  def apply[Option_12, IsMulti_12 /* <: Boolean */, Group_12 /* <: GroupBase[Option_12] */](props: NoticeProps[Option_12, IsMulti_12, Group_12]): Element = js.native
  
  var defaultProps: Children = js.native
}
