...
import typingsJapgolly.reactCollapsed.anon.Current
import typingsJapgolly.reactCollapsed.distTypesMod.AssignableRef
...
object distUtilsMod {
  ...
  inline def usePaddingWarning(element: RefHandle[HTMLElement]): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("usePaddingWarning")(element.asInstanceOf[js.Any]).asInstanceOf[Unit]
