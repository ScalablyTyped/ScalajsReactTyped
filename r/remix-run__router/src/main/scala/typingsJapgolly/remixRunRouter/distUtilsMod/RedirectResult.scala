...
trait RedirectResult
  ...
  var `type`: typingsJapgolly.remixRunRouter.distUtilsMod.ResultType.redirect
}
object RedirectResult {
  ...
  extension [Self <: RedirectResult](x: Self) {
    ...
    inline def setType(value: typingsJapgolly.remixRunRouter.distUtilsMod.ResultType.redirect): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
