package typingsJapgolly.reactNativeGestureHandler

import typingsJapgolly.reactNativeGestureHandler.libTypescriptActionTypeMod.ActionType
import typingsJapgolly.reactNativeGestureHandler.libTypescriptComponentsTouchablesGenericTouchableMod.TouchableState
import typingsJapgolly.reactNativeGestureHandler.libTypescriptDirectionsMod.Directions
import typingsJapgolly.reactNativeGestureHandler.libTypescriptHandlersGesturesGestureMod.CALLBACK_TYPE
import typingsJapgolly.reactNativeGestureHandler.libTypescriptStateMod.State
import typingsJapgolly.reactNativeGestureHandler.libTypescriptTouchEventTypeMod.TouchEventType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object reactNativeGestureHandlerInts {
  
  @js.native
  sealed trait `0`
    extends StObject
       with CALLBACK_TYPE
       with State
       with TouchEventType
       with TouchableState
  inline def `0`: `0` = 0.asInstanceOf[`0`]
  
  @js.native
  sealed trait `1`
    extends StObject
       with ActionType
       with CALLBACK_TYPE
       with Directions
       with State
       with TouchEventType
       with TouchableState
  inline def `1`: `1` = 1.asInstanceOf[`1`]
  
  @js.native
  sealed trait `10`
    extends StObject
       with CALLBACK_TYPE
  inline def `10`: `10` = 10.asInstanceOf[`10`]
  
  @js.native
  sealed trait `2`
    extends StObject
       with ActionType
       with CALLBACK_TYPE
       with Directions
       with State
       with TouchEventType
       with TouchableState
  inline def `2`: `2` = 2.asInstanceOf[`2`]
  
  @js.native
  sealed trait `3`
    extends StObject
       with ActionType
       with CALLBACK_TYPE
       with State
       with TouchEventType
  inline def `3`: `3` = 3.asInstanceOf[`3`]
  
  @js.native
  sealed trait `4`
    extends StObject
       with ActionType
       with CALLBACK_TYPE
       with Directions
       with State
       with TouchEventType
  inline def `4`: `4` = 4.asInstanceOf[`4`]
  
  @js.native
  sealed trait `5`
    extends StObject
       with CALLBACK_TYPE
       with State
  inline def `5`: `5` = 5.asInstanceOf[`5`]
  
  @js.native
  sealed trait `6`
    extends StObject
       with CALLBACK_TYPE
  inline def `6`: `6` = 6.asInstanceOf[`6`]
  
  @js.native
  sealed trait `7`
    extends StObject
       with CALLBACK_TYPE
  inline def `7`: `7` = 7.asInstanceOf[`7`]
  
  @js.native
  sealed trait `8`
    extends StObject
       with CALLBACK_TYPE
       with Directions
  inline def `8`: `8` = 8.asInstanceOf[`8`]
  
  @js.native
  sealed trait `9`
    extends StObject
       with CALLBACK_TYPE
  inline def `9`: `9` = 9.asInstanceOf[`9`]
}
