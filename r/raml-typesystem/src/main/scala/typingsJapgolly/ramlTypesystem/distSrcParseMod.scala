...
import typingsJapgolly.ramlTypesystem.anon.Key
import typingsJapgolly.ramlTypesystem.distSrcMetainfoMod.Annotation
import typingsJapgolly.ramlTypesystem.distSrcMetainfoMod.CustomFacet
...
import typingsJapgolly.ramlTypesystem.distSrcTypesystemMod.TypeInformation
...
object distSrcParseMod {
  ...
  object NodeKind extends StObject {
    ...
    sealed trait ARRAY
      extends StObject
         with NodeKind
    /* 1 */ val ARRAY: typingsJapgolly.ramlTypesystem.distSrcParseMod.NodeKind.ARRAY & Double = js.native
    
    @js.native
    sealed trait MAP
      extends StObject
         with NodeKind
    /* 2 */ val MAP: typingsJapgolly.ramlTypesystem.distSrcParseMod.NodeKind.MAP & Double = js.native
    
    @js.native
    sealed trait SCALAR
      extends StObject
         with NodeKind
    /* 0 */ val SCALAR: typingsJapgolly.ramlTypesystem.distSrcParseMod.NodeKind.SCALAR & Double = js.native
  }
  ...
  object ParseNode {
    ...
    inline def apply(
      ...
      val __obj = js.Dynamic.literal(childWithKey = js.Any.fromFunction1(childWithKey), children = children.toJsFn, getMeta = js.Any.fromFunction1(getMeta), key = key.toJsFn, kind = kind.toJsFn, path = path.toJsFn, value = value.toJsFn)
      __obj.asInstanceOf[ParseNode]
    }
    
    extension [Self <: ParseNode](x: Self) {
      
      inline def setAnchor(value: CallbackTo[Any]): Self = StObject.set(x, "anchor", value.toJsFn)
