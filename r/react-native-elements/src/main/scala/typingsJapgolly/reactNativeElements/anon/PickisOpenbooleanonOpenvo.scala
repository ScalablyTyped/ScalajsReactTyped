...
object PickisOpenbooleanonOpenvo {
  ...
  inline def apply(isOpen: Boolean, onClose: Callback, onOpen: Callback): PickisOpenbooleanonOpenvo = {
    val __obj = js.Dynamic.literal(isOpen = isOpen.asInstanceOf[js.Any], onClose = onClose.toJsFn, onOpen = onOpen.toJsFn)
