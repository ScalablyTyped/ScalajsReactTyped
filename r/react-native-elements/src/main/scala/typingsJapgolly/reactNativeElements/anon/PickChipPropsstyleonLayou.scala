...
/* Inlined std.Pick<react-native-elements.react-native-elements/dist/buttons/Chip.ChipProps, 'style' | 'onLayout' | 'testID' | 'accessible' | 'accessibilityActions' | 'accessibilityLabel' | 'accessibilityRole' | 'accessibilityState' | 'accessibilityHint' | 'accessibilityValue' | 'onAccessibilityAction' | 'accessibilityLiveRegion' | 'importantForAccessibility' | 'accessibilityElementsHidden' | 'accessibilityViewIsModal' | 'onAccessibilityEscape' | 'onAccessibilityTap' | 'onMagicTap' | 'accessibilityIgnoresInvertColors' | 'raised' | 'disabled' | 'onPress' | 'onLongPress' | 'activeOpacity' | 'delayLongPress' | 'delayPressIn' | 'delayPressOut' | 'hitSlop' | 'onBlur' | 'onFocus' | 'onPressIn' | 'onPressOut' | 'pressRetentionOffset' | 'hasTVPreferredFocus' | 'tvParallaxProperties' | 'touchSoundDisabled' | 'background' | 'useForeground' | 'type' | 'containerStyle' | 'disabledStyle' | 'title' | 'icon' | 'titleStyle' | 'titleProps' | 'buttonStyle' | 'iconContainerStyle' | 'iconRight' | 'linearGradientProps' | 'TouchableComponent' | 'ViewComponent' | 'disabledTitleStyle' | 'iconPosition'> */
trait PickChipPropsstyleonLayou extends StObject {
...
object PickChipPropsstyleonLayou {
  
  inline def apply(): PickChipPropsstyleonLayou = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[PickChipPropsstyleonLayou]
  }
  
  extension [Self <: PickChipPropsstyleonLayou](x: Self) {
