...
object NormalizationModuleImport {
  ...
  extension [Self <: NormalizationModuleImport](x: Self) {
    ...
    inline def setComponentModuleProvider(value: CallbackTo[Any | js.Promise[Any] | JSResourceReference[Any]]): Self = StObject.set(x, "componentModuleProvider", value.toJsFn)
    ...
    ): Self = StObject.set(x, "operationModuleProvider", value.toJsFn)
