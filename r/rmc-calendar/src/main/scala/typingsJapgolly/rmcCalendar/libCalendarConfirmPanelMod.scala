...
object libCalendarConfirmPanelMod {
  ...
  object ConfirmPanelPropsType {
    ...
    inline def apply(locale: Locale, onConfirm: Callback): ConfirmPanelPropsType = {
      val __obj = js.Dynamic.literal(locale = locale.asInstanceOf[js.Any], onConfirm = onConfirm.toJsFn)
