...
object Query {
  ...
  object Observer {
    ...
    inline def apply(next: /* querySnapshot */ QuerySnapshot => Callback): Observer = {
      val __obj = js.Dynamic.literal(next = js.Any.fromFunction1((t0: /* querySnapshot */ QuerySnapshot) => next(t0).runNow()))
    ...
    extension [Self <: Observer](x: Self) {
      ...
      inline def setNext(value: /* querySnapshot */ QuerySnapshot => Callback): Self = StObject.set(x, "next", js.Any.fromFunction1((t0: /* querySnapshot */ QuerySnapshot) => value(t0).runNow()))
    }
  }
