package typingsJapgolly.rethinkdb

import typingsJapgolly.rethinkdb.mod.waitFor
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object rethinkdbStrings {
  
  @js.native
  sealed trait BINARY extends StObject
  inline def BINARY: BINARY = "BINARY".asInstanceOf[BINARY]
  
  @js.native
  sealed trait TIME extends StObject
  inline def TIME: TIME = "TIME".asInstanceOf[TIME]
  
  @js.native
  sealed trait WGS84 extends StObject
  inline def WGS84: WGS84 = "WGS84".asInstanceOf[WGS84]
  
  @js.native
  sealed trait always extends StObject
  inline def always: always = "always".asInstanceOf[always]
  
  @js.native
  sealed trait array extends StObject
  inline def array: array = "array".asInstanceOf[array]
  
  @js.native
  sealed trait error extends StObject
  inline def error: error = "error".asInstanceOf[error]
  
  @js.native
  sealed trait ft extends StObject
  inline def ft: ft = "ft".asInstanceOf[ft]
  
  @js.native
  sealed trait hard extends StObject
  inline def hard: hard = "hard".asInstanceOf[hard]
  
  @js.native
  sealed trait km extends StObject
  inline def km: km = "km".asInstanceOf[km]
  
  @js.native
  sealed trait m extends StObject
  inline def m: m = "m".asInstanceOf[m]
  
  @js.native
  sealed trait majority extends StObject
  inline def majority: majority = "majority".asInstanceOf[majority]
  
  @js.native
  sealed trait mi extends StObject
  inline def mi: mi = "mi".asInstanceOf[mi]
  
  @js.native
  sealed trait native extends StObject
  inline def native: native = "native".asInstanceOf[native]
  
  @js.native
  sealed trait nm extends StObject
  inline def nm: nm = "nm".asInstanceOf[nm]
  
  @js.native
  sealed trait `object` extends StObject
  inline def `object`: `object` = "object".asInstanceOf[`object`]
  
  @js.native
  sealed trait outdated extends StObject
  inline def outdated: outdated = "outdated".asInstanceOf[outdated]
  
  @js.native
  sealed trait raw extends StObject
  inline def raw: raw = "raw".asInstanceOf[raw]
  
  @js.native
  sealed trait ready_for_outdated_reads
    extends StObject
       with waitFor
  inline def ready_for_outdated_reads: ready_for_outdated_reads = "ready_for_outdated_reads".asInstanceOf[ready_for_outdated_reads]
  
  @js.native
  sealed trait ready_for_reads
    extends StObject
       with waitFor
  inline def ready_for_reads: ready_for_reads = "ready_for_reads".asInstanceOf[ready_for_reads]
  
  @js.native
  sealed trait ready_for_writes
    extends StObject
       with waitFor
  inline def ready_for_writes: ready_for_writes = "ready_for_writes".asInstanceOf[ready_for_writes]
  
  @js.native
  sealed trait replace extends StObject
  inline def replace: replace = "replace".asInstanceOf[replace]
  
  @js.native
  sealed trait single extends StObject
  inline def single: single = "single".asInstanceOf[single]
  
  @js.native
  sealed trait soft extends StObject
  inline def soft: soft = "soft".asInstanceOf[soft]
  
  @js.native
  sealed trait unit_sphere extends StObject
  inline def unit_sphere: unit_sphere = "unit_sphere".asInstanceOf[unit_sphere]
  
  @js.native
  sealed trait update extends StObject
  inline def update: update = "update".asInstanceOf[update]
}
