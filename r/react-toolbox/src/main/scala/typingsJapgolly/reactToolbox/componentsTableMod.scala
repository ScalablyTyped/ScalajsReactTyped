package typingsJapgolly.reactToolbox

import typingsJapgolly.reactToolbox.componentsTableTableCellMod.TableCellProps
import typingsJapgolly.reactToolbox.componentsTableTableHeadMod.TableHeadProps
import typingsJapgolly.reactToolbox.componentsTableTableMod.TableProps
import typingsJapgolly.reactToolbox.componentsTableTableRowMod.TableRowProps
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object componentsTableMod {
  
  @JSImport("react-toolbox/components/table", JSImport.Default)
  @js.native
  open class default protected ()
    extends typingsJapgolly.reactToolbox.componentsTableTableMod.Table {
    def this(props: TableProps) = this()
    /**
      * @deprecated
      * @see https://reactjs.org/docs/legacy-context.html
      */
    def this(props: TableProps, context: Any) = this()
  }
  
  @JSImport("react-toolbox/components/table", "Table")
  @js.native
  open class Table protected ()
    extends typingsJapgolly.reactToolbox.componentsTableTableMod.Table {
    def this(props: TableProps) = this()
    /**
      * @deprecated
      * @see https://reactjs.org/docs/legacy-context.html
      */
    def this(props: TableProps, context: Any) = this()
  }
  
  @JSImport("react-toolbox/components/table", "TableCell")
  @js.native
  open class TableCell protected ()
    extends typingsJapgolly.reactToolbox.componentsTableTableCellMod.TableCell {
    def this(props: TableCellProps) = this()
    /**
      * @deprecated
      * @see https://reactjs.org/docs/legacy-context.html
      */
    def this(props: TableCellProps, context: Any) = this()
  }
  
  @JSImport("react-toolbox/components/table", "TableHead")
  @js.native
  open class TableHead protected ()
    extends typingsJapgolly.reactToolbox.componentsTableTableHeadMod.TableHead {
    def this(props: TableHeadProps) = this()
    /**
      * @deprecated
      * @see https://reactjs.org/docs/legacy-context.html
      */
    def this(props: TableHeadProps, context: Any) = this()
  }
  
  @JSImport("react-toolbox/components/table", "TableRow")
  @js.native
  open class TableRow protected ()
    extends typingsJapgolly.reactToolbox.componentsTableTableRowMod.TableRow {
    def this(props: TableRowProps) = this()
    /**
      * @deprecated
      * @see https://reactjs.org/docs/legacy-context.html
      */
    def this(props: TableRowProps, context: Any) = this()
  }
}
