package typingsJapgolly.reactToolbox

import typingsJapgolly.reactToolbox.componentsRadioRadioButtonMod.RadioButtonProps
import typingsJapgolly.reactToolbox.componentsRadioRadioGroupMod.RadioGroupProps
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object componentsRadioMod {
  
  @JSImport("react-toolbox/components/radio", JSImport.Default)
  @js.native
  open class default protected ()
    extends typingsJapgolly.reactToolbox.componentsRadioRadioButtonMod.RadioButton {
    def this(props: RadioButtonProps) = this()
    /**
      * @deprecated
      * @see https://reactjs.org/docs/legacy-context.html
      */
    def this(props: RadioButtonProps, context: Any) = this()
  }
  
  @JSImport("react-toolbox/components/radio", "RadioButton")
  @js.native
  open class RadioButton protected ()
    extends typingsJapgolly.reactToolbox.componentsRadioRadioButtonMod.RadioButton {
    def this(props: RadioButtonProps) = this()
    /**
      * @deprecated
      * @see https://reactjs.org/docs/legacy-context.html
      */
    def this(props: RadioButtonProps, context: Any) = this()
  }
  
  @JSImport("react-toolbox/components/radio", "RadioGroup")
  @js.native
  open class RadioGroup protected ()
    extends typingsJapgolly.reactToolbox.componentsRadioRadioGroupMod.RadioGroup {
    def this(props: RadioGroupProps) = this()
    /**
      * @deprecated
      * @see https://reactjs.org/docs/legacy-context.html
      */
    def this(props: RadioGroupProps, context: Any) = this()
  }
}
