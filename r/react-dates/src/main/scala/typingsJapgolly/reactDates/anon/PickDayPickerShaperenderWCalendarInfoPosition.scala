package typingsJapgolly.reactDates.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Inlined std.Pick<react-dates.react-dates.DayPickerShape, 'renderWeekHeaderElement' | 'enableOutsideDays' | 'numberOfMonths' | 'withPortal' | 'hideKeyboardShortcutsPanel' | 'daySize' | 'noBorder' | 'verticalBorderSpacing' | 'horizontalMonthPadding' | 'dayPickerNavigationInlineStyles' | 'navPosition' | 'navPrev' | 'navNext' | 'renderNavPrevButton' | 'renderNavNextButton' | 'noNavButtons' | 'noNavNextButton' | 'noNavPrevButton' | 'onOutsideClick' | 'renderCalendarDay' | 'renderDayContents' | 'renderCalendarInfo' | 'calendarInfoPosition' | 'firstDayOfWeek' | 'verticalHeight' | 'transitionDuration' | 'onBlur' | 'isFocused' | 'showKeyboardShortcuts' | 'onTab' | 'onShiftTab' | 'monthFormat' | 'weekDayFormat' | 'phrases' | 'dayAriaLabelFormat' | 'isRTL'> */
trait PickDayPickerShaperenderWCalendarInfoPosition extends StObject {
  
  var calendarInfoPosition: js.UndefOr[Any] = js.undefined
  
  var dayAriaLabelFormat: js.UndefOr[Any] = js.undefined
  
  var dayPickerNavigationInlineStyles: js.UndefOr[Any] = js.undefined
  
  var daySize: js.UndefOr[Any] = js.undefined
  
  var enableOutsideDays: js.UndefOr[Any] = js.undefined
  
  var firstDayOfWeek: js.UndefOr[Any] = js.undefined
  
  var hideKeyboardShortcutsPanel: js.UndefOr[Any] = js.undefined
  
  var horizontalMonthPadding: js.UndefOr[Any] = js.undefined
  
  var isFocused: js.UndefOr[Any] = js.undefined
  
  var isRTL: js.UndefOr[Any] = js.undefined
  
  var monthFormat: js.UndefOr[Any] = js.undefined
  
  var navNext: js.UndefOr[Any] = js.undefined
  
  var navPosition: js.UndefOr[Any] = js.undefined
  
  var navPrev: js.UndefOr[Any] = js.undefined
  
  var noBorder: js.UndefOr[Any] = js.undefined
  
  var noNavButtons: js.UndefOr[Any] = js.undefined
  
  var noNavNextButton: js.UndefOr[Any] = js.undefined
  
  var noNavPrevButton: js.UndefOr[Any] = js.undefined
  
  var numberOfMonths: js.UndefOr[Any] = js.undefined
  
  var onBlur: js.UndefOr[Any] = js.undefined
  
  var onOutsideClick: js.UndefOr[Any] = js.undefined
  
  var onShiftTab: js.UndefOr[Any] = js.undefined
  
  var onTab: js.UndefOr[Any] = js.undefined
  
  var phrases: js.UndefOr[Any] = js.undefined
  
  var renderCalendarDay: js.UndefOr[Any] = js.undefined
  
  var renderCalendarInfo: js.UndefOr[Any] = js.undefined
  
  var renderDayContents: js.UndefOr[Any] = js.undefined
  
  var renderNavNextButton: js.UndefOr[Any] = js.undefined
  
  var renderNavPrevButton: js.UndefOr[Any] = js.undefined
  
  var renderWeekHeaderElement: js.UndefOr[Any] = js.undefined
  
  var showKeyboardShortcuts: js.UndefOr[Any] = js.undefined
  
  var transitionDuration: js.UndefOr[Any] = js.undefined
  
  var verticalBorderSpacing: js.UndefOr[Any] = js.undefined
  
  var verticalHeight: js.UndefOr[Any] = js.undefined
  
  var weekDayFormat: js.UndefOr[Any] = js.undefined
  
  var withPortal: js.UndefOr[Any] = js.undefined
}
object PickDayPickerShaperenderWCalendarInfoPosition {
  
  inline def apply(): PickDayPickerShaperenderWCalendarInfoPosition = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[PickDayPickerShaperenderWCalendarInfoPosition]
  }
  
  extension [Self <: PickDayPickerShaperenderWCalendarInfoPosition](x: Self) {
    
    inline def setCalendarInfoPosition(value: Any): Self = StObject.set(x, "calendarInfoPosition", value.asInstanceOf[js.Any])
    
    inline def setCalendarInfoPositionUndefined: Self = StObject.set(x, "calendarInfoPosition", js.undefined)
    
    inline def setDayAriaLabelFormat(value: Any): Self = StObject.set(x, "dayAriaLabelFormat", value.asInstanceOf[js.Any])
    
    inline def setDayAriaLabelFormatUndefined: Self = StObject.set(x, "dayAriaLabelFormat", js.undefined)
    
    inline def setDayPickerNavigationInlineStyles(value: Any): Self = StObject.set(x, "dayPickerNavigationInlineStyles", value.asInstanceOf[js.Any])
    
    inline def setDayPickerNavigationInlineStylesUndefined: Self = StObject.set(x, "dayPickerNavigationInlineStyles", js.undefined)
    
    inline def setDaySize(value: Any): Self = StObject.set(x, "daySize", value.asInstanceOf[js.Any])
    
    inline def setDaySizeUndefined: Self = StObject.set(x, "daySize", js.undefined)
    
    inline def setEnableOutsideDays(value: Any): Self = StObject.set(x, "enableOutsideDays", value.asInstanceOf[js.Any])
    
    inline def setEnableOutsideDaysUndefined: Self = StObject.set(x, "enableOutsideDays", js.undefined)
    
    inline def setFirstDayOfWeek(value: Any): Self = StObject.set(x, "firstDayOfWeek", value.asInstanceOf[js.Any])
    
    inline def setFirstDayOfWeekUndefined: Self = StObject.set(x, "firstDayOfWeek", js.undefined)
    
    inline def setHideKeyboardShortcutsPanel(value: Any): Self = StObject.set(x, "hideKeyboardShortcutsPanel", value.asInstanceOf[js.Any])
    
    inline def setHideKeyboardShortcutsPanelUndefined: Self = StObject.set(x, "hideKeyboardShortcutsPanel", js.undefined)
    
    inline def setHorizontalMonthPadding(value: Any): Self = StObject.set(x, "horizontalMonthPadding", value.asInstanceOf[js.Any])
    
    inline def setHorizontalMonthPaddingUndefined: Self = StObject.set(x, "horizontalMonthPadding", js.undefined)
    
    inline def setIsFocused(value: Any): Self = StObject.set(x, "isFocused", value.asInstanceOf[js.Any])
    
    inline def setIsFocusedUndefined: Self = StObject.set(x, "isFocused", js.undefined)
    
    inline def setIsRTL(value: Any): Self = StObject.set(x, "isRTL", value.asInstanceOf[js.Any])
    
    inline def setIsRTLUndefined: Self = StObject.set(x, "isRTL", js.undefined)
    
    inline def setMonthFormat(value: Any): Self = StObject.set(x, "monthFormat", value.asInstanceOf[js.Any])
    
    inline def setMonthFormatUndefined: Self = StObject.set(x, "monthFormat", js.undefined)
    
    inline def setNavNext(value: Any): Self = StObject.set(x, "navNext", value.asInstanceOf[js.Any])
    
    inline def setNavNextUndefined: Self = StObject.set(x, "navNext", js.undefined)
    
    inline def setNavPosition(value: Any): Self = StObject.set(x, "navPosition", value.asInstanceOf[js.Any])
    
    inline def setNavPositionUndefined: Self = StObject.set(x, "navPosition", js.undefined)
    
    inline def setNavPrev(value: Any): Self = StObject.set(x, "navPrev", value.asInstanceOf[js.Any])
    
    inline def setNavPrevUndefined: Self = StObject.set(x, "navPrev", js.undefined)
    
    inline def setNoBorder(value: Any): Self = StObject.set(x, "noBorder", value.asInstanceOf[js.Any])
    
    inline def setNoBorderUndefined: Self = StObject.set(x, "noBorder", js.undefined)
    
    inline def setNoNavButtons(value: Any): Self = StObject.set(x, "noNavButtons", value.asInstanceOf[js.Any])
    
    inline def setNoNavButtonsUndefined: Self = StObject.set(x, "noNavButtons", js.undefined)
    
    inline def setNoNavNextButton(value: Any): Self = StObject.set(x, "noNavNextButton", value.asInstanceOf[js.Any])
    
    inline def setNoNavNextButtonUndefined: Self = StObject.set(x, "noNavNextButton", js.undefined)
    
    inline def setNoNavPrevButton(value: Any): Self = StObject.set(x, "noNavPrevButton", value.asInstanceOf[js.Any])
    
    inline def setNoNavPrevButtonUndefined: Self = StObject.set(x, "noNavPrevButton", js.undefined)
    
    inline def setNumberOfMonths(value: Any): Self = StObject.set(x, "numberOfMonths", value.asInstanceOf[js.Any])
    
    inline def setNumberOfMonthsUndefined: Self = StObject.set(x, "numberOfMonths", js.undefined)
    
    inline def setOnBlur(value: Any): Self = StObject.set(x, "onBlur", value.asInstanceOf[js.Any])
    
    inline def setOnBlurUndefined: Self = StObject.set(x, "onBlur", js.undefined)
    
    inline def setOnOutsideClick(value: Any): Self = StObject.set(x, "onOutsideClick", value.asInstanceOf[js.Any])
    
    inline def setOnOutsideClickUndefined: Self = StObject.set(x, "onOutsideClick", js.undefined)
    
    inline def setOnShiftTab(value: Any): Self = StObject.set(x, "onShiftTab", value.asInstanceOf[js.Any])
    
    inline def setOnShiftTabUndefined: Self = StObject.set(x, "onShiftTab", js.undefined)
    
    inline def setOnTab(value: Any): Self = StObject.set(x, "onTab", value.asInstanceOf[js.Any])
    
    inline def setOnTabUndefined: Self = StObject.set(x, "onTab", js.undefined)
    
    inline def setPhrases(value: Any): Self = StObject.set(x, "phrases", value.asInstanceOf[js.Any])
    
    inline def setPhrasesUndefined: Self = StObject.set(x, "phrases", js.undefined)
    
    inline def setRenderCalendarDay(value: Any): Self = StObject.set(x, "renderCalendarDay", value.asInstanceOf[js.Any])
    
    inline def setRenderCalendarDayUndefined: Self = StObject.set(x, "renderCalendarDay", js.undefined)
    
    inline def setRenderCalendarInfo(value: Any): Self = StObject.set(x, "renderCalendarInfo", value.asInstanceOf[js.Any])
    
    inline def setRenderCalendarInfoUndefined: Self = StObject.set(x, "renderCalendarInfo", js.undefined)
    
    inline def setRenderDayContents(value: Any): Self = StObject.set(x, "renderDayContents", value.asInstanceOf[js.Any])
    
    inline def setRenderDayContentsUndefined: Self = StObject.set(x, "renderDayContents", js.undefined)
    
    inline def setRenderNavNextButton(value: Any): Self = StObject.set(x, "renderNavNextButton", value.asInstanceOf[js.Any])
    
    inline def setRenderNavNextButtonUndefined: Self = StObject.set(x, "renderNavNextButton", js.undefined)
    
    inline def setRenderNavPrevButton(value: Any): Self = StObject.set(x, "renderNavPrevButton", value.asInstanceOf[js.Any])
    
    inline def setRenderNavPrevButtonUndefined: Self = StObject.set(x, "renderNavPrevButton", js.undefined)
    
    inline def setRenderWeekHeaderElement(value: Any): Self = StObject.set(x, "renderWeekHeaderElement", value.asInstanceOf[js.Any])
    
    inline def setRenderWeekHeaderElementUndefined: Self = StObject.set(x, "renderWeekHeaderElement", js.undefined)
    
    inline def setShowKeyboardShortcuts(value: Any): Self = StObject.set(x, "showKeyboardShortcuts", value.asInstanceOf[js.Any])
    
    inline def setShowKeyboardShortcutsUndefined: Self = StObject.set(x, "showKeyboardShortcuts", js.undefined)
    
    inline def setTransitionDuration(value: Any): Self = StObject.set(x, "transitionDuration", value.asInstanceOf[js.Any])
    
    inline def setTransitionDurationUndefined: Self = StObject.set(x, "transitionDuration", js.undefined)
    
    inline def setVerticalBorderSpacing(value: Any): Self = StObject.set(x, "verticalBorderSpacing", value.asInstanceOf[js.Any])
    
    inline def setVerticalBorderSpacingUndefined: Self = StObject.set(x, "verticalBorderSpacing", js.undefined)
    
    inline def setVerticalHeight(value: Any): Self = StObject.set(x, "verticalHeight", value.asInstanceOf[js.Any])
    
    inline def setVerticalHeightUndefined: Self = StObject.set(x, "verticalHeight", js.undefined)
    
    inline def setWeekDayFormat(value: Any): Self = StObject.set(x, "weekDayFormat", value.asInstanceOf[js.Any])
    
    inline def setWeekDayFormatUndefined: Self = StObject.set(x, "weekDayFormat", js.undefined)
    
    inline def setWithPortal(value: Any): Self = StObject.set(x, "withPortal", value.asInstanceOf[js.Any])
    
    inline def setWithPortalUndefined: Self = StObject.set(x, "withPortal", js.undefined)
  }
}
