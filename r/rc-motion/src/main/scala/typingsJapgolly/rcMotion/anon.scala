...
import typingsJapgolly.rcMotion.esUtilDiffMod.DiffStatus
...
object anon {
  ...
  object Dictname {
    ...
    inline def apply(key: typingsJapgolly.react.mod.Key): Dictname = {
  ...
  object IsCanceled {
    ...
    inline def apply(isCanceled: CallbackTo[Boolean]): IsCanceled = {
      val __obj = js.Dynamic.literal(isCanceled = isCanceled.toJsFn)
