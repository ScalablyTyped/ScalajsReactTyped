...
import typingsJapgolly.reduxApiMiddleware.anon.FnCall
import typingsJapgolly.reduxApiMiddleware.reduxApiMiddlewareBooleans.`true`
...
object mod {
  ...
  open class ApiError[T] protected ()
    ...
    @JSName("name")
    var name_ApiError: typingsJapgolly.reduxApiMiddleware.reduxApiMiddlewareStrings.ApiError = js.native
  ...
  type ValidAction[Payload, Meta] = typingsJapgolly.reduxApiMiddleware.anon.Error & (/* import warning: importer.ImportType#apply Failed type conversion: [Payload] extends [never] ? {} : {  payload :Payload} */ js.Any)
