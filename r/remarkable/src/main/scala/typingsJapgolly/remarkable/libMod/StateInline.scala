...
object StateInline {
  ...
  extension [Self <: StateInline](x: Self) {
    ...
    inline def setPush(value: ContentToken => Callback): Self = StObject.set(x, "push", js.Any.fromFunction1((t0: ContentToken) => value(t0).runNow()))
    
    inline def setPushPending(value: Callback): Self = StObject.set(x, "pushPending", value.toJsFn)
