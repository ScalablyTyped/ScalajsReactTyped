...
import typingsJapgolly.reactInfiniteCalendar.reactInfiniteCalendarStrings.landscape
import typingsJapgolly.reactInfiniteCalendar.reactInfiniteCalendarStrings.portrait
...
object anon {
  ...
  object DistanceInWords {
    ...
    inline def apply(distanceInWords: (Any, Any, Any) => Any, format: CallbackTo[Any]): DistanceInWords = {
      val __obj = js.Dynamic.literal(distanceInWords = js.Any.fromFunction3(distanceInWords), format = format.toJsFn)
    ...
    extension [Self <: DistanceInWords](x: Self) {
      ...
      inline def setFormat(value: CallbackTo[Any]): Self = StObject.set(x, "format", value.toJsFn)
    }
  }
