package typingsJapgolly.reactNativePaypalLib

import typingsJapgolly.reactNativePaypalLib.mod.CURRENCIES
import typingsJapgolly.reactNativePaypalLib.mod.ENVIRONMENT
import typingsJapgolly.reactNativePaypalLib.mod.INTENT
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object reactNativePaypalLibStrings {
  
  @js.native
  sealed trait AUD
    extends StObject
       with CURRENCIES
  inline def AUD: AUD = "AUD".asInstanceOf[AUD]
  
  @js.native
  sealed trait AUTHORIZE
    extends StObject
       with INTENT
  inline def AUTHORIZE: AUTHORIZE = "AUTHORIZE".asInstanceOf[AUTHORIZE]
  
  @js.native
  sealed trait BRL
    extends StObject
       with CURRENCIES
  inline def BRL: BRL = "BRL".asInstanceOf[BRL]
  
  @js.native
  sealed trait CAD
    extends StObject
       with CURRENCIES
  inline def CAD: CAD = "CAD".asInstanceOf[CAD]
  
  @js.native
  sealed trait CHF
    extends StObject
       with CURRENCIES
  inline def CHF: CHF = "CHF".asInstanceOf[CHF]
  
  @js.native
  sealed trait CNY
    extends StObject
       with CURRENCIES
  inline def CNY: CNY = "CNY".asInstanceOf[CNY]
  
  @js.native
  sealed trait CZK
    extends StObject
       with CURRENCIES
  inline def CZK: CZK = "CZK".asInstanceOf[CZK]
  
  @js.native
  sealed trait DKK
    extends StObject
       with CURRENCIES
  inline def DKK: DKK = "DKK".asInstanceOf[DKK]
  
  @js.native
  sealed trait EUR
    extends StObject
       with CURRENCIES
  inline def EUR: EUR = "EUR".asInstanceOf[EUR]
  
  @js.native
  sealed trait GBP
    extends StObject
       with CURRENCIES
  inline def GBP: GBP = "GBP".asInstanceOf[GBP]
  
  @js.native
  sealed trait HKD
    extends StObject
       with CURRENCIES
  inline def HKD: HKD = "HKD".asInstanceOf[HKD]
  
  @js.native
  sealed trait HUF
    extends StObject
       with CURRENCIES
  inline def HUF: HUF = "HUF".asInstanceOf[HUF]
  
  @js.native
  sealed trait ILS
    extends StObject
       with CURRENCIES
  inline def ILS: ILS = "ILS".asInstanceOf[ILS]
  
  @js.native
  sealed trait INR
    extends StObject
       with CURRENCIES
  inline def INR: INR = "INR".asInstanceOf[INR]
  
  @js.native
  sealed trait JPY
    extends StObject
       with CURRENCIES
  inline def JPY: JPY = "JPY".asInstanceOf[JPY]
  
  @js.native
  sealed trait MXN
    extends StObject
       with CURRENCIES
  inline def MXN: MXN = "MXN".asInstanceOf[MXN]
  
  @js.native
  sealed trait MYR
    extends StObject
       with CURRENCIES
  inline def MYR: MYR = "MYR".asInstanceOf[MYR]
  
  @js.native
  sealed trait NOK
    extends StObject
       with CURRENCIES
  inline def NOK: NOK = "NOK".asInstanceOf[NOK]
  
  @js.native
  sealed trait NO_NETWORK
    extends StObject
       with ENVIRONMENT
  inline def NO_NETWORK: NO_NETWORK = "NO_NETWORK".asInstanceOf[NO_NETWORK]
  
  @js.native
  sealed trait NZD
    extends StObject
       with CURRENCIES
  inline def NZD: NZD = "NZD".asInstanceOf[NZD]
  
  @js.native
  sealed trait ORDER
    extends StObject
       with INTENT
  inline def ORDER: ORDER = "ORDER".asInstanceOf[ORDER]
  
  @js.native
  sealed trait PHP
    extends StObject
       with CURRENCIES
  inline def PHP: PHP = "PHP".asInstanceOf[PHP]
  
  @js.native
  sealed trait PLN
    extends StObject
       with CURRENCIES
  inline def PLN: PLN = "PLN".asInstanceOf[PLN]
  
  @js.native
  sealed trait PRODUCTION
    extends StObject
       with ENVIRONMENT
  inline def PRODUCTION: PRODUCTION = "PRODUCTION".asInstanceOf[PRODUCTION]
  
  @js.native
  sealed trait RUB
    extends StObject
       with CURRENCIES
  inline def RUB: RUB = "RUB".asInstanceOf[RUB]
  
  @js.native
  sealed trait SALE
    extends StObject
       with INTENT
  inline def SALE: SALE = "SALE".asInstanceOf[SALE]
  
  @js.native
  sealed trait SANDBOX
    extends StObject
       with ENVIRONMENT
  inline def SANDBOX: SANDBOX = "SANDBOX".asInstanceOf[SANDBOX]
  
  @js.native
  sealed trait SEK
    extends StObject
       with CURRENCIES
  inline def SEK: SEK = "SEK".asInstanceOf[SEK]
  
  @js.native
  sealed trait SGD
    extends StObject
       with CURRENCIES
  inline def SGD: SGD = "SGD".asInstanceOf[SGD]
  
  @js.native
  sealed trait THB
    extends StObject
       with CURRENCIES
  inline def THB: THB = "THB".asInstanceOf[THB]
  
  @js.native
  sealed trait TWD
    extends StObject
       with CURRENCIES
  inline def TWD: TWD = "TWD".asInstanceOf[TWD]
  
  @js.native
  sealed trait USD
    extends StObject
       with CURRENCIES
  inline def USD: USD = "USD".asInstanceOf[USD]
}
