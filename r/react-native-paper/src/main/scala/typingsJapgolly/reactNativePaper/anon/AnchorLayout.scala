...
trait AnchorLayout extends StObject {
  ...
  var opacityAnimation: typingsJapgolly.reactNative.mod.Animated.Value
...
object AnchorLayout {
  ...
  inline def apply(
    ...
    left: Double,
    menuLayout: Height,
    opacityAnimation: typingsJapgolly.reactNative.mod.Animated.Value,
    rendered: Boolean,
  ...
  extension [Self <: AnchorLayout](x: Self) {
    ...
    inline def setOpacityAnimation(value: typingsJapgolly.reactNative.mod.Animated.Value): Self = StObject.set(x, "opacityAnimation", value.asInstanceOf[js.Any])
