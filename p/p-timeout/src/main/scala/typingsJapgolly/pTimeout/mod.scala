...
import typingsJapgolly.pTimeout.anon.ClearTimeout
...
object mod {
  ...
  open class TimeoutError ()
    ...
    @JSName("name")
    val name_TimeoutError: typingsJapgolly.pTimeout.pTimeoutStrings.TimeoutError = js.native
  }
  ...
  object Options {
    ...
    extension [Self <: Options[?], ReturnType](x: Self & Options[ReturnType]) {
      ...
      inline def setFallback(value: CallbackTo[ReturnType | js.Promise[ReturnType]]): Self = StObject.set(x, "fallback", value.toJsFn)
