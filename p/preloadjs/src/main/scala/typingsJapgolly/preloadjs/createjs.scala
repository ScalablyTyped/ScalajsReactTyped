package typingsJapgolly.preloadjs
...
object createjs {
  ...
  object AbstractRequest {
    ...
    inline def apply(cancel: Callback, destroy: Callback, load: Callback): AbstractRequest = {
      val __obj = js.Dynamic.literal(cancel = cancel.toJsFn, destroy = destroy.toJsFn, load = load.toJsFn)
