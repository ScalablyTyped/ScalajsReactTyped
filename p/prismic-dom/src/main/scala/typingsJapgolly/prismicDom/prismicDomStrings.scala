package typingsJapgolly.prismicDom

import typingsJapgolly.prismicDom.mod.ElementType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object prismicDomStrings {
  
  @js.native
  sealed trait em
    extends StObject
       with ElementType
  inline def em: em = "em".asInstanceOf[em]
  
  @js.native
  sealed trait embed
    extends StObject
       with ElementType
  inline def embed: embed = "embed".asInstanceOf[embed]
  
  @js.native
  sealed trait `group-list-item`
    extends StObject
       with ElementType
  inline def `group-list-item`: `group-list-item` = "group-list-item".asInstanceOf[`group-list-item`]
  
  @js.native
  sealed trait `group-o-list-item`
    extends StObject
       with ElementType
  inline def `group-o-list-item`: `group-o-list-item` = "group-o-list-item".asInstanceOf[`group-o-list-item`]
  
  @js.native
  sealed trait heading1
    extends StObject
       with ElementType
  inline def heading1: heading1 = "heading1".asInstanceOf[heading1]
  
  @js.native
  sealed trait heading2
    extends StObject
       with ElementType
  inline def heading2: heading2 = "heading2".asInstanceOf[heading2]
  
  @js.native
  sealed trait heading3
    extends StObject
       with ElementType
  inline def heading3: heading3 = "heading3".asInstanceOf[heading3]
  
  @js.native
  sealed trait heading4
    extends StObject
       with ElementType
  inline def heading4: heading4 = "heading4".asInstanceOf[heading4]
  
  @js.native
  sealed trait heading5
    extends StObject
       with ElementType
  inline def heading5: heading5 = "heading5".asInstanceOf[heading5]
  
  @js.native
  sealed trait heading6
    extends StObject
       with ElementType
  inline def heading6: heading6 = "heading6".asInstanceOf[heading6]
  
  @js.native
  sealed trait hyperlink
    extends StObject
       with ElementType
  inline def hyperlink: hyperlink = "hyperlink".asInstanceOf[hyperlink]
  
  @js.native
  sealed trait image
    extends StObject
       with ElementType
  inline def image: image = "image".asInstanceOf[image]
  
  @js.native
  sealed trait label
    extends StObject
       with ElementType
  inline def label: label = "label".asInstanceOf[label]
  
  @js.native
  sealed trait `list-item`
    extends StObject
       with ElementType
  inline def `list-item`: `list-item` = "list-item".asInstanceOf[`list-item`]
  
  @js.native
  sealed trait `o-list-item`
    extends StObject
       with ElementType
  inline def `o-list-item`: `o-list-item` = "o-list-item".asInstanceOf[`o-list-item`]
  
  @js.native
  sealed trait paragraph
    extends StObject
       with ElementType
  inline def paragraph: paragraph = "paragraph".asInstanceOf[paragraph]
  
  @js.native
  sealed trait preformatted
    extends StObject
       with ElementType
  inline def preformatted: preformatted = "preformatted".asInstanceOf[preformatted]
  
  @js.native
  sealed trait span
    extends StObject
       with ElementType
  inline def span: span = "span".asInstanceOf[span]
  
  @js.native
  sealed trait strong
    extends StObject
       with ElementType
  inline def strong: strong = "strong".asInstanceOf[strong]
}
