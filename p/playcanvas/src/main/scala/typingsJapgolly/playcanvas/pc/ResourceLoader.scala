...
class ResourceLoader protected () extends js.Object {
  ...
  /**
    ...
    * });
    */
  def load(url: String, `type`: String, callback: typingsJapgolly.playcanvas.pc.callbacks.ResourceLoader): Unit = js.native
  def load(
    url: String,
    `type`: String,
    callback: typingsJapgolly.playcanvas.pc.callbacks.ResourceLoader,
    asset: Asset
  ): Unit = js.native
  /**
    * @function
