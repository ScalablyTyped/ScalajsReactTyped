...
object GeometryData {
  ...
  inline def apply(
    _changeVertexCount: (Any, Any) => Callback,
    indexCount: Double,
    indexStreamUpdated: Boolean,
    indices: Any,
    indicesUsage: Double,
    initDefaults: Callback,
    maxIndices: Double,
    maxVertices: Double,
    recreate: Boolean,
    vertexCount: Any,
  ...
  extension [Self <: GeometryData](x: Self) {
    ...
    inline def setInitDefaults(value: Callback): Self = StObject.set(x, "initDefaults", value.toJsFn)
    ...
    inline def set_changeVertexCount(value: (Any, Any) => Callback): Self = StObject.set(x, "_changeVertexCount", js.Any.fromFunction2((t0: Any, t1: Any) => (value(t0, t1)).runNow()))
  }
}
