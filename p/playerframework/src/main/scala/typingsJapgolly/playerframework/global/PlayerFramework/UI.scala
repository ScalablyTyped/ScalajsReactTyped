package typingsJapgolly.playerframework.global.PlayerFramework

import org.scalajs.dom.Element
import org.scalajs.dom.HTMLElement
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object UI {
  
  @JSGlobal("PlayerFramework.UI.Button")
  @js.native
  open class Button ()
    extends StObject
       with typingsJapgolly.playerframework.PlayerFramework.UI.Button {
    
    /* CompleteClass */
    var content: String = js.native
    
    /* CompleteClass */
    var disabled: Boolean = js.native
    
    /* CompleteClass */
    var element: HTMLElement = js.native
    
    /* CompleteClass */
    var flyout: Element = js.native
    
    /* CompleteClass */
    var hidden: Boolean = js.native
    
    /* CompleteClass */
    var hoverContent: String = js.native
    
    /* CompleteClass */
    var label: String = js.native
    
    /* CompleteClass */
    var tooltip: String = js.native
    
    /* CompleteClass */
    var `type`: String = js.native
  }
  
  @JSGlobal("PlayerFramework.UI.ControlPanel")
  @js.native
  open class ControlPanel ()
    extends StObject
       with typingsJapgolly.playerframework.PlayerFramework.UI.ControlPanel {
    
    /* CompleteClass */
    var element: HTMLElement = js.native
    
    /* CompleteClass */
    var flyoutContainerElement: HTMLElement = js.native
    
    /* CompleteClass */
    var hidden: Boolean = js.native
    
    /* CompleteClass */
    var isAudioHidden: Boolean = js.native
    
    /* CompleteClass */
    var isCaptionsHidden: Boolean = js.native
    
    /* CompleteClass */
    var isElapsedTimeHidden: Boolean = js.native
    
    /* CompleteClass */
    var isFastForwardHidden: Boolean = js.native
    
    /* CompleteClass */
    var isFullScreenHidden: Boolean = js.native
    
    /* CompleteClass */
    var isGoLiveHidden: Boolean = js.native
    
    /* CompleteClass */
    var isInfoHidden: Boolean = js.native
    
    /* CompleteClass */
    var isMediaQualityHidden: Boolean = js.native
    
    /* CompleteClass */
    var isMoreHidden: Boolean = js.native
    
    /* CompleteClass */
    var isMuteHidden: Boolean = js.native
    
    /* CompleteClass */
    var isPauseHidden: Boolean = js.native
    
    /* CompleteClass */
    var isPlayPauseHidden: Boolean = js.native
    
    /* CompleteClass */
    var isPlayResumeHidden: Boolean = js.native
    
    /* CompleteClass */
    var isRemainingTimeHidden: Boolean = js.native
    
    /* CompleteClass */
    var isReplayHidden: Boolean = js.native
    
    /* CompleteClass */
    var isRewindHidden: Boolean = js.native
    
    /* CompleteClass */
    var isSignalStrengthHidden: Boolean = js.native
    
    /* CompleteClass */
    var isSkipAheadHidden: Boolean = js.native
    
    /* CompleteClass */
    var isSkipBackHidden: Boolean = js.native
    
    /* CompleteClass */
    var isSkipNextHidden: Boolean = js.native
    
    /* CompleteClass */
    var isSkipPreviousHidden: Boolean = js.native
    
    /* CompleteClass */
    var isSlowMotionHidden: Boolean = js.native
    
    /* CompleteClass */
    var isStopHidden: Boolean = js.native
    
    /* CompleteClass */
    var isTimelineHidden: Boolean = js.native
    
    /* CompleteClass */
    var isTotalTimeHidden: Boolean = js.native
    
    /* CompleteClass */
    var isVolumeHidden: Boolean = js.native
    
    /* CompleteClass */
    var isVolumeMuteHidden: Boolean = js.native
    
    /* CompleteClass */
    var isZoomHidden: Boolean = js.native
  }
  
  @JSGlobal("PlayerFramework.UI.Indicator")
  @js.native
  open class Indicator ()
    extends StObject
       with typingsJapgolly.playerframework.PlayerFramework.UI.Indicator {
    
    /* CompleteClass */
    var disabled: Boolean = js.native
    
    /* CompleteClass */
    var element: HTMLElement = js.native
    
    /* CompleteClass */
    var hidden: Boolean = js.native
    
    /* CompleteClass */
    var label: String = js.native
    
    /* CompleteClass */
    var tooltip: String = js.native
    
    /* CompleteClass */
    var value: String = js.native
  }
  
  @JSGlobal("PlayerFramework.UI.Meter")
  @js.native
  open class Meter ()
    extends StObject
       with typingsJapgolly.playerframework.PlayerFramework.UI.Meter {
    
    /* CompleteClass */
    var disabled: Boolean = js.native
    
    /* CompleteClass */
    var element: HTMLElement = js.native
    
    /* CompleteClass */
    var hidden: Boolean = js.native
    
    /* CompleteClass */
    var label: String = js.native
    
    /* CompleteClass */
    var tooltip: String = js.native
    
    /* CompleteClass */
    var value: Double = js.native
  }
  
  @JSGlobal("PlayerFramework.UI.Slider")
  @js.native
  open class Slider ()
    extends StObject
       with typingsJapgolly.playerframework.PlayerFramework.UI.Slider {
    
    /* CompleteClass */
    var altStep1: Double = js.native
    
    /* CompleteClass */
    var altStep2: Double = js.native
    
    /* CompleteClass */
    var altStep3: Double = js.native
    
    /* CompleteClass */
    var disabled: Boolean = js.native
    
    /* CompleteClass */
    var element: HTMLElement = js.native
    
    /* CompleteClass */
    var hidden: Boolean = js.native
    
    /* CompleteClass */
    var isThumbnailVisible: Boolean = js.native
    
    /* CompleteClass */
    var label: String = js.native
    
    /* CompleteClass */
    var markers: js.Array[Any] = js.native
    
    /* CompleteClass */
    var max: Double = js.native
    
    /* CompleteClass */
    var min: Double = js.native
    
    /* CompleteClass */
    var progress: Double = js.native
    
    /* CompleteClass */
    var step: Double = js.native
    
    /* CompleteClass */
    var thumbnailImageSrc: String = js.native
    
    /* CompleteClass */
    var tooltip: String = js.native
    
    /* CompleteClass */
    var value: Double = js.native
    
    /* CompleteClass */
    var vertical: Boolean = js.native
  }
}
