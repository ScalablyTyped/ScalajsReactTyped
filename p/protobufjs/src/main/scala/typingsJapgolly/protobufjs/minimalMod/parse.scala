package typingsJapgolly.protobufjs.minimalMod

import typingsJapgolly.protobufjs.mod.IParseOptions
import typingsJapgolly.protobufjs.mod.IParserResult
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("protobufjs/minimal", "parse")
@js.native
object parse extends js.Object {
  def apply(source: String): IParserResult = js.native
  def apply(source: String, options: IParseOptions): IParserResult = js.native
  def apply(source: String, root: typingsJapgolly.protobufjs.mod.Root): IParserResult = js.native
  def apply(source: String, root: typingsJapgolly.protobufjs.mod.Root, options: IParseOptions): IParserResult = js.native
}

