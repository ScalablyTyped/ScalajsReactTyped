package typingsJapgolly.pollyjsPersister
...
object mod {
  ...
  object Persister {
    ...
    inline def apply(options: StringDictionary[Any], persist: CallbackTo[js.Promise[Unit]]): Persister = {
      val __obj = js.Dynamic.literal(options = options.asInstanceOf[js.Any], persist = persist.toJsFn)
    ...
    extension [Self <: Persister](x: Self) {
      ...
      inline def setPersist(value: CallbackTo[js.Promise[Unit]]): Self = StObject.set(x, "persist", value.toJsFn)
    }
  }
}
