...
import typingsJapgolly.popoto.graphLinkMod.Link
import typingsJapgolly.popoto.graphNodeMod.Node
import typingsJapgolly.popoto.queryMod.QueryStructure
...
object TypeofQuery {
  ...
  inline def apply(
    ...
    generateNegativeQueryElements: CallbackTo[PickQueryStructurewhereEl],
  ...
  extension [Self <: TypeofQuery](x: Self) {
    ...
    inline def setGenerateNegativeQueryElements(value: CallbackTo[PickQueryStructurewhereEl]): Self = StObject.set(x, "generateNegativeQueryElements", value.toJsFn)
