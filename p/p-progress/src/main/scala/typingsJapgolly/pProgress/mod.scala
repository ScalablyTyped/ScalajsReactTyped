...
object mod {
  ...
  /* Rewritten from type alias, can be one of: 
    - typingsJapgolly.pProgress.anon.Status[ResolveValueType]
    - typingsJapgolly.pProgress.anon.Reason
  */
  trait PromiseSettledResult[ResolveValueType] extends StObject
  object PromiseSettledResult {
    
    inline def Reason(reason: Any): typingsJapgolly.pProgress.anon.Reason = {
      val __obj = js.Dynamic.literal(reason = reason.asInstanceOf[js.Any], status = "rejected")
      __obj.asInstanceOf[typingsJapgolly.pProgress.anon.Reason]
    }
    
    inline def Status[ResolveValueType](value: ResolveValueType): typingsJapgolly.pProgress.anon.Status[ResolveValueType] = {
      val __obj = js.Dynamic.literal(status = "fulfilled", value = value.asInstanceOf[js.Any])
      __obj.asInstanceOf[typingsJapgolly.pProgress.anon.Status[ResolveValueType]]
    }
  }
}
