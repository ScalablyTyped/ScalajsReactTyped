...
object anon {
  ...
  object Explode {
    ...
    inline def apply(explode: Rule_ => Callback, merge: Rule_ => Callback): Explode = {
      val __obj = js.Dynamic.literal(explode = js.Any.fromFunction1((t0: Rule_) => explode(t0).runNow()), merge = js.Any.fromFunction1((t0: Rule_) => merge(t0).runNow()))
      __obj.asInstanceOf[Explode]
    }
    
    extension [Self <: Explode](x: Self) {
      
      inline def setExplode(value: Rule_ => Callback): Self = StObject.set(x, "explode", js.Any.fromFunction1((t0: Rule_) => value(t0).runNow()))
      
      inline def setMerge(value: Rule_ => Callback): Self = StObject.set(x, "merge", js.Any.fromFunction1((t0: Rule_) => value(t0).runNow()))
    }
  }
