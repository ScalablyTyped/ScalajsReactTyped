...
object libPreviousMapMod {
  ...
  object PreviousMap {
    ...
    inline def apply(consumer: CallbackTo[SourceMapConsumer], `inline`: Boolean, withContent: CallbackTo[Boolean]): PreviousMap = {
      val __obj = js.Dynamic.literal(consumer = consumer.toJsFn, withContent = withContent.toJsFn)
    ...
    extension [Self <: PreviousMap](x: Self) {
      ...
      inline def setConsumer(value: CallbackTo[SourceMapConsumer]): Self = StObject.set(x, "consumer", value.toJsFn)
      ...
      inline def setWithContent(value: CallbackTo[Boolean]): Self = StObject.set(x, "withContent", value.toJsFn)
    }
  }
}
