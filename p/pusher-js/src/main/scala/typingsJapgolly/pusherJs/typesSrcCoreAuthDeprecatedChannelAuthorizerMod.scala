...
import typingsJapgolly.pusherJs.pusherJsStrings.ajax
import typingsJapgolly.pusherJs.pusherJsStrings.jsonp
...
import typingsJapgolly.pusherJs.typesSrcCoreChannelsChannelMod.default
...
object typesSrcCoreAuthDeprecatedChannelAuthorizerMod {
  ...
  object DeprecatedChannelAuthorizer {
    ...
    inline def apply(authorize: (String, ChannelAuthorizationCallback) => Callback): DeprecatedChannelAuthorizer = {
      val __obj = js.Dynamic.literal(authorize = js.Any.fromFunction2((t0: String, t1: ChannelAuthorizationCallback) => (authorize(t0, t1)).runNow()))
      __obj.asInstanceOf[DeprecatedChannelAuthorizer]
    }
    
    extension [Self <: DeprecatedChannelAuthorizer](x: Self) {
      
      inline def setAuthorize(value: (String, ChannelAuthorizationCallback) => Callback): Self = StObject.set(x, "authorize", js.Any.fromFunction2((t0: String, t1: ChannelAuthorizationCallback) => (value(t0, t1)).runNow()))
    }
  }
}
