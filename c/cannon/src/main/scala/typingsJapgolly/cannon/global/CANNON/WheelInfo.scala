package typingsJapgolly.cannon.global.CANNON

import typingsJapgolly.cannon.CANNON.IWheelInfoOptions
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("CANNON.WheelInfo")
@js.native
open class WheelInfo ()
  extends StObject
     with typingsJapgolly.cannon.CANNON.WheelInfo {
  def this(options: IWheelInfoOptions) = this()
  
  /* CompleteClass */
  var axleLocal: typingsJapgolly.cannon.CANNON.Vec3 = js.native
  
  /* CompleteClass */
  var axleWorld: typingsJapgolly.cannon.CANNON.Vec3 = js.native
  
  /* CompleteClass */
  var brake: Double = js.native
  
  /* CompleteClass */
  var chassisConnectionPointLocal: typingsJapgolly.cannon.CANNON.Vec3 = js.native
  
  /* CompleteClass */
  var chassisConnectionPointWorld: typingsJapgolly.cannon.CANNON.Vec3 = js.native
  
  /* CompleteClass */
  var clippedInvContactDotSuspension: Double = js.native
  
  /* CompleteClass */
  var customSlidingRotationalSpeed: Double = js.native
  
  /* CompleteClass */
  var dampingCompression: Double = js.native
  
  /* CompleteClass */
  var dampingRelaxation: Double = js.native
  
  /* CompleteClass */
  var deltaRotation: Double = js.native
  
  /* CompleteClass */
  var directionLocal: typingsJapgolly.cannon.CANNON.Vec3 = js.native
  
  /* CompleteClass */
  var directionWorld: typingsJapgolly.cannon.CANNON.Vec3 = js.native
  
  /* CompleteClass */
  var engineForce: Double = js.native
  
  /* CompleteClass */
  var forwardImpulse: Double = js.native
  
  /* CompleteClass */
  var frictionSlip: Double = js.native
  
  /* CompleteClass */
  var isFrontWheel: Boolean = js.native
  
  /* CompleteClass */
  var isInContact: Boolean = js.native
  
  /* CompleteClass */
  var maxSuspensionForce: Double = js.native
  
  /* CompleteClass */
  var maxSuspensionTravbel: Double = js.native
  
  /* CompleteClass */
  var radius: Double = js.native
  
  /* CompleteClass */
  var raycastResult: typingsJapgolly.cannon.CANNON.RaycastResult = js.native
  
  /* CompleteClass */
  var rollInfluence: Double = js.native
  
  /* CompleteClass */
  var rotation: Double = js.native
  
  /* CompleteClass */
  var sideImpulse: Double = js.native
  
  /* CompleteClass */
  var skidInfo: Double = js.native
  
  /* CompleteClass */
  var sliding: Boolean = js.native
  
  /* CompleteClass */
  var steering: Double = js.native
  
  /* CompleteClass */
  var suspensionForce: Double = js.native
  
  /* CompleteClass */
  var suspensionLength: Double = js.native
  
  /* CompleteClass */
  var suspensionMaxLength: Double = js.native
  
  /* CompleteClass */
  var suspensionRelativeVelocity: Double = js.native
  
  /* CompleteClass */
  var suspensionRestLength: Double = js.native
  
  /* CompleteClass */
  var suspensionStiffness: Double = js.native
  
  /* CompleteClass */
  var useCustomSlidingRotationalSpeed: Boolean = js.native
  
  /* CompleteClass */
  var worldTransform: typingsJapgolly.cannon.CANNON.Transform = js.native
}
