...
import typingsJapgolly.connectMongo.buildMainLibMongoStoreMod.CryptoOptions
import typingsJapgolly.connectMongo.buildMainLibMongoStoreMod.default
import typingsJapgolly.connectMongo.connectMongoStrings.disabled
import typingsJapgolly.connectMongo.connectMongoStrings.interval
import typingsJapgolly.connectMongo.connectMongoStrings.native
import typingsJapgolly.expressSession.mod.Cookie
import typingsJapgolly.expressSession.mod.SessionData
...
object anon {
  ...
  object All {
    ...
    inline def apply(
      all: CallbackTo[
          js.Promise[js.UndefOr[js.Array[SessionData] | StringDictionary[SessionData] | Null]]
        ],
      ...
      length: CallbackTo[js.Promise[Double]],
      set: (String, SessionData) => js.Promise[Unit],
      touch: (String, SessionDatalastModifiedDa) => js.Promise[Unit]
    ): All = {
      val __obj = js.Dynamic.literal(all = all.toJsFn, clear = clear.toJsFn, close = close.toJsFn, destroy = js.Any.fromFunction1(destroy), get = js.Any.fromFunction1(get), length = length.toJsFn, set = js.Any.fromFunction2(set), touch = js.Any.fromFunction2(touch))
    ...
    extension [Self <: All](x: Self) {
      ...
      inline def setLength(value: CallbackTo[js.Promise[Double]]): Self = StObject.set(x, "length", value.toJsFn)
