package typingsJapgolly.cliTable2

import typingsJapgolly.cliTable2.mod.CharName
import typingsJapgolly.cliTable2.mod.HorizontalAlignment
import typingsJapgolly.cliTable2.mod.VerticalAlignment
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object cliTable2Strings {
  
  @js.native
  sealed trait bottom
    extends StObject
       with CharName
       with VerticalAlignment
  inline def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @js.native
  sealed trait `bottom-left`
    extends StObject
       with CharName
  inline def `bottom-left`: `bottom-left` = "bottom-left".asInstanceOf[`bottom-left`]
  
  @js.native
  sealed trait `bottom-mid`
    extends StObject
       with CharName
  inline def `bottom-mid`: `bottom-mid` = "bottom-mid".asInstanceOf[`bottom-mid`]
  
  @js.native
  sealed trait `bottom-right`
    extends StObject
       with CharName
  inline def `bottom-right`: `bottom-right` = "bottom-right".asInstanceOf[`bottom-right`]
  
  @js.native
  sealed trait center
    extends StObject
       with HorizontalAlignment
       with VerticalAlignment
  inline def center: center = "center".asInstanceOf[center]
  
  @js.native
  sealed trait left
    extends StObject
       with CharName
       with HorizontalAlignment
  inline def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait `left-mid`
    extends StObject
       with CharName
  inline def `left-mid`: `left-mid` = "left-mid".asInstanceOf[`left-mid`]
  
  @js.native
  sealed trait mid
    extends StObject
       with CharName
  inline def mid: mid = "mid".asInstanceOf[mid]
  
  @js.native
  sealed trait `mid-mid`
    extends StObject
       with CharName
  inline def `mid-mid`: `mid-mid` = "mid-mid".asInstanceOf[`mid-mid`]
  
  @js.native
  sealed trait middle
    extends StObject
       with CharName
  inline def middle: middle = "middle".asInstanceOf[middle]
  
  @js.native
  sealed trait right
    extends StObject
       with CharName
       with HorizontalAlignment
  inline def right: right = "right".asInstanceOf[right]
  
  @js.native
  sealed trait `right-mid`
    extends StObject
       with CharName
  inline def `right-mid`: `right-mid` = "right-mid".asInstanceOf[`right-mid`]
  
  @js.native
  sealed trait top
    extends StObject
       with CharName
       with VerticalAlignment
  inline def top: top = "top".asInstanceOf[top]
  
  @js.native
  sealed trait `top-left`
    extends StObject
       with CharName
  inline def `top-left`: `top-left` = "top-left".asInstanceOf[`top-left`]
  
  @js.native
  sealed trait `top-mid`
    extends StObject
       with CharName
  inline def `top-mid`: `top-mid` = "top-mid".asInstanceOf[`top-mid`]
  
  @js.native
  sealed trait `top-right`
    extends StObject
       with CharName
  inline def `top-right`: `top-right` = "top-right".asInstanceOf[`top-right`]
}
