...
import typingsJapgolly.cypress.CyEventEmitter
import typingsJapgolly.cypress.typesBluebirdMod.Resolvable
...
object Cypress extends Shortcut {
  ...
  @JSGlobal("Cypress")
  @js.native
  val ^ : typingsJapgolly.cypress.Cypress.Cypress & CyEventEmitter = js.native
  ...
  open class Promise[R] protected ()
    extends typingsJapgolly.cypress.typesBluebirdMod.^[R] {
    /**
  ...
  type _To = typingsJapgolly.cypress.Cypress.Cypress & CyEventEmitter
  
  /* This means you don't have to write `^`, but can instead just say `Cypress.foo` */
  override def _to: typingsJapgolly.cypress.Cypress.Cypress & CyEventEmitter = ^
}
