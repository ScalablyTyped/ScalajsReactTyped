...
import typingsJapgolly.cypress.Chai.Assertion
import typingsJapgolly.cypress.Cypress.Chainable
import typingsJapgolly.cypressRecurse.cypressRecurseStrings.both
import typingsJapgolly.cypressRecurse.cypressRecurseStrings.reduced
import typingsJapgolly.cypressRecurse.cypressRecurseStrings.value
...
object mod {
  ...
  object RecurseOptions {
    ...
    extension [Self <: RecurseOptions[?], T](x: Self & RecurseOptions[T]) {
      ...
      inline def setLogFunction1(value: T => Callback): Self = StObject.set(x, "log", js.Any.fromFunction1((t0: T) => value(t0).runNow()))
