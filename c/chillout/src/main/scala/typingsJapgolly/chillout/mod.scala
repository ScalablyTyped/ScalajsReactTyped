...
import typingsJapgolly.chillout.chilloutBooleans.`false`
import typingsJapgolly.chillout.chilloutBooleans.`true`
...
object mod {
  ...
  object ChilloutIterator {
    ...
    inline def apply[TValue, TReturn](next: CallbackTo[js.Tuple2[`false` | `true`, js.Symbol | TReturn | TValue]]): ChilloutIterator[TValue, TReturn] = {
    ...
    extension [Self <: ChilloutIterator[?, ?], TValue, TReturn](x: Self & (ChilloutIterator[TValue, TReturn])) {
      ...
      inline def setNext(value: CallbackTo[js.Tuple2[`false` | `true`, js.Symbol | TReturn | TValue]]): Self = StObject.set(x, "next", value.toJsFn)
    }
  }
