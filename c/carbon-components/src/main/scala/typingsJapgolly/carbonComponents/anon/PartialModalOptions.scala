package typingsJapgolly.carbonComponents.anon

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/* Inlined std.Partial<carbon-components.carbon-components/components/modal/modal.ModalOptions> */
trait PartialModalOptions extends StObject {
  
  var attribInitTarget: js.UndefOr[String] = js.undefined
  
  var classBody: js.UndefOr[String] = js.undefined
  
  var classVisible: js.UndefOr[String] = js.undefined
  
  var eventAfterHidden: js.UndefOr[String] = js.undefined
  
  var eventAfterShown: js.UndefOr[String] = js.undefined
  
  var eventBeforeHidden: js.UndefOr[String] = js.undefined
  
  var eventBeforeShown: js.UndefOr[String] = js.undefined
  
  var initEventNames: js.UndefOr[js.Array[String]] = js.undefined
  
  var selectorInit: js.UndefOr[String] = js.undefined
  
  var selectorModalClose: js.UndefOr[String] = js.undefined
  
  var selectorModalContainer: js.UndefOr[String] = js.undefined
  
  var selectorPrimaryFocus: js.UndefOr[String] = js.undefined
  
  var selectorsFloatingMenus: js.UndefOr[js.Array[String]] = js.undefined
}
object PartialModalOptions {
  
  inline def apply(): PartialModalOptions = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[PartialModalOptions]
  }
  
  extension [Self <: PartialModalOptions](x: Self) {
    
    inline def setAttribInitTarget(value: String): Self = StObject.set(x, "attribInitTarget", value.asInstanceOf[js.Any])
    
    inline def setAttribInitTargetUndefined: Self = StObject.set(x, "attribInitTarget", js.undefined)
    
    inline def setClassBody(value: String): Self = StObject.set(x, "classBody", value.asInstanceOf[js.Any])
    
    inline def setClassBodyUndefined: Self = StObject.set(x, "classBody", js.undefined)
    
    inline def setClassVisible(value: String): Self = StObject.set(x, "classVisible", value.asInstanceOf[js.Any])
    
    inline def setClassVisibleUndefined: Self = StObject.set(x, "classVisible", js.undefined)
    
    inline def setEventAfterHidden(value: String): Self = StObject.set(x, "eventAfterHidden", value.asInstanceOf[js.Any])
    
    inline def setEventAfterHiddenUndefined: Self = StObject.set(x, "eventAfterHidden", js.undefined)
    
    inline def setEventAfterShown(value: String): Self = StObject.set(x, "eventAfterShown", value.asInstanceOf[js.Any])
    
    inline def setEventAfterShownUndefined: Self = StObject.set(x, "eventAfterShown", js.undefined)
    
    inline def setEventBeforeHidden(value: String): Self = StObject.set(x, "eventBeforeHidden", value.asInstanceOf[js.Any])
    
    inline def setEventBeforeHiddenUndefined: Self = StObject.set(x, "eventBeforeHidden", js.undefined)
    
    inline def setEventBeforeShown(value: String): Self = StObject.set(x, "eventBeforeShown", value.asInstanceOf[js.Any])
    
    inline def setEventBeforeShownUndefined: Self = StObject.set(x, "eventBeforeShown", js.undefined)
    
    inline def setInitEventNames(value: js.Array[String]): Self = StObject.set(x, "initEventNames", value.asInstanceOf[js.Any])
    
    inline def setInitEventNamesUndefined: Self = StObject.set(x, "initEventNames", js.undefined)
    
    inline def setInitEventNamesVarargs(value: String*): Self = StObject.set(x, "initEventNames", js.Array(value*))
    
    inline def setSelectorInit(value: String): Self = StObject.set(x, "selectorInit", value.asInstanceOf[js.Any])
    
    inline def setSelectorInitUndefined: Self = StObject.set(x, "selectorInit", js.undefined)
    
    inline def setSelectorModalClose(value: String): Self = StObject.set(x, "selectorModalClose", value.asInstanceOf[js.Any])
    
    inline def setSelectorModalCloseUndefined: Self = StObject.set(x, "selectorModalClose", js.undefined)
    
    inline def setSelectorModalContainer(value: String): Self = StObject.set(x, "selectorModalContainer", value.asInstanceOf[js.Any])
    
    inline def setSelectorModalContainerUndefined: Self = StObject.set(x, "selectorModalContainer", js.undefined)
    
    inline def setSelectorPrimaryFocus(value: String): Self = StObject.set(x, "selectorPrimaryFocus", value.asInstanceOf[js.Any])
    
    inline def setSelectorPrimaryFocusUndefined: Self = StObject.set(x, "selectorPrimaryFocus", js.undefined)
    
    inline def setSelectorsFloatingMenus(value: js.Array[String]): Self = StObject.set(x, "selectorsFloatingMenus", value.asInstanceOf[js.Any])
    
    inline def setSelectorsFloatingMenusUndefined: Self = StObject.set(x, "selectorsFloatingMenus", js.undefined)
    
    inline def setSelectorsFloatingMenusVarargs(value: String*): Self = StObject.set(x, "selectorsFloatingMenus", js.Array(value*))
  }
}
