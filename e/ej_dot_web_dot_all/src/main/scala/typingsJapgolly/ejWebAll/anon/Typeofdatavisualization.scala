package typingsJapgolly.ejWebAll.anon

import org.scalablytyped.runtime.Instantiable2
import org.scalajs.dom.Element
import typingsJapgolly.ejWebAll.JQuery
import typingsJapgolly.ejWebAll.ej.Model
import typingsJapgolly.ejWebAll.ej.datavisualization.Barcode
import typingsJapgolly.ejWebAll.ej.datavisualization.BulletGraph
import typingsJapgolly.ejWebAll.ej.datavisualization.Chart
import typingsJapgolly.ejWebAll.ej.datavisualization.CircularGauge
import typingsJapgolly.ejWebAll.ej.datavisualization.Diagram
import typingsJapgolly.ejWebAll.ej.datavisualization.DigitalGauge
import typingsJapgolly.ejWebAll.ej.datavisualization.HeatMap
import typingsJapgolly.ejWebAll.ej.datavisualization.HeatMapLegend
import typingsJapgolly.ejWebAll.ej.datavisualization.LinearGauge
import typingsJapgolly.ejWebAll.ej.datavisualization.Map
import typingsJapgolly.ejWebAll.ej.datavisualization.Overview
import typingsJapgolly.ejWebAll.ej.datavisualization.RangeNavigator
import typingsJapgolly.ejWebAll.ej.datavisualization.Sparkline
import typingsJapgolly.ejWebAll.ej.datavisualization.SunburstChart
import typingsJapgolly.ejWebAll.ej.datavisualization.SymbolPalette
import typingsJapgolly.ejWebAll.ej.datavisualization.TreeMap
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait Typeofdatavisualization extends StObject {
  
  val Barcode: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.Barcode
  ]) & TypeofBarcode
  
  val BulletGraph: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.BulletGraph
  ]) & TypeofBulletGraph
  
  val Chart: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.Chart
  ]) & TypeofChart
  
  val CircularGauge: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.CircularGauge
  ]) & TypeofCircularGauge
  
  val Diagram: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.Diagram
  ]) & TypeofDiagram
  
  val DigitalGauge: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.DigitalGauge
  ]) & TypeofDigitalGauge
  
  val HeatMap: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.HeatMap
  ]) & TypeofHeatMap
  
  val HeatMapLegend: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.HeatMapLegend
  ]) & TypeofHeatMapLegend
  
  val LinearGauge: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.LinearGauge
  ]) & TypeofLinearGauge
  
  val Map: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.Map
  ]) & TypeofMap
  
  val Overview: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.Overview
  ]) & TypeofOverview
  
  val RangeNavigator: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.RangeNavigator
  ]) & TypeofRangeNavigator
  
  val Sparkline: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[typingsJapgolly.ejWebAll.ej.Sparkline.Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.Sparkline
  ]) & TypeofSparklineFn
  
  val Sunburst: Any
  
  val SunburstChart: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[typingsJapgolly.ejWebAll.ej.SunburstChart.Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.SunburstChart
  ]) & TypeofSunburstChartFn
  
  val SymbolPalette: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.SymbolPalette
  ]) & TypeofSymbolPalette
  
  val TreeMap: (Instantiable2[
    /* element */ JQuery | Element, 
    /* options */ js.UndefOr[Model], 
    typingsJapgolly.ejWebAll.ej.datavisualization.TreeMap
  ]) & TypeofTreeMap
}
object Typeofdatavisualization {
  
  inline def apply(
    Barcode: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Barcode]) & TypeofBarcode,
    BulletGraph: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], BulletGraph]) & TypeofBulletGraph,
    Chart: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Chart]) & TypeofChart,
    CircularGauge: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], CircularGauge]) & TypeofCircularGauge,
    Diagram: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Diagram]) & TypeofDiagram,
    DigitalGauge: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], DigitalGauge]) & TypeofDigitalGauge,
    HeatMap: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], HeatMap]) & TypeofHeatMap,
    HeatMapLegend: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], HeatMapLegend]) & TypeofHeatMapLegend,
    LinearGauge: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], LinearGauge]) & TypeofLinearGauge,
    Map: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Map]) & TypeofMap,
    Overview: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Overview]) & TypeofOverview,
    RangeNavigator: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], RangeNavigator]) & TypeofRangeNavigator,
    Sparkline: (Instantiable2[
      /* element */ JQuery | Element, 
      /* options */ js.UndefOr[typingsJapgolly.ejWebAll.ej.Sparkline.Model], 
      Sparkline
    ]) & TypeofSparklineFn,
    Sunburst: Any,
    SunburstChart: (Instantiable2[
      /* element */ JQuery | Element, 
      /* options */ js.UndefOr[typingsJapgolly.ejWebAll.ej.SunburstChart.Model], 
      SunburstChart
    ]) & TypeofSunburstChartFn,
    SymbolPalette: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], SymbolPalette]) & TypeofSymbolPalette,
    TreeMap: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], TreeMap]) & TypeofTreeMap
  ): Typeofdatavisualization = {
    val __obj = js.Dynamic.literal(Barcode = Barcode.asInstanceOf[js.Any], BulletGraph = BulletGraph.asInstanceOf[js.Any], Chart = Chart.asInstanceOf[js.Any], CircularGauge = CircularGauge.asInstanceOf[js.Any], Diagram = Diagram.asInstanceOf[js.Any], DigitalGauge = DigitalGauge.asInstanceOf[js.Any], HeatMap = HeatMap.asInstanceOf[js.Any], HeatMapLegend = HeatMapLegend.asInstanceOf[js.Any], LinearGauge = LinearGauge.asInstanceOf[js.Any], Map = Map.asInstanceOf[js.Any], Overview = Overview.asInstanceOf[js.Any], RangeNavigator = RangeNavigator.asInstanceOf[js.Any], Sparkline = Sparkline.asInstanceOf[js.Any], Sunburst = Sunburst.asInstanceOf[js.Any], SunburstChart = SunburstChart.asInstanceOf[js.Any], SymbolPalette = SymbolPalette.asInstanceOf[js.Any], TreeMap = TreeMap.asInstanceOf[js.Any])
    __obj.asInstanceOf[Typeofdatavisualization]
  }
  
  extension [Self <: Typeofdatavisualization](x: Self) {
    
    inline def setBarcode(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Barcode]) & TypeofBarcode
    ): Self = StObject.set(x, "Barcode", value.asInstanceOf[js.Any])
    
    inline def setBulletGraph(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], BulletGraph]) & TypeofBulletGraph
    ): Self = StObject.set(x, "BulletGraph", value.asInstanceOf[js.Any])
    
    inline def setChart(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Chart]) & TypeofChart
    ): Self = StObject.set(x, "Chart", value.asInstanceOf[js.Any])
    
    inline def setCircularGauge(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], CircularGauge]) & TypeofCircularGauge
    ): Self = StObject.set(x, "CircularGauge", value.asInstanceOf[js.Any])
    
    inline def setDiagram(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Diagram]) & TypeofDiagram
    ): Self = StObject.set(x, "Diagram", value.asInstanceOf[js.Any])
    
    inline def setDigitalGauge(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], DigitalGauge]) & TypeofDigitalGauge
    ): Self = StObject.set(x, "DigitalGauge", value.asInstanceOf[js.Any])
    
    inline def setHeatMap(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], HeatMap]) & TypeofHeatMap
    ): Self = StObject.set(x, "HeatMap", value.asInstanceOf[js.Any])
    
    inline def setHeatMapLegend(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], HeatMapLegend]) & TypeofHeatMapLegend
    ): Self = StObject.set(x, "HeatMapLegend", value.asInstanceOf[js.Any])
    
    inline def setLinearGauge(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], LinearGauge]) & TypeofLinearGauge
    ): Self = StObject.set(x, "LinearGauge", value.asInstanceOf[js.Any])
    
    inline def setMap(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Map]) & TypeofMap
    ): Self = StObject.set(x, "Map", value.asInstanceOf[js.Any])
    
    inline def setOverview(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], Overview]) & TypeofOverview
    ): Self = StObject.set(x, "Overview", value.asInstanceOf[js.Any])
    
    inline def setRangeNavigator(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], RangeNavigator]) & TypeofRangeNavigator
    ): Self = StObject.set(x, "RangeNavigator", value.asInstanceOf[js.Any])
    
    inline def setSparkline(
      value: (Instantiable2[
          /* element */ JQuery | Element, 
          /* options */ js.UndefOr[typingsJapgolly.ejWebAll.ej.Sparkline.Model], 
          Sparkline
        ]) & TypeofSparklineFn
    ): Self = StObject.set(x, "Sparkline", value.asInstanceOf[js.Any])
    
    inline def setSunburst(value: Any): Self = StObject.set(x, "Sunburst", value.asInstanceOf[js.Any])
    
    inline def setSunburstChart(
      value: (Instantiable2[
          /* element */ JQuery | Element, 
          /* options */ js.UndefOr[typingsJapgolly.ejWebAll.ej.SunburstChart.Model], 
          SunburstChart
        ]) & TypeofSunburstChartFn
    ): Self = StObject.set(x, "SunburstChart", value.asInstanceOf[js.Any])
    
    inline def setSymbolPalette(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], SymbolPalette]) & TypeofSymbolPalette
    ): Self = StObject.set(x, "SymbolPalette", value.asInstanceOf[js.Any])
    
    inline def setTreeMap(
      value: (Instantiable2[/* element */ JQuery | Element, /* options */ js.UndefOr[Model], TreeMap]) & TypeofTreeMap
    ): Self = StObject.set(x, "TreeMap", value.asInstanceOf[js.Any])
  }
}
