...
object distBytesMod {
  ...
  */
  ...
  object _ToBufferInputTypes {
    ...
    inline def TransformableToArray(toArray: CallbackTo[js.typedarray.Uint8Array]): typingsJapgolly.ethereumjsUtil.distTypesMod.TransformableToArray = {
      val __obj = js.Dynamic.literal(toArray = toArray.toJsFn)
      __obj.asInstanceOf[typingsJapgolly.ethereumjsUtil.distTypesMod.TransformableToArray]
    }
    
    inline def TransformableToBuffer(toBuffer: CallbackTo[Buffer]): typingsJapgolly.ethereumjsUtil.distTypesMod.TransformableToBuffer = {
      val __obj = js.Dynamic.literal(toBuffer = toBuffer.toJsFn)
      __obj.asInstanceOf[typingsJapgolly.ethereumjsUtil.distTypesMod.TransformableToBuffer]
    }
  }
}
