...
trait MappingFloatNumberProperty
  ...
  var `type`: typingsJapgolly.elasticElasticsearch.elasticElasticsearchStrings.float
}
object MappingFloatNumberProperty {
  ...
  extension [Self <: MappingFloatNumberProperty](x: Self) {
    ...
    inline def setType(value: typingsJapgolly.elasticElasticsearch.elasticElasticsearchStrings.float): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
  }
}
