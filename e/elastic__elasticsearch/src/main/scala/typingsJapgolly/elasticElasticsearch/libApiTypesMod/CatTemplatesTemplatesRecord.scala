package typingsJapgolly.elasticElasticsearch.libApiTypesMod

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

trait CatTemplatesTemplatesRecord extends StObject {
  
  var c: js.UndefOr[String] = js.undefined
  
  var composed_of: js.UndefOr[String] = js.undefined
  
  var index_patterns: js.UndefOr[String] = js.undefined
  
  var n: js.UndefOr[Name] = js.undefined
  
  var name: js.UndefOr[Name] = js.undefined
  
  var o: js.UndefOr[String] = js.undefined
  
  var order: js.UndefOr[String] = js.undefined
  
  var p: js.UndefOr[String] = js.undefined
  
  var t: js.UndefOr[String] = js.undefined
  
  var v: js.UndefOr[VersionString | Null] = js.undefined
  
  var version: js.UndefOr[VersionString | Null] = js.undefined
}
object CatTemplatesTemplatesRecord {
  
  inline def apply(): CatTemplatesTemplatesRecord = {
    val __obj = js.Dynamic.literal()
    __obj.asInstanceOf[CatTemplatesTemplatesRecord]
  }
  
  extension [Self <: CatTemplatesTemplatesRecord](x: Self) {
    
    inline def setC(value: String): Self = StObject.set(x, "c", value.asInstanceOf[js.Any])
    
    inline def setCUndefined: Self = StObject.set(x, "c", js.undefined)
    
    inline def setComposed_of(value: String): Self = StObject.set(x, "composed_of", value.asInstanceOf[js.Any])
    
    inline def setComposed_ofUndefined: Self = StObject.set(x, "composed_of", js.undefined)
    
    inline def setIndex_patterns(value: String): Self = StObject.set(x, "index_patterns", value.asInstanceOf[js.Any])
    
    inline def setIndex_patternsUndefined: Self = StObject.set(x, "index_patterns", js.undefined)
    
    inline def setN(value: Name): Self = StObject.set(x, "n", value.asInstanceOf[js.Any])
    
    inline def setNUndefined: Self = StObject.set(x, "n", js.undefined)
    
    inline def setName(value: Name): Self = StObject.set(x, "name", value.asInstanceOf[js.Any])
    
    inline def setNameUndefined: Self = StObject.set(x, "name", js.undefined)
    
    inline def setO(value: String): Self = StObject.set(x, "o", value.asInstanceOf[js.Any])
    
    inline def setOUndefined: Self = StObject.set(x, "o", js.undefined)
    
    inline def setOrder(value: String): Self = StObject.set(x, "order", value.asInstanceOf[js.Any])
    
    inline def setOrderUndefined: Self = StObject.set(x, "order", js.undefined)
    
    inline def setP(value: String): Self = StObject.set(x, "p", value.asInstanceOf[js.Any])
    
    inline def setPUndefined: Self = StObject.set(x, "p", js.undefined)
    
    inline def setT(value: String): Self = StObject.set(x, "t", value.asInstanceOf[js.Any])
    
    inline def setTUndefined: Self = StObject.set(x, "t", js.undefined)
    
    inline def setV(value: VersionString): Self = StObject.set(x, "v", value.asInstanceOf[js.Any])
    
    inline def setVNull: Self = StObject.set(x, "v", null)
    
    inline def setVUndefined: Self = StObject.set(x, "v", js.undefined)
    
    inline def setVersion(value: VersionString): Self = StObject.set(x, "version", value.asInstanceOf[js.Any])
    
    inline def setVersionNull: Self = StObject.set(x, "version", null)
    
    inline def setVersionUndefined: Self = StObject.set(x, "version", js.undefined)
  }
}
