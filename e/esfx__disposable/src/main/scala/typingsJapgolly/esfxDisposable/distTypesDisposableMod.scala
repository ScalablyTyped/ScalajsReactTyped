...
import typingsJapgolly.esfxDisposable.distTypesAsyncDisposableStackMod._AsyncDisposableLike
...
object distTypesDisposableMod {
  ...
  object DisposableScope {
    ...
    inline def apply(fail: Any => Callback, `using`: Any => Any): DisposableScope = {
      val __obj = js.Dynamic.literal(fail = js.Any.fromFunction1((t0: Any) => fail(t0).runNow()))
    ...
    extension [Self <: DisposableScope](x: Self) {
      ...
      inline def setFail(value: Any => Callback): Self = StObject.set(x, "fail", js.Any.fromFunction1((t0: Any) => value(t0).runNow()))
