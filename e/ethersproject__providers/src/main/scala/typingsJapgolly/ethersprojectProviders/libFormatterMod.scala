...
import typingsJapgolly.ethersprojectAbstractProvider.mod.Block
import typingsJapgolly.ethersprojectAbstractProvider.mod.TransactionReceipt
...
object libFormatterMod {
  ...
  object CommunityResourcable {
    ...
    inline def apply(isCommunityResource: CallbackTo[Boolean]): CommunityResourcable = {
      val __obj = js.Dynamic.literal(isCommunityResource = isCommunityResource.toJsFn)
      __obj.asInstanceOf[CommunityResourcable]
    }
    
    extension [Self <: CommunityResourcable](x: Self) {
      
      inline def setIsCommunityResource(value: CallbackTo[Boolean]): Self = StObject.set(x, "isCommunityResource", value.toJsFn)
    }
  }
