package typingsJapgolly.extjs.global.Ext

import org.scalajs.dom.HTMLElement
import typingsJapgolly.extjs.Ext.util.IPoint
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Ext.EventObject")
@js.native
open class EventObject ()
  extends StObject
     with typingsJapgolly.extjs.Ext.EventObject
/* static members */
object EventObject {
  
  @JSGlobal("Ext.EventObject")
  @js.native
  val ^ : js.Any = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.A")
  @js.native
  def A: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.ALT")
  @js.native
  def ALT: Double = js.native
  inline def ALT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ALT")(x.asInstanceOf[js.Any])
  
  inline def A_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("A")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.B")
  @js.native
  def B: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.BACKSPACE")
  @js.native
  def BACKSPACE: Double = js.native
  inline def BACKSPACE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("BACKSPACE")(x.asInstanceOf[js.Any])
  
  inline def B_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("B")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.C")
  @js.native
  def C: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.CAPS_LOCK")
  @js.native
  def CAPS_LOCK: Double = js.native
  inline def CAPS_LOCK_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("CAPS_LOCK")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.CONTEXT_MENU")
  @js.native
  def CONTEXT_MENU: Double = js.native
  inline def CONTEXT_MENU_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("CONTEXT_MENU")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.CTRL")
  @js.native
  def CTRL: Double = js.native
  inline def CTRL_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("CTRL")(x.asInstanceOf[js.Any])
  
  inline def C_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("C")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.D")
  @js.native
  def D: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.DELETE")
  @js.native
  def DELETE: Double = js.native
  inline def DELETE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("DELETE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.DOWN")
  @js.native
  def DOWN: Double = js.native
  inline def DOWN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("DOWN")(x.asInstanceOf[js.Any])
  
  inline def D_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("D")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.E")
  @js.native
  def E: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.EIGHT")
  @js.native
  def EIGHT: Double = js.native
  inline def EIGHT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("EIGHT")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.END")
  @js.native
  def END: Double = js.native
  inline def END_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("END")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.ENTER")
  @js.native
  def ENTER: Double = js.native
  inline def ENTER_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ENTER")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.ESC")
  @js.native
  def ESC: Double = js.native
  inline def ESC_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ESC")(x.asInstanceOf[js.Any])
  
  inline def E_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("E")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F")
  @js.native
  def F: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F1")
  @js.native
  def F1: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F10")
  @js.native
  def F10: Double = js.native
  inline def F10_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F10")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F11")
  @js.native
  def F11: Double = js.native
  inline def F11_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F11")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F12")
  @js.native
  def F12: Double = js.native
  inline def F12_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F12")(x.asInstanceOf[js.Any])
  
  inline def F1_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F1")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F2")
  @js.native
  def F2: Double = js.native
  inline def F2_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F2")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F3")
  @js.native
  def F3: Double = js.native
  inline def F3_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F3")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F4")
  @js.native
  def F4: Double = js.native
  inline def F4_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F4")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F5")
  @js.native
  def F5: Double = js.native
  inline def F5_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F5")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F6")
  @js.native
  def F6: Double = js.native
  inline def F6_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F6")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F7")
  @js.native
  def F7: Double = js.native
  inline def F7_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F7")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F8")
  @js.native
  def F8: Double = js.native
  inline def F8_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F8")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.F9")
  @js.native
  def F9: Double = js.native
  inline def F9_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F9")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.FIVE")
  @js.native
  def FIVE: Double = js.native
  inline def FIVE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("FIVE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.FOUR")
  @js.native
  def FOUR: Double = js.native
  inline def FOUR_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("FOUR")(x.asInstanceOf[js.Any])
  
  inline def F_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("F")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.G")
  @js.native
  def G: Double = js.native
  inline def G_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("G")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.H")
  @js.native
  def H: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.HOME")
  @js.native
  def HOME: Double = js.native
  inline def HOME_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("HOME")(x.asInstanceOf[js.Any])
  
  inline def H_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("H")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.I")
  @js.native
  def I: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.INSERT")
  @js.native
  def INSERT: Double = js.native
  inline def INSERT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("INSERT")(x.asInstanceOf[js.Any])
  
  inline def I_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("I")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.J")
  @js.native
  def J: Double = js.native
  inline def J_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("J")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.K")
  @js.native
  def K: Double = js.native
  inline def K_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("K")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.L")
  @js.native
  def L: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.LEFT")
  @js.native
  def LEFT: Double = js.native
  inline def LEFT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("LEFT")(x.asInstanceOf[js.Any])
  
  inline def L_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("L")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.M")
  @js.native
  def M: Double = js.native
  inline def M_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("M")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.N")
  @js.native
  def N: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NINE")
  @js.native
  def NINE: Double = js.native
  inline def NINE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NINE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_CENTER")
  @js.native
  def NUM_CENTER: Double = js.native
  inline def NUM_CENTER_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_CENTER")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_DIVISION")
  @js.native
  def NUM_DIVISION: Double = js.native
  inline def NUM_DIVISION_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_DIVISION")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_EIGHT")
  @js.native
  def NUM_EIGHT: Double = js.native
  inline def NUM_EIGHT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_EIGHT")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_FIVE")
  @js.native
  def NUM_FIVE: Double = js.native
  inline def NUM_FIVE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_FIVE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_FOUR")
  @js.native
  def NUM_FOUR: Double = js.native
  inline def NUM_FOUR_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_FOUR")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_MINUS")
  @js.native
  def NUM_MINUS: Double = js.native
  inline def NUM_MINUS_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_MINUS")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_MULTIPLY")
  @js.native
  def NUM_MULTIPLY: Double = js.native
  inline def NUM_MULTIPLY_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_MULTIPLY")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_NINE")
  @js.native
  def NUM_NINE: Double = js.native
  inline def NUM_NINE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_NINE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_ONE")
  @js.native
  def NUM_ONE: Double = js.native
  inline def NUM_ONE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_ONE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_PERIOD")
  @js.native
  def NUM_PERIOD: Double = js.native
  inline def NUM_PERIOD_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_PERIOD")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_PLUS")
  @js.native
  def NUM_PLUS: Double = js.native
  inline def NUM_PLUS_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_PLUS")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_SEVEN")
  @js.native
  def NUM_SEVEN: Double = js.native
  inline def NUM_SEVEN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_SEVEN")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_SIX")
  @js.native
  def NUM_SIX: Double = js.native
  inline def NUM_SIX_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_SIX")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_THREE")
  @js.native
  def NUM_THREE: Double = js.native
  inline def NUM_THREE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_THREE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_TWO")
  @js.native
  def NUM_TWO: Double = js.native
  inline def NUM_TWO_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_TWO")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.NUM_ZERO")
  @js.native
  def NUM_ZERO: Double = js.native
  inline def NUM_ZERO_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("NUM_ZERO")(x.asInstanceOf[js.Any])
  
  inline def N_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("N")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.O")
  @js.native
  def O: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.ONE")
  @js.native
  def ONE: Double = js.native
  inline def ONE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ONE")(x.asInstanceOf[js.Any])
  
  inline def O_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("O")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.P")
  @js.native
  def P: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.PAGE_DOWN")
  @js.native
  def PAGE_DOWN: Double = js.native
  inline def PAGE_DOWN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("PAGE_DOWN")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.PAGE_UP")
  @js.native
  def PAGE_UP: Double = js.native
  inline def PAGE_UP_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("PAGE_UP")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.PAUSE")
  @js.native
  def PAUSE: Double = js.native
  inline def PAUSE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("PAUSE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.PRINT_SCREEN")
  @js.native
  def PRINT_SCREEN: Double = js.native
  inline def PRINT_SCREEN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("PRINT_SCREEN")(x.asInstanceOf[js.Any])
  
  inline def P_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("P")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.Q")
  @js.native
  def Q: Double = js.native
  inline def Q_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Q")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.R")
  @js.native
  def R: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.RETURN")
  @js.native
  def RETURN: Double = js.native
  inline def RETURN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("RETURN")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.RIGHT")
  @js.native
  def RIGHT: Double = js.native
  inline def RIGHT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("RIGHT")(x.asInstanceOf[js.Any])
  
  inline def R_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("R")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.S")
  @js.native
  def S: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.SEVEN")
  @js.native
  def SEVEN: Double = js.native
  inline def SEVEN_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("SEVEN")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.SHIFT")
  @js.native
  def SHIFT: Double = js.native
  inline def SHIFT_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("SHIFT")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.SIX")
  @js.native
  def SIX: Double = js.native
  inline def SIX_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("SIX")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.SPACE")
  @js.native
  def SPACE: Double = js.native
  inline def SPACE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("SPACE")(x.asInstanceOf[js.Any])
  
  inline def S_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("S")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.T")
  @js.native
  def T: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.TAB")
  @js.native
  def TAB: Double = js.native
  inline def TAB_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("TAB")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.THREE")
  @js.native
  def THREE: Double = js.native
  inline def THREE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("THREE")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.TWO")
  @js.native
  def TWO: Double = js.native
  inline def TWO_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("TWO")(x.asInstanceOf[js.Any])
  
  inline def T_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("T")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.U")
  @js.native
  def U: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.UP")
  @js.native
  def UP: Double = js.native
  inline def UP_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("UP")(x.asInstanceOf[js.Any])
  
  inline def U_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("U")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.V")
  @js.native
  def V: Double = js.native
  inline def V_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("V")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.W")
  @js.native
  def W: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.WHEEL_SCALE")
  @js.native
  def WHEEL_SCALE: Double = js.native
  inline def WHEEL_SCALE_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("WHEEL_SCALE")(x.asInstanceOf[js.Any])
  
  inline def W_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("W")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.X")
  @js.native
  def X: Double = js.native
  inline def X_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("X")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.Y")
  @js.native
  def Y: Double = js.native
  inline def Y_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Y")(x.asInstanceOf[js.Any])
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.Z")
  @js.native
  def Z: Double = js.native
  
  /** [Property] (Number) */
  @JSGlobal("Ext.EventObject.ZERO")
  @js.native
  def ZERO: Double = js.native
  inline def ZERO_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ZERO")(x.asInstanceOf[js.Any])
  
  inline def Z_=(x: Double): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Z")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.EventObject.altKey")
  @js.native
  def altKey: Boolean = js.native
  inline def altKey_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("altKey")(x.asInstanceOf[js.Any])
  
  /** [Method] Correctly scales a given wheel delta
    * @param delta Number The delta value.
    */
  inline def correctWheelDelta(): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("correctWheelDelta")().asInstanceOf[Unit]
  inline def correctWheelDelta(delta: Double): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("correctWheelDelta")(delta.asInstanceOf[js.Any]).asInstanceOf[Unit]
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.EventObject.ctrlKey")
  @js.native
  def ctrlKey: Boolean = js.native
  inline def ctrlKey_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("ctrlKey")(x.asInstanceOf[js.Any])
  
  /** [Method] Gets the character code for the event
    * @returns Number
    */
  inline def getCharCode(): Double = ^.asInstanceOf[js.Dynamic].applyDynamic("getCharCode")().asInstanceOf[Double]
  
  /** [Method] Returns a normalized keyCode for the event
    * @returns Number The key code
    */
  inline def getKey(): Double = ^.asInstanceOf[js.Dynamic].applyDynamic("getKey")().asInstanceOf[Double]
  
  /** [Method] Gets the x coordinate of the event
    * @returns Number
    */
  inline def getPageX(): Double = ^.asInstanceOf[js.Dynamic].applyDynamic("getPageX")().asInstanceOf[Double]
  
  /** [Method] Gets the y coordinate of the event
    * @returns Number
    */
  inline def getPageY(): Double = ^.asInstanceOf[js.Dynamic].applyDynamic("getPageY")().asInstanceOf[Double]
  
  /** [Method] Returns a point object that consists of the object coordinates
    * @returns Ext.util.Point point
    */
  inline def getPoint(): IPoint = ^.asInstanceOf[js.Dynamic].applyDynamic("getPoint")().asInstanceOf[IPoint]
  
  /** [Method] Gets the related target
    * @param selector String A simple selector to filter the target or look for an ancestor of the target
    * @param maxDepth Number/HTMLElement The max depth to search as a number or element (defaults to 10 || document.body)
    * @param returnEl Boolean True to return a Ext.Element object instead of DOM node
    * @returns HTMLElement
    */
  inline def getRelatedTarget(): HTMLElement = ^.asInstanceOf[js.Dynamic].applyDynamic("getRelatedTarget")().asInstanceOf[HTMLElement]
  inline def getRelatedTarget(selector: java.lang.String): HTMLElement = ^.asInstanceOf[js.Dynamic].applyDynamic("getRelatedTarget")(selector.asInstanceOf[js.Any]).asInstanceOf[HTMLElement]
  inline def getRelatedTarget(selector: java.lang.String, maxDepth: Any): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getRelatedTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getRelatedTarget(selector: java.lang.String, maxDepth: Any, returnEl: Boolean): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getRelatedTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any], returnEl.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getRelatedTarget(selector: java.lang.String, maxDepth: Unit, returnEl: Boolean): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getRelatedTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any], returnEl.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getRelatedTarget(selector: Unit, maxDepth: Any): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getRelatedTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getRelatedTarget(selector: Unit, maxDepth: Any, returnEl: Boolean): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getRelatedTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any], returnEl.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getRelatedTarget(selector: Unit, maxDepth: Unit, returnEl: Boolean): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getRelatedTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any], returnEl.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  
  /** [Method] Gets the target for the event
    * @param selector String A simple selector to filter the target or look for an ancestor of the target
    * @param maxDepth Number/HTMLElement The max depth to search as a number or element (defaults to 10 || document.body)
    * @param returnEl Boolean True to return a Ext.Element object instead of DOM node
    * @returns HTMLElement
    */
  inline def getTarget(): HTMLElement = ^.asInstanceOf[js.Dynamic].applyDynamic("getTarget")().asInstanceOf[HTMLElement]
  inline def getTarget(selector: java.lang.String): HTMLElement = ^.asInstanceOf[js.Dynamic].applyDynamic("getTarget")(selector.asInstanceOf[js.Any]).asInstanceOf[HTMLElement]
  inline def getTarget(selector: java.lang.String, maxDepth: Any): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getTarget(selector: java.lang.String, maxDepth: Any, returnEl: Boolean): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any], returnEl.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getTarget(selector: java.lang.String, maxDepth: Unit, returnEl: Boolean): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any], returnEl.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getTarget(selector: Unit, maxDepth: Any): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getTarget(selector: Unit, maxDepth: Any, returnEl: Boolean): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any], returnEl.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  inline def getTarget(selector: Unit, maxDepth: Unit, returnEl: Boolean): HTMLElement = (^.asInstanceOf[js.Dynamic].applyDynamic("getTarget")(selector.asInstanceOf[js.Any], maxDepth.asInstanceOf[js.Any], returnEl.asInstanceOf[js.Any])).asInstanceOf[HTMLElement]
  
  /** [Method] Normalizes mouse wheel y delta across browsers
    * @returns Number The mouse wheel y-delta
    */
  inline def getWheelDelta(): Double = ^.asInstanceOf[js.Dynamic].applyDynamic("getWheelDelta")().asInstanceOf[Double]
  
  /** [Method] Returns the mouse wheel deltas for this event
    * @returns Object An object with "x" and "y" properties holding the mouse wheel deltas.
    */
  inline def getWheelDeltas(): Any = ^.asInstanceOf[js.Dynamic].applyDynamic("getWheelDeltas")().asInstanceOf[Any]
  
  /** [Method] Gets the x coordinate of the event
    * @returns Number
    */
  inline def getX(): Double = ^.asInstanceOf[js.Dynamic].applyDynamic("getX")().asInstanceOf[Double]
  
  /** [Method] Gets the page coordinates of the event
    * @returns Number[] The xy values like [x, y]
    */
  inline def getXY(): typingsJapgolly.extjs.Ext.Array = ^.asInstanceOf[js.Dynamic].applyDynamic("getXY")().asInstanceOf[typingsJapgolly.extjs.Ext.Array]
  
  /** [Method] Gets the y coordinate of the event
    * @returns Number
    */
  inline def getY(): Double = ^.asInstanceOf[js.Dynamic].applyDynamic("getY")().asInstanceOf[Double]
  
  /** [Method] Returns true if the control meta shift or alt key was pressed during this event
    * @returns Boolean
    */
  inline def hasModifier(): Boolean = ^.asInstanceOf[js.Dynamic].applyDynamic("hasModifier")().asInstanceOf[Boolean]
  
  /** [Method] Injects a DOM event using the data in this object and optionally a new target
    * @param target Ext.Element/HTMLElement If specified, the target for the event. This is likely to be used when relaying a DOM event. If not specified, getTarget is used to determine the target.
    */
  inline def injectEvent(): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("injectEvent")().asInstanceOf[Unit]
  inline def injectEvent(target: Any): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("injectEvent")(target.asInstanceOf[js.Any]).asInstanceOf[Unit]
  
  /** [Method] Checks if the key pressed was a navigation key
    * @returns Boolean True if the press is a navigation keypress
    */
  inline def isNavKeyPress(): Boolean = ^.asInstanceOf[js.Dynamic].applyDynamic("isNavKeyPress")().asInstanceOf[Boolean]
  
  /** [Method] Checks if the key pressed was a special key
    * @returns Boolean True if the press is a special keypress
    */
  inline def isSpecialKey(): Boolean = ^.asInstanceOf[js.Dynamic].applyDynamic("isSpecialKey")().asInstanceOf[Boolean]
  
  /** [Method] Prevents the browsers default handling of the event  */
  inline def preventDefault(): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("preventDefault")().asInstanceOf[Unit]
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.EventObject.shiftKey")
  @js.native
  def shiftKey: Boolean = js.native
  inline def shiftKey_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("shiftKey")(x.asInstanceOf[js.Any])
  
  /** [Method] Stop the event preventDefault and stopPropagation  */
  inline def stopEvent(): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("stopEvent")().asInstanceOf[Unit]
  
  /** [Method] Cancels bubbling of the event  */
  inline def stopPropagation(): Unit = ^.asInstanceOf[js.Dynamic].applyDynamic("stopPropagation")().asInstanceOf[Unit]
  
  /** [Method] Returns true if the target of this event is a child of el
    * @param el String/HTMLElement/Ext.Element The id, DOM element or Ext.Element to check
    * @param related Boolean true to test if the related target is within el instead of the target
    * @param allowEl Boolean true to also check if the passed element is the target or related target
    * @returns Boolean
    */
  inline def within(): Boolean = ^.asInstanceOf[js.Dynamic].applyDynamic("within")().asInstanceOf[Boolean]
  inline def within(el: Any): Boolean = ^.asInstanceOf[js.Dynamic].applyDynamic("within")(el.asInstanceOf[js.Any]).asInstanceOf[Boolean]
  inline def within(el: Any, related: Boolean): Boolean = (^.asInstanceOf[js.Dynamic].applyDynamic("within")(el.asInstanceOf[js.Any], related.asInstanceOf[js.Any])).asInstanceOf[Boolean]
  inline def within(el: Any, related: Boolean, allowEl: Boolean): Boolean = (^.asInstanceOf[js.Dynamic].applyDynamic("within")(el.asInstanceOf[js.Any], related.asInstanceOf[js.Any], allowEl.asInstanceOf[js.Any])).asInstanceOf[Boolean]
  inline def within(el: Any, related: Unit, allowEl: Boolean): Boolean = (^.asInstanceOf[js.Dynamic].applyDynamic("within")(el.asInstanceOf[js.Any], related.asInstanceOf[js.Any], allowEl.asInstanceOf[js.Any])).asInstanceOf[Boolean]
  inline def within(el: Unit, related: Boolean): Boolean = (^.asInstanceOf[js.Dynamic].applyDynamic("within")(el.asInstanceOf[js.Any], related.asInstanceOf[js.Any])).asInstanceOf[Boolean]
  inline def within(el: Unit, related: Boolean, allowEl: Boolean): Boolean = (^.asInstanceOf[js.Dynamic].applyDynamic("within")(el.asInstanceOf[js.Any], related.asInstanceOf[js.Any], allowEl.asInstanceOf[js.Any])).asInstanceOf[Boolean]
  inline def within(el: Unit, related: Unit, allowEl: Boolean): Boolean = (^.asInstanceOf[js.Dynamic].applyDynamic("within")(el.asInstanceOf[js.Any], related.asInstanceOf[js.Any], allowEl.asInstanceOf[js.Any])).asInstanceOf[Boolean]
}
