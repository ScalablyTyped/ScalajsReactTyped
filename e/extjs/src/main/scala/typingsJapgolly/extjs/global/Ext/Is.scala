package typingsJapgolly.extjs.global.Ext

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("Ext.Is")
@js.native
open class Is ()
  extends StObject
     with typingsJapgolly.extjs.Ext.Is
/* static members */
object Is {
  
  @JSGlobal("Ext.Is")
  @js.native
  val ^ : js.Any = js.native
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.Android")
  @js.native
  def Android: Boolean = js.native
  inline def Android_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Android")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.Blackberry")
  @js.native
  def Blackberry: Boolean = js.native
  inline def Blackberry_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Blackberry")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.Desktop")
  @js.native
  def Desktop: Boolean = js.native
  inline def Desktop_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Desktop")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.Linux")
  @js.native
  def Linux: Boolean = js.native
  inline def Linux_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Linux")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.Mac")
  @js.native
  def Mac: Boolean = js.native
  inline def Mac_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Mac")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.Phone")
  @js.native
  def Phone: Boolean = js.native
  inline def Phone_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Phone")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.Standalone")
  @js.native
  def Standalone: Boolean = js.native
  inline def Standalone_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Standalone")(x.asInstanceOf[js.Any])
  
  /** [Property] (Object) */
  @JSGlobal("Ext.Is.Tablet")
  @js.native
  def Tablet: Any = js.native
  inline def Tablet_=(x: Any): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Tablet")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.Windows")
  @js.native
  def Windows: Boolean = js.native
  inline def Windows_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("Windows")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.iOS")
  @js.native
  def iOS: Boolean = js.native
  inline def iOS_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("iOS")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.iPad")
  @js.native
  def iPad: Boolean = js.native
  inline def iPad_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("iPad")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.iPhone")
  @js.native
  def iPhone: Boolean = js.native
  inline def iPhone_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("iPhone")(x.asInstanceOf[js.Any])
  
  /** [Property] (Boolean) */
  @JSGlobal("Ext.Is.iPod")
  @js.native
  def iPod: Boolean = js.native
  inline def iPod_=(x: Boolean): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("iPod")(x.asInstanceOf[js.Any])
}
