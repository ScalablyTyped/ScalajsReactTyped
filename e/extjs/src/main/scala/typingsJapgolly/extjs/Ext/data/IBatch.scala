...
object IBatch {
  ...
  extension [Self <: IBatch](x: Self) {
    ...
    inline def setPause(value: CallbackTo[IBatch]): Self = StObject.set(x, "pause", value.toJsFn)
    ...
    inline def setRetry(value: CallbackTo[IBatch]): Self = StObject.set(x, "retry", value.toJsFn)
