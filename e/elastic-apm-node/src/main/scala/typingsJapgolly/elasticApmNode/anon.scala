...
import typingsJapgolly.elasticApmNode.typesConnectMod.Connect.ErrorHandleFunction
...
object anon {
  ...
  object Connect {
    ...
    inline def apply(connect: CallbackTo[ErrorHandleFunction]): Connect = {
      val __obj = js.Dynamic.literal(connect = connect.toJsFn)
      __obj.asInstanceOf[Connect]
    }
    
    extension [Self <: Connect](x: Self) {
      
      inline def setConnect(value: CallbackTo[ErrorHandleFunction]): Self = StObject.set(x, "connect", value.toJsFn)
    }
  }
