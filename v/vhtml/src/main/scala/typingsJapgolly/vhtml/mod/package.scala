...
import typingsJapgolly.vhtml.anon.ChildrenChildren
import typingsJapgolly.vhtml.anon.Dictattr
import typingsJapgolly.vhtml.mod.^
import typingsJapgolly.vhtml.vhtmlStrings.children
...
/**
  ...
  */
type ComponentPropTransform[TComp, TProps] = (SafeEmptyType[typingsJapgolly.std.Omit[TProps, children]]) & (/* import warning: importer.ImportType#apply Failed type conversion: TProps extends {  children :[]} ? {} : TProps extends {  children :[infer ChildType]} ? {  children :ChildType} : TProps extends {  children :[infer ChildType | undefined]} ? {  children :ChildType | undefined} : TProps extends {  children :std.Array<infer ChildrenType>} ? {  children :ChildrenType | std.Array<ChildrenType> | undefined} : TProps extends {  children :any} ? never : {} */ js.Any)
