package typingsJapgolly.vueRuntimeCore

import typingsJapgolly.vueRuntimeCore.mod.BooleanFlags
import typingsJapgolly.vueRuntimeCore.mod.ErrorCodes
import typingsJapgolly.vueRuntimeCore.mod.MoveType
import typingsJapgolly.vueRuntimeCore.mod.TeleportMoveTypes
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object vueRuntimeCoreInts {
  
  @js.native
  sealed trait `0`
    extends StObject
       with BooleanFlags
       with ErrorCodes
       with MoveType
       with TeleportMoveTypes
  inline def `0`: `0` = 0.asInstanceOf[`0`]
  
  @js.native
  sealed trait `1`
    extends StObject
       with BooleanFlags
       with ErrorCodes
       with MoveType
       with TeleportMoveTypes
  inline def `1`: `1` = 1.asInstanceOf[`1`]
  
  @js.native
  sealed trait `10`
    extends StObject
       with ErrorCodes
  inline def `10`: `10` = 10.asInstanceOf[`10`]
  
  @js.native
  sealed trait `11`
    extends StObject
       with ErrorCodes
  inline def `11`: `11` = 11.asInstanceOf[`11`]
  
  @js.native
  sealed trait `12`
    extends StObject
       with ErrorCodes
  inline def `12`: `12` = 12.asInstanceOf[`12`]
  
  @js.native
  sealed trait `13`
    extends StObject
       with ErrorCodes
  inline def `13`: `13` = 13.asInstanceOf[`13`]
  
  @js.native
  sealed trait `14`
    extends StObject
       with ErrorCodes
  inline def `14`: `14` = 14.asInstanceOf[`14`]
  
  @js.native
  sealed trait `2`
    extends StObject
       with ErrorCodes
       with MoveType
       with TeleportMoveTypes
  inline def `2`: `2` = 2.asInstanceOf[`2`]
  
  @js.native
  sealed trait `3`
    extends StObject
       with ErrorCodes
  inline def `3`: `3` = 3.asInstanceOf[`3`]
  
  @js.native
  sealed trait `4`
    extends StObject
       with ErrorCodes
  inline def `4`: `4` = 4.asInstanceOf[`4`]
  
  @js.native
  sealed trait `5`
    extends StObject
       with ErrorCodes
  inline def `5`: `5` = 5.asInstanceOf[`5`]
  
  @js.native
  sealed trait `6`
    extends StObject
       with ErrorCodes
  inline def `6`: `6` = 6.asInstanceOf[`6`]
  
  @js.native
  sealed trait `7`
    extends StObject
       with ErrorCodes
  inline def `7`: `7` = 7.asInstanceOf[`7`]
  
  @js.native
  sealed trait `8`
    extends StObject
       with ErrorCodes
  inline def `8`: `8` = 8.asInstanceOf[`8`]
  
  @js.native
  sealed trait `9`
    extends StObject
       with ErrorCodes
  inline def `9`: `9` = 9.asInstanceOf[`9`]
}
