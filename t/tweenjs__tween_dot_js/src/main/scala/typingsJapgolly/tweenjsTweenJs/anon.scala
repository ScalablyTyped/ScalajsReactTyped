...
object anon {
  ...
  object TypeofSequence {
    ...
    inline def apply(_nextId: Any, nextId: CallbackTo[Double]): TypeofSequence = {
      val __obj = js.Dynamic.literal(_nextId = _nextId.asInstanceOf[js.Any], nextId = nextId.toJsFn)
      __obj.asInstanceOf[TypeofSequence]
    }
    
    extension [Self <: TypeofSequence](x: Self) {
      
      inline def setNextId(value: CallbackTo[Double]): Self = StObject.set(x, "nextId", value.toJsFn)
