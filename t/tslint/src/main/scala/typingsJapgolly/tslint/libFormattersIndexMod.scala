package typingsJapgolly.tslint

import typingsJapgolly.tslint.libFormattersCodeFrameFormatterMod.Formatter
import typingsJapgolly.tslint.libLanguageFormatterFormatterMod.IFormatterMetadata
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object libFormattersIndexMod {
  
  @JSImport("tslint/lib/formatters/index", "CodeFrameFormatter")
  @js.native
  open class CodeFrameFormatter () extends Formatter
  /* static members */
  object CodeFrameFormatter {
    
    @JSImport("tslint/lib/formatters/index", "CodeFrameFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "CodeFrameFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
  
  @JSImport("tslint/lib/formatters/index", "FileslistFormatter")
  @js.native
  open class FileslistFormatter ()
    extends typingsJapgolly.tslint.libFormattersFileslistFormatterMod.Formatter
  /* static members */
  object FileslistFormatter {
    
    @JSImport("tslint/lib/formatters/index", "FileslistFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "FileslistFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
  
  @JSImport("tslint/lib/formatters/index", "JUnitFormatter")
  @js.native
  open class JUnitFormatter ()
    extends typingsJapgolly.tslint.libFormattersJunitFormatterMod.Formatter
  /* static members */
  object JUnitFormatter {
    
    @JSImport("tslint/lib/formatters/index", "JUnitFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "JUnitFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
  
  @JSImport("tslint/lib/formatters/index", "JsonFormatter")
  @js.native
  open class JsonFormatter ()
    extends typingsJapgolly.tslint.libFormattersJsonFormatterMod.Formatter
  /* static members */
  object JsonFormatter {
    
    @JSImport("tslint/lib/formatters/index", "JsonFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "JsonFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
  
  @JSImport("tslint/lib/formatters/index", "PmdFormatter")
  @js.native
  open class PmdFormatter ()
    extends typingsJapgolly.tslint.libFormattersPmdFormatterMod.Formatter
  /* static members */
  object PmdFormatter {
    
    @JSImport("tslint/lib/formatters/index", "PmdFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "PmdFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
  
  @JSImport("tslint/lib/formatters/index", "ProseFormatter")
  @js.native
  open class ProseFormatter ()
    extends typingsJapgolly.tslint.libFormattersProseFormatterMod.Formatter
  /* static members */
  object ProseFormatter {
    
    @JSImport("tslint/lib/formatters/index", "ProseFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "ProseFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
  
  @JSImport("tslint/lib/formatters/index", "StylishFormatter")
  @js.native
  open class StylishFormatter ()
    extends typingsJapgolly.tslint.libFormattersStylishFormatterMod.Formatter
  /* static members */
  object StylishFormatter {
    
    @JSImport("tslint/lib/formatters/index", "StylishFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "StylishFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
  
  @JSImport("tslint/lib/formatters/index", "TapFormatter")
  @js.native
  open class TapFormatter ()
    extends typingsJapgolly.tslint.libFormattersTapFormatterMod.Formatter
  /* static members */
  object TapFormatter {
    
    @JSImport("tslint/lib/formatters/index", "TapFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "TapFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
  
  @JSImport("tslint/lib/formatters/index", "VerboseFormatter")
  @js.native
  open class VerboseFormatter ()
    extends typingsJapgolly.tslint.libFormattersVerboseFormatterMod.Formatter
  /* static members */
  object VerboseFormatter {
    
    @JSImport("tslint/lib/formatters/index", "VerboseFormatter")
    @js.native
    val ^ : js.Any = js.native
    
    @JSImport("tslint/lib/formatters/index", "VerboseFormatter.metadata")
    @js.native
    def metadata: IFormatterMetadata = js.native
    inline def metadata_=(x: IFormatterMetadata): Unit = ^.asInstanceOf[js.Dynamic].updateDynamic("metadata")(x.asInstanceOf[js.Any])
  }
}
