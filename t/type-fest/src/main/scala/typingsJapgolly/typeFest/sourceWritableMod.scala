...
object sourceWritableMod {
  ...
  Except[BaseType, Keys]) & (/* import warning: importer.ImportType#apply Failed type conversion: // Pick the keys that should be writable from the base type and make them writable by removing the `readonly` modifier from the key.
