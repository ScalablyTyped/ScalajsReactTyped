...
trait Async extends StObject {
  ...
  var sync: typingsJapgolly.tsToolbelt.anon.List
}
object Async {
  
  inline def apply(async: Multi, sync: typingsJapgolly.tsToolbelt.anon.List): Async = {
  ...
  extension [Self <: Async](x: Self) {
    ...
    inline def setSync(value: typingsJapgolly.tsToolbelt.anon.List): Self = StObject.set(x, "sync", value.asInstanceOf[js.Any])
  }
}
