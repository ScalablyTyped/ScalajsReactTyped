...
trait RW[Path /* <: typingsJapgolly.tsToolbelt.outListListMod.List[Key] */, A] extends StObject {
...
object RW {
  ...
  inline def apply[Path /* <: typingsJapgolly.tsToolbelt.outListListMod.List[Key] */, A](Exclamationmark: R[Path, A], Questionmark: W[Path, A]): RW[Path, A] = {
  ...
  extension [Self <: RW[?, ?], Path /* <: typingsJapgolly.tsToolbelt.outListListMod.List[Key] */, A](x: Self & (RW[Path, A])) {
