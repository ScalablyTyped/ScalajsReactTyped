...
object mod {
  ...
  open class UnsupportedTerminalError ()
    ...
    @JSName("name")
    val name_UnsupportedTerminalError: typingsJapgolly.termImg.termImgStrings.UnsupportedTerminalError = js.native
  }
  ...
  object Options {
    ...
    extension [Self <: Options[?], FallbackType](x: Self & Options[FallbackType]) {
      ...
      inline def setFallback(value: CallbackTo[FallbackType]): Self = StObject.set(x, "fallback", value.toJsFn)
