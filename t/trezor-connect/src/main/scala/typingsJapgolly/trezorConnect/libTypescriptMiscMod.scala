...
import typingsJapgolly.trezorConnect.anon.Params
...
object libTypescriptMiscMod {
  ...
  object RequestLoginAsync {
    ...
    inline def apply(callback: CallbackTo[LoginChallenge]): RequestLoginAsync = {
      val __obj = js.Dynamic.literal(callback = callback.toJsFn)
    ...
    extension [Self <: RequestLoginAsync](x: Self) {
      ...
      inline def setCallback(value: CallbackTo[LoginChallenge]): Self = StObject.set(x, "callback", value.toJsFn)
    }
  }
