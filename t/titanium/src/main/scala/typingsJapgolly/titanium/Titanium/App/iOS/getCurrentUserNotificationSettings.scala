package typingsJapgolly.titanium.Titanium.App.iOS

import typingsJapgolly.titanium.UserNotificationSettings
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSGlobal("Titanium.App.iOS.getCurrentUserNotificationSettings")
@js.native
object getCurrentUserNotificationSettings extends js.Object {
  /**
  			 * Gets the value of the <Titanium.App.iOS.currentUserNotificationSettings> property.
  			 */
  def apply(): UserNotificationSettings = js.native
}

