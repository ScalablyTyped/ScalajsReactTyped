...
object mod {
  ...
  object TwixIter {
    ...
    inline def apply(hasNext: CallbackTo[Boolean], next: CallbackTo[Twix]): TwixIter = {
    ...
    extension [Self <: TwixIter](x: Self) {
      ...
      inline def setNext(value: CallbackTo[Twix]): Self = StObject.set(x, "next", value.toJsFn)
    }
  }
