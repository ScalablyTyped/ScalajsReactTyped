package typingsJapgolly.tensorflowTfjsBackendWebgl

import typingsJapgolly.tensorflowTfjsCore.distKernelRegistryMod.KernelConfig
import typingsJapgolly.tensorflowTfjsCore.distKernelRegistryMod.KernelFunc
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object distKernelsLogicalAndMod {
  
  @JSImport("@tensorflow/tfjs-backend-webgl/dist/kernels/LogicalAnd", "logicalAnd")
  @js.native
  val logicalAnd: KernelFunc = js.native
  
  @JSImport("@tensorflow/tfjs-backend-webgl/dist/kernels/LogicalAnd", "logicalAndConfig")
  @js.native
  val logicalAndConfig: KernelConfig = js.native
}
