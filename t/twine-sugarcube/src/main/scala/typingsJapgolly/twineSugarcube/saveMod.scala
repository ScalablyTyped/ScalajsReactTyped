...
import typingsJapgolly.twineSugarcube.anon.Count
import typingsJapgolly.twineSugarcube.anon.Get
import typingsJapgolly.twineSugarcube.extensionsMod.global.Array
import typingsJapgolly.twineSugarcube.twineSugarcubeStrings.autosave
import typingsJapgolly.twineSugarcube.twineSugarcubeStrings.disk
import typingsJapgolly.twineSugarcube.twineSugarcubeStrings.serialize
import typingsJapgolly.twineSugarcube.twineSugarcubeStrings.slot
...
object saveMod {
  ...
  object SaveEventAPI {
    ...
    inline def apply[HandlerType](add: HandlerType => Callback, clear: Callback, delete: HandlerType => Boolean, size: Double): SaveEventAPI[HandlerType] = {
      val __obj = js.Dynamic.literal(add = js.Any.fromFunction1((t0: HandlerType) => add(t0).runNow()), clear = clear.toJsFn, delete = js.Any.fromFunction1(delete), size = size.asInstanceOf[js.Any])
      __obj.asInstanceOf[SaveEventAPI[HandlerType]]
    }
    
    extension [Self <: SaveEventAPI[?], HandlerType](x: Self & SaveEventAPI[HandlerType]) {
      
      inline def setAdd(value: HandlerType => Callback): Self = StObject.set(x, "add", js.Any.fromFunction1((t0: HandlerType) => value(t0).runNow()))
