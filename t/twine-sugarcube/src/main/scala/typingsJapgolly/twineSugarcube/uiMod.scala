...
import typingsJapgolly.twineSugarcube.extensionsMod.global.JQuery
import typingsJapgolly.twineSugarcube.twineSugarcubeStrings.auto
import typingsJapgolly.twineSugarcube.twineSugarcubeStrings.hide
import typingsJapgolly.twineSugarcube.twineSugarcubeStrings.show
...
object uiMod {
  ...
  object LoadScreenAPI {
    ...
    inline def apply(lock: CallbackTo[Double], unlock: Double => Callback): LoadScreenAPI = {
      val __obj = js.Dynamic.literal(lock = lock.toJsFn, unlock = js.Any.fromFunction1((t0: Double) => unlock(t0).runNow()))
      __obj.asInstanceOf[LoadScreenAPI]
    }
    
    extension [Self <: LoadScreenAPI](x: Self) {
      
      inline def setLock(value: CallbackTo[Double]): Self = StObject.set(x, "lock", value.toJsFn)
      
      inline def setUnlock(value: Double => Callback): Self = StObject.set(x, "unlock", js.Any.fromFunction1((t0: Double) => value(t0).runNow()))
    }
  }
