...
import typingsJapgolly.tensorflowModelsFaceLandmarksDetection.tensorflowModelsFaceLandmarksDetectionStrings.canvasimagesource
import typingsJapgolly.tensorflowModelsFaceLandmarksDetection.tensorflowModelsFaceLandmarksDetectionStrings.imagedata
import typingsJapgolly.tensorflowModelsFaceLandmarksDetection.tensorflowModelsFaceLandmarksDetectionStrings.tensor
...
object distSharedCalculatorsInterfacesCommonInterfacesMod {
  ...
  object KeypointsFilter {
    ...
    inline def apply(apply: (js.Array[Keypoint], Double, Double) => js.Array[Keypoint], reset: Callback): KeypointsFilter = {
      val __obj = js.Dynamic.literal(apply = js.Any.fromFunction3(apply), reset = reset.toJsFn)
  ...
  object Mask {
    ...
    inline def apply(
      getUnderlyingType: CallbackTo[canvasimagesource | imagedata | tensor],
      toCanvasImageSource: CallbackTo[js.Promise[CanvasImageSource]],
      toImageData: CallbackTo[js.Promise[ImageData]],
      toTensor: CallbackTo[js.Promise[Tensor3D]]
    ): Mask = {
      val __obj = js.Dynamic.literal(getUnderlyingType = getUnderlyingType.toJsFn, toCanvasImageSource = toCanvasImageSource.toJsFn, toImageData = toImageData.toJsFn, toTensor = toTensor.toJsFn)
      __obj.asInstanceOf[Mask]
    }
    
    extension [Self <: Mask](x: Self) {
      
      inline def setGetUnderlyingType(value: CallbackTo[canvasimagesource | imagedata | tensor]): Self = StObject.set(x, "getUnderlyingType", value.toJsFn)
      
      inline def setToCanvasImageSource(value: CallbackTo[js.Promise[CanvasImageSource]]): Self = StObject.set(x, "toCanvasImageSource", value.toJsFn)
      
      inline def setToImageData(value: CallbackTo[js.Promise[ImageData]]): Self = StObject.set(x, "toImageData", value.toJsFn)
      
      inline def setToTensor(value: CallbackTo[js.Promise[Tensor3D]]): Self = StObject.set(x, "toTensor", value.toJsFn)
    }
  }
