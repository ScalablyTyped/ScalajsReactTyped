...
import typingsJapgolly.tsInterfaceChecker.tsInterfaceCheckerBooleans.`false`
...
object distUtilMod {
  ...
  object IUnionResolver {
    ...
    inline def apply(createContext: CallbackTo[IContext]): IUnionResolver = {
      val __obj = js.Dynamic.literal(createContext = createContext.toJsFn)
      __obj.asInstanceOf[IUnionResolver]
    }
    
    extension [Self <: IUnionResolver](x: Self) {
      
      inline def setCreateContext(value: CallbackTo[IContext]): Self = StObject.set(x, "createContext", value.toJsFn)
    }
  }
}
