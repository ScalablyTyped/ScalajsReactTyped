...
object mod {
  ...
  /* Rewritten from type alias, can be one of: 
    - typingsJapgolly.transitJs.transitJsStrings.json
    - typingsJapgolly.transitJs.transitJsStrings.`json-verbose`
  */
  trait Encodings extends StObject
  object Encodings {
    
    inline def json: typingsJapgolly.transitJs.transitJsStrings.json = "json".asInstanceOf[typingsJapgolly.transitJs.transitJsStrings.json]
    
    inline def `json-verbose`: typingsJapgolly.transitJs.transitJsStrings.`json-verbose` = "json-verbose".asInstanceOf[typingsJapgolly.transitJs.transitJsStrings.`json-verbose`]
  }
  ...
  object TransitBuilder {
    ...
    inline def apply[T](add: /* repeated */ Any => T, finalize_ : T => T, init: CallbackTo[T]): TransitBuilder[T] = {
      val __obj = js.Dynamic.literal(add = js.Any.fromFunction1(add), init = init.toJsFn)
      __obj.updateDynamic("finalize")(js.Any.fromFunction1(finalize_))
    ...
    extension [Self <: TransitBuilder[?], T](x: Self & TransitBuilder[T]) {
      ...
      inline def setInit(value: CallbackTo[T]): Self = StObject.set(x, "init", value.toJsFn)
    }
  }
