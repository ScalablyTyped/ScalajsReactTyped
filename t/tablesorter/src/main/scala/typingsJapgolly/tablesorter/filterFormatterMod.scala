package typingsJapgolly.tablesorter

import typingsJapgolly.tablesorter.html5FormatterMod.Html5Formatter
import typingsJapgolly.tablesorter.select2FormatterMod.Select2Formatter
import typingsJapgolly.tablesorter.uiformatterMod.UIFormatter
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

@JSImport("tablesorter/Filtering/Formatter/FilterFormatter", JSImport.Namespace)
@js.native
object filterFormatterMod extends js.Object {
  @js.native
  trait FilterFormatter
    extends Html5Formatter
       with UIFormatter
       with Select2Formatter
  
}

