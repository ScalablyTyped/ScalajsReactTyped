package typingsJapgolly.typescriptTuple

import typingsJapgolly.typescriptTuple.typescriptTupleStrings.Infinite
import typingsJapgolly.typescriptTuple.typescriptTupleStrings.InfiniteTupleSet
import typingsJapgolly.typescriptTuple.typescriptTupleStrings.`TupleSet is not finite`
import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

trait AnonCODENAMEERROR extends js.Object {
  var CODENAME: InfiniteTupleSet with Infinite
  var ERROR: `TupleSet is not finite`
}

object AnonCODENAMEERROR {
  @scala.inline
  def apply(CODENAME: InfiniteTupleSet with Infinite, ERROR: `TupleSet is not finite`): AnonCODENAMEERROR = {
    val __obj = js.Dynamic.literal(CODENAME = CODENAME.asInstanceOf[js.Any], ERROR = ERROR.asInstanceOf[js.Any])
  
    __obj.asInstanceOf[AnonCODENAMEERROR]
  }
}

