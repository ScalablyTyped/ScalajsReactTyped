...
object distOpsRandUtilMod {
  ...
  object RandomBase {
    ...
    inline def apply(nextValue: CallbackTo[Double]): RandomBase = {
      val __obj = js.Dynamic.literal(nextValue = nextValue.toJsFn)
      __obj.asInstanceOf[RandomBase]
    }
    
    extension [Self <: RandomBase](x: Self) {
      
      inline def setNextValue(value: CallbackTo[Double]): Self = StObject.set(x, "nextValue", value.toJsFn)
    }
  }
  ...
  object RandomGamma {
    ...
    inline def apply(nextValue: CallbackTo[Double]): RandomGamma = {
      val __obj = js.Dynamic.literal(nextValue = nextValue.toJsFn)
