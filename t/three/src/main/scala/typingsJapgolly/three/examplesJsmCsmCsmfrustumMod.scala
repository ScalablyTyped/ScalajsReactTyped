...
object examplesJsmCsmCsmfrustumMod {
  ...
  object CSMFrustum {
    ...
    inline def apply(
      ...
      split: (js.Array[Double], js.Array[CSMFrustum]) => Callback,
      toSpace: (Matrix4, CSMFrustum) => Callback,
      vertices: CSMFrustumVerticies
    ): CSMFrustum = {
      val __obj = js.Dynamic.literal(setFromProjectionMatrix = js.Any.fromFunction2(setFromProjectionMatrix), split = js.Any.fromFunction2((t0: js.Array[Double], t1: js.Array[CSMFrustum]) => (split(t0, t1)).runNow()), toSpace = js.Any.fromFunction2((t0: Matrix4, t1: CSMFrustum) => (toSpace(t0, t1)).runNow()), vertices = vertices.asInstanceOf[js.Any])
    ...
    extension [Self <: CSMFrustum](x: Self) {
      ...
      inline def setSplit(value: (js.Array[Double], js.Array[CSMFrustum]) => Callback): Self = StObject.set(x, "split", js.Any.fromFunction2((t0: js.Array[Double], t1: js.Array[CSMFrustum]) => (value(t0, t1)).runNow()))
      
      inline def setToSpace(value: (Matrix4, CSMFrustum) => Callback): Self = StObject.set(x, "toSpace", js.Any.fromFunction2((t0: Matrix4, t1: CSMFrustum) => (value(t0, t1)).runNow()))
