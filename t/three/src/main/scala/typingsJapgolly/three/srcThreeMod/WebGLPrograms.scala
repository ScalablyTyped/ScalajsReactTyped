package typingsJapgolly.three.srcThreeMod

import typingsJapgolly.three.srcRenderersWebglWebGLBindingStatesMod.WebGLBindingStates
import typingsJapgolly.three.srcRenderersWebglWebGLCubeMapsMod.WebGLCubeMaps
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSImport("three/src/Three", "WebGLPrograms")
@js.native
open class WebGLPrograms protected ()
  extends typingsJapgolly.three.srcRenderersWebglWebGLProgramsMod.WebGLPrograms {
  def this(
    renderer: typingsJapgolly.three.srcRenderersWebGLRendererMod.WebGLRenderer,
    cubemaps: WebGLCubeMaps,
    extensions: typingsJapgolly.three.srcRenderersWebglWebGLExtensionsMod.WebGLExtensions,
    capabilities: typingsJapgolly.three.srcRenderersWebglWebGLCapabilitiesMod.WebGLCapabilities,
    bindingStates: WebGLBindingStates,
    clipping: typingsJapgolly.three.srcRenderersWebglWebGLClippingMod.WebGLClipping
  ) = this()
}
