package typingsJapgolly.typescriptServices.global.TypeScript.Services

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("TypeScript.Services.FormatCodeOptions")
@js.native
open class FormatCodeOptions ()
  extends StObject
     with typingsJapgolly.typescriptServices.TypeScript.Services.FormatCodeOptions {
  
  /* CompleteClass */
  var ConvertTabsToSpaces: Boolean = js.native
  
  /* CompleteClass */
  var IndentSize: Double = js.native
  
  /* CompleteClass */
  var InsertSpaceAfterCommaDelimiter: Boolean = js.native
  
  /* CompleteClass */
  var InsertSpaceAfterFunctionKeywordForAnonymousFunctions: Boolean = js.native
  
  /* CompleteClass */
  var InsertSpaceAfterKeywordsInControlFlowStatements: Boolean = js.native
  
  /* CompleteClass */
  var InsertSpaceAfterOpeningAndBeforeClosingNonemptyParenthesis: Boolean = js.native
  
  /* CompleteClass */
  var InsertSpaceAfterSemicolonInForStatements: Boolean = js.native
  
  /* CompleteClass */
  var InsertSpaceBeforeAndAfterBinaryOperators: Boolean = js.native
  
  /* CompleteClass */
  var NewLineCharacter: String = js.native
  
  /* CompleteClass */
  var PlaceOpenBraceOnNewLineForControlBlocks: Boolean = js.native
  
  /* CompleteClass */
  var PlaceOpenBraceOnNewLineForFunctions: Boolean = js.native
  
  /* CompleteClass */
  var TabSize: Double = js.native
}
object FormatCodeOptions {
  
  @JSGlobal("TypeScript.Services.FormatCodeOptions")
  @js.native
  val ^ : js.Any = js.native
  
  /* static member */
  inline def clone_(objectToClone: typingsJapgolly.typescriptServices.TypeScript.Services.FormatCodeOptions): typingsJapgolly.typescriptServices.TypeScript.Services.FormatCodeOptions = ^.asInstanceOf[js.Dynamic].applyDynamic("clone")(objectToClone.asInstanceOf[js.Any]).asInstanceOf[typingsJapgolly.typescriptServices.TypeScript.Services.FormatCodeOptions]
}
