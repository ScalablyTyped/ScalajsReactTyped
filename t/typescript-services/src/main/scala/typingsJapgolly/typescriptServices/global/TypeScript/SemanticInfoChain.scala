package typingsJapgolly.typescriptServices.global.TypeScript

import typingsJapgolly.typescriptServices.TypeScript.ILogger
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("TypeScript.SemanticInfoChain")
@js.native
open class SemanticInfoChain protected ()
  extends StObject
     with typingsJapgolly.typescriptServices.TypeScript.SemanticInfoChain {
  def this(compiler: typingsJapgolly.typescriptServices.TypeScript.TypeScriptCompiler, logger: ILogger) = this()
}
