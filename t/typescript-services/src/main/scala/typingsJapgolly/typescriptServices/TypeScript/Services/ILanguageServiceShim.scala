...
object ILanguageServiceShim {
  ...
  extension [Self <: ILanguageServiceShim](x: Self) {
    ...
    inline def setGetCompilerOptionsDiagnostics(value: CallbackTo[String]): Self = StObject.set(x, "getCompilerOptionsDiagnostics", value.toJsFn)
    ...
    inline def setRefresh(value: Boolean => Callback): Self = StObject.set(x, "refresh", js.Any.fromFunction1((t0: Boolean) => value(t0).runNow()))
  }
}
