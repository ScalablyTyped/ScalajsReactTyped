package typingsJapgolly.typescript.mod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Rewritten from type alias, can be one of: 
  - typingsJapgolly.typescript.mod.SignatureDeclaration
  - typingsJapgolly.typescript.mod.VariableDeclaration
  - typingsJapgolly.typescript.mod.ParameterDeclaration
  - typingsJapgolly.typescript.mod.PropertySignature
  - typingsJapgolly.typescript.mod.PropertyDeclaration
  - typingsJapgolly.typescript.mod.TypePredicateNode
  - typingsJapgolly.typescript.mod.ParenthesizedTypeNode
  - typingsJapgolly.typescript.mod.TypeOperatorNode
  - typingsJapgolly.typescript.mod.MappedTypeNode
  - typingsJapgolly.typescript.mod.AssertionExpression
  - typingsJapgolly.typescript.mod.TypeAliasDeclaration
  - typingsJapgolly.typescript.mod.JSDocTypeExpression
  - typingsJapgolly.typescript.mod.JSDocNonNullableType
  - typingsJapgolly.typescript.mod.JSDocNullableType
  - typingsJapgolly.typescript.mod.JSDocOptionalType
  - typingsJapgolly.typescript.mod.JSDocVariadicType
*/
trait HasType extends js.Object

