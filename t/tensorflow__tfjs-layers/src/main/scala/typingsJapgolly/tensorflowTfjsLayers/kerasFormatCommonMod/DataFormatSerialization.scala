package typingsJapgolly.tensorflowTfjsLayers.kerasFormatCommonMod

import scala.scalajs.js
import scala.scalajs.js.`|`
import scala.scalajs.js.annotation._

/* Rewritten from type alias, can be one of: 
  - typingsJapgolly.tensorflowTfjsLayers.tensorflowTfjsLayersStrings.channels_first
  - typingsJapgolly.tensorflowTfjsLayers.tensorflowTfjsLayersStrings.channels_last
*/
trait DataFormatSerialization extends js.Object

object DataFormatSerialization {
  @scala.inline
  /* private */ def cast[T](in: js.Any): T = in.asInstanceOf[T]
  @scala.inline
  def channels_first: typingsJapgolly.tensorflowTfjsLayers.tensorflowTfjsLayersStrings.channels_first = this.cast("channels_first")
  @scala.inline
  def channels_last: typingsJapgolly.tensorflowTfjsLayers.tensorflowTfjsLayersStrings.channels_last = this.cast("channels_last")
}

