...
object mod {
  ...
  object Tether {
    ...
    inline def apply(
      ...
      val __obj = js.Dynamic.literal(destroy = destroy.toJsFn, disable = disable.toJsFn, enable = enable.toJsFn, position = position.toJsFn, setOptions = js.Any.fromFunction1((t0: ITetherOptions) => setOptions(t0).runNow()))
    ...
    extension [Self <: Tether](x: Self) {
      ...
      inline def setPosition(value: Callback): Self = StObject.set(x, "position", value.toJsFn)
      
      inline def setSetOptions(value: ITetherOptions => Callback): Self = StObject.set(x, "setOptions", js.Any.fromFunction1((t0: ITetherOptions) => value(t0).runNow()))
    }
  }
}
