package typingsJapgolly.flatpickr

import typingsJapgolly.flatpickr.distTypesLocaleMod.key
import typingsJapgolly.flatpickr.distTypesOptionsMod.HookKey
import typingsJapgolly.flatpickr.distUtilsFormattingMod.token
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object flatpickrStrings {
  
  @js.native
  sealed trait default
    extends StObject
       with key
  inline def default: default = "default".asInstanceOf[default]
  
  @js.native
  sealed trait D
    extends StObject
       with token
  inline def D: D = "D".asInstanceOf[D]
  
  @js.native
  sealed trait F
    extends StObject
       with token
  inline def F: F = "F".asInstanceOf[F]
  
  @js.native
  sealed trait G
    extends StObject
       with token
  inline def G: G = "G".asInstanceOf[G]
  
  @js.native
  sealed trait H
    extends StObject
       with token
  inline def H: H = "H".asInstanceOf[H]
  
  @js.native
  sealed trait J
    extends StObject
       with token
  inline def J: J = "J".asInstanceOf[J]
  
  @js.native
  sealed trait K
    extends StObject
       with token
  inline def K: K = "K".asInstanceOf[K]
  
  @js.native
  sealed trait M
    extends StObject
       with token
  inline def M: M = "M".asInstanceOf[M]
  
  @js.native
  sealed trait S
    extends StObject
       with token
  inline def S: S = "S".asInstanceOf[S]
  
  @js.native
  sealed trait U
    extends StObject
       with token
  inline def U: U = "U".asInstanceOf[U]
  
  @js.native
  sealed trait W
    extends StObject
       with token
  inline def W: W = "W".asInstanceOf[W]
  
  @js.native
  sealed trait Y
    extends StObject
       with token
  inline def Y: Y = "Y".asInstanceOf[Y]
  
  @js.native
  sealed trait Z
    extends StObject
       with token
  inline def Z: Z = "Z".asInstanceOf[Z]
  
  @js.native
  sealed trait a extends StObject
  inline def a: a = "a".asInstanceOf[a]
  
  @js.native
  sealed trait abbr extends StObject
  inline def abbr: abbr = "abbr".asInstanceOf[abbr]
  
  @js.native
  sealed trait above extends StObject
  inline def above: above = "above".asInstanceOf[above]
  
  @js.native
  sealed trait `above center` extends StObject
  inline def `above center`: `above center` = ("above center").asInstanceOf[`above center`]
  
  @js.native
  sealed trait `above left` extends StObject
  inline def `above left`: `above left` = ("above left").asInstanceOf[`above left`]
  
  @js.native
  sealed trait `above right` extends StObject
  inline def `above right`: `above right` = ("above right").asInstanceOf[`above right`]
  
  @js.native
  sealed trait address extends StObject
  inline def address: address = "address".asInstanceOf[address]
  
  @js.native
  sealed trait allowInput extends StObject
  inline def allowInput: allowInput = "allowInput".asInstanceOf[allowInput]
  
  @js.native
  sealed trait allowInvalidPreload extends StObject
  inline def allowInvalidPreload: allowInvalidPreload = "allowInvalidPreload".asInstanceOf[allowInvalidPreload]
  
  @js.native
  sealed trait altFormat extends StObject
  inline def altFormat: altFormat = "altFormat".asInstanceOf[altFormat]
  
  @js.native
  sealed trait altInput extends StObject
  inline def altInput: altInput = "altInput".asInstanceOf[altInput]
  
  @js.native
  sealed trait altInputClass extends StObject
  inline def altInputClass: altInputClass = "altInputClass".asInstanceOf[altInputClass]
  
  @js.native
  sealed trait animate extends StObject
  inline def animate: animate = "animate".asInstanceOf[animate]
  
  @js.native
  sealed trait appendTo extends StObject
  inline def appendTo: appendTo = "appendTo".asInstanceOf[appendTo]
  
  @js.native
  sealed trait ar
    extends StObject
       with key
  inline def ar: ar = "ar".asInstanceOf[ar]
  
  @js.native
  sealed trait area extends StObject
  inline def area: area = "area".asInstanceOf[area]
  
  @js.native
  sealed trait ariaDateFormat extends StObject
  inline def ariaDateFormat: ariaDateFormat = "ariaDateFormat".asInstanceOf[ariaDateFormat]
  
  @js.native
  sealed trait article extends StObject
  inline def article: article = "article".asInstanceOf[article]
  
  @js.native
  sealed trait aside extends StObject
  inline def aside: aside = "aside".asInstanceOf[aside]
  
  @js.native
  sealed trait at
    extends StObject
       with key
  inline def at: at = "at".asInstanceOf[at]
  
  @js.native
  sealed trait audio extends StObject
  inline def audio: audio = "audio".asInstanceOf[audio]
  
  @js.native
  sealed trait auto extends StObject
  inline def auto: auto = "auto".asInstanceOf[auto]
  
  @js.native
  sealed trait `auto center` extends StObject
  inline def `auto center`: `auto center` = ("auto center").asInstanceOf[`auto center`]
  
  @js.native
  sealed trait `auto left` extends StObject
  inline def `auto left`: `auto left` = ("auto left").asInstanceOf[`auto left`]
  
  @js.native
  sealed trait `auto right` extends StObject
  inline def `auto right`: `auto right` = ("auto right").asInstanceOf[`auto right`]
  
  @js.native
  sealed trait autoFillDefaultTime extends StObject
  inline def autoFillDefaultTime: autoFillDefaultTime = "autoFillDefaultTime".asInstanceOf[autoFillDefaultTime]
  
  @js.native
  sealed trait az
    extends StObject
       with key
  inline def az: az = "az".asInstanceOf[az]
  
  @js.native
  sealed trait b extends StObject
  inline def b: b = "b".asInstanceOf[b]
  
  @js.native
  sealed trait base extends StObject
  inline def base: base = "base".asInstanceOf[base]
  
  @js.native
  sealed trait bdi extends StObject
  inline def bdi: bdi = "bdi".asInstanceOf[bdi]
  
  @js.native
  sealed trait bdo extends StObject
  inline def bdo: bdo = "bdo".asInstanceOf[bdo]
  
  @js.native
  sealed trait be
    extends StObject
       with key
  inline def be: be = "be".asInstanceOf[be]
  
  @js.native
  sealed trait below extends StObject
  inline def below: below = "below".asInstanceOf[below]
  
  @js.native
  sealed trait `below center` extends StObject
  inline def `below center`: `below center` = ("below center").asInstanceOf[`below center`]
  
  @js.native
  sealed trait `below left` extends StObject
  inline def `below left`: `below left` = ("below left").asInstanceOf[`below left`]
  
  @js.native
  sealed trait `below right` extends StObject
  inline def `below right`: `below right` = ("below right").asInstanceOf[`below right`]
  
  @js.native
  sealed trait bg
    extends StObject
       with key
  inline def bg: bg = "bg".asInstanceOf[bg]
  
  @js.native
  sealed trait blockquote extends StObject
  inline def blockquote: blockquote = "blockquote".asInstanceOf[blockquote]
  
  @js.native
  sealed trait bn
    extends StObject
       with key
  inline def bn: bn = "bn".asInstanceOf[bn]
  
  @js.native
  sealed trait body extends StObject
  inline def body: body = "body".asInstanceOf[body]
  
  @js.native
  sealed trait br extends StObject
  inline def br: br = "br".asInstanceOf[br]
  
  @js.native
  sealed trait bs
    extends StObject
       with key
  inline def bs: bs = "bs".asInstanceOf[bs]
  
  @js.native
  sealed trait button extends StObject
  inline def button: button = "button".asInstanceOf[button]
  
  @js.native
  sealed trait ca
    extends StObject
       with key
  inline def ca: ca = "ca".asInstanceOf[ca]
  
  @js.native
  sealed trait canvas extends StObject
  inline def canvas: canvas = "canvas".asInstanceOf[canvas]
  
  @js.native
  sealed trait caption extends StObject
  inline def caption: caption = "caption".asInstanceOf[caption]
  
  @js.native
  sealed trait cat
    extends StObject
       with key
  inline def cat: cat = "cat".asInstanceOf[cat]
  
  @js.native
  sealed trait cite extends StObject
  inline def cite: cite = "cite".asInstanceOf[cite]
  
  @js.native
  sealed trait ckb
    extends StObject
       with key
  inline def ckb: ckb = "ckb".asInstanceOf[ckb]
  
  @js.native
  sealed trait clickOpens extends StObject
  inline def clickOpens: clickOpens = "clickOpens".asInstanceOf[clickOpens]
  
  @js.native
  sealed trait closeOnSelect extends StObject
  inline def closeOnSelect: closeOnSelect = "closeOnSelect".asInstanceOf[closeOnSelect]
  
  @js.native
  sealed trait code extends StObject
  inline def code: code = "code".asInstanceOf[code]
  
  @js.native
  sealed trait col extends StObject
  inline def col: col = "col".asInstanceOf[col]
  
  @js.native
  sealed trait colgroup extends StObject
  inline def colgroup: colgroup = "colgroup".asInstanceOf[colgroup]
  
  @js.native
  sealed trait conjunction extends StObject
  inline def conjunction: conjunction = "conjunction".asInstanceOf[conjunction]
  
  @js.native
  sealed trait cs
    extends StObject
       with key
  inline def cs: cs = "cs".asInstanceOf[cs]
  
  @js.native
  sealed trait cy
    extends StObject
       with key
  inline def cy: cy = "cy".asInstanceOf[cy]
  
  @js.native
  sealed trait d_
    extends StObject
       with token
  inline def d_ : d_ = "d".asInstanceOf[d_]
  
  @js.native
  sealed trait da
    extends StObject
       with key
  inline def da: da = "da".asInstanceOf[da]
  
  @js.native
  sealed trait data extends StObject
  inline def data: data = "data".asInstanceOf[data]
  
  @js.native
  sealed trait datalist extends StObject
  inline def datalist: datalist = "datalist".asInstanceOf[datalist]
  
  @js.native
  sealed trait dateFormat extends StObject
  inline def dateFormat: dateFormat = "dateFormat".asInstanceOf[dateFormat]
  
  @js.native
  sealed trait dd extends StObject
  inline def dd: dd = "dd".asInstanceOf[dd]
  
  @js.native
  sealed trait de
    extends StObject
       with key
  inline def de: de = "de".asInstanceOf[de]
  
  @js.native
  sealed trait defaultDate extends StObject
  inline def defaultDate: defaultDate = "defaultDate".asInstanceOf[defaultDate]
  
  @js.native
  sealed trait defaultHour extends StObject
  inline def defaultHour: defaultHour = "defaultHour".asInstanceOf[defaultHour]
  
  @js.native
  sealed trait defaultMinute extends StObject
  inline def defaultMinute: defaultMinute = "defaultMinute".asInstanceOf[defaultMinute]
  
  @js.native
  sealed trait defaultSeconds extends StObject
  inline def defaultSeconds: defaultSeconds = "defaultSeconds".asInstanceOf[defaultSeconds]
  
  @js.native
  sealed trait del extends StObject
  inline def del: del = "del".asInstanceOf[del]
  
  @js.native
  sealed trait details extends StObject
  inline def details: details = "details".asInstanceOf[details]
  
  @js.native
  sealed trait dfn extends StObject
  inline def dfn: dfn = "dfn".asInstanceOf[dfn]
  
  @js.native
  sealed trait dialog extends StObject
  inline def dialog: dialog = "dialog".asInstanceOf[dialog]
  
  @js.native
  sealed trait disable extends StObject
  inline def disable: disable = "disable".asInstanceOf[disable]
  
  @js.native
  sealed trait disableMobile extends StObject
  inline def disableMobile: disableMobile = "disableMobile".asInstanceOf[disableMobile]
  
  @js.native
  sealed trait div extends StObject
  inline def div: div = "div".asInstanceOf[div]
  
  @js.native
  sealed trait dl extends StObject
  inline def dl: dl = "dl".asInstanceOf[dl]
  
  @js.native
  sealed trait dropdown extends StObject
  inline def dropdown: dropdown = "dropdown".asInstanceOf[dropdown]
  
  @js.native
  sealed trait dt extends StObject
  inline def dt: dt = "dt".asInstanceOf[dt]
  
  @js.native
  sealed trait em extends StObject
  inline def em: em = "em".asInstanceOf[em]
  
  @js.native
  sealed trait embed extends StObject
  inline def embed: embed = "embed".asInstanceOf[embed]
  
  @js.native
  sealed trait en
    extends StObject
       with key
  inline def en: en = "en".asInstanceOf[en]
  
  @js.native
  sealed trait enable extends StObject
  inline def enable: enable = "enable".asInstanceOf[enable]
  
  @js.native
  sealed trait enableSeconds extends StObject
  inline def enableSeconds: enableSeconds = "enableSeconds".asInstanceOf[enableSeconds]
  
  @js.native
  sealed trait enableTime extends StObject
  inline def enableTime: enableTime = "enableTime".asInstanceOf[enableTime]
  
  @js.native
  sealed trait eo
    extends StObject
       with key
  inline def eo: eo = "eo".asInstanceOf[eo]
  
  @js.native
  sealed trait errorHandler extends StObject
  inline def errorHandler: errorHandler = "errorHandler".asInstanceOf[errorHandler]
  
  @js.native
  sealed trait es
    extends StObject
       with key
  inline def es: es = "es".asInstanceOf[es]
  
  @js.native
  sealed trait et
    extends StObject
       with key
  inline def et: et = "et".asInstanceOf[et]
  
  @js.native
  sealed trait fa
    extends StObject
       with key
  inline def fa: fa = "fa".asInstanceOf[fa]
  
  @js.native
  sealed trait fi
    extends StObject
       with key
  inline def fi: fi = "fi".asInstanceOf[fi]
  
  @js.native
  sealed trait fieldset extends StObject
  inline def fieldset: fieldset = "fieldset".asInstanceOf[fieldset]
  
  @js.native
  sealed trait figcaption extends StObject
  inline def figcaption: figcaption = "figcaption".asInstanceOf[figcaption]
  
  @js.native
  sealed trait figure extends StObject
  inline def figure: figure = "figure".asInstanceOf[figure]
  
  @js.native
  sealed trait fo
    extends StObject
       with key
  inline def fo: fo = "fo".asInstanceOf[fo]
  
  @js.native
  sealed trait footer extends StObject
  inline def footer: footer = "footer".asInstanceOf[footer]
  
  @js.native
  sealed trait form extends StObject
  inline def form: form = "form".asInstanceOf[form]
  
  @js.native
  sealed trait formatDate extends StObject
  inline def formatDate: formatDate = "formatDate".asInstanceOf[formatDate]
  
  @js.native
  sealed trait fr
    extends StObject
       with key
  inline def fr: fr = "fr".asInstanceOf[fr]
  
  @js.native
  sealed trait getWeek extends StObject
  inline def getWeek: getWeek = "getWeek".asInstanceOf[getWeek]
  
  @js.native
  sealed trait gr
    extends StObject
       with key
  inline def gr: gr = "gr".asInstanceOf[gr]
  
  @js.native
  sealed trait h1 extends StObject
  inline def h1: h1 = "h1".asInstanceOf[h1]
  
  @js.native
  sealed trait h2 extends StObject
  inline def h2: h2 = "h2".asInstanceOf[h2]
  
  @js.native
  sealed trait h3 extends StObject
  inline def h3: h3 = "h3".asInstanceOf[h3]
  
  @js.native
  sealed trait h4 extends StObject
  inline def h4: h4 = "h4".asInstanceOf[h4]
  
  @js.native
  sealed trait h5 extends StObject
  inline def h5: h5 = "h5".asInstanceOf[h5]
  
  @js.native
  sealed trait h6 extends StObject
  inline def h6: h6 = "h6".asInstanceOf[h6]
  
  @js.native
  sealed trait h_
    extends StObject
       with token
  inline def h_ : h_ = "h".asInstanceOf[h_]
  
  @js.native
  sealed trait he
    extends StObject
       with key
  inline def he: he = "he".asInstanceOf[he]
  
  @js.native
  sealed trait head extends StObject
  inline def head: head = "head".asInstanceOf[head]
  
  @js.native
  sealed trait header extends StObject
  inline def header: header = "header".asInstanceOf[header]
  
  @js.native
  sealed trait hgroup extends StObject
  inline def hgroup: hgroup = "hgroup".asInstanceOf[hgroup]
  
  @js.native
  sealed trait hi
    extends StObject
       with key
  inline def hi: hi = "hi".asInstanceOf[hi]
  
  @js.native
  sealed trait hourIncrement extends StObject
  inline def hourIncrement: hourIncrement = "hourIncrement".asInstanceOf[hourIncrement]
  
  @js.native
  sealed trait hr
    extends StObject
       with key
  inline def hr: hr = "hr".asInstanceOf[hr]
  
  @js.native
  sealed trait html extends StObject
  inline def html: html = "html".asInstanceOf[html]
  
  @js.native
  sealed trait hu
    extends StObject
       with key
  inline def hu: hu = "hu".asInstanceOf[hu]
  
  @js.native
  sealed trait hy
    extends StObject
       with key
  inline def hy: hy = "hy".asInstanceOf[hy]
  
  @js.native
  sealed trait i
    extends StObject
       with token
  inline def i: i = "i".asInstanceOf[i]
  
  @js.native
  sealed trait id
    extends StObject
       with key
  inline def id: id = "id".asInstanceOf[id]
  
  @js.native
  sealed trait iframe extends StObject
  inline def iframe: iframe = "iframe".asInstanceOf[iframe]
  
  @js.native
  sealed trait ignoredFocusElements extends StObject
  inline def ignoredFocusElements: ignoredFocusElements = "ignoredFocusElements".asInstanceOf[ignoredFocusElements]
  
  @js.native
  sealed trait img extends StObject
  inline def img: img = "img".asInstanceOf[img]
  
  @js.native
  sealed trait increment extends StObject
  inline def increment: increment = "increment".asInstanceOf[increment]
  
  @js.native
  sealed trait `inline` extends StObject
  inline def `inline`: `inline` = "inline".asInstanceOf[`inline`]
  
  @js.native
  sealed trait input extends StObject
  inline def input: input = "input".asInstanceOf[input]
  
  @js.native
  sealed trait ins extends StObject
  inline def ins: ins = "ins".asInstanceOf[ins]
  
  @js.native
  sealed trait is
    extends StObject
       with key
  inline def is: is = "is".asInstanceOf[is]
  
  @js.native
  sealed trait it
    extends StObject
       with key
  inline def it: it = "it".asInstanceOf[it]
  
  @js.native
  sealed trait j_
    extends StObject
       with token
  inline def j_ : j_ = "j".asInstanceOf[j_]
  
  @js.native
  sealed trait ja
    extends StObject
       with key
  inline def ja: ja = "ja".asInstanceOf[ja]
  
  @js.native
  sealed trait ka
    extends StObject
       with key
  inline def ka: ka = "ka".asInstanceOf[ka]
  
  @js.native
  sealed trait kbd extends StObject
  inline def kbd: kbd = "kbd".asInstanceOf[kbd]
  
  @js.native
  sealed trait km
    extends StObject
       with key
  inline def km: km = "km".asInstanceOf[km]
  
  @js.native
  sealed trait ko
    extends StObject
       with key
  inline def ko: ko = "ko".asInstanceOf[ko]
  
  @js.native
  sealed trait kz
    extends StObject
       with key
  inline def kz: kz = "kz".asInstanceOf[kz]
  
  @js.native
  sealed trait l
    extends StObject
       with token
  inline def l: l = "l".asInstanceOf[l]
  
  @js.native
  sealed trait label extends StObject
  inline def label: label = "label".asInstanceOf[label]
  
  @js.native
  sealed trait left extends StObject
  inline def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait legend extends StObject
  inline def legend: legend = "legend".asInstanceOf[legend]
  
  @js.native
  sealed trait li extends StObject
  inline def li: li = "li".asInstanceOf[li]
  
  @js.native
  sealed trait link extends StObject
  inline def link: link = "link".asInstanceOf[link]
  
  @js.native
  sealed trait locale extends StObject
  inline def locale: locale = "locale".asInstanceOf[locale]
  
  @js.native
  sealed trait lt
    extends StObject
       with key
  inline def lt: lt = "lt".asInstanceOf[lt]
  
  @js.native
  sealed trait lv
    extends StObject
       with key
  inline def lv: lv = "lv".asInstanceOf[lv]
  
  @js.native
  sealed trait m_
    extends StObject
       with token
  inline def m_ : m_ = "m".asInstanceOf[m_]
  
  @js.native
  sealed trait main extends StObject
  inline def main: main = "main".asInstanceOf[main]
  
  @js.native
  sealed trait map extends StObject
  inline def map: map = "map".asInstanceOf[map]
  
  @js.native
  sealed trait mark extends StObject
  inline def mark: mark = "mark".asInstanceOf[mark]
  
  @js.native
  sealed trait maxDate extends StObject
  inline def maxDate: maxDate = "maxDate".asInstanceOf[maxDate]
  
  @js.native
  sealed trait maxTime extends StObject
  inline def maxTime: maxTime = "maxTime".asInstanceOf[maxTime]
  
  @js.native
  sealed trait menu extends StObject
  inline def menu: menu = "menu".asInstanceOf[menu]
  
  @js.native
  sealed trait meta extends StObject
  inline def meta: meta = "meta".asInstanceOf[meta]
  
  @js.native
  sealed trait meter extends StObject
  inline def meter: meter = "meter".asInstanceOf[meter]
  
  @js.native
  sealed trait minDate extends StObject
  inline def minDate: minDate = "minDate".asInstanceOf[minDate]
  
  @js.native
  sealed trait minTime extends StObject
  inline def minTime: minTime = "minTime".asInstanceOf[minTime]
  
  @js.native
  sealed trait minuteIncrement extends StObject
  inline def minuteIncrement: minuteIncrement = "minuteIncrement".asInstanceOf[minuteIncrement]
  
  @js.native
  sealed trait mk
    extends StObject
       with key
  inline def mk: mk = "mk".asInstanceOf[mk]
  
  @js.native
  sealed trait mn
    extends StObject
       with key
  inline def mn: mn = "mn".asInstanceOf[mn]
  
  @js.native
  sealed trait mode extends StObject
  inline def mode: mode = "mode".asInstanceOf[mode]
  
  @js.native
  sealed trait monthSelectorType extends StObject
  inline def monthSelectorType: monthSelectorType = "monthSelectorType".asInstanceOf[monthSelectorType]
  
  @js.native
  sealed trait ms
    extends StObject
       with key
  inline def ms: ms = "ms".asInstanceOf[ms]
  
  @js.native
  sealed trait multiple extends StObject
  inline def multiple: multiple = "multiple".asInstanceOf[multiple]
  
  @js.native
  sealed trait my
    extends StObject
       with key
  inline def my: my = "my".asInstanceOf[my]
  
  @js.native
  sealed trait n
    extends StObject
       with token
  inline def n: n = "n".asInstanceOf[n]
  
  @js.native
  sealed trait nav extends StObject
  inline def nav: nav = "nav".asInstanceOf[nav]
  
  @js.native
  sealed trait nextArrow extends StObject
  inline def nextArrow: nextArrow = "nextArrow".asInstanceOf[nextArrow]
  
  @js.native
  sealed trait nl
    extends StObject
       with key
  inline def nl: nl = "nl".asInstanceOf[nl]
  
  @js.native
  sealed trait nn
    extends StObject
       with key
  inline def nn: nn = "nn".asInstanceOf[nn]
  
  @js.native
  sealed trait no
    extends StObject
       with key
  inline def no: no = "no".asInstanceOf[no]
  
  @js.native
  sealed trait noCalendar extends StObject
  inline def noCalendar: noCalendar = "noCalendar".asInstanceOf[noCalendar]
  
  @js.native
  sealed trait noscript extends StObject
  inline def noscript: noscript = "noscript".asInstanceOf[noscript]
  
  @js.native
  sealed trait now extends StObject
  inline def now: now = "now".asInstanceOf[now]
  
  @js.native
  sealed trait `object` extends StObject
  inline def `object`: `object` = "object".asInstanceOf[`object`]
  
  @js.native
  sealed trait ol extends StObject
  inline def ol: ol = "ol".asInstanceOf[ol]
  
  @js.native
  sealed trait onChange
    extends StObject
       with HookKey
  inline def onChange: onChange = "onChange".asInstanceOf[onChange]
  
  @js.native
  sealed trait onClose
    extends StObject
       with HookKey
  inline def onClose: onClose = "onClose".asInstanceOf[onClose]
  
  @js.native
  sealed trait onDayCreate
    extends StObject
       with HookKey
  inline def onDayCreate: onDayCreate = "onDayCreate".asInstanceOf[onDayCreate]
  
  @js.native
  sealed trait onDestroy
    extends StObject
       with HookKey
  inline def onDestroy: onDestroy = "onDestroy".asInstanceOf[onDestroy]
  
  @js.native
  sealed trait onKeyDown
    extends StObject
       with HookKey
  inline def onKeyDown: onKeyDown = "onKeyDown".asInstanceOf[onKeyDown]
  
  @js.native
  sealed trait onMonthChange
    extends StObject
       with HookKey
  inline def onMonthChange: onMonthChange = "onMonthChange".asInstanceOf[onMonthChange]
  
  @js.native
  sealed trait onOpen
    extends StObject
       with HookKey
  inline def onOpen: onOpen = "onOpen".asInstanceOf[onOpen]
  
  @js.native
  sealed trait onParseConfig
    extends StObject
       with HookKey
  inline def onParseConfig: onParseConfig = "onParseConfig".asInstanceOf[onParseConfig]
  
  @js.native
  sealed trait onPreCalendarPosition
    extends StObject
       with HookKey
  inline def onPreCalendarPosition: onPreCalendarPosition = "onPreCalendarPosition".asInstanceOf[onPreCalendarPosition]
  
  @js.native
  sealed trait onReady
    extends StObject
       with HookKey
  inline def onReady: onReady = "onReady".asInstanceOf[onReady]
  
  @js.native
  sealed trait onValueUpdate
    extends StObject
       with HookKey
  inline def onValueUpdate: onValueUpdate = "onValueUpdate".asInstanceOf[onValueUpdate]
  
  @js.native
  sealed trait onYearChange
    extends StObject
       with HookKey
  inline def onYearChange: onYearChange = "onYearChange".asInstanceOf[onYearChange]
  
  @js.native
  sealed trait optgroup extends StObject
  inline def optgroup: optgroup = "optgroup".asInstanceOf[optgroup]
  
  @js.native
  sealed trait option extends StObject
  inline def option: option = "option".asInstanceOf[option]
  
  @js.native
  sealed trait output extends StObject
  inline def output: output = "output".asInstanceOf[output]
  
  @js.native
  sealed trait p extends StObject
  inline def p: p = "p".asInstanceOf[p]
  
  @js.native
  sealed trait pa
    extends StObject
       with key
  inline def pa: pa = "pa".asInstanceOf[pa]
  
  @js.native
  sealed trait parseDate extends StObject
  inline def parseDate: parseDate = "parseDate".asInstanceOf[parseDate]
  
  @js.native
  sealed trait picture extends StObject
  inline def picture: picture = "picture".asInstanceOf[picture]
  
  @js.native
  sealed trait pl
    extends StObject
       with key
  inline def pl: pl = "pl".asInstanceOf[pl]
  
  @js.native
  sealed trait plugins extends StObject
  inline def plugins: plugins = "plugins".asInstanceOf[plugins]
  
  @js.native
  sealed trait position extends StObject
  inline def position: position = "position".asInstanceOf[position]
  
  @js.native
  sealed trait positionElement extends StObject
  inline def positionElement: positionElement = "positionElement".asInstanceOf[positionElement]
  
  @js.native
  sealed trait pre extends StObject
  inline def pre: pre = "pre".asInstanceOf[pre]
  
  @js.native
  sealed trait prevArrow extends StObject
  inline def prevArrow: prevArrow = "prevArrow".asInstanceOf[prevArrow]
  
  @js.native
  sealed trait progress extends StObject
  inline def progress: progress = "progress".asInstanceOf[progress]
  
  @js.native
  sealed trait pt
    extends StObject
       with key
  inline def pt: pt = "pt".asInstanceOf[pt]
  
  @js.native
  sealed trait q extends StObject
  inline def q: q = "q".asInstanceOf[q]
  
  @js.native
  sealed trait range extends StObject
  inline def range: range = "range".asInstanceOf[range]
  
  @js.native
  sealed trait ro
    extends StObject
       with key
  inline def ro: ro = "ro".asInstanceOf[ro]
  
  @js.native
  sealed trait rp extends StObject
  inline def rp: rp = "rp".asInstanceOf[rp]
  
  @js.native
  sealed trait rt extends StObject
  inline def rt: rt = "rt".asInstanceOf[rt]
  
  @js.native
  sealed trait ru
    extends StObject
       with key
  inline def ru: ru = "ru".asInstanceOf[ru]
  
  @js.native
  sealed trait ruby extends StObject
  inline def ruby: ruby = "ruby".asInstanceOf[ruby]
  
  @js.native
  sealed trait s_
    extends StObject
       with token
  inline def s_ : s_ = "s".asInstanceOf[s_]
  
  @js.native
  sealed trait samp extends StObject
  inline def samp: samp = "samp".asInstanceOf[samp]
  
  @js.native
  sealed trait script extends StObject
  inline def script: script = "script".asInstanceOf[script]
  
  @js.native
  sealed trait section extends StObject
  inline def section: section = "section".asInstanceOf[section]
  
  @js.native
  sealed trait select extends StObject
  inline def select: select = "select".asInstanceOf[select]
  
  @js.native
  sealed trait shorthandCurrentMonth extends StObject
  inline def shorthandCurrentMonth: shorthandCurrentMonth = "shorthandCurrentMonth".asInstanceOf[shorthandCurrentMonth]
  
  @js.native
  sealed trait showMonths extends StObject
  inline def showMonths: showMonths = "showMonths".asInstanceOf[showMonths]
  
  @js.native
  sealed trait si
    extends StObject
       with key
  inline def si: si = "si".asInstanceOf[si]
  
  @js.native
  sealed trait single extends StObject
  inline def single: single = "single".asInstanceOf[single]
  
  @js.native
  sealed trait sk
    extends StObject
       with key
  inline def sk: sk = "sk".asInstanceOf[sk]
  
  @js.native
  sealed trait sl
    extends StObject
       with key
  inline def sl: sl = "sl".asInstanceOf[sl]
  
  @js.native
  sealed trait slot extends StObject
  inline def slot: slot = "slot".asInstanceOf[slot]
  
  @js.native
  sealed trait small extends StObject
  inline def small: small = "small".asInstanceOf[small]
  
  @js.native
  sealed trait source extends StObject
  inline def source: source = "source".asInstanceOf[source]
  
  @js.native
  sealed trait span extends StObject
  inline def span: span = "span".asInstanceOf[span]
  
  @js.native
  sealed trait sq
    extends StObject
       with key
  inline def sq: sq = "sq".asInstanceOf[sq]
  
  @js.native
  sealed trait sr
    extends StObject
       with key
  inline def sr: sr = "sr".asInstanceOf[sr]
  
  @js.native
  sealed trait static extends StObject
  inline def static: static = "static".asInstanceOf[static]
  
  @js.native
  sealed trait strong extends StObject
  inline def strong: strong = "strong".asInstanceOf[strong]
  
  @js.native
  sealed trait style extends StObject
  inline def style: style = "style".asInstanceOf[style]
  
  @js.native
  sealed trait sub extends StObject
  inline def sub: sub = "sub".asInstanceOf[sub]
  
  @js.native
  sealed trait summary extends StObject
  inline def summary: summary = "summary".asInstanceOf[summary]
  
  @js.native
  sealed trait sup extends StObject
  inline def sup: sup = "sup".asInstanceOf[sup]
  
  @js.native
  sealed trait sv
    extends StObject
       with key
  inline def sv: sv = "sv".asInstanceOf[sv]
  
  @js.native
  sealed trait table extends StObject
  inline def table: table = "table".asInstanceOf[table]
  
  @js.native
  sealed trait tbody extends StObject
  inline def tbody: tbody = "tbody".asInstanceOf[tbody]
  
  @js.native
  sealed trait td extends StObject
  inline def td: td = "td".asInstanceOf[td]
  
  @js.native
  sealed trait template extends StObject
  inline def template: template = "template".asInstanceOf[template]
  
  @js.native
  sealed trait textarea extends StObject
  inline def textarea: textarea = "textarea".asInstanceOf[textarea]
  
  @js.native
  sealed trait tfoot extends StObject
  inline def tfoot: tfoot = "tfoot".asInstanceOf[tfoot]
  
  @js.native
  sealed trait th
    extends StObject
       with key
  inline def th: th = "th".asInstanceOf[th]
  
  @js.native
  sealed trait thead extends StObject
  inline def thead: thead = "thead".asInstanceOf[thead]
  
  @js.native
  sealed trait time extends StObject
  inline def time: time = "time".asInstanceOf[time]
  
  @js.native
  sealed trait time_24hr extends StObject
  inline def time_24hr: time_24hr = "time_24hr".asInstanceOf[time_24hr]
  
  @js.native
  sealed trait title extends StObject
  inline def title: title = "title".asInstanceOf[title]
  
  @js.native
  sealed trait tr
    extends StObject
       with key
  inline def tr: tr = "tr".asInstanceOf[tr]
  
  @js.native
  sealed trait track extends StObject
  inline def track: track = "track".asInstanceOf[track]
  
  @js.native
  sealed trait u_
    extends StObject
       with token
  inline def u_ : u_ = "u".asInstanceOf[u_]
  
  @js.native
  sealed trait uk
    extends StObject
       with key
  inline def uk: uk = "uk".asInstanceOf[uk]
  
  @js.native
  sealed trait ul extends StObject
  inline def ul: ul = "ul".asInstanceOf[ul]
  
  @js.native
  sealed trait uz
    extends StObject
       with key
  inline def uz: uz = "uz".asInstanceOf[uz]
  
  @js.native
  sealed trait uz_latn
    extends StObject
       with key
  inline def uz_latn: uz_latn = "uz_latn".asInstanceOf[uz_latn]
  
  @js.native
  sealed trait `var` extends StObject
  inline def `var`: `var` = "var".asInstanceOf[`var`]
  
  @js.native
  sealed trait video extends StObject
  inline def video: video = "video".asInstanceOf[video]
  
  @js.native
  sealed trait vn
    extends StObject
       with key
  inline def vn: vn = "vn".asInstanceOf[vn]
  
  @js.native
  sealed trait w_
    extends StObject
       with token
  inline def w_ : w_ = "w".asInstanceOf[w_]
  
  @js.native
  sealed trait wbr extends StObject
  inline def wbr: wbr = "wbr".asInstanceOf[wbr]
  
  @js.native
  sealed trait weekNumbers extends StObject
  inline def weekNumbers: weekNumbers = "weekNumbers".asInstanceOf[weekNumbers]
  
  @js.native
  sealed trait wrap extends StObject
  inline def wrap: wrap = "wrap".asInstanceOf[wrap]
  
  @js.native
  sealed trait y_
    extends StObject
       with token
  inline def y_ : y_ = "y".asInstanceOf[y_]
  
  @js.native
  sealed trait zh
    extends StObject
       with key
  inline def zh: zh = "zh".asInstanceOf[zh]
  
  @js.native
  sealed trait zh_tw
    extends StObject
       with key
  inline def zh_tw: zh_tw = "zh_tw".asInstanceOf[zh_tw]
}
