package typingsJapgolly.fileSize

import typingsJapgolly.fileSize.mod.Spec
import typingsJapgolly.fileSize.mod.Unit
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object fileSizeStrings {
  
  @js.native
  sealed trait B
    extends StObject
       with Unit
  inline def B: B = "B".asInstanceOf[B]
  
  @js.native
  sealed trait EB
    extends StObject
       with Unit
  inline def EB: EB = "EB".asInstanceOf[EB]
  
  @js.native
  sealed trait GB
    extends StObject
       with Unit
  inline def GB: GB = "GB".asInstanceOf[GB]
  
  @js.native
  sealed trait KB
    extends StObject
       with Unit
  inline def KB: KB = "KB".asInstanceOf[KB]
  
  @js.native
  sealed trait MB
    extends StObject
       with Unit
  inline def MB: MB = "MB".asInstanceOf[MB]
  
  @js.native
  sealed trait PB
    extends StObject
       with Unit
  inline def PB: PB = "PB".asInstanceOf[PB]
  
  @js.native
  sealed trait TB
    extends StObject
       with Unit
  inline def TB: TB = "TB".asInstanceOf[TB]
  
  @js.native
  sealed trait YB
    extends StObject
       with Unit
  inline def YB: YB = "YB".asInstanceOf[YB]
  
  @js.native
  sealed trait ZB
    extends StObject
       with Unit
  inline def ZB: ZB = "ZB".asInstanceOf[ZB]
  
  @js.native
  sealed trait iec
    extends StObject
       with Spec
  inline def iec: iec = "iec".asInstanceOf[iec]
  
  @js.native
  sealed trait jedec
    extends StObject
       with Spec
  inline def jedec: jedec = "jedec".asInstanceOf[jedec]
  
  @js.native
  sealed trait si
    extends StObject
       with Spec
  inline def si: si = "si".asInstanceOf[si]
}
