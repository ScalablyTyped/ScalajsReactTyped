...
object mod {
  ...
  object firebaseAppCompatAugmentingMod {
    ...
    object FirebaseApp {
      ...
      inline def apply(appCheck: CallbackTo[FirebaseAppCheck]): FirebaseApp = {
        val __obj = js.Dynamic.literal(appCheck = appCheck.toJsFn)
        __obj.asInstanceOf[FirebaseApp]
      }
      
      extension [Self <: FirebaseApp](x: Self) {
        
        inline def setAppCheck(value: CallbackTo[FirebaseAppCheck]): Self = StObject.set(x, "appCheck", value.toJsFn)
      }
    }
