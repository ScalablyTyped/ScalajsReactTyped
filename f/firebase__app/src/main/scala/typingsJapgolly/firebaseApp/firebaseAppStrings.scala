package typingsJapgolly.firebaseApp

import typingsJapgolly.firebaseApp.distEsmAppSrcErrorsMod.AppError
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object firebaseAppStrings {
  
  @js.native
  sealed trait `app-deleted`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `app-deleted`: `app-deleted` = "app-deleted".asInstanceOf[`app-deleted`]
  
  @js.native
  sealed trait `bad-app-name`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `bad-app-name`: `bad-app-name` = "bad-app-name".asInstanceOf[`bad-app-name`]
  
  @js.native
  sealed trait `duplicate-app`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `duplicate-app`: `duplicate-app` = "duplicate-app".asInstanceOf[`duplicate-app`]
  
  @js.native
  sealed trait `fire-js` extends StObject
  inline def `fire-js`: `fire-js` = "fire-js".asInstanceOf[`fire-js`]
  
  @js.native
  sealed trait `idb-delete`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `idb-delete`: `idb-delete` = "idb-delete".asInstanceOf[`idb-delete`]
  
  @js.native
  sealed trait `idb-get`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `idb-get`: `idb-get` = "idb-get".asInstanceOf[`idb-get`]
  
  @js.native
  sealed trait `idb-open`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `idb-open`: `idb-open` = "idb-open".asInstanceOf[`idb-open`]
  
  @js.native
  sealed trait `idb-set`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `idb-set`: `idb-set` = "idb-set".asInstanceOf[`idb-set`]
  
  @js.native
  sealed trait `invalid-app-argument`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `invalid-app-argument`: `invalid-app-argument` = "invalid-app-argument".asInstanceOf[`invalid-app-argument`]
  
  @js.native
  sealed trait `invalid-log-argument`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `invalid-log-argument`: `invalid-log-argument` = "invalid-log-argument".asInstanceOf[`invalid-log-argument`]
  
  @js.native
  sealed trait `no-app`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `no-app`: `no-app` = "no-app".asInstanceOf[`no-app`]
  
  @js.native
  sealed trait `no-options`
    extends StObject
       with AppError
       with typingsJapgolly.firebaseApp.distAppSrcErrorsMod.AppError
  inline def `no-options`: `no-options` = "no-options".asInstanceOf[`no-options`]
}
