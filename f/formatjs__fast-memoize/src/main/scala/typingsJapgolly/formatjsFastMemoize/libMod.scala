...
object libMod {
  ...
  object Cache {
    ...
    inline def apply[K, V](create: CallbackTo[DefaultCache[K, V]]): Cache[K, V] = {
    ...
    extension [Self <: Cache[?, ?], K, V](x: Self & (Cache[K, V])) {
      ...
      inline def setCreate(value: CallbackTo[DefaultCache[K, V]]): Self = StObject.set(x, "create", value.toJsFn)
    }
  }
  ...
  object DefaultCache {
    ...
    inline def apply[K, V](get: K => V, set: (K, V) => Callback): DefaultCache[K, V] = {
      val __obj = js.Dynamic.literal(get = js.Any.fromFunction1(get), set = js.Any.fromFunction2((t0: K, t1: V) => (set(t0, t1)).runNow()))
