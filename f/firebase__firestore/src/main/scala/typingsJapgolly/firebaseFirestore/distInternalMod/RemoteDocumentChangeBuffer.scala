...
trait RemoteDocumentChangeBuffer extends StObject {
  ...
  /* protected */ var changes: ObjectMap[DocumentKey, MutableDocument]
  ...
  /**
    ...
    * cached.
    */
  def getEntry(transaction: PersistenceTransaction, documentKey: DocumentKey): PersistencePromise[MutableDocument]
  
  /* protected */ def getFromCache(transaction: PersistenceTransaction, documentKey: DocumentKey): PersistencePromise[MutableDocument]
  ...
  /**
    ...
    * `getEntry()/getEntries()` (enforced via IndexedDbs `apply()`).
    */
  def removeEntry(key: DocumentKey, readTime: SnapshotVersion): Unit
}
object RemoteDocumentChangeBuffer {
  ...
  inline def apply(
    ...
    changes: ObjectMap[DocumentKey, MutableDocument],
    changesApplied: Any,
  ...
  extension [Self <: RemoteDocumentChangeBuffer](x: Self) {
    ...
    inline def setChanges(value: ObjectMap[DocumentKey, MutableDocument]): Self = StObject.set(x, "changes", value.asInstanceOf[js.Any])
    ...
    inline def setGetEntry(value: (PersistenceTransaction, DocumentKey) => PersistencePromise[MutableDocument]): Self = StObject.set(x, "getEntry", js.Any.fromFunction2(value))
    
    inline def setGetFromCache(value: (PersistenceTransaction, DocumentKey) => PersistencePromise[MutableDocument]): Self = StObject.set(x, "getFromCache", js.Any.fromFunction2(value))
    
    inline def setRemoveEntry(value: (DocumentKey, SnapshotVersion) => Callback): Self = StObject.set(x, "removeEntry", js.Any.fromFunction2((t0: DocumentKey, t1: SnapshotVersion) => (value(t0, t1)).runNow()))
  }
}
