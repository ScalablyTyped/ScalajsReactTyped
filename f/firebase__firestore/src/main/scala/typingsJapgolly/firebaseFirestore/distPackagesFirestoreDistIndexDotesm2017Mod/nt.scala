...
import typingsJapgolly.firebaseFirestore.anon.NanosecondsSeconds
...
object nt {
  ...
  inline def apply(
    ...
    toJSON: CallbackTo[NanosecondsSeconds],
    toMillis: CallbackTo[Double]
  ): nt = {
    val __obj = js.Dynamic.literal(_compareTo = js.Any.fromFunction1(_compareTo), isEqual = js.Any.fromFunction1(isEqual), nanoseconds = nanoseconds.asInstanceOf[js.Any], seconds = seconds.asInstanceOf[js.Any], toDate = toDate.toJsFn, toJSON = toJSON.toJsFn, toMillis = toMillis.toJsFn)
    __obj.asInstanceOf[nt]
  }
  
  extension [Self <: nt](x: Self) {
    ...
    inline def setToJSON(value: CallbackTo[NanosecondsSeconds]): Self = StObject.set(x, "toJSON", value.toJsFn)
