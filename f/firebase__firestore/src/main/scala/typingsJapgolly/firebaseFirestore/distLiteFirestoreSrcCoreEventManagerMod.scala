...
object distLiteFirestoreSrcCoreEventManagerMod {
  ...
  object Observer {
    ...
    inline def apply[T](error: FirestoreError => Callback, next: T => Callback): Observer[T] = {
      val __obj = js.Dynamic.literal(error = js.Any.fromFunction1((t0: FirestoreError) => error(t0).runNow()), next = js.Any.fromFunction1((t0: T) => next(t0).runNow()))
