...
import typingsJapgolly.firebaseAdmin.libAppCredentialInternalMod.ServiceAccountCredential
import typingsJapgolly.firebaseAdmin.libUtilsApiRequestMod.AuthorizedHttpClient
import typingsJapgolly.firebaseAdmin.libUtilsErrorMod.ErrorInfo
...
object libUtilsCryptoSignerMod {
  ...
  object CryptoSigner {
    ...
    inline def apply(
      algorithm: Algorithm,
      getAccountId: CallbackTo[js.Promise[String]],
      sign: Buffer => js.Promise[Buffer]
    ): CryptoSigner = {
      val __obj = js.Dynamic.literal(algorithm = algorithm.asInstanceOf[js.Any], getAccountId = getAccountId.toJsFn, sign = js.Any.fromFunction1(sign))
    ...
    extension [Self <: CryptoSigner](x: Self) {
      ...
      inline def setGetAccountId(value: CallbackTo[js.Promise[String]]): Self = StObject.set(x, "getAccountId", value.toJsFn)
