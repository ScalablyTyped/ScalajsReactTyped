...
import typingsJapgolly.formatjsEcma402Abstract.utilsMod.LiteralPart
...
object mod {
  ...
  object ListFormat {
    ...
    inline def apply(
      ...
      val __obj = js.Dynamic.literal(format = js.Any.fromFunction1(format), formatToParts = js.Any.fromFunction1(formatToParts), resolvedOptions = resolvedOptions.toJsFn)
    ...
    extension [Self <: ListFormat](x: Self) {
      ...
      inline def setResolvedOptions(value: CallbackTo[ResolvedIntlListFormatOptions]): Self = StObject.set(x, "resolvedOptions", value.toJsFn)
    }
  }
