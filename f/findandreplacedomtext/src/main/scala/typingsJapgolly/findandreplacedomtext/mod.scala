...
import typingsJapgolly.findandreplacedomtext.findandreplacedomtextStrings.first
import typingsJapgolly.findandreplacedomtext.findandreplacedomtextStrings.prose
import typingsJapgolly.findandreplacedomtext.findandreplacedomtextStrings.retain
...
object mod {
  ...
  object Return {
    ...
    inline def apply(revert: CallbackTo[Return]): Return = {
      val __obj = js.Dynamic.literal(revert = revert.toJsFn)
      __obj.asInstanceOf[Return]
    }
    
    extension [Self <: Return](x: Self) {
      
      inline def setRevert(value: CallbackTo[Return]): Self = StObject.set(x, "revert", value.toJsFn)
    }
  }
}
