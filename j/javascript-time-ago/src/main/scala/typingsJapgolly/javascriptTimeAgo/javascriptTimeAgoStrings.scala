package typingsJapgolly.javascriptTimeAgo

import typingsJapgolly.javascriptTimeAgo.gradationMod.Unit
import typingsJapgolly.javascriptTimeAgo.localeMod.DefaultFormats
import typingsJapgolly.javascriptTimeAgo.localeMod.ExtendedFormats
import typingsJapgolly.javascriptTimeAgo.localeMod.Formats
import typingsJapgolly.javascriptTimeAgo.localeMod.TimeUnit
import typingsJapgolly.javascriptTimeAgo.styleMod.Flavour
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object javascriptTimeAgoStrings {
  
  @js.native
  sealed trait af extends StObject
  inline def af: af = "af".asInstanceOf[af]
  
  @js.native
  sealed trait am extends StObject
  inline def am: am = "am".asInstanceOf[am]
  
  @js.native
  sealed trait ar extends StObject
  inline def ar: ar = "ar".asInstanceOf[ar]
  
  @js.native
  sealed trait `ar-AE` extends StObject
  inline def `ar-AE`: `ar-AE` = "ar-AE".asInstanceOf[`ar-AE`]
  
  @js.native
  sealed trait as extends StObject
  inline def as: as = "as".asInstanceOf[as]
  
  @js.native
  sealed trait ast extends StObject
  inline def ast: ast = "ast".asInstanceOf[ast]
  
  @js.native
  sealed trait az extends StObject
  inline def az: az = "az".asInstanceOf[az]
  
  @js.native
  sealed trait be extends StObject
  inline def be: be = "be".asInstanceOf[be]
  
  @js.native
  sealed trait bg extends StObject
  inline def bg: bg = "bg".asInstanceOf[bg]
  
  @js.native
  sealed trait bn extends StObject
  inline def bn: bn = "bn".asInstanceOf[bn]
  
  @js.native
  sealed trait br extends StObject
  inline def br: br = "br".asInstanceOf[br]
  
  @js.native
  sealed trait bs extends StObject
  inline def bs: bs = "bs".asInstanceOf[bs]
  
  @js.native
  sealed trait `bs-Cyrl` extends StObject
  inline def `bs-Cyrl`: `bs-Cyrl` = "bs-Cyrl".asInstanceOf[`bs-Cyrl`]
  
  @js.native
  sealed trait ca extends StObject
  inline def ca: ca = "ca".asInstanceOf[ca]
  
  @js.native
  sealed trait ccp extends StObject
  inline def ccp: ccp = "ccp".asInstanceOf[ccp]
  
  @js.native
  sealed trait ce extends StObject
  inline def ce: ce = "ce".asInstanceOf[ce]
  
  @js.native
  sealed trait chr extends StObject
  inline def chr: chr = "chr".asInstanceOf[chr]
  
  @js.native
  sealed trait cs extends StObject
  inline def cs: cs = "cs".asInstanceOf[cs]
  
  @js.native
  sealed trait cy extends StObject
  inline def cy: cy = "cy".asInstanceOf[cy]
  
  @js.native
  sealed trait da extends StObject
  inline def da: da = "da".asInstanceOf[da]
  
  @js.native
  sealed trait day
    extends StObject
       with TimeUnit
       with Unit
  inline def day: day = "day".asInstanceOf[day]
  
  @js.native
  sealed trait de extends StObject
  inline def de: de = "de".asInstanceOf[de]
  
  @js.native
  sealed trait dsb extends StObject
  inline def dsb: dsb = "dsb".asInstanceOf[dsb]
  
  @js.native
  sealed trait dz extends StObject
  inline def dz: dz = "dz".asInstanceOf[dz]
  
  @js.native
  sealed trait ee extends StObject
  inline def ee: ee = "ee".asInstanceOf[ee]
  
  @js.native
  sealed trait el extends StObject
  inline def el: el = "el".asInstanceOf[el]
  
  @js.native
  sealed trait en extends StObject
  inline def en: en = "en".asInstanceOf[en]
  
  @js.native
  sealed trait `es-AR` extends StObject
  inline def `es-AR`: `es-AR` = "es-AR".asInstanceOf[`es-AR`]
  
  @js.native
  sealed trait `es-MX` extends StObject
  inline def `es-MX`: `es-MX` = "es-MX".asInstanceOf[`es-MX`]
  
  @js.native
  sealed trait `es-PY` extends StObject
  inline def `es-PY`: `es-PY` = "es-PY".asInstanceOf[`es-PY`]
  
  @js.native
  sealed trait `es-SV` extends StObject
  inline def `es-SV`: `es-SV` = "es-SV".asInstanceOf[`es-SV`]
  
  @js.native
  sealed trait `es-US` extends StObject
  inline def `es-US`: `es-US` = "es-US".asInstanceOf[`es-US`]
  
  @js.native
  sealed trait et extends StObject
  inline def et: et = "et".asInstanceOf[et]
  
  @js.native
  sealed trait eu extends StObject
  inline def eu: eu = "eu".asInstanceOf[eu]
  
  @js.native
  sealed trait fa extends StObject
  inline def fa: fa = "fa".asInstanceOf[fa]
  
  @js.native
  sealed trait few extends StObject
  inline def few: few = "few".asInstanceOf[few]
  
  @js.native
  sealed trait fi extends StObject
  inline def fi: fi = "fi".asInstanceOf[fi]
  
  @js.native
  sealed trait fil extends StObject
  inline def fil: fil = "fil".asInstanceOf[fil]
  
  @js.native
  sealed trait fo extends StObject
  inline def fo: fo = "fo".asInstanceOf[fo]
  
  @js.native
  sealed trait fr extends StObject
  inline def fr: fr = "fr".asInstanceOf[fr]
  
  @js.native
  sealed trait `fr-CA` extends StObject
  inline def `fr-CA`: `fr-CA` = "fr-CA".asInstanceOf[`fr-CA`]
  
  @js.native
  sealed trait fur extends StObject
  inline def fur: fur = "fur".asInstanceOf[fur]
  
  @js.native
  sealed trait fy extends StObject
  inline def fy: fy = "fy".asInstanceOf[fy]
  
  @js.native
  sealed trait ga extends StObject
  inline def ga: ga = "ga".asInstanceOf[ga]
  
  @js.native
  sealed trait gd extends StObject
  inline def gd: gd = "gd".asInstanceOf[gd]
  
  @js.native
  sealed trait gl extends StObject
  inline def gl: gl = "gl".asInstanceOf[gl]
  
  @js.native
  sealed trait gu extends StObject
  inline def gu: gu = "gu".asInstanceOf[gu]
  
  @js.native
  sealed trait he extends StObject
  inline def he: he = "he".asInstanceOf[he]
  
  @js.native
  sealed trait hi extends StObject
  inline def hi: hi = "hi".asInstanceOf[hi]
  
  @js.native
  sealed trait hour
    extends StObject
       with TimeUnit
       with Unit
  inline def hour: hour = "hour".asInstanceOf[hour]
  
  @js.native
  sealed trait hr extends StObject
  inline def hr: hr = "hr".asInstanceOf[hr]
  
  @js.native
  sealed trait hsb extends StObject
  inline def hsb: hsb = "hsb".asInstanceOf[hsb]
  
  @js.native
  sealed trait hu extends StObject
  inline def hu: hu = "hu".asInstanceOf[hu]
  
  @js.native
  sealed trait hy extends StObject
  inline def hy: hy = "hy".asInstanceOf[hy]
  
  @js.native
  sealed trait ia extends StObject
  inline def ia: ia = "ia".asInstanceOf[ia]
  
  @js.native
  sealed trait id extends StObject
  inline def id: id = "id".asInstanceOf[id]
  
  @js.native
  sealed trait is extends StObject
  inline def is: is = "is".asInstanceOf[is]
  
  @js.native
  sealed trait it extends StObject
  inline def it: it = "it".asInstanceOf[it]
  
  @js.native
  sealed trait ja extends StObject
  inline def ja: ja = "ja".asInstanceOf[ja]
  
  @js.native
  sealed trait jgo extends StObject
  inline def jgo: jgo = "jgo".asInstanceOf[jgo]
  
  @js.native
  sealed trait jv extends StObject
  inline def jv: jv = "jv".asInstanceOf[jv]
  
  @js.native
  sealed trait ka extends StObject
  inline def ka: ka = "ka".asInstanceOf[ka]
  
  @js.native
  sealed trait kea extends StObject
  inline def kea: kea = "kea".asInstanceOf[kea]
  
  @js.native
  sealed trait kk extends StObject
  inline def kk: kk = "kk".asInstanceOf[kk]
  
  @js.native
  sealed trait kl extends StObject
  inline def kl: kl = "kl".asInstanceOf[kl]
  
  @js.native
  sealed trait km extends StObject
  inline def km: km = "km".asInstanceOf[km]
  
  @js.native
  sealed trait kn extends StObject
  inline def kn: kn = "kn".asInstanceOf[kn]
  
  @js.native
  sealed trait ko extends StObject
  inline def ko: ko = "ko".asInstanceOf[ko]
  
  @js.native
  sealed trait kok extends StObject
  inline def kok: kok = "kok".asInstanceOf[kok]
  
  @js.native
  sealed trait ksh extends StObject
  inline def ksh: ksh = "ksh".asInstanceOf[ksh]
  
  @js.native
  sealed trait ku extends StObject
  inline def ku: ku = "ku".asInstanceOf[ku]
  
  @js.native
  sealed trait ky extends StObject
  inline def ky: ky = "ky".asInstanceOf[ky]
  
  @js.native
  sealed trait lb extends StObject
  inline def lb: lb = "lb".asInstanceOf[lb]
  
  @js.native
  sealed trait lkt extends StObject
  inline def lkt: lkt = "lkt".asInstanceOf[lkt]
  
  @js.native
  sealed trait lo extends StObject
  inline def lo: lo = "lo".asInstanceOf[lo]
  
  @js.native
  sealed trait long
    extends StObject
       with DefaultFormats
       with Flavour
       with Formats
  inline def long: long = "long".asInstanceOf[long]
  
  @js.native
  sealed trait `long-convenient`
    extends StObject
       with ExtendedFormats
       with Formats
  inline def `long-convenient`: `long-convenient` = "long-convenient".asInstanceOf[`long-convenient`]
  
  @js.native
  sealed trait `long-time`
    extends StObject
       with ExtendedFormats
       with Formats
  inline def `long-time`: `long-time` = "long-time".asInstanceOf[`long-time`]
  
  @js.native
  sealed trait lt extends StObject
  inline def lt: lt = "lt".asInstanceOf[lt]
  
  @js.native
  sealed trait lv extends StObject
  inline def lv: lv = "lv".asInstanceOf[lv]
  
  @js.native
  sealed trait minute
    extends StObject
       with TimeUnit
       with Unit
  inline def minute: minute = "minute".asInstanceOf[minute]
  
  @js.native
  sealed trait mk extends StObject
  inline def mk: mk = "mk".asInstanceOf[mk]
  
  @js.native
  sealed trait ml extends StObject
  inline def ml: ml = "ml".asInstanceOf[ml]
  
  @js.native
  sealed trait mn extends StObject
  inline def mn: mn = "mn".asInstanceOf[mn]
  
  @js.native
  sealed trait month
    extends StObject
       with TimeUnit
       with Unit
  inline def month: month = "month".asInstanceOf[month]
  
  @js.native
  sealed trait mr extends StObject
  inline def mr: mr = "mr".asInstanceOf[mr]
  
  @js.native
  sealed trait ms extends StObject
  inline def ms: ms = "ms".asInstanceOf[ms]
  
  @js.native
  sealed trait mt extends StObject
  inline def mt: mt = "mt".asInstanceOf[mt]
  
  @js.native
  sealed trait my extends StObject
  inline def my: my = "my".asInstanceOf[my]
  
  @js.native
  sealed trait mzn extends StObject
  inline def mzn: mzn = "mzn".asInstanceOf[mzn]
  
  @js.native
  sealed trait narrow
    extends StObject
       with DefaultFormats
       with Flavour
       with Formats
  inline def narrow: narrow = "narrow".asInstanceOf[narrow]
  
  @js.native
  sealed trait nb extends StObject
  inline def nb: nb = "nb".asInstanceOf[nb]
  
  @js.native
  sealed trait ne extends StObject
  inline def ne: ne = "ne".asInstanceOf[ne]
  
  @js.native
  sealed trait nl extends StObject
  inline def nl: nl = "nl".asInstanceOf[nl]
  
  @js.native
  sealed trait nn extends StObject
  inline def nn: nn = "nn".asInstanceOf[nn]
  
  @js.native
  sealed trait now
    extends StObject
       with TimeUnit
       with Unit
  inline def now: now = "now".asInstanceOf[now]
  
  @js.native
  sealed trait one extends StObject
  inline def one: one = "one".asInstanceOf[one]
  
  @js.native
  sealed trait or extends StObject
  inline def or: or = "or".asInstanceOf[or]
  
  @js.native
  sealed trait other extends StObject
  inline def other: other = "other".asInstanceOf[other]
  
  @js.native
  sealed trait pa extends StObject
  inline def pa: pa = "pa".asInstanceOf[pa]
  
  @js.native
  sealed trait pl extends StObject
  inline def pl: pl = "pl".asInstanceOf[pl]
  
  @js.native
  sealed trait ps extends StObject
  inline def ps: ps = "ps".asInstanceOf[ps]
  
  @js.native
  sealed trait pt extends StObject
  inline def pt: pt = "pt".asInstanceOf[pt]
  
  @js.native
  sealed trait qu extends StObject
  inline def qu: qu = "qu".asInstanceOf[qu]
  
  @js.native
  sealed trait quarter
    extends StObject
       with TimeUnit
  inline def quarter: quarter = "quarter".asInstanceOf[quarter]
  
  @js.native
  sealed trait ro extends StObject
  inline def ro: ro = "ro".asInstanceOf[ro]
  
  @js.native
  sealed trait ru extends StObject
  inline def ru: ru = "ru".asInstanceOf[ru]
  
  @js.native
  sealed trait sah extends StObject
  inline def sah: sah = "sah".asInstanceOf[sah]
  
  @js.native
  sealed trait sd extends StObject
  inline def sd: sd = "sd".asInstanceOf[sd]
  
  @js.native
  sealed trait se extends StObject
  inline def se: se = "se".asInstanceOf[se]
  
  @js.native
  sealed trait `se-FI` extends StObject
  inline def `se-FI`: `se-FI` = "se-FI".asInstanceOf[`se-FI`]
  
  @js.native
  sealed trait second
    extends StObject
       with TimeUnit
       with Unit
  inline def second: second = "second".asInstanceOf[second]
  
  @js.native
  sealed trait short
    extends StObject
       with DefaultFormats
       with Flavour
       with Formats
  inline def short: short = "short".asInstanceOf[short]
  
  @js.native
  sealed trait `short-convenient`
    extends StObject
       with ExtendedFormats
       with Formats
  inline def `short-convenient`: `short-convenient` = "short-convenient".asInstanceOf[`short-convenient`]
  
  @js.native
  sealed trait `short-time`
    extends StObject
       with ExtendedFormats
       with Flavour
       with Formats
  inline def `short-time`: `short-time` = "short-time".asInstanceOf[`short-time`]
  
  @js.native
  sealed trait si extends StObject
  inline def si: si = "si".asInstanceOf[si]
  
  @js.native
  sealed trait sk extends StObject
  inline def sk: sk = "sk".asInstanceOf[sk]
  
  @js.native
  sealed trait sl extends StObject
  inline def sl: sl = "sl".asInstanceOf[sl]
  
  @js.native
  sealed trait so extends StObject
  inline def so: so = "so".asInstanceOf[so]
  
  @js.native
  sealed trait sq extends StObject
  inline def sq: sq = "sq".asInstanceOf[sq]
  
  @js.native
  sealed trait sr extends StObject
  inline def sr: sr = "sr".asInstanceOf[sr]
  
  @js.native
  sealed trait `sr-Latn` extends StObject
  inline def `sr-Latn`: `sr-Latn` = "sr-Latn".asInstanceOf[`sr-Latn`]
  
  @js.native
  sealed trait sv extends StObject
  inline def sv: sv = "sv".asInstanceOf[sv]
  
  @js.native
  sealed trait sw extends StObject
  inline def sw: sw = "sw".asInstanceOf[sw]
  
  @js.native
  sealed trait ta extends StObject
  inline def ta: ta = "ta".asInstanceOf[ta]
  
  @js.native
  sealed trait te extends StObject
  inline def te: te = "te".asInstanceOf[te]
  
  @js.native
  sealed trait th extends StObject
  inline def th: th = "th".asInstanceOf[th]
  
  @js.native
  sealed trait ti extends StObject
  inline def ti: ti = "ti".asInstanceOf[ti]
  
  @js.native
  sealed trait tiny
    extends StObject
       with ExtendedFormats
       with Flavour
       with Formats
  inline def tiny: tiny = "tiny".asInstanceOf[tiny]
  
  @js.native
  sealed trait tk extends StObject
  inline def tk: tk = "tk".asInstanceOf[tk]
  
  @js.native
  sealed trait to extends StObject
  inline def to: to = "to".asInstanceOf[to]
  
  @js.native
  sealed trait tr extends StObject
  inline def tr: tr = "tr".asInstanceOf[tr]
  
  @js.native
  sealed trait two extends StObject
  inline def two: two = "two".asInstanceOf[two]
  
  @js.native
  sealed trait ug extends StObject
  inline def ug: ug = "ug".asInstanceOf[ug]
  
  @js.native
  sealed trait uk extends StObject
  inline def uk: uk = "uk".asInstanceOf[uk]
  
  @js.native
  sealed trait ur extends StObject
  inline def ur: ur = "ur".asInstanceOf[ur]
  
  @js.native
  sealed trait `ur-IN` extends StObject
  inline def `ur-IN`: `ur-IN` = "ur-IN".asInstanceOf[`ur-IN`]
  
  @js.native
  sealed trait uz extends StObject
  inline def uz: uz = "uz".asInstanceOf[uz]
  
  @js.native
  sealed trait `uz-Cyrl` extends StObject
  inline def `uz-Cyrl`: `uz-Cyrl` = "uz-Cyrl".asInstanceOf[`uz-Cyrl`]
  
  @js.native
  sealed trait vi extends StObject
  inline def vi: vi = "vi".asInstanceOf[vi]
  
  @js.native
  sealed trait wae extends StObject
  inline def wae: wae = "wae".asInstanceOf[wae]
  
  @js.native
  sealed trait week
    extends StObject
       with TimeUnit
       with Unit
  inline def week: week = "week".asInstanceOf[week]
  
  @js.native
  sealed trait year
    extends StObject
       with TimeUnit
       with Unit
  inline def year: year = "year".asInstanceOf[year]
  
  @js.native
  sealed trait yi extends StObject
  inline def yi: yi = "yi".asInstanceOf[yi]
  
  @js.native
  sealed trait yue extends StObject
  inline def yue: yue = "yue".asInstanceOf[yue]
  
  @js.native
  sealed trait `yue-Hans` extends StObject
  inline def `yue-Hans`: `yue-Hans` = "yue-Hans".asInstanceOf[`yue-Hans`]
  
  @js.native
  sealed trait zh extends StObject
  inline def zh: zh = "zh".asInstanceOf[zh]
  
  @js.native
  sealed trait `zh-Hans-HK` extends StObject
  inline def `zh-Hans-HK`: `zh-Hans-HK` = "zh-Hans-HK".asInstanceOf[`zh-Hans-HK`]
  
  @js.native
  sealed trait `zh-Hans-MO` extends StObject
  inline def `zh-Hans-MO`: `zh-Hans-MO` = "zh-Hans-MO".asInstanceOf[`zh-Hans-MO`]
  
  @js.native
  sealed trait `zh-Hans-SG` extends StObject
  inline def `zh-Hans-SG`: `zh-Hans-SG` = "zh-Hans-SG".asInstanceOf[`zh-Hans-SG`]
  
  @js.native
  sealed trait `zh-Hant` extends StObject
  inline def `zh-Hant`: `zh-Hant` = "zh-Hant".asInstanceOf[`zh-Hant`]
  
  @js.native
  sealed trait `zh-Hant-HK` extends StObject
  inline def `zh-Hant-HK`: `zh-Hant-HK` = "zh-Hant-HK".asInstanceOf[`zh-Hant-HK`]
  
  @js.native
  sealed trait `zh-Hant-MO` extends StObject
  inline def `zh-Hant-MO`: `zh-Hant-MO` = "zh-Hant-MO".asInstanceOf[`zh-Hant-MO`]
  
  @js.native
  sealed trait zu extends StObject
  inline def zu: zu = "zu".asInstanceOf[zu]
}
