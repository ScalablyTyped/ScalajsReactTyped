package typingsJapgolly.jsPriorityQueue
...
object mod {
  ...
  object AbstractPriorityQueue {
    ...
    inline def apply[T](clear: Callback, dequeue: CallbackTo[T], length: Double, peek: CallbackTo[T], queue: T => Callback): AbstractPriorityQueue[T] = {
      val __obj = js.Dynamic.literal(clear = clear.toJsFn, dequeue = dequeue.toJsFn, length = length.asInstanceOf[js.Any], peek = peek.toJsFn, queue = js.Any.fromFunction1((t0: T) => queue(t0).runNow()))
    ...
    extension [Self <: AbstractPriorityQueue[?], T](x: Self & AbstractPriorityQueue[T]) {
      ...
      inline def setQueue(value: T => Callback): Self = StObject.set(x, "queue", js.Any.fromFunction1((t0: T) => value(t0).runNow()))
    }
  }
