...
import typingsJapgolly.jweixin.anon.ErrMsg
...
object IchooseImage {
  ...
  inline def apply(cancel: Callback, success: ErrMsg => Callback): IchooseImage = {
    val __obj = js.Dynamic.literal(cancel = cancel.toJsFn, success = js.Any.fromFunction1((t0: ErrMsg) => success(t0).runNow()))
    __obj.asInstanceOf[IchooseImage]
  }
  
  extension [Self <: IchooseImage](x: Self) {
