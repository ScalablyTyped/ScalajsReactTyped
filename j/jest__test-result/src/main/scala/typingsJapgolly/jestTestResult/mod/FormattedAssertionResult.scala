...
import typingsJapgolly.jestTypes.mod.Callsite
...
trait FormattedAssertionResult extends StObject {
  ...
  var status: typingsJapgolly.jestTypes.mod.Status
...
object FormattedAssertionResult {
  ...
  inline def apply(
    ...
    fullName: String,
    status: typingsJapgolly.jestTypes.mod.Status,
    title: String
  ...
  extension [Self <: FormattedAssertionResult](x: Self) {
    ...
    inline def setStatus(value: typingsJapgolly.jestTypes.mod.Status): Self = StObject.set(x, "status", value.asInstanceOf[js.Any])
