...
import typingsJapgolly.jquery.JQuery.TypeEventHandlers
...
trait JQuery_[TElement]
  ...
  def insertAfter(target: JQuery[typingsJapgolly.std.Node]): this.type = js.native
  /**
  ...
  def insertBefore(target: JQuery[typingsJapgolly.std.Node]): this.type = js.native
  def is(
