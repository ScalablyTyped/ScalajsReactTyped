...
trait AnonTrigger[TTarget, TData] extends _SpecialEventHook[TTarget, TData] {
  /**
    ...
    */
  def trigger(event: typingsJapgolly.jquery.JQuery_.Event, data: TData): Unit | `false`
}

object AnonTrigger {
  @scala.inline
  def apply[TTarget, TData](trigger: (typingsJapgolly.jquery.JQuery_.Event, TData) => CallbackTo[Unit | `false`]): AnonTrigger[TTarget, TData] = {
