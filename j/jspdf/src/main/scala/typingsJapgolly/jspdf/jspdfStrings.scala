package typingsJapgolly.jspdf

import typingsJapgolly.jspdf.mod.ColorSpace
import typingsJapgolly.jspdf.mod.ImageCompression
import typingsJapgolly.jspdf.mod.ImageFormat
import typingsJapgolly.jspdf.mod.ShadingPatternType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object jspdfStrings {
  
  @js.native
  sealed trait `100` extends StObject
  inline def `100`: `100` = "100".asInstanceOf[`100`]
  
  @js.native
  sealed trait `200` extends StObject
  inline def `200`: `200` = "200".asInstanceOf[`200`]
  
  @js.native
  sealed trait `300` extends StObject
  inline def `300`: `300` = "300".asInstanceOf[`300`]
  
  @js.native
  sealed trait `400` extends StObject
  inline def `400`: `400` = "400".asInstanceOf[`400`]
  
  @js.native
  sealed trait `500` extends StObject
  inline def `500`: `500` = "500".asInstanceOf[`500`]
  
  @js.native
  sealed trait `600` extends StObject
  inline def `600`: `600` = "600".asInstanceOf[`600`]
  
  @js.native
  sealed trait `700` extends StObject
  inline def `700`: `700` = "700".asInstanceOf[`700`]
  
  @js.native
  sealed trait `800` extends StObject
  inline def `800`: `800` = "800".asInstanceOf[`800`]
  
  @js.native
  sealed trait `900` extends StObject
  inline def `900`: `900` = "900".asInstanceOf[`900`]
  
  @js.native
  sealed trait AppDefault extends StObject
  inline def AppDefault: AppDefault = "AppDefault".asInstanceOf[AppDefault]
  
  @js.native
  sealed trait ArtBox extends StObject
  inline def ArtBox: ArtBox = "ArtBox".asInstanceOf[ArtBox]
  
  @js.native
  sealed trait BMP
    extends StObject
       with ImageFormat
  inline def BMP: BMP = "BMP".asInstanceOf[BMP]
  
  @js.native
  sealed trait BleedBox extends StObject
  inline def BleedBox: BleedBox = "BleedBox".asInstanceOf[BleedBox]
  
  @js.native
  sealed trait CalGray
    extends StObject
       with ColorSpace
  inline def CalGray: CalGray = "CalGray".asInstanceOf[CalGray]
  
  @js.native
  sealed trait CalRGB
    extends StObject
       with ColorSpace
  inline def CalRGB: CalRGB = "CalRGB".asInstanceOf[CalRGB]
  
  @js.native
  sealed trait CropBox extends StObject
  inline def CropBox: CropBox = "CropBox".asInstanceOf[CropBox]
  
  @js.native
  sealed trait DeviceCMYK
    extends StObject
       with ColorSpace
  inline def DeviceCMYK: DeviceCMYK = "DeviceCMYK".asInstanceOf[DeviceCMYK]
  
  @js.native
  sealed trait DeviceGray
    extends StObject
       with ColorSpace
  inline def DeviceGray: DeviceGray = "DeviceGray".asInstanceOf[DeviceGray]
  
  @js.native
  sealed trait DeviceN
    extends StObject
       with ColorSpace
  inline def DeviceN: DeviceN = "DeviceN".asInstanceOf[DeviceN]
  
  @js.native
  sealed trait DeviceRGB
    extends StObject
       with ColorSpace
  inline def DeviceRGB: DeviceRGB = "DeviceRGB".asInstanceOf[DeviceRGB]
  
  @js.native
  sealed trait DuplexFlipLongEdge extends StObject
  inline def DuplexFlipLongEdge: DuplexFlipLongEdge = "DuplexFlipLongEdge".asInstanceOf[DuplexFlipLongEdge]
  
  @js.native
  sealed trait DuplexFlipShortEdge extends StObject
  inline def DuplexFlipShortEdge: DuplexFlipShortEdge = "DuplexFlipShortEdge".asInstanceOf[DuplexFlipShortEdge]
  
  @js.native
  sealed trait FAST
    extends StObject
       with ImageCompression
  inline def FAST: FAST = "FAST".asInstanceOf[FAST]
  
  @js.native
  sealed trait Fit extends StObject
  inline def Fit: Fit = "Fit".asInstanceOf[Fit]
  
  @js.native
  sealed trait FitH extends StObject
  inline def FitH: FitH = "FitH".asInstanceOf[FitH]
  
  @js.native
  sealed trait FitV extends StObject
  inline def FitV: FitV = "FitV".asInstanceOf[FitV]
  
  @js.native
  sealed trait FullScreen extends StObject
  inline def FullScreen: FullScreen = "FullScreen".asInstanceOf[FullScreen]
  
  @js.native
  sealed trait GIF87a
    extends StObject
       with ImageFormat
  inline def GIF87a: GIF87a = "GIF87a".asInstanceOf[GIF87a]
  
  @js.native
  sealed trait GIF89a
    extends StObject
       with ImageFormat
  inline def GIF89a: GIF89a = "GIF89a".asInstanceOf[GIF89a]
  
  @js.native
  sealed trait ICCBased
    extends StObject
       with ColorSpace
  inline def ICCBased: ICCBased = "ICCBased".asInstanceOf[ICCBased]
  
  @js.native
  sealed trait `Identity-H` extends StObject
  inline def `Identity-H`: `Identity-H` = "Identity-H".asInstanceOf[`Identity-H`]
  
  @js.native
  sealed trait Indexed
    extends StObject
       with ColorSpace
  inline def Indexed: Indexed = "Indexed".asInstanceOf[Indexed]
  
  @js.native
  sealed trait JPEG
    extends StObject
       with ImageFormat
  inline def JPEG: JPEG = "JPEG".asInstanceOf[JPEG]
  
  @js.native
  sealed trait JPEG2000
    extends StObject
       with ImageFormat
  inline def JPEG2000: JPEG2000 = "JPEG2000".asInstanceOf[JPEG2000]
  
  @js.native
  sealed trait JPG
    extends StObject
       with ImageFormat
  inline def JPG: JPG = "JPG".asInstanceOf[JPG]
  
  @js.native
  sealed trait L2R extends StObject
  inline def L2R: L2R = "L2R".asInstanceOf[L2R]
  
  @js.native
  sealed trait Lab
    extends StObject
       with ColorSpace
  inline def Lab: Lab = "Lab".asInstanceOf[Lab]
  
  @js.native
  sealed trait MEDIUM
    extends StObject
       with ImageCompression
  inline def MEDIUM: MEDIUM = "MEDIUM".asInstanceOf[MEDIUM]
  
  @js.native
  sealed trait MacRomanEncoding extends StObject
  inline def MacRomanEncoding: MacRomanEncoding = "MacRomanEncoding".asInstanceOf[MacRomanEncoding]
  
  @js.native
  sealed trait MediaBox extends StObject
  inline def MediaBox: MediaBox = "MediaBox".asInstanceOf[MediaBox]
  
  @js.native
  sealed trait NONE
    extends StObject
       with ImageCompression
  inline def NONE: NONE = "NONE".asInstanceOf[NONE]
  
  @js.native
  sealed trait None_ extends StObject
  inline def None_ : None_ = "None".asInstanceOf[None_]
  
  @js.native
  sealed trait Off extends StObject
  inline def Off: Off = "Off".asInstanceOf[Off]
  
  @js.native
  sealed trait On extends StObject
  inline def On: On = "On".asInstanceOf[On]
  
  @js.native
  sealed trait PNG
    extends StObject
       with ImageFormat
  inline def PNG: PNG = "PNG".asInstanceOf[PNG]
  
  @js.native
  sealed trait Pattern
    extends StObject
       with ColorSpace
  inline def Pattern: Pattern = "Pattern".asInstanceOf[Pattern]
  
  @js.native
  sealed trait R2L extends StObject
  inline def R2L: R2L = "R2L".asInstanceOf[R2L]
  
  @js.native
  sealed trait RGBA
    extends StObject
       with ImageFormat
  inline def RGBA: RGBA = "RGBA".asInstanceOf[RGBA]
  
  @js.native
  sealed trait SLOW
    extends StObject
       with ImageCompression
  inline def SLOW: SLOW = "SLOW".asInstanceOf[SLOW]
  
  @js.native
  sealed trait Separation
    extends StObject
       with ColorSpace
  inline def Separation: Separation = "Separation".asInstanceOf[Separation]
  
  @js.native
  sealed trait Simplex extends StObject
  inline def Simplex: Simplex = "Simplex".asInstanceOf[Simplex]
  
  @js.native
  sealed trait StandardEncoding extends StObject
  inline def StandardEncoding: StandardEncoding = "StandardEncoding".asInstanceOf[StandardEncoding]
  
  @js.native
  sealed trait TIFF
    extends StObject
       with ImageFormat
  inline def TIFF: TIFF = "TIFF".asInstanceOf[TIFF]
  
  @js.native
  sealed trait TrimBox extends StObject
  inline def TrimBox: TrimBox = "TrimBox".asInstanceOf[TrimBox]
  
  @js.native
  sealed trait UNKNOWN
    extends StObject
       with ImageFormat
  inline def UNKNOWN: UNKNOWN = "UNKNOWN".asInstanceOf[UNKNOWN]
  
  @js.native
  sealed trait UseNone extends StObject
  inline def UseNone: UseNone = "UseNone".asInstanceOf[UseNone]
  
  @js.native
  sealed trait UseOC extends StObject
  inline def UseOC: UseOC = "UseOC".asInstanceOf[UseOC]
  
  @js.native
  sealed trait UseOutlines extends StObject
  inline def UseOutlines: UseOutlines = "UseOutlines".asInstanceOf[UseOutlines]
  
  @js.native
  sealed trait UseThumbs extends StObject
  inline def UseThumbs: UseThumbs = "UseThumbs".asInstanceOf[UseThumbs]
  
  @js.native
  sealed trait WEBP
    extends StObject
       with ImageFormat
  inline def WEBP: WEBP = "WEBP".asInstanceOf[WEBP]
  
  @js.native
  sealed trait WinAnsiEncoding extends StObject
  inline def WinAnsiEncoding: WinAnsiEncoding = "WinAnsiEncoding".asInstanceOf[WinAnsiEncoding]
  
  @js.native
  sealed trait XYZ extends StObject
  inline def XYZ: XYZ = "XYZ".asInstanceOf[XYZ]
  
  @js.native
  sealed trait addToPathForClipping extends StObject
  inline def addToPathForClipping: addToPathForClipping = "addToPathForClipping".asInstanceOf[addToPathForClipping]
  
  @js.native
  sealed trait af extends StObject
  inline def af: af = "af".asInstanceOf[af]
  
  @js.native
  sealed trait alphabetic extends StObject
  inline def alphabetic: alphabetic = "alphabetic".asInstanceOf[alphabetic]
  
  @js.native
  sealed trait an extends StObject
  inline def an: an = "an".asInstanceOf[an]
  
  @js.native
  sealed trait `annot-forms` extends StObject
  inline def `annot-forms`: `annot-forms` = "annot-forms".asInstanceOf[`annot-forms`]
  
  @js.native
  sealed trait ar extends StObject
  inline def ar: ar = "ar".asInstanceOf[ar]
  
  @js.native
  sealed trait `ar-AE` extends StObject
  inline def `ar-AE`: `ar-AE` = "ar-AE".asInstanceOf[`ar-AE`]
  
  @js.native
  sealed trait `ar-BH` extends StObject
  inline def `ar-BH`: `ar-BH` = "ar-BH".asInstanceOf[`ar-BH`]
  
  @js.native
  sealed trait `ar-DZ` extends StObject
  inline def `ar-DZ`: `ar-DZ` = "ar-DZ".asInstanceOf[`ar-DZ`]
  
  @js.native
  sealed trait `ar-EG` extends StObject
  inline def `ar-EG`: `ar-EG` = "ar-EG".asInstanceOf[`ar-EG`]
  
  @js.native
  sealed trait `ar-IQ` extends StObject
  inline def `ar-IQ`: `ar-IQ` = "ar-IQ".asInstanceOf[`ar-IQ`]
  
  @js.native
  sealed trait `ar-JO` extends StObject
  inline def `ar-JO`: `ar-JO` = "ar-JO".asInstanceOf[`ar-JO`]
  
  @js.native
  sealed trait `ar-KW` extends StObject
  inline def `ar-KW`: `ar-KW` = "ar-KW".asInstanceOf[`ar-KW`]
  
  @js.native
  sealed trait `ar-LB` extends StObject
  inline def `ar-LB`: `ar-LB` = "ar-LB".asInstanceOf[`ar-LB`]
  
  @js.native
  sealed trait `ar-LY` extends StObject
  inline def `ar-LY`: `ar-LY` = "ar-LY".asInstanceOf[`ar-LY`]
  
  @js.native
  sealed trait `ar-MA` extends StObject
  inline def `ar-MA`: `ar-MA` = "ar-MA".asInstanceOf[`ar-MA`]
  
  @js.native
  sealed trait `ar-OM` extends StObject
  inline def `ar-OM`: `ar-OM` = "ar-OM".asInstanceOf[`ar-OM`]
  
  @js.native
  sealed trait `ar-QA` extends StObject
  inline def `ar-QA`: `ar-QA` = "ar-QA".asInstanceOf[`ar-QA`]
  
  @js.native
  sealed trait `ar-SA` extends StObject
  inline def `ar-SA`: `ar-SA` = "ar-SA".asInstanceOf[`ar-SA`]
  
  @js.native
  sealed trait `ar-SY` extends StObject
  inline def `ar-SY`: `ar-SY` = "ar-SY".asInstanceOf[`ar-SY`]
  
  @js.native
  sealed trait `ar-TN` extends StObject
  inline def `ar-TN`: `ar-TN` = "ar-TN".asInstanceOf[`ar-TN`]
  
  @js.native
  sealed trait `ar-YE` extends StObject
  inline def `ar-YE`: `ar-YE` = "ar-YE".asInstanceOf[`ar-YE`]
  
  @js.native
  sealed trait arraybuffer extends StObject
  inline def arraybuffer: arraybuffer = "arraybuffer".asInstanceOf[arraybuffer]
  
  @js.native
  sealed trait as extends StObject
  inline def as: as = "as".asInstanceOf[as]
  
  @js.native
  sealed trait ast extends StObject
  inline def ast: ast = "ast".asInstanceOf[ast]
  
  @js.native
  sealed trait axial
    extends StObject
       with ShadingPatternType
  inline def axial: axial = "axial".asInstanceOf[axial]
  
  @js.native
  sealed trait az extends StObject
  inline def az: az = "az".asInstanceOf[az]
  
  @js.native
  sealed trait be extends StObject
  inline def be: be = "be".asInstanceOf[be]
  
  @js.native
  sealed trait bevel extends StObject
  inline def bevel: bevel = "bevel".asInstanceOf[bevel]
  
  @js.native
  sealed trait bg extends StObject
  inline def bg: bg = "bg".asInstanceOf[bg]
  
  @js.native
  sealed trait blob extends StObject
  inline def blob: blob = "blob".asInstanceOf[blob]
  
  @js.native
  sealed trait bloburi extends StObject
  inline def bloburi: bloburi = "bloburi".asInstanceOf[bloburi]
  
  @js.native
  sealed trait bloburl extends StObject
  inline def bloburl: bloburl = "bloburl".asInstanceOf[bloburl]
  
  @js.native
  sealed trait bn extends StObject
  inline def bn: bn = "bn".asInstanceOf[bn]
  
  @js.native
  sealed trait bold extends StObject
  inline def bold: bold = "bold".asInstanceOf[bold]
  
  @js.native
  sealed trait bottom extends StObject
  inline def bottom: bottom = "bottom".asInstanceOf[bottom]
  
  @js.native
  sealed trait br extends StObject
  inline def br: br = "br".asInstanceOf[br]
  
  @js.native
  sealed trait bs extends StObject
  inline def bs: bs = "bs".asInstanceOf[bs]
  
  @js.native
  sealed trait butt extends StObject
  inline def butt: butt = "butt".asInstanceOf[butt]
  
  @js.native
  sealed trait ca extends StObject
  inline def ca: ca = "ca".asInstanceOf[ca]
  
  @js.native
  sealed trait canvas extends StObject
  inline def canvas: canvas = "canvas".asInstanceOf[canvas]
  
  @js.native
  sealed trait ce extends StObject
  inline def ce: ce = "ce".asInstanceOf[ce]
  
  @js.native
  sealed trait center extends StObject
  inline def center: center = "center".asInstanceOf[center]
  
  @js.native
  sealed trait ch extends StObject
  inline def ch: ch = "ch".asInstanceOf[ch]
  
  @js.native
  sealed trait cm extends StObject
  inline def cm: cm = "cm".asInstanceOf[cm]
  
  @js.native
  sealed trait co extends StObject
  inline def co: co = "co".asInstanceOf[co]
  
  @js.native
  sealed trait condensed extends StObject
  inline def condensed: condensed = "condensed".asInstanceOf[condensed]
  
  @js.native
  sealed trait container extends StObject
  inline def container: container = "container".asInstanceOf[container]
  
  @js.native
  sealed trait context2d extends StObject
  inline def context2d: context2d = "context2d".asInstanceOf[context2d]
  
  @js.native
  sealed trait continuous extends StObject
  inline def continuous: continuous = "continuous".asInstanceOf[continuous]
  
  @js.native
  sealed trait copy extends StObject
  inline def copy: copy = "copy".asInstanceOf[copy]
  
  @js.native
  sealed trait cr extends StObject
  inline def cr: cr = "cr".asInstanceOf[cr]
  
  @js.native
  sealed trait cs extends StObject
  inline def cs: cs = "cs".asInstanceOf[cs]
  
  @js.native
  sealed trait cv extends StObject
  inline def cv: cv = "cv".asInstanceOf[cv]
  
  @js.native
  sealed trait cy extends StObject
  inline def cy: cy = "cy".asInstanceOf[cy]
  
  @js.native
  sealed trait da extends StObject
  inline def da: da = "da".asInstanceOf[da]
  
  @js.native
  sealed trait datauri extends StObject
  inline def datauri: datauri = "datauri".asInstanceOf[datauri]
  
  @js.native
  sealed trait datauristring extends StObject
  inline def datauristring: datauristring = "datauristring".asInstanceOf[datauristring]
  
  @js.native
  sealed trait dataurl extends StObject
  inline def dataurl: dataurl = "dataurl".asInstanceOf[dataurl]
  
  @js.native
  sealed trait dataurlnewwindow extends StObject
  inline def dataurlnewwindow: dataurlnewwindow = "dataurlnewwindow".asInstanceOf[dataurlnewwindow]
  
  @js.native
  sealed trait dataurlstring extends StObject
  inline def dataurlstring: dataurlstring = "dataurlstring".asInstanceOf[dataurlstring]
  
  @js.native
  sealed trait de extends StObject
  inline def de: de = "de".asInstanceOf[de]
  
  @js.native
  sealed trait `de-AT` extends StObject
  inline def `de-AT`: `de-AT` = "de-AT".asInstanceOf[`de-AT`]
  
  @js.native
  sealed trait `de-CH` extends StObject
  inline def `de-CH`: `de-CH` = "de-CH".asInstanceOf[`de-CH`]
  
  @js.native
  sealed trait `de-DE` extends StObject
  inline def `de-DE`: `de-DE` = "de-DE".asInstanceOf[`de-DE`]
  
  @js.native
  sealed trait `de-LI` extends StObject
  inline def `de-LI`: `de-LI` = "de-LI".asInstanceOf[`de-LI`]
  
  @js.native
  sealed trait `de-LU` extends StObject
  inline def `de-LU`: `de-LU` = "de-LU".asInstanceOf[`de-LU`]
  
  @js.native
  sealed trait el extends StObject
  inline def el: el = "el".asInstanceOf[el]
  
  @js.native
  sealed trait em extends StObject
  inline def em: em = "em".asInstanceOf[em]
  
  @js.native
  sealed trait en extends StObject
  inline def en: en = "en".asInstanceOf[en]
  
  @js.native
  sealed trait `en-AU` extends StObject
  inline def `en-AU`: `en-AU` = "en-AU".asInstanceOf[`en-AU`]
  
  @js.native
  sealed trait `en-BZ` extends StObject
  inline def `en-BZ`: `en-BZ` = "en-BZ".asInstanceOf[`en-BZ`]
  
  @js.native
  sealed trait `en-CA` extends StObject
  inline def `en-CA`: `en-CA` = "en-CA".asInstanceOf[`en-CA`]
  
  @js.native
  sealed trait `en-GB` extends StObject
  inline def `en-GB`: `en-GB` = "en-GB".asInstanceOf[`en-GB`]
  
  @js.native
  sealed trait `en-IE` extends StObject
  inline def `en-IE`: `en-IE` = "en-IE".asInstanceOf[`en-IE`]
  
  @js.native
  sealed trait `en-JM` extends StObject
  inline def `en-JM`: `en-JM` = "en-JM".asInstanceOf[`en-JM`]
  
  @js.native
  sealed trait `en-NZ` extends StObject
  inline def `en-NZ`: `en-NZ` = "en-NZ".asInstanceOf[`en-NZ`]
  
  @js.native
  sealed trait `en-PH` extends StObject
  inline def `en-PH`: `en-PH` = "en-PH".asInstanceOf[`en-PH`]
  
  @js.native
  sealed trait `en-TT` extends StObject
  inline def `en-TT`: `en-TT` = "en-TT".asInstanceOf[`en-TT`]
  
  @js.native
  sealed trait `en-US` extends StObject
  inline def `en-US`: `en-US` = "en-US".asInstanceOf[`en-US`]
  
  @js.native
  sealed trait `en-ZA` extends StObject
  inline def `en-ZA`: `en-ZA` = "en-ZA".asInstanceOf[`en-ZA`]
  
  @js.native
  sealed trait `en-ZW` extends StObject
  inline def `en-ZW`: `en-ZW` = "en-ZW".asInstanceOf[`en-ZW`]
  
  @js.native
  sealed trait end extends StObject
  inline def end: end = "end".asInstanceOf[end]
  
  @js.native
  sealed trait eo extends StObject
  inline def eo: eo = "eo".asInstanceOf[eo]
  
  @js.native
  sealed trait es extends StObject
  inline def es: es = "es".asInstanceOf[es]
  
  @js.native
  sealed trait `es-AR` extends StObject
  inline def `es-AR`: `es-AR` = "es-AR".asInstanceOf[`es-AR`]
  
  @js.native
  sealed trait `es-BO` extends StObject
  inline def `es-BO`: `es-BO` = "es-BO".asInstanceOf[`es-BO`]
  
  @js.native
  sealed trait `es-CL` extends StObject
  inline def `es-CL`: `es-CL` = "es-CL".asInstanceOf[`es-CL`]
  
  @js.native
  sealed trait `es-CO` extends StObject
  inline def `es-CO`: `es-CO` = "es-CO".asInstanceOf[`es-CO`]
  
  @js.native
  sealed trait `es-CR` extends StObject
  inline def `es-CR`: `es-CR` = "es-CR".asInstanceOf[`es-CR`]
  
  @js.native
  sealed trait `es-DO` extends StObject
  inline def `es-DO`: `es-DO` = "es-DO".asInstanceOf[`es-DO`]
  
  @js.native
  sealed trait `es-EC` extends StObject
  inline def `es-EC`: `es-EC` = "es-EC".asInstanceOf[`es-EC`]
  
  @js.native
  sealed trait `es-ES` extends StObject
  inline def `es-ES`: `es-ES` = "es-ES".asInstanceOf[`es-ES`]
  
  @js.native
  sealed trait `es-GT` extends StObject
  inline def `es-GT`: `es-GT` = "es-GT".asInstanceOf[`es-GT`]
  
  @js.native
  sealed trait `es-HN` extends StObject
  inline def `es-HN`: `es-HN` = "es-HN".asInstanceOf[`es-HN`]
  
  @js.native
  sealed trait `es-MX` extends StObject
  inline def `es-MX`: `es-MX` = "es-MX".asInstanceOf[`es-MX`]
  
  @js.native
  sealed trait `es-NI` extends StObject
  inline def `es-NI`: `es-NI` = "es-NI".asInstanceOf[`es-NI`]
  
  @js.native
  sealed trait `es-PA` extends StObject
  inline def `es-PA`: `es-PA` = "es-PA".asInstanceOf[`es-PA`]
  
  @js.native
  sealed trait `es-PE` extends StObject
  inline def `es-PE`: `es-PE` = "es-PE".asInstanceOf[`es-PE`]
  
  @js.native
  sealed trait `es-PR` extends StObject
  inline def `es-PR`: `es-PR` = "es-PR".asInstanceOf[`es-PR`]
  
  @js.native
  sealed trait `es-PY` extends StObject
  inline def `es-PY`: `es-PY` = "es-PY".asInstanceOf[`es-PY`]
  
  @js.native
  sealed trait `es-SV` extends StObject
  inline def `es-SV`: `es-SV` = "es-SV".asInstanceOf[`es-SV`]
  
  @js.native
  sealed trait `es-UY` extends StObject
  inline def `es-UY`: `es-UY` = "es-UY".asInstanceOf[`es-UY`]
  
  @js.native
  sealed trait `es-VE` extends StObject
  inline def `es-VE`: `es-VE` = "es-VE".asInstanceOf[`es-VE`]
  
  @js.native
  sealed trait et extends StObject
  inline def et: et = "et".asInstanceOf[et]
  
  @js.native
  sealed trait eu extends StObject
  inline def eu: eu = "eu".asInstanceOf[eu]
  
  @js.native
  sealed trait evenodd extends StObject
  inline def evenodd: evenodd = "evenodd".asInstanceOf[evenodd]
  
  @js.native
  sealed trait ex extends StObject
  inline def ex: ex = "ex".asInstanceOf[ex]
  
  @js.native
  sealed trait expanded extends StObject
  inline def expanded: expanded = "expanded".asInstanceOf[expanded]
  
  @js.native
  sealed trait `extra-condensed` extends StObject
  inline def `extra-condensed`: `extra-condensed` = "extra-condensed".asInstanceOf[`extra-condensed`]
  
  @js.native
  sealed trait `extra-expanded` extends StObject
  inline def `extra-expanded`: `extra-expanded` = "extra-expanded".asInstanceOf[`extra-expanded`]
  
  @js.native
  sealed trait fa extends StObject
  inline def fa: fa = "fa".asInstanceOf[fa]
  
  @js.native
  sealed trait `fa-IR` extends StObject
  inline def `fa-IR`: `fa-IR` = "fa-IR".asInstanceOf[`fa-IR`]
  
  @js.native
  sealed trait fi extends StObject
  inline def fi: fi = "fi".asInstanceOf[fi]
  
  @js.native
  sealed trait fill extends StObject
  inline def fill: fill = "fill".asInstanceOf[fill]
  
  @js.native
  sealed trait fillAndAddForClipping extends StObject
  inline def fillAndAddForClipping: fillAndAddForClipping = "fillAndAddForClipping".asInstanceOf[fillAndAddForClipping]
  
  @js.native
  sealed trait fillThenStroke extends StObject
  inline def fillThenStroke: fillThenStroke = "fillThenStroke".asInstanceOf[fillThenStroke]
  
  @js.native
  sealed trait fillThenStrokeAndAddToPathForClipping extends StObject
  inline def fillThenStrokeAndAddToPathForClipping: fillThenStrokeAndAddToPathForClipping = "fillThenStrokeAndAddToPathForClipping".asInstanceOf[fillThenStrokeAndAddToPathForClipping]
  
  @js.native
  sealed trait fj extends StObject
  inline def fj: fj = "fj".asInstanceOf[fj]
  
  @js.native
  sealed trait fo extends StObject
  inline def fo: fo = "fo".asInstanceOf[fo]
  
  @js.native
  sealed trait fr extends StObject
  inline def fr: fr = "fr".asInstanceOf[fr]
  
  @js.native
  sealed trait `fr-BE` extends StObject
  inline def `fr-BE`: `fr-BE` = "fr-BE".asInstanceOf[`fr-BE`]
  
  @js.native
  sealed trait `fr-CA` extends StObject
  inline def `fr-CA`: `fr-CA` = "fr-CA".asInstanceOf[`fr-CA`]
  
  @js.native
  sealed trait `fr-CH` extends StObject
  inline def `fr-CH`: `fr-CH` = "fr-CH".asInstanceOf[`fr-CH`]
  
  @js.native
  sealed trait `fr-FR` extends StObject
  inline def `fr-FR`: `fr-FR` = "fr-FR".asInstanceOf[`fr-FR`]
  
  @js.native
  sealed trait `fr-LU` extends StObject
  inline def `fr-LU`: `fr-LU` = "fr-LU".asInstanceOf[`fr-LU`]
  
  @js.native
  sealed trait `fr-MC` extends StObject
  inline def `fr-MC`: `fr-MC` = "fr-MC".asInstanceOf[`fr-MC`]
  
  @js.native
  sealed trait freetext extends StObject
  inline def freetext: freetext = "freetext".asInstanceOf[freetext]
  
  @js.native
  sealed trait fullheight extends StObject
  inline def fullheight: fullheight = "fullheight".asInstanceOf[fullheight]
  
  @js.native
  sealed trait fullpage extends StObject
  inline def fullpage: fullpage = "fullpage".asInstanceOf[fullpage]
  
  @js.native
  sealed trait fullwidth extends StObject
  inline def fullwidth: fullwidth = "fullwidth".asInstanceOf[fullwidth]
  
  @js.native
  sealed trait fur extends StObject
  inline def fur: fur = "fur".asInstanceOf[fur]
  
  @js.native
  sealed trait fy extends StObject
  inline def fy: fy = "fy".asInstanceOf[fy]
  
  @js.native
  sealed trait ga extends StObject
  inline def ga: ga = "ga".asInstanceOf[ga]
  
  @js.native
  sealed trait gd extends StObject
  inline def gd: gd = "gd".asInstanceOf[gd]
  
  @js.native
  sealed trait `gd-IE` extends StObject
  inline def `gd-IE`: `gd-IE` = "gd-IE".asInstanceOf[`gd-IE`]
  
  @js.native
  sealed trait gl extends StObject
  inline def gl: gl = "gl".asInstanceOf[gl]
  
  @js.native
  sealed trait gu extends StObject
  inline def gu: gu = "gu".asInstanceOf[gu]
  
  @js.native
  sealed trait hanging extends StObject
  inline def hanging: hanging = "hanging".asInstanceOf[hanging]
  
  @js.native
  sealed trait he extends StObject
  inline def he: he = "he".asInstanceOf[he]
  
  @js.native
  sealed trait hi extends StObject
  inline def hi: hi = "hi".asInstanceOf[hi]
  
  @js.native
  sealed trait high extends StObject
  inline def high: high = "high".asInstanceOf[high]
  
  @js.native
  sealed trait hr extends StObject
  inline def hr: hr = "hr".asInstanceOf[hr]
  
  @js.native
  sealed trait hsb extends StObject
  inline def hsb: hsb = "hsb".asInstanceOf[hsb]
  
  @js.native
  sealed trait ht extends StObject
  inline def ht: ht = "ht".asInstanceOf[ht]
  
  @js.native
  sealed trait hu extends StObject
  inline def hu: hu = "hu".asInstanceOf[hu]
  
  @js.native
  sealed trait hy extends StObject
  inline def hy: hy = "hy".asInstanceOf[hy]
  
  @js.native
  sealed trait id extends StObject
  inline def id: id = "id".asInstanceOf[id]
  
  @js.native
  sealed trait ideographic extends StObject
  inline def ideographic: ideographic = "ideographic".asInstanceOf[ideographic]
  
  @js.native
  sealed trait img extends StObject
  inline def img: img = "img".asInstanceOf[img]
  
  @js.native
  sealed trait in extends StObject
  inline def in: in = "in".asInstanceOf[in]
  
  @js.native
  sealed trait invisible extends StObject
  inline def invisible: invisible = "invisible".asInstanceOf[invisible]
  
  @js.native
  sealed trait is extends StObject
  inline def is: is = "is".asInstanceOf[is]
  
  @js.native
  sealed trait it extends StObject
  inline def it: it = "it".asInstanceOf[it]
  
  @js.native
  sealed trait `it-CH` extends StObject
  inline def `it-CH`: `it-CH` = "it-CH".asInstanceOf[`it-CH`]
  
  @js.native
  sealed trait italic extends StObject
  inline def italic: italic = "italic".asInstanceOf[italic]
  
  @js.native
  sealed trait iu extends StObject
  inline def iu: iu = "iu".asInstanceOf[iu]
  
  @js.native
  sealed trait ja extends StObject
  inline def ja: ja = "ja".asInstanceOf[ja]
  
  @js.native
  sealed trait javascript extends StObject
  inline def javascript: javascript = "javascript".asInstanceOf[javascript]
  
  @js.native
  sealed trait ji extends StObject
  inline def ji: ji = "ji".asInstanceOf[ji]
  
  @js.native
  sealed trait jpeg_ extends StObject
  inline def jpeg_ : jpeg_ = "jpeg".asInstanceOf[jpeg_]
  
  @js.native
  sealed trait justify extends StObject
  inline def justify: justify = "justify".asInstanceOf[justify]
  
  @js.native
  sealed trait ka extends StObject
  inline def ka: ka = "ka".asInstanceOf[ka]
  
  @js.native
  sealed trait kk extends StObject
  inline def kk: kk = "kk".asInstanceOf[kk]
  
  @js.native
  sealed trait km extends StObject
  inline def km: km = "km".asInstanceOf[km]
  
  @js.native
  sealed trait kn extends StObject
  inline def kn: kn = "kn".asInstanceOf[kn]
  
  @js.native
  sealed trait ko extends StObject
  inline def ko: ko = "ko".asInstanceOf[ko]
  
  @js.native
  sealed trait `ko-KP` extends StObject
  inline def `ko-KP`: `ko-KP` = "ko-KP".asInstanceOf[`ko-KP`]
  
  @js.native
  sealed trait `ko-KR` extends StObject
  inline def `ko-KR`: `ko-KR` = "ko-KR".asInstanceOf[`ko-KR`]
  
  @js.native
  sealed trait ks extends StObject
  inline def ks: ks = "ks".asInstanceOf[ks]
  
  @js.native
  sealed trait ky extends StObject
  inline def ky: ky = "ky".asInstanceOf[ky]
  
  @js.native
  sealed trait l extends StObject
  inline def l: l = "l".asInstanceOf[l]
  
  @js.native
  sealed trait la extends StObject
  inline def la: la = "la".asInstanceOf[la]
  
  @js.native
  sealed trait landscape extends StObject
  inline def landscape: landscape = "landscape".asInstanceOf[landscape]
  
  @js.native
  sealed trait lb extends StObject
  inline def lb: lb = "lb".asInstanceOf[lb]
  
  @js.native
  sealed trait left extends StObject
  inline def left: left = "left".asInstanceOf[left]
  
  @js.native
  sealed trait link extends StObject
  inline def link: link = "link".asInstanceOf[link]
  
  @js.native
  sealed trait low extends StObject
  inline def low: low = "low".asInstanceOf[low]
  
  @js.native
  sealed trait lt extends StObject
  inline def lt: lt = "lt".asInstanceOf[lt]
  
  @js.native
  sealed trait lv extends StObject
  inline def lv: lv = "lv".asInstanceOf[lv]
  
  @js.native
  sealed trait mi extends StObject
  inline def mi: mi = "mi".asInstanceOf[mi]
  
  @js.native
  sealed trait middle extends StObject
  inline def middle: middle = "middle".asInstanceOf[middle]
  
  @js.native
  sealed trait miter extends StObject
  inline def miter: miter = "miter".asInstanceOf[miter]
  
  @js.native
  sealed trait mk extends StObject
  inline def mk: mk = "mk".asInstanceOf[mk]
  
  @js.native
  sealed trait ml extends StObject
  inline def ml: ml = "ml".asInstanceOf[ml]
  
  @js.native
  sealed trait mm extends StObject
  inline def mm: mm = "mm".asInstanceOf[mm]
  
  @js.native
  sealed trait mo extends StObject
  inline def mo: mo = "mo".asInstanceOf[mo]
  
  @js.native
  sealed trait modify extends StObject
  inline def modify: modify = "modify".asInstanceOf[modify]
  
  @js.native
  sealed trait mr extends StObject
  inline def mr: mr = "mr".asInstanceOf[mr]
  
  @js.native
  sealed trait ms extends StObject
  inline def ms: ms = "ms".asInstanceOf[ms]
  
  @js.native
  sealed trait mt extends StObject
  inline def mt: mt = "mt".asInstanceOf[mt]
  
  @js.native
  sealed trait my extends StObject
  inline def my: my = "my".asInstanceOf[my]
  
  @js.native
  sealed trait nb extends StObject
  inline def nb: nb = "nb".asInstanceOf[nb]
  
  @js.native
  sealed trait ne extends StObject
  inline def ne: ne = "ne".asInstanceOf[ne]
  
  @js.native
  sealed trait ng extends StObject
  inline def ng: ng = "ng".asInstanceOf[ng]
  
  @js.native
  sealed trait nl extends StObject
  inline def nl: nl = "nl".asInstanceOf[nl]
  
  @js.native
  sealed trait `nl-BE` extends StObject
  inline def `nl-BE`: `nl-BE` = "nl-BE".asInstanceOf[`nl-BE`]
  
  @js.native
  sealed trait nn extends StObject
  inline def nn: nn = "nn".asInstanceOf[nn]
  
  @js.native
  sealed trait no extends StObject
  inline def no: no = "no".asInstanceOf[no]
  
  @js.native
  sealed trait `non-conform` extends StObject
  inline def `non-conform`: `non-conform` = "non-conform".asInstanceOf[`non-conform`]
  
  @js.native
  sealed trait none__ extends StObject
  inline def none__ : none__ = "none".asInstanceOf[none__]
  
  @js.native
  sealed trait normal extends StObject
  inline def normal: normal = "normal".asInstanceOf[normal]
  
  @js.native
  sealed trait nv extends StObject
  inline def nv: nv = "nv".asInstanceOf[nv]
  
  @js.native
  sealed trait oblique extends StObject
  inline def oblique: oblique = "oblique".asInstanceOf[oblique]
  
  @js.native
  sealed trait oc extends StObject
  inline def oc: oc = "oc".asInstanceOf[oc]
  
  @js.native
  sealed trait om extends StObject
  inline def om: om = "om".asInstanceOf[om]
  
  @js.native
  sealed trait or extends StObject
  inline def or: or = "or".asInstanceOf[or]
  
  @js.native
  sealed trait original extends StObject
  inline def original: original = "original".asInstanceOf[original]
  
  @js.native
  sealed trait p extends StObject
  inline def p: p = "p".asInstanceOf[p]
  
  @js.native
  sealed trait pa extends StObject
  inline def pa: pa = "pa".asInstanceOf[pa]
  
  @js.native
  sealed trait `pa-IN` extends StObject
  inline def `pa-IN`: `pa-IN` = "pa-IN".asInstanceOf[`pa-IN`]
  
  @js.native
  sealed trait `pa-PK` extends StObject
  inline def `pa-PK`: `pa-PK` = "pa-PK".asInstanceOf[`pa-PK`]
  
  @js.native
  sealed trait pc extends StObject
  inline def pc: pc = "pc".asInstanceOf[pc]
  
  @js.native
  sealed trait pdf extends StObject
  inline def pdf: pdf = "pdf".asInstanceOf[pdf]
  
  @js.native
  sealed trait pdfjsnewwindow extends StObject
  inline def pdfjsnewwindow: pdfjsnewwindow = "pdfjsnewwindow".asInstanceOf[pdfjsnewwindow]
  
  @js.native
  sealed trait pdfobjectnewwindow extends StObject
  inline def pdfobjectnewwindow: pdfobjectnewwindow = "pdfobjectnewwindow".asInstanceOf[pdfobjectnewwindow]
  
  @js.native
  sealed trait pl extends StObject
  inline def pl: pl = "pl".asInstanceOf[pl]
  
  @js.native
  sealed trait png_ extends StObject
  inline def png_ : png_ = "png".asInstanceOf[png_]
  
  @js.native
  sealed trait portrait extends StObject
  inline def portrait: portrait = "portrait".asInstanceOf[portrait]
  
  @js.native
  sealed trait print extends StObject
  inline def print: print = "print".asInstanceOf[print]
  
  @js.native
  sealed trait pt extends StObject
  inline def pt: pt = "pt".asInstanceOf[pt]
  
  @js.native
  sealed trait `pt-BR` extends StObject
  inline def `pt-BR`: `pt-BR` = "pt-BR".asInstanceOf[`pt-BR`]
  
  @js.native
  sealed trait px extends StObject
  inline def px: px = "px".asInstanceOf[px]
  
  @js.native
  sealed trait qu extends StObject
  inline def qu: qu = "qu".asInstanceOf[qu]
  
  @js.native
  sealed trait radial
    extends StObject
       with ShadingPatternType
  inline def radial: radial = "radial".asInstanceOf[radial]
  
  @js.native
  sealed trait reset extends StObject
  inline def reset: reset = "reset".asInstanceOf[reset]
  
  @js.native
  sealed trait right extends StObject
  inline def right: right = "right".asInstanceOf[right]
  
  @js.native
  sealed trait rm extends StObject
  inline def rm: rm = "rm".asInstanceOf[rm]
  
  @js.native
  sealed trait ro extends StObject
  inline def ro: ro = "ro".asInstanceOf[ro]
  
  @js.native
  sealed trait `ro-MO` extends StObject
  inline def `ro-MO`: `ro-MO` = "ro-MO".asInstanceOf[`ro-MO`]
  
  @js.native
  sealed trait round extends StObject
  inline def round: round = "round".asInstanceOf[round]
  
  @js.native
  sealed trait ru extends StObject
  inline def ru: ru = "ru".asInstanceOf[ru]
  
  @js.native
  sealed trait `ru-MO` extends StObject
  inline def `ru-MO`: `ru-MO` = "ru-MO".asInstanceOf[`ru-MO`]
  
  @js.native
  sealed trait sa extends StObject
  inline def sa: sa = "sa".asInstanceOf[sa]
  
  @js.native
  sealed trait sb extends StObject
  inline def sb: sb = "sb".asInstanceOf[sb]
  
  @js.native
  sealed trait sc extends StObject
  inline def sc: sc = "sc".asInstanceOf[sc]
  
  @js.native
  sealed trait sd extends StObject
  inline def sd: sd = "sd".asInstanceOf[sd]
  
  @js.native
  sealed trait `semi-condensed` extends StObject
  inline def `semi-condensed`: `semi-condensed` = "semi-condensed".asInstanceOf[`semi-condensed`]
  
  @js.native
  sealed trait `semi-expanded` extends StObject
  inline def `semi-expanded`: `semi-expanded` = "semi-expanded".asInstanceOf[`semi-expanded`]
  
  @js.native
  sealed trait sg extends StObject
  inline def sg: sg = "sg".asInstanceOf[sg]
  
  @js.native
  sealed trait si extends StObject
  inline def si: si = "si".asInstanceOf[si]
  
  @js.native
  sealed trait single extends StObject
  inline def single: single = "single".asInstanceOf[single]
  
  @js.native
  sealed trait sk extends StObject
  inline def sk: sk = "sk".asInstanceOf[sk]
  
  @js.native
  sealed trait sl extends StObject
  inline def sl: sl = "sl".asInstanceOf[sl]
  
  @js.native
  sealed trait slice extends StObject
  inline def slice: slice = "slice".asInstanceOf[slice]
  
  @js.native
  sealed trait smart extends StObject
  inline def smart: smart = "smart".asInstanceOf[smart]
  
  @js.native
  sealed trait so extends StObject
  inline def so: so = "so".asInstanceOf[so]
  
  @js.native
  sealed trait `source-over` extends StObject
  inline def `source-over`: `source-over` = "source-over".asInstanceOf[`source-over`]
  
  @js.native
  sealed trait sq extends StObject
  inline def sq: sq = "sq".asInstanceOf[sq]
  
  @js.native
  sealed trait square extends StObject
  inline def square: square = "square".asInstanceOf[square]
  
  @js.native
  sealed trait sr extends StObject
  inline def sr: sr = "sr".asInstanceOf[sr]
  
  @js.native
  sealed trait start extends StObject
  inline def start: start = "start".asInstanceOf[start]
  
  @js.native
  sealed trait string extends StObject
  inline def string: string = "string".asInstanceOf[string]
  
  @js.native
  sealed trait stroke extends StObject
  inline def stroke: stroke = "stroke".asInstanceOf[stroke]
  
  @js.native
  sealed trait strokeAndAddPathForClipping extends StObject
  inline def strokeAndAddPathForClipping: strokeAndAddPathForClipping = "strokeAndAddPathForClipping".asInstanceOf[strokeAndAddPathForClipping]
  
  @js.native
  sealed trait sv extends StObject
  inline def sv: sv = "sv".asInstanceOf[sv]
  
  @js.native
  sealed trait `sv-FI` extends StObject
  inline def `sv-FI`: `sv-FI` = "sv-FI".asInstanceOf[`sv-FI`]
  
  @js.native
  sealed trait `sv-SV` extends StObject
  inline def `sv-SV`: `sv-SV` = "sv-SV".asInstanceOf[`sv-SV`]
  
  @js.native
  sealed trait sw extends StObject
  inline def sw: sw = "sw".asInstanceOf[sw]
  
  @js.native
  sealed trait sx extends StObject
  inline def sx: sx = "sx".asInstanceOf[sx]
  
  @js.native
  sealed trait sz extends StObject
  inline def sz: sz = "sz".asInstanceOf[sz]
  
  @js.native
  sealed trait ta extends StObject
  inline def ta: ta = "ta".asInstanceOf[ta]
  
  @js.native
  sealed trait te extends StObject
  inline def te: te = "te".asInstanceOf[te]
  
  @js.native
  sealed trait text extends StObject
  inline def text: text = "text".asInstanceOf[text]
  
  @js.native
  sealed trait th extends StObject
  inline def th: th = "th".asInstanceOf[th]
  
  @js.native
  sealed trait tig extends StObject
  inline def tig: tig = "tig".asInstanceOf[tig]
  
  @js.native
  sealed trait tk extends StObject
  inline def tk: tk = "tk".asInstanceOf[tk]
  
  @js.native
  sealed trait tlh extends StObject
  inline def tlh: tlh = "tlh".asInstanceOf[tlh]
  
  @js.native
  sealed trait tn extends StObject
  inline def tn: tn = "tn".asInstanceOf[tn]
  
  @js.native
  sealed trait top extends StObject
  inline def top: top = "top".asInstanceOf[top]
  
  @js.native
  sealed trait tr extends StObject
  inline def tr: tr = "tr".asInstanceOf[tr]
  
  @js.native
  sealed trait truetype extends StObject
  inline def truetype: truetype = "truetype".asInstanceOf[truetype]
  
  @js.native
  sealed trait ts extends StObject
  inline def ts: ts = "ts".asInstanceOf[ts]
  
  @js.native
  sealed trait tt extends StObject
  inline def tt: tt = "tt".asInstanceOf[tt]
  
  @js.native
  sealed trait two extends StObject
  inline def two: two = "two".asInstanceOf[two]
  
  @js.native
  sealed trait twoleft extends StObject
  inline def twoleft: twoleft = "twoleft".asInstanceOf[twoleft]
  
  @js.native
  sealed trait tworight extends StObject
  inline def tworight: tworight = "tworight".asInstanceOf[tworight]
  
  @js.native
  sealed trait uk extends StObject
  inline def uk: uk = "uk".asInstanceOf[uk]
  
  @js.native
  sealed trait `ultra-condensed` extends StObject
  inline def `ultra-condensed`: `ultra-condensed` = "ultra-condensed".asInstanceOf[`ultra-condensed`]
  
  @js.native
  sealed trait `ultra-expanded` extends StObject
  inline def `ultra-expanded`: `ultra-expanded` = "ultra-expanded".asInstanceOf[`ultra-expanded`]
  
  @js.native
  sealed trait ur extends StObject
  inline def ur: ur = "ur".asInstanceOf[ur]
  
  @js.native
  sealed trait ve extends StObject
  inline def ve: ve = "ve".asInstanceOf[ve]
  
  @js.native
  sealed trait vi extends StObject
  inline def vi: vi = "vi".asInstanceOf[vi]
  
  @js.native
  sealed trait vo extends StObject
  inline def vo: vo = "vo".asInstanceOf[vo]
  
  @js.native
  sealed trait wa extends StObject
  inline def wa: wa = "wa".asInstanceOf[wa]
  
  @js.native
  sealed trait webp_ extends StObject
  inline def webp_ : webp_ = "webp".asInstanceOf[webp_]
  
  @js.native
  sealed trait xh extends StObject
  inline def xh: xh = "xh".asInstanceOf[xh]
  
  @js.native
  sealed trait zh extends StObject
  inline def zh: zh = "zh".asInstanceOf[zh]
  
  @js.native
  sealed trait `zh-CN` extends StObject
  inline def `zh-CN`: `zh-CN` = "zh-CN".asInstanceOf[`zh-CN`]
  
  @js.native
  sealed trait `zh-HK` extends StObject
  inline def `zh-HK`: `zh-HK` = "zh-HK".asInstanceOf[`zh-HK`]
  
  @js.native
  sealed trait `zh-SG` extends StObject
  inline def `zh-SG`: `zh-SG` = "zh-SG".asInstanceOf[`zh-SG`]
  
  @js.native
  sealed trait `zh-TW` extends StObject
  inline def `zh-TW`: `zh-TW` = "zh-TW".asInstanceOf[`zh-TW`]
  
  @js.native
  sealed trait zu extends StObject
  inline def zu: zu = "zu".asInstanceOf[zu]
}
