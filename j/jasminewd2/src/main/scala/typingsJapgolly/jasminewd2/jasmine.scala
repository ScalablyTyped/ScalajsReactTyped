...
import typingsJapgolly.jasmine.jasmine.ArrayLike
import typingsJapgolly.jasmine.jasmine.CustomEqualityTester
import typingsJapgolly.jasmine.jasmine.Expected
import typingsJapgolly.jasmine.jasmine.MatchersUtil
...
object jasmine {
  ...
  object Spec {
    ...
    inline def apply(addMatchers: AsyncCustomMatcherFactories => Callback): Spec = {
      val __obj = js.Dynamic.literal(addMatchers = js.Any.fromFunction1((t0: AsyncCustomMatcherFactories) => addMatchers(t0).runNow()))
      __obj.asInstanceOf[Spec]
    }
    
    extension [Self <: Spec](x: Self) {
      
      inline def setAddMatchers(value: AsyncCustomMatcherFactories => Callback): Self = StObject.set(x, "addMatchers", js.Any.fromFunction1((t0: AsyncCustomMatcherFactories) => value(t0).runNow()))
    }
  }
}
