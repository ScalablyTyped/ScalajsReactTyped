...
import typingsJapgolly.jsmediatags.anon.EndIx
...
object build2ChunkedFileDataMod {
  ...
  object ChunkedFileData {
    ...
    inline def apply(
      ...
      val __obj = js.Dynamic.literal(_concatData = js.Any.fromFunction2(_concatData), _getChunkRange = js.Any.fromFunction2(_getChunkRange), _sliceData = js.Any.fromFunction3(_sliceData), addData = js.Any.fromFunction2((t0: Double, t1: DataType) => (addData(t0, t1)).runNow()), getByteAt = js.Any.fromFunction1(getByteAt), hasDataRange = js.Any.fromFunction2(hasDataRange))
      __obj.asInstanceOf[ChunkedFileData]
    }
    
    extension [Self <: ChunkedFileData](x: Self) {
      
      inline def setAddData(value: (Double, DataType) => Callback): Self = StObject.set(x, "addData", js.Any.fromFunction2((t0: Double, t1: DataType) => (value(t0, t1)).runNow()))
