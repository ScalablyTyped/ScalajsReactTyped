...
object SchedulerView {
  ...
  extension [Self <: SchedulerView](x: Self) {
    ...
    inline def setEndDate(value: CallbackTo[js.Date]): Self = StObject.set(x, "endDate", value.toJsFn)
    ...
    inline def setStartDate(value: CallbackTo[js.Date]): Self = StObject.set(x, "startDate", value.toJsFn)
