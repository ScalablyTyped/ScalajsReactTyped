package typingsJapgolly.koaSubdomain
...
object mod {
  ...
  object Subdomain {
    ...
    inline def apply(
      ...
      routes: CallbackTo[Middleware[DefaultState, DefaultContext, Any]],
      use: (String, IMiddleware[Any, js.Object]) => Subdomain
    ): Subdomain = {
      val __obj = js.Dynamic.literal(routes = routes.toJsFn, use = js.Any.fromFunction2(use))
    ...
    extension [Self <: Subdomain](x: Self) {
      ...
      inline def setRoutes(value: CallbackTo[Middleware[DefaultState, DefaultContext, Any]]): Self = StObject.set(x, "routes", value.toJsFn)
