...
object KiiUserBuilder {
  ...
  inline def apply(
    ...
    val __obj = js.Dynamic.literal(build = build.toJsFn, setEmailAddress = js.Any.fromFunction1(setEmailAddress), setGlobalPhoneNumber = js.Any.fromFunction1(setGlobalPhoneNumber), setLocalPhoneNumber = js.Any.fromFunction2(setLocalPhoneNumber), setUsername = js.Any.fromFunction1(setUsername))
    __obj.asInstanceOf[KiiUserBuilder]
  }
  
  extension [Self <: KiiUserBuilder](x: Self) {
    
    inline def setBuild(value: CallbackTo[KiiUser]): Self = StObject.set(x, "build", value.toJsFn)
