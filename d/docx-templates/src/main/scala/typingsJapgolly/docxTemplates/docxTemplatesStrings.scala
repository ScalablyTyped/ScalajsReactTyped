package typingsJapgolly.docxTemplates

import typingsJapgolly.docxTemplates.libBundledMod.BuiltInCommand
import typingsJapgolly.docxTemplates.libTypesMod.ImageExtension
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object docxTemplatesStrings {
  
  @js.native
  sealed trait ALIAS
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def ALIAS: ALIAS = "ALIAS".asInstanceOf[ALIAS]
  
  @js.native
  sealed trait CMD_NODE
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def CMD_NODE: CMD_NODE = "CMD_NODE".asInstanceOf[CMD_NODE]
  
  @js.native
  sealed trait Dotgif
    extends StObject
       with ImageExtension
  inline def Dotgif: Dotgif = ".gif".asInstanceOf[Dotgif]
  
  @js.native
  sealed trait Dotjpeg
    extends StObject
       with ImageExtension
  inline def Dotjpeg: Dotjpeg = ".jpeg".asInstanceOf[Dotjpeg]
  
  @js.native
  sealed trait Dotjpg
    extends StObject
       with ImageExtension
  inline def Dotjpg: Dotjpg = ".jpg".asInstanceOf[Dotjpg]
  
  @js.native
  sealed trait Dotpng
    extends StObject
       with ImageExtension
  inline def Dotpng: Dotpng = ".png".asInstanceOf[Dotpng]
  
  @js.native
  sealed trait Dotsvg
    extends StObject
       with ImageExtension
  inline def Dotsvg: Dotsvg = ".svg".asInstanceOf[Dotsvg]
  
  @js.native
  sealed trait `END-FOR`
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def `END-FOR`: `END-FOR` = "END-FOR".asInstanceOf[`END-FOR`]
  
  @js.native
  sealed trait `END-IF`
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def `END-IF`: `END-IF` = "END-IF".asInstanceOf[`END-IF`]
  
  @js.native
  sealed trait EXEC
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def EXEC: EXEC = "EXEC".asInstanceOf[EXEC]
  
  @js.native
  sealed trait FOR
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def FOR: FOR = "FOR".asInstanceOf[FOR]
  
  @js.native
  sealed trait HTML
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def HTML: HTML = "HTML".asInstanceOf[HTML]
  
  @js.native
  sealed trait IF
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def IF: IF = "IF".asInstanceOf[IF]
  
  @js.native
  sealed trait IMAGE
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def IMAGE: IMAGE = "IMAGE".asInstanceOf[IMAGE]
  
  @js.native
  sealed trait INS
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def INS: INS = "INS".asInstanceOf[INS]
  
  @js.native
  sealed trait JS extends StObject
  inline def JS: JS = "JS".asInstanceOf[JS]
  
  @js.native
  sealed trait LINK
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def LINK: LINK = "LINK".asInstanceOf[LINK]
  
  @js.native
  sealed trait QUERY
    extends StObject
       with BuiltInCommand
       with typingsJapgolly.docxTemplates.libTypesMod.BuiltInCommand
  inline def QUERY: QUERY = "QUERY".asInstanceOf[QUERY]
  
  @js.native
  sealed trait XML extends StObject
  inline def XML: XML = "XML".asInstanceOf[XML]
  
  @js.native
  sealed trait errors extends StObject
  inline def errors: errors = "errors".asInstanceOf[errors]
  
  @js.native
  sealed trait success extends StObject
  inline def success: success = "success".asInstanceOf[success]
}
