...
import typingsJapgolly.datatablesNetKeytable.anon.Disable
...
object DataTables {
  ...
  object CellMethods {
    ...
    inline def apply(focus: CallbackTo[Api]): CellMethods = {
    ...
    extension [Self <: CellMethods](x: Self) {
      ...
      inline def setFocus(value: CallbackTo[Api]): Self = StObject.set(x, "focus", value.toJsFn)
    }
  }
