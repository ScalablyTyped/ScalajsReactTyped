package typingsJapgolly.devexpressAspnetcoreBootstrap.global.DevExpress.AspNetCore

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("DevExpress.AspNetCore.BootstrapAccordionGroup")
@js.native
open class BootstrapAccordionGroup protected ()
  extends StObject
     with typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapAccordionGroup {
  /* protected */ def this(instance: Any) = this()
  
  /* CompleteClass */
  override def getEnabled(): Boolean = js.native
  
  /* CompleteClass */
  override def getExpanded(): Boolean = js.native
  
  /* CompleteClass */
  override def getHeaderBadgeIconCssClass(): String = js.native
  
  /* CompleteClass */
  override def getHeaderBadgeText(): String = js.native
  
  /* CompleteClass */
  override def getItem(index: Double): typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapAccordionItem | Null = js.native
  
  /* CompleteClass */
  override def getItemByName(name: String): typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapAccordionItem | Null = js.native
  
  /* CompleteClass */
  override def getItemCount(): Double = js.native
  
  /* CompleteClass */
  override def getText(): String = js.native
  
  /* CompleteClass */
  override def getVisible(): Boolean = js.native
  
  /* CompleteClass */
  override val index: Double = js.native
  
  /* protected */ /* CompleteClass */
  override val instance: Any = js.native
  
  /* CompleteClass */
  override val name: String = js.native
  
  /* CompleteClass */
  override val navBar: typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapAccordion | Null = js.native
  
  /* CompleteClass */
  override def setExpanded(value: Boolean): Unit = js.native
  
  /* CompleteClass */
  override def setHeaderBadgeIconCssClass(cssClass: String): Unit = js.native
  
  /* CompleteClass */
  override def setHeaderBadgeText(text: String): Unit = js.native
  
  /* CompleteClass */
  override def setText(text: String): Unit = js.native
  
  /* CompleteClass */
  override def setVisible(value: Boolean): Unit = js.native
}
