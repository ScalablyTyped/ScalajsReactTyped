package typingsJapgolly.devexpressAspnetcoreBootstrap.global.DevExpress.AspNetCore

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("DevExpress.AspNetCore.BootstrapTimeInterval")
@js.native
open class BootstrapTimeInterval protected ()
  extends StObject
     with typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapTimeInterval {
  /* protected */ def this(instance: Any) = this()
  
  /* CompleteClass */
  override def contains(
    interval: typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapTimeInterval
  ): Boolean = js.native
  
  /* CompleteClass */
  override def equals(
    interval: typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapTimeInterval
  ): Boolean = js.native
  
  /* CompleteClass */
  override def getAllDay(): Boolean = js.native
  
  /* CompleteClass */
  override def getDuration(): Double = js.native
  
  /* CompleteClass */
  override def getEnd(): js.Date = js.native
  
  /* CompleteClass */
  override def getStart(): js.Date = js.native
  
  /* protected */ /* CompleteClass */
  override val instance: Any = js.native
  
  /* CompleteClass */
  override def intersectsWith(
    interval: typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapTimeInterval
  ): Boolean = js.native
  
  /* CompleteClass */
  override def intersectsWithExcludingBounds(
    interval: typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapTimeInterval
  ): Boolean = js.native
  
  /* CompleteClass */
  override def setAllDay(allDayValue: Boolean): Unit = js.native
  
  /* CompleteClass */
  override def setDuration(value: Double): Unit = js.native
  
  /* CompleteClass */
  override def setEnd(value: js.Date): Unit = js.native
  
  /* CompleteClass */
  override def setStart(value: js.Date): Unit = js.native
}
