package typingsJapgolly.devexpressAspnetcoreBootstrap.global.DevExpress.AspNetCore

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

@JSGlobal("DevExpress.AspNetCore.BootstrapPopupWindow")
@js.native
open class BootstrapPopupWindow protected ()
  extends StObject
     with typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapPopupWindow {
  /* protected */ def this(instance: Any) = this()
  
  /* CompleteClass */
  override def getFooterImageUrl(): String = js.native
  
  /* CompleteClass */
  override def getFooterNavigateUrl(): String = js.native
  
  /* CompleteClass */
  override def getFooterText(): String = js.native
  
  /* CompleteClass */
  override def getHeaderImageUrl(): String = js.native
  
  /* CompleteClass */
  override def getHeaderNavigateUrl(): String = js.native
  
  /* CompleteClass */
  override def getHeaderText(): String = js.native
  
  /* CompleteClass */
  override val index: Double = js.native
  
  /* protected */ /* CompleteClass */
  override val instance: Any = js.native
  
  /* CompleteClass */
  override val name: String = js.native
  
  /* CompleteClass */
  override val popupControl: typingsJapgolly.devexpressAspnetcoreBootstrap.DevExpress.AspNetCore.BootstrapPopupControl | Null = js.native
  
  /* CompleteClass */
  override def setFooterImageUrl(value: String): Unit = js.native
  
  /* CompleteClass */
  override def setFooterNavigateUrl(value: String): Unit = js.native
  
  /* CompleteClass */
  override def setFooterText(value: String): Unit = js.native
  
  /* CompleteClass */
  override def setHeaderImageUrl(value: String): Unit = js.native
  
  /* CompleteClass */
  override def setHeaderNavigateUrl(value: String): Unit = js.native
  
  /* CompleteClass */
  override def setHeaderText(value: String): Unit = js.native
}
