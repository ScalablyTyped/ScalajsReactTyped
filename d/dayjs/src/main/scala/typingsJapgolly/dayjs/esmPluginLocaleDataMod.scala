...
object esmPluginLocaleDataMod extends Shortcut {
  ...
  object dayjsEsmAugmentingMod {
    ...
    object Dayjs {
      ...
      inline def apply(localeData: CallbackTo[InstanceLocaleDataReturn]): Dayjs = {
        val __obj = js.Dynamic.literal(localeData = localeData.toJsFn)
        __obj.asInstanceOf[Dayjs]
      }
      
      extension [Self <: Dayjs](x: Self) {
        
        inline def setLocaleData(value: CallbackTo[InstanceLocaleDataReturn]): Self = StObject.set(x, "localeData", value.toJsFn)
      }
    }
