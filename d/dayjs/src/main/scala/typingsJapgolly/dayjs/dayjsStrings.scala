package typingsJapgolly.dayjs

import typingsJapgolly.dayjs.esmPluginIsoWeekMod.ISOUnitType
import typingsJapgolly.dayjs.mod.ManipulateType
import typingsJapgolly.dayjs.mod.OpUnitType
import typingsJapgolly.dayjs.mod.QUnitType
import typingsJapgolly.dayjs.mod.UnitType
import typingsJapgolly.dayjs.mod.UnitTypeLong
import typingsJapgolly.dayjs.mod.UnitTypeLongPlural
import typingsJapgolly.dayjs.mod.UnitTypeShort
import typingsJapgolly.dayjs.pluginDurationMod.DurationUnitType
import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

object dayjsStrings {
  
  @js.native
  sealed trait D
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeShort
       with typingsJapgolly.dayjs.esmMod.UnitTypeShort
  inline def D: D = "D".asInstanceOf[D]
  
  @js.native
  sealed trait M
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeShort
       with typingsJapgolly.dayjs.esmMod.UnitTypeShort
  inline def M: M = "M".asInstanceOf[M]
  
  @js.native
  sealed trait Q
    extends StObject
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
  inline def Q: Q = "Q".asInstanceOf[Q]
  
  @js.native
  sealed trait `[]` extends StObject
  inline def `[]`: `[]` = "[]".asInstanceOf[`[]`]
  
  @js.native
  sealed trait d_
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeShort
       with typingsJapgolly.dayjs.esmMod.UnitTypeShort
  inline def d_ : d_ = "d".asInstanceOf[d_]
  
  @js.native
  sealed trait date
    extends StObject
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLong
       with typingsJapgolly.dayjs.esmMod.UnitTypeLong
  inline def date: date = "date".asInstanceOf[date]
  
  @js.native
  sealed trait dates
    extends StObject
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLongPlural
       with typingsJapgolly.dayjs.esmMod.UnitTypeLongPlural
  inline def dates: dates = "dates".asInstanceOf[dates]
  
  @js.native
  sealed trait day
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLong
       with typingsJapgolly.dayjs.esmMod.UnitTypeLong
  inline def day: day = "day".asInstanceOf[day]
  
  @js.native
  sealed trait days
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLongPlural
       with typingsJapgolly.dayjs.esmMod.UnitTypeLongPlural
  inline def days: days = "days".asInstanceOf[days]
  
  @js.native
  sealed trait h
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeShort
       with typingsJapgolly.dayjs.esmMod.UnitTypeShort
  inline def h: h = "h".asInstanceOf[h]
  
  @js.native
  sealed trait hour
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLong
       with typingsJapgolly.dayjs.esmMod.UnitTypeLong
  inline def hour: hour = "hour".asInstanceOf[hour]
  
  @js.native
  sealed trait hours
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLongPlural
       with typingsJapgolly.dayjs.esmMod.UnitTypeLongPlural
  inline def hours: hours = "hours".asInstanceOf[hours]
  
  @js.native
  sealed trait isoWeek
    extends StObject
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
  inline def isoWeek: isoWeek = "isoWeek".asInstanceOf[isoWeek]
  
  @js.native
  sealed trait long extends StObject
  inline def long: long = "long".asInstanceOf[long]
  
  @js.native
  sealed trait m_
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeShort
       with typingsJapgolly.dayjs.esmMod.UnitTypeShort
  inline def m_ : m_ = "m".asInstanceOf[m_]
  
  @js.native
  sealed trait millisecond
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLong
       with typingsJapgolly.dayjs.esmMod.UnitTypeLong
  inline def millisecond: millisecond = "millisecond".asInstanceOf[millisecond]
  
  @js.native
  sealed trait milliseconds
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLongPlural
       with typingsJapgolly.dayjs.esmMod.UnitTypeLongPlural
  inline def milliseconds: milliseconds = "milliseconds".asInstanceOf[milliseconds]
  
  @js.native
  sealed trait minute
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLong
       with typingsJapgolly.dayjs.esmMod.UnitTypeLong
  inline def minute: minute = "minute".asInstanceOf[minute]
  
  @js.native
  sealed trait minutes
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLongPlural
       with typingsJapgolly.dayjs.esmMod.UnitTypeLongPlural
  inline def minutes: minutes = "minutes".asInstanceOf[minutes]
  
  @js.native
  sealed trait month
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLong
       with typingsJapgolly.dayjs.esmMod.UnitTypeLong
  inline def month: month = "month".asInstanceOf[month]
  
  @js.native
  sealed trait months
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLongPlural
       with typingsJapgolly.dayjs.esmMod.UnitTypeLongPlural
  inline def months: months = "months".asInstanceOf[months]
  
  @js.native
  sealed trait ms
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeShort
       with typingsJapgolly.dayjs.esmMod.UnitTypeShort
  inline def ms: ms = "ms".asInstanceOf[ms]
  
  @js.native
  sealed trait quarter
    extends StObject
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
  inline def quarter: quarter = "quarter".asInstanceOf[quarter]
  
  @js.native
  sealed trait quarters
    extends StObject
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
  inline def quarters: quarters = "quarters".asInstanceOf[quarters]
  
  @js.native
  sealed trait s
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeShort
       with typingsJapgolly.dayjs.esmMod.UnitTypeShort
  inline def s: s = "s".asInstanceOf[s]
  
  @js.native
  sealed trait second
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLong
       with typingsJapgolly.dayjs.esmMod.UnitTypeLong
  inline def second: second = "second".asInstanceOf[second]
  
  @js.native
  sealed trait seconds
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLongPlural
       with typingsJapgolly.dayjs.esmMod.UnitTypeLongPlural
  inline def seconds: seconds = "seconds".asInstanceOf[seconds]
  
  @js.native
  sealed trait short extends StObject
  inline def short: short = "short".asInstanceOf[short]
  
  @js.native
  sealed trait w
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
  inline def w: w = "w".asInstanceOf[w]
  
  @js.native
  sealed trait week
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
  inline def week: week = "week".asInstanceOf[week]
  
  @js.native
  sealed trait weeks
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
  inline def weeks: weeks = "weeks".asInstanceOf[weeks]
  
  @js.native
  sealed trait y
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeShort
       with typingsJapgolly.dayjs.esmMod.UnitTypeShort
  inline def y: y = "y".asInstanceOf[y]
  
  @js.native
  sealed trait year
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLong
       with typingsJapgolly.dayjs.esmMod.UnitTypeLong
  inline def year: year = "year".asInstanceOf[year]
  
  @js.native
  sealed trait years
    extends StObject
       with DurationUnitType
       with typingsJapgolly.dayjs.esmPluginDurationMod.DurationUnitType
       with ISOUnitType
       with typingsJapgolly.dayjs.pluginIsoWeekMod.ISOUnitType
       with ManipulateType
       with typingsJapgolly.dayjs.esmMod.ManipulateType
       with OpUnitType
       with typingsJapgolly.dayjs.esmMod.OpUnitType
       with QUnitType
       with typingsJapgolly.dayjs.esmMod.QUnitType
       with UnitType
       with typingsJapgolly.dayjs.esmMod.UnitType
       with UnitTypeLongPlural
       with typingsJapgolly.dayjs.esmMod.UnitTypeLongPlural
  inline def years: years = "years".asInstanceOf[years]
}
