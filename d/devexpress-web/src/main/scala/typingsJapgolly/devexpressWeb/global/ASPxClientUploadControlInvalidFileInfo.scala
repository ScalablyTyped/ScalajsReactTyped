package typingsJapgolly.devexpressWeb.global

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/**
  * Contains settings of the file that hasn't passed validation.
  */
@JSGlobal("ASPxClientUploadControlInvalidFileInfo")
@js.native
open class ASPxClientUploadControlInvalidFileInfo ()
  extends StObject
     with typingsJapgolly.devexpressWeb.ASPxClientUploadControlInvalidFileInfo {
  
  /**
    * Gets the error type.
    */
  /* CompleteClass */
  var errorType: typingsJapgolly.devexpressWeb.ASPxClientUploadControlValidationErrorTypeConsts = js.native
  
  /**
    * Gets the name of the invalid file.
    */
  /* CompleteClass */
  var fileName: String = js.native
  
  /**
    * Gets the size of the invalid file.
    */
  /* CompleteClass */
  var fileSize: Double = js.native
}
