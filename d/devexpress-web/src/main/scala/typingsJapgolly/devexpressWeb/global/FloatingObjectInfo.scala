package typingsJapgolly.devexpressWeb.global

import org.scalablytyped.runtime.StObject
import scala.scalajs.js
import scala.scalajs.js.annotation.{JSGlobalScope, JSGlobal, JSImport, JSName, JSBracketAccess}

/**
  * Contains floating objects base settings.
  */
@JSGlobal("FloatingObjectInfo")
@js.native
open class FloatingObjectInfo ()
  extends StObject
     with typingsJapgolly.devexpressWeb.FloatingObjectInfo {
  
  /**
    * Gets the parent sub-document what stores the current floating object.
    */
  /* CompleteClass */
  override val parentSubDocument: typingsJapgolly.devexpressWeb.SubDocument = js.native
  
  /**
    * Gets the position of the floating object's anchor in the parent sub-document.
    */
  /* CompleteClass */
  override val position: Double = js.native
}
