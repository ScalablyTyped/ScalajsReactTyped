...
object BindingToSyntax {
  ...
  extension [Self <: BindingToSyntax[?], T](x: Self & BindingToSyntax[T]) {
    ...
    inline def setToSelf(value: CallbackTo[BindingInWhenOnSyntax[T]]): Self = StObject.set(x, "toSelf", value.toJsFn)
    
    inline def setToService(value: ServiceIdentifier[T] => Callback): Self = StObject.set(x, "toService", js.Any.fromFunction1((t0: ServiceIdentifier[T]) => value(t0).runNow()))
  }
}
