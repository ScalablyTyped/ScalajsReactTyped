...
import typingsJapgolly.immer.distUtilsPluginsMod.ES5ArrayState
import typingsJapgolly.immer.distUtilsPluginsMod.ES5ObjectState
import typingsJapgolly.immer.distUtilsPluginsMod.PatchPath
import typingsJapgolly.immer.immerStrings.`Illegal state`
import typingsJapgolly.immer.immerStrings.`Immer drafts cannot have computed properties`
import typingsJapgolly.immer.immerStrings.`Immer forbids circular references`
import typingsJapgolly.immer.immerStrings.`Immer only supports deleting array indices`
import typingsJapgolly.immer.immerStrings.`Sets cannot have QuotationmarkreplaceQuotationmark patchesDot`
import typingsJapgolly.immer.immerStrings.`The first or second argument to GraveaccentproduceGraveaccent must be a function`
import typingsJapgolly.immer.immerStrings.`The given draft is already finalized`
import typingsJapgolly.immer.immerStrings.`This object has been frozen and should not be mutated`
...
object anon {
  ...
  object ApplyPatches {
    ...
    inline def apply(
      ...
      generatePatches_ : (ImmerState, PatchPath, js.Array[Patch], js.Array[Patch]) => Callback,
      generateReplacementPatches_ : (Any, Any, js.Array[Patch], js.Array[Patch]) => Callback
    ): ApplyPatches = {
      val __obj = js.Dynamic.literal(applyPatches_ = js.Any.fromFunction2(applyPatches_), generatePatches_ = js.Any.fromFunction4((t0: ImmerState, t1: PatchPath, t2: js.Array[Patch], t3: js.Array[Patch]) => (generatePatches_(t0, t1, t2, t3)).runNow()), generateReplacementPatches_ = js.Any.fromFunction4((t0: Any, t1: Any, t2: js.Array[Patch], t3: js.Array[Patch]) => (generateReplacementPatches_(t0, t1, t2, t3)).runNow()))
    ...
    extension [Self <: ApplyPatches](x: Self) {
      ...
      inline def setGeneratePatches_(value: (ImmerState, PatchPath, js.Array[Patch], js.Array[Patch]) => Callback): Self = StObject.set(x, "generatePatches_", js.Any.fromFunction4((t0: ImmerState, t1: PatchPath, t2: js.Array[Patch], t3: js.Array[Patch]) => (value(t0, t1, t2, t3)).runNow()))
      
      inline def setGenerateReplacementPatches_(value: (Any, Any, js.Array[Patch], js.Array[Patch]) => Callback): Self = StObject.set(x, "generateReplacementPatches_", js.Any.fromFunction4((t0: Any, t1: Any, t2: js.Array[Patch], t3: js.Array[Patch]) => (value(t0, t1, t2, t3)).runNow()))
    }
  }
