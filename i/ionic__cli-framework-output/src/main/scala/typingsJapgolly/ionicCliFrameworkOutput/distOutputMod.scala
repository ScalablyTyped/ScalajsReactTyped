...
import typingsJapgolly.ionicCliFrameworkOutput.distTasksMod.TaskChain
...
object distOutputMod {
  ...
  object OutputStrategy {
    ...
    inline def apply(
      colors: Colors,
      createTaskChain: CallbackTo[TaskChain],
      stream: WritableStream,
      write: String => Boolean
    ): OutputStrategy = {
      val __obj = js.Dynamic.literal(colors = colors.asInstanceOf[js.Any], createTaskChain = createTaskChain.toJsFn, stream = stream.asInstanceOf[js.Any], write = js.Any.fromFunction1(write))
    ...
    extension [Self <: OutputStrategy](x: Self) {
      ...
      inline def setCreateTaskChain(value: CallbackTo[TaskChain]): Self = StObject.set(x, "createTaskChain", value.toJsFn)
