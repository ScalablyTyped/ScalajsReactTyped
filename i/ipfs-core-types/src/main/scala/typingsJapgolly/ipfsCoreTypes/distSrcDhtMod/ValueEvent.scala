...
import typingsJapgolly.ipfsCoreTypes.ipfsCoreTypesStrings.VALUE
...
trait ValueEvent
  ...
  var `type`: typingsJapgolly.ipfsCoreTypes.distSrcDhtMod.EventTypes.VALUE
...
object ValueEvent {
  ...
  inline def apply(
    `type`: typingsJapgolly.ipfsCoreTypes.distSrcDhtMod.EventTypes.VALUE,
  ...
  extension [Self <: ValueEvent](x: Self) {
    ...
    inline def setType(value: typingsJapgolly.ipfsCoreTypes.distSrcDhtMod.EventTypes.VALUE): Self = StObject.set(x, "type", value.asInstanceOf[js.Any])
