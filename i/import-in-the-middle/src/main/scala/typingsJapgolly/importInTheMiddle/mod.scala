package typingsJapgolly.importInTheMiddle
...
object mod {
  ...
  object Hook {
    ...
    inline def apply(unhook: Callback): Hook = {
      val __obj = js.Dynamic.literal(unhook = unhook.toJsFn)
      __obj.asInstanceOf[Hook]
    }
    
    extension [Self <: Hook](x: Self) {
      
      inline def setUnhook(value: Callback): Self = StObject.set(x, "unhook", value.toJsFn)
    }
  }
