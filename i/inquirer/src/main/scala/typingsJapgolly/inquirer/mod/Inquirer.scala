...
import typingsJapgolly.inquirer.anon.Choices
import typingsJapgolly.inquirer.anon.Default
import typingsJapgolly.inquirer.inquirerBooleans.`false`
import typingsJapgolly.inquirer.inquirerBooleans.`true`
...
object inquirer {
  ...
  object prompts {
    ...
    object PromptBase {
      ...
      inline def apply(run: CallbackTo[js.Promise[Any]], status: PromptState): PromptBase = {
        val __obj = js.Dynamic.literal(run = run.toJsFn, status = status.asInstanceOf[js.Any])
  ...
  object ui {
    ...
    @js.native
    trait BottomBar
      extends typingsJapgolly.inquirer.libUiBaseUIMod.default {
