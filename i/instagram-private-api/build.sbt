organization := "org.scalablytyped.japgolly"
name := "instagram-private-api"
version := "1.39.0-a9c9c4"
scalaVersion := "2.13.1"
enablePlugins(ScalaJSPlugin)
libraryDependencies ++= Seq(
  "com.github.japgolly.scalajs-react" %%% "core" % "1.5.0",
  "com.olvind" %%% "scalablytyped-runtime" % "2.1.0",
  "org.scalablytyped.japgolly" %%% "bluebird" % "3.5-dt-20200302Z-b175b6",
  "org.scalablytyped.japgolly" %%% "caseless" % "0.12-dt-20190311Z-d016a6",
  "org.scalablytyped.japgolly" %%% "chance" % "1.0-dt-20200226Z-99ea41",
  "org.scalablytyped.japgolly" %%% "form-data" % "2.5.1-b08b8b",
  "org.scalablytyped.japgolly" %%% "lifeomic__attempt" % "3.0.0-55a7fd",
  "org.scalablytyped.japgolly" %%% "luxon" % "1.21-dt-20191115Z-63d8c3",
  "org.scalablytyped.japgolly" %%% "node" % "13.7-dt-20200228Z-e4cd2e",
  "org.scalablytyped.japgolly" %%% "request" % "2.48-dt-20191217Z-011ed5",
  "org.scalablytyped.japgolly" %%% "rxjs" % "6.5.4-a27765",
  "org.scalablytyped.japgolly" %%% "rxjs-compat" % "6.5.4-968493",
  "org.scalablytyped.japgolly" %%% "std" % "3.8-8884a5",
  "org.scalablytyped.japgolly" %%% "tough-cookie" % "2.3-dt-20191206Z-d45a45",
  "org.scalablytyped.japgolly" %%% "ts-custom-error" % "2.2.2-616aba",
  "org.scalablytyped.japgolly" %%% "ts-xor" % "1.0.8-617751")
publishArtifact in packageDoc := false
scalacOptions ++= List("-encoding", "utf-8", "-g:notailcalls", "-P:scalajs:sjsDefinedByDefault")
licenses += ("MIT", url("http://opensource.org/licenses/MIT"))
bintrayRepository := "ScalajsReactTyped"
resolvers += Resolver.bintrayRepo("oyvindberg", "ScalajsReactTyped")
