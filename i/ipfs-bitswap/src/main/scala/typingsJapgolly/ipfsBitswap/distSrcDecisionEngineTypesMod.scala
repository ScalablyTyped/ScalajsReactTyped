...
object distSrcDecisionEngineTypesMod {
  ...
  object TaskMerger {
    ...
    inline def apply(hasNewInfo: (Task, js.Array[Task]) => Boolean, merge: (Task, Task) => Callback): TaskMerger = {
      val __obj = js.Dynamic.literal(hasNewInfo = js.Any.fromFunction2(hasNewInfo), merge = js.Any.fromFunction2((t0: Task, t1: Task) => (merge(t0, t1)).runNow()))
    ...
    extension [Self <: TaskMerger](x: Self) {
      ...
      inline def setMerge(value: (Task, Task) => Callback): Self = StObject.set(x, "merge", js.Any.fromFunction2((t0: Task, t1: Task) => (value(t0, t1)).runNow()))
    }
  }
}
